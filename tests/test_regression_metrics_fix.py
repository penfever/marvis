...
def test_regression_metrics_fix():
    ...
    try:
        ...
        metrics = calculate_llm_metrics(
            y_true, y_pred, unique_classes=None, task_type="regression", logger=logger
        )
        ...
        if balanced_accuracy is None:
            ...
            logger.error(
                f"❌ balanced_accuracy should be None for regression, got {balanced_accuracy}"
            )
        ...
        if len(cls_metrics_none) == len(expected_cls_metrics):
            logger.info(
                f"✅ All classification metrics correctly None: {cls_metrics_none}"
            )
            tests_passed += 1
        else:
            not_none = [m for m in expected_cls_metrics if metrics.get(m) is not None]
            logger.error(
                f"❌ Classification metrics should be None, but found: {not_none}"
            )
...
def test_classification_metrics_unchanged():
    ...
    try:
        ...
        if accuracy is not None and isinstance(accuracy, (int, float)):
            ...
            logger.error(
                f"❌ accuracy should be numeric for classification, got {accuracy}"
            )

        # Test 2: balanced_accuracy should have numeric value for classification
        if balanced_accuracy is not None and isinstance(
            balanced_accuracy, (int, float)
        ):
            logger.info(
                f"✅ balanced_accuracy has correct numeric value: {balanced_accuracy:.4f}"
            )
            tests_passed += 1
        else:
            logger.error(
                f"❌ balanced_accuracy should be numeric for classification, got {balanced_accuracy}"
            )
        ...
        if len(reg_none) == len(reg_metrics):
            logger.info(
                f"✅ Regression metrics correctly None for classification: {reg_none}"
            )
            tests_passed += 1
        else:
            not_none = [m for m in reg_metrics if metrics.get(m) is not None]
            logger.error(
                f"❌ Regression metrics should be None for classification: {not_none}"
            )
        ...
        if len(cls_with_values) == len(cls_metrics):
            ...
            logger.error(
                f"❌ Classification metrics should have values, but None: {none_values}"
            )
