...
class VLMPromptingTestSuite:
    ...
    def __init__(
        self,
        output_dir: str,
        task_id_or_dataset_name: str = "23",
        vlm_model: str = "Qwen/Qwen2-VL-2B-Instruct",
        num_tests: int = None,
        num_samples_per_test: int = 10,
        backend: str = "auto",
        zoom_factor: float = 6.5,
        ...
        self.modality, self.task_id = self._parse_dataset_identifier(
            task_id_or_dataset_name
        )
        ...
        logger.info(
            f"Initialized test suite with dataset: {task_id_or_dataset_name}, modality: {self.modality}"
        )
        logger.info(
            f"Dataset shape: {self.X.shape if hasattr(self.X, 'shape') else len(self.X) if isinstance(self.X, list) else 'N/A'}"
        )
        logger.info(
            f"Task type: {self.task_type} (detected via: {self.task_type_method})"
        )
    ...
    def _parse_dataset_identifier(self, identifier: str) -> Tuple[str, Optional[int]]:
        ...
        logger.warning(
            f"Unknown dataset identifier '{identifier}', defaulting to tabular modality"
        )
        return "tabular", None
    ...
    def _determine_task_type(self) -> Tuple[str, str]:
        ...
        try:
            task_type, method = detect_task_type(
                task_id=self.task_id, y=self.y, dataset_info=self.dataset_info
            )
            return task_type, method
        except Exception as e:
            logger.warning(
                f"Task type detection failed: {e}, defaulting to classification"
            )
            return "classification", "fallback_default"

    def _validate_detailed_vlm_outputs(
        ...
        try:
            ...
            if not detailed_output_path.exists():
                return (
                    False,
                    f"detailed_vlm_outputs.json file not found at {detailed_output_path}",
                )
            ...
            with open(detailed_output_path, "r") as f:
                ...
                try:
                    ...
                    return (
                        False,
                        f"detailed_vlm_outputs.json contains invalid JSON: {e}",
                    )
            ...
            if missing_fields:
                return (
                    False,
                    f"detailed_vlm_outputs.json missing required fields: {missing_fields}",
                )
            ...
            if len(prediction_details) == 0:
                if pipeline_failed:
                    ...
                    return (
                        True,
                        "detailed_vlm_outputs.json validation passed (failed test case with expected empty predictions)",
                    )
                else:
                    return (
                        False,
                        "prediction_details is empty - no predictions were recorded",
                    )
            ...
            for i, pred in enumerate(prediction_details):
                ...
                pred_missing_fields = [
                    field for field in pred_required_fields if field not in pred
                ]
                if pred_missing_fields:
                    return (
                        False,
                        f"prediction_details[{i}] missing required fields: {pred_missing_fields}",
                    )
    ...
    def _load_semantic_class_names(
        self, task_id: int, num_classes: int
    ) -> Optional[List[str]]:
        """
        ...
        try:
            ...
            if "target_values" in data:
                ...
                if isinstance(target_values, dict):
                    ...
                    try:
                        sorted_items = sorted(
                            target_values.items(), key=lambda x: int(x[0])
                        )
                    except ValueError:
            ...
            if class_names:
                ...
                if len(class_names) >= num_classes:
                    ...
                    logger.info(
                        f"Loaded {len(class_names)} semantic class names for task {task_id}: {class_names}"
                    )
                    return class_names
                else:
                    logger.warning(
                        f"Found {len(class_names)} semantic names but need {num_classes} for task {task_id}"
                    )
    ...
    def _calculate_metrics(self, prediction_details, ground_truth_labels, config_name):
        ...
        if self.task_type == "classification":
            ...
            return {
                ...
                "num_test_samples": len(predictions),
                "target_range": [
                    float(np.min(true_labels)),
                    float(np.max(true_labels)),
                ],
                "prediction_range": [
    ...
    def _load_dataset(self):
        ...
        try:
            ...
            for col in range(X.shape[1]):
                col_data = X[:, col]
                if (
                    col_data.dtype == "object" or col_data.dtype.kind in "SU"
                ):  # String/Unicode
            ...
            dataset_info = {
                "name": (
                    ...
                    else f"OpenML_Task_{self.task_id}"
                ),
                "n_samples": len(X),
    ...
    def _load_audio_dataset(self):
        ...
        if not AUDIO_AVAILABLE:
            raise ImportError(
                "Audio datasets not available. Install required dependencies for audio support."
            )
        ...
        try:
            ...
            logger.warning(
                f"Failed to load audio dataset {self.task_id_or_dataset_name}: {e}"
            )
            logger.info("Using synthetic dataset as fallback")
    ...
    def _load_vision_dataset(self):
        ...
        if not VISION_AVAILABLE:
            raise ImportError(
                "Vision datasets not available. Install required dependencies for vision support."
            )
        ...
        try:
            ...
            if dataset_name in ["cifar10", "cifar100"]:
                train_paths, train_labels, test_paths, test_labels, class_names = (
                    rm.prepare_cifar_dataset(dataset_name)
                )
            ...
            logger.warning(
                f"Failed to load vision dataset {self.task_id_or_dataset_name}: {e}"
            )
            logger.info("Using synthetic dataset as fallback")
    ...
    def _get_test_configurations(self) -> List[Dict[str, Any]]:
        ...
        if self.modality != "tabular":
            all_configs = (
                modality_configs + single_viz_configs[:3]
            )  # Just a few basic configs
        else:
    ...
    def run_single_test(self, config: Dict[str, Any], test_idx: int) -> Dict[str, Any]:
        ...
        if self.test_indices is None:
            ...
            if len(X_test) > self.responses_per_test:
                self.test_indices = np.random.choice(
                    len(X_test), self.responses_per_test, replace=False
                )
        ...
        try:
            ...
            if config.get("load_semantic_from_cc18", False):
                semantic_class_names = self._load_semantic_class_names(
                    self.task_id, len(np.unique(y_train))
                )
                if semantic_class_names:
                    config["semantic_class_names"] = semantic_class_names
                    logger.info(
                        f"Loaded semantic class names for test {test_idx + 1}: {semantic_class_names}"
                    )
                else:
                    logger.info(
                        f"No semantic class names found for task {self.task_id}, using Class_<NUM> fallback"
                    )
                    config["semantic_class_names"] = [
                        f"Class_{i}" for i in range(len(np.unique(y_train)))
                    ]
                    config["use_semantic_names"] = (
                        False  # Disable semantic names since we couldn't load them
                    )
            ...
            classifier_config = {
                ...
                "semantic_axes_method": config.get(
                    "semantic_axes_method", "pca_loadings"
                ),  # NEW: Support different semantic axes methods
                "use_smart_feature_selection": config.get(
                    "use_smart_feature_selection", True
                ),  # NEW: Enable smart feature selection by default
                "max_features_to_test": config.get(
                    "max_features_to_test", 10
                ),  # NEW: Limit features for perturbation testing
                "feature_selection_method": config.get(
            ...
            if config.get("enable_multi_viz", False):
                ...
                classifier_config.update(
                    {
                        "enable_multi_viz": True,
                        "visualization_methods": config.get(
                            "visualization_methods", ["tsne"]
                        ),
                        "layout_strategy": config.get("layout_strategy", "sequential"),
                        "reasoning_focus": config.get(
            ...
            if config.get("use_semantic_names", False) and config.get(
                "semantic_class_names"
            ):
                fit_kwargs["class_names"] = config["semantic_class_names"][
            ...
            try:
                ...
                if "prediction_details" in results and results["prediction_details"]:
                    ...
                    for detail in prediction_details:
                        if "vlm_response" in detail:
                            ...
                            all_responses.append(
                                f"Class: Class_{parsed_pred} | Reasoning: Parsed prediction from MARVIS"
                            )

                    logger.info(
                        f"Extracted {len(all_responses)} actual VLM responses from prediction_details"
                    )
                    ...
                    if config.get("enable_multi_viz", False):
                        ...
                        multi_viz_info = []
                        if (
                            hasattr(classifier, "context_composer")
                            and classifier.context_composer
                        ):
                    ...
                    if isinstance(predictions, (list, np.ndarray)):
                        all_responses = [
                            f"Class: Class_{pred} | Reasoning: MARVIS prediction (no detailed response)"
                    ...
                    if config.get("use_semantic_names", False) and config.get(
                        ...
                        fallback_class_names = config["semantic_class_names"][
                            : len(np.unique(y_train))
                        ]
                    else:
                ...
                if "prediction_details" in results and results["prediction_details"]:
                    ...
                    if metrics:
                        ...
                        metrics_logger = MetricsLogger(
                            ...
                            dataset_name=self.dataset_info.get(
                                "name", f"dataset_{self.task_id}"
                            ),
                        ...
                        logger.info(
                            f"✓ Test {test_idx + 1} metrics: Accuracy={metrics['accuracy']:.3f}, F1={metrics['f1_macro']:.3f}"
                        )
                    else:
                        logger.warning(
                            f"Could not calculate metrics for test {test_idx + 1}"
                        )
                ...
                viz_path = (
                    self.output_dir
                    / "visualizations"
                    / f"test_{test_idx:02d}_{config['name']}.png"
                )

                # MARVIS saves visualizations in its temp directory during prediction
                viz_found = False
                if (
                    hasattr(classifier, "temp_dir")
                    and classifier.temp_dir
                    and os.path.exists(classifier.temp_dir)
                ):
                    temp_viz_files = list(Path(classifier.temp_dir).glob("*.png"))
                    if temp_viz_files:
                        # Copy the most recent visualization file
                        latest_viz = max(
                            temp_viz_files, key=lambda p: p.stat().st_mtime
                        )
                        shutil.copy2(latest_viz, viz_path)
                        logger.info(
                            f"Copied MARVIS visualization from {latest_viz} to {viz_path}"
                        )
                        viz_found = True
                        ...
                        if len(temp_viz_files) > 1:
                            for i, viz_file in enumerate(temp_viz_files):
                                additional_viz_path = (
                                    test_dir / f"visualization_{i}.png"
                                )
                                shutil.copy2(viz_file, additional_viz_path)

                if not viz_found:
                    logger.warning(
                        f"No visualization found for test {test_idx + 1}. Check MARVIS's temp_dir: {getattr(classifier, 'temp_dir', 'Not set')}"
                    )

                logger.info(
                    f"✓ Test {test_idx + 1} completed successfully using real MARVIS pipeline"
                )
                ...
                ground_truth_labels = [int(label) for label in y_test]
                all_responses = [
                    f"Class: FAILED | Reasoning: Pipeline error: {str(e)}"
                ] * len(X_test)
                all_prompts = ["FAILED"] * len(X_test)
                viz_path = (
                    self.output_dir
                    / "visualizations"
                    / f"test_{test_idx:02d}_{config['name']}_failed.png"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Created minimal detailed_vlm_outputs.json for failed test: {detailed_output_path}"
                    )
                except Exception as json_error:
                    logger.error(
                        f"Failed to create detailed_vlm_outputs.json for failed test: {json_error}"
                    )
            ...
            prompt_path = (
                self.output_dir
                / "prompts"
                / f"test_{test_idx:02d}_{config['name']}_all.txt"
            )
            ...
            if config.get("use_semantic_names", False) and config.get(
                ...
                gt_class_names = config["semantic_class_names"][
                    : len(np.unique(y_train))
                ]
            else:
            ...
            ground_truth_path = (
                self.output_dir
                / "ground_truth"
                / f"test_{test_idx:02d}_{config['name']}.json"
            )
            ground_truth_data = {
                "test_indices": (
                    self.test_indices.tolist() if self.test_indices is not None else []
                ),
            ...
            vlm_outputs_valid, validation_error = self._validate_detailed_vlm_outputs(
            ...
            result = {
                ...
                "ground_truth_path": str(
                    ground_truth_path.relative_to(self.output_dir)
                ),
                ...
                "detailed_vlm_outputs_path": str(
                    detailed_output_path.relative_to(self.output_dir)
                ),
            }
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        if hasattr(self, "_target_configs") and self._target_configs:
            matching_configs = [
                config for config in configs if config["name"] in self._target_configs
            ]
            if matching_configs:
                configs = matching_configs
                if len(self._target_configs) == 1:
                    logger.info(
                        f"Running specific test configuration: {self._target_configs[0]}"
                    )
                else:
                    logger.info(
                        f"Running {len(self._target_configs)} specific test configurations: {', '.join(self._target_configs)}"
                    )
            else:
                logger.error(
                    f"Target configurations not found: {', '.join(self._target_configs)}"
                )
                return {}
        # Handle legacy single target config (for backward compatibility)
        elif hasattr(self, "_target_config") and self._target_config:
            matching_configs = [
                config for config in configs if config["name"] == self._target_config
            ]
            if matching_configs:
                configs = [matching_configs[0]]
                logger.info(
                    f"Running specific test configuration: {self._target_config}"
                )
            else:
            ...
            if self.num_tests is not None and self.num_tests < len(configs):
                ...
                logger.info(
                    f"Limited to {self.num_tests} test configurations (out of {len(self._get_test_configurations())} available)"
                )

        logger.info(
            f"Running {len(configs)} test configurations with {self.num_samples_per_test} samples each..."
        )
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        vlm_outputs_valid_tests = [
            r for r in self.test_results if r.get("detailed_vlm_outputs_valid", False)
        ]
        vlm_outputs_invalid_tests = [
            r
            for r in self.test_results
            if not r.get("detailed_vlm_outputs_valid", False)
        ]
        pipeline_failed_tests = [
            r for r in failed_tests if "pipeline failed" in r.get("error", "").lower()
        ]
        validation_failed_tests = [
            r for r in failed_tests if "validation failed" in r.get("error", "").lower()
        ]

        single_viz_tests = [
            r for r in successful_tests if not r.get("is_multi_viz", False)
        ]
        ...
        for result in successful_tests:
            if result.get("metrics"):
                ...
                if metrics.get("balanced_accuracy") is not None:
                    overall_metrics["balanced_accuracy"].append(
        ...
        for metric_name in ["accuracy", "f1_macro", "balanced_accuracy"]:
            ...
            if best_config:
                best_configs[metric_name] = {"config": best_config, "score": best_score}

        # Compare single-viz vs multi-viz performance
        single_viz_metrics = [
            r["metrics"] for r in single_viz_tests if r.get("metrics")
        ]
        ...
        if single_viz_metrics and multi_viz_metrics:
            for metric_name in ["accuracy", "f1_macro", "balanced_accuracy"]:
                single_scores = [
                    m[metric_name]
                    for m in single_viz_metrics
                    if m.get(metric_name) is not None
                ]
                multi_scores = [
                    m[metric_name]
                    for m in multi_viz_metrics
                    if m.get(metric_name) is not None
                ]
        ...
        summary = {
            ...
            "average_prompt_length": np.mean(
                [r.get("avg_prompt_length", 0) for r in successful_tests]
            ),
            "total_test_samples": sum(
                [r.get("num_test_samples", 0) for r in successful_tests]
            ),
            ...
            "validation_statistics": {
                ...
                "validation_success_rate": (
                    len(vlm_outputs_valid_tests) / len(self.test_results) * 100
            ...
            "files_generated": {
                "visualizations": len(
                    list((self.output_dir / "visualizations").glob("*.png"))
                ),
                ...
                "ground_truth": len(
                    list((self.output_dir / "ground_truth").glob("*.json"))
                ),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive VLM Prompting Test Suite"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./test_vlm_outputs",
    ...
    if args.test_config:
        ...
        for config_name in args.test_config:
            matches = [
                config for config in test_configs if config["name"] == config_name
            ]
            if matches:
        ...
        if missing_configs:
            print(
                f"Error: Test configuration(s) not found: {', '.join(missing_configs)}"
            )
        ...
        if len(args.test_config) == 1:
            ...
            print(
                f"Running {len(args.test_config)} specific test configurations: {', '.join(args.test_config)}"
            )
    ...
    if summary.get("overall_metrics"):
        ...
        if summary.get("single_vs_multi_viz_comparison"):
            ...
            for metric, comp in summary["single_vs_multi_viz_comparison"].items():
                ...
                print(
                    f"  {metric}: Single={comp['single_viz_mean']:.3f}, Multi={comp['multi_viz_mean']:.3f}"
                )
        ...
        if summary.get("insights"):
            ...
            if insights.get("best_overall_config"):
                print(
                    f"  • Best overall configuration: {insights['best_overall_config']}"
                )
            if insights.get("most_tested_viz_method"):
                print(
                    f"  • Most tested visualization method: {insights['most_tested_viz_method']}"
                )
            if insights.get("multi_viz_advantage"):
                multi_advantages = [
                    k for k, v in insights["multi_viz_advantage"].items() if v
                ]
                if multi_advantages:
                    print(
                        f"  • Multi-viz shows advantage in: {', '.join(multi_advantages)}"
                    )
