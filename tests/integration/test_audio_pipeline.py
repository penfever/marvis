#!/usr/bin/env python3
"""
Integration tests for MARVIS audio processing pipeline.
...
def test_whisper_embeddings():
    ...
    try:
        ...
        embeddings = get_whisper_embeddings(
            test_paths, model_name="tiny", cache_dir="./cache", device="cpu"
        )
...
def test_tsne_visualization():
    ...
    try:
        ...
        logger.info(
            f"✓ t-SNE visualization created: train={train_tsne.shape}, test={test_tsne.shape}"
        )
...
def test_dataset_loading():
    ...
    try:
        ...
        logger.info(
            f"✓ Dataset loaded: {len(train_paths)} train, {len(test_paths)} test samples"
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for class_idx in range(num_classes):
            for sample_idx in range(num_samples_per_class):
                ...
                audio_path = (
                    Path(temp_dir) / f"class_{class_idx}_sample_{sample_idx}.wav"
                )
...
def test_whisper_knn_baseline():
    ...
    try:
        ...
        # Create test data
        audio_paths, labels, class_names = create_synthetic_test_data(
            num_samples_per_class=3, num_classes=2
        )
...
def test_clap_zero_shot_baseline():
    ...
    try:
        ...
        # Create test data
        audio_paths, labels, class_names = create_synthetic_test_data(
            num_samples_per_class=2, num_classes=2
        )
    ...
    try:
        ...
        for class_idx in range(2):
            for sample_idx in range(2):
                ...
                audio = create_synthetic_audio(
                    frequency=frequency, duration=1.0, sample_rate=16000
                )
...
def run_comprehensive_tests():
    ...
    logger.info(
        f"Original Pipeline Tests: {'✅ PASS' if original_success else '❌ FAIL'}"
    )
