...
class MarvisTsneRegressionMetricsTestSuite:
    ...
    def create_regression_dataset(
        self, n_samples: int = 100, n_features: int = 5, noise: float = 0.1
    ):
    ...
    def create_classification_dataset(
        self, n_samples: int = 100, n_features: int = 5, n_classes: int = 3
    ):
    ...
    def test_task_type_detection(self):
        ...
        try:
            ...
            task_type_reg, method_reg = detect_task_type(
                y=y_reg, manual_override="regression"
            )
            logger.info(
                f"Regression dataset detected as: {task_type_reg} (method: {method_reg})"
            )
            ...
            if task_type_reg == "regression":
                ...
                logger.error(
                    f"❌ Regression dataset incorrectly detected as {task_type_reg}"
                )
            ...
            task_type_cls, method_cls = detect_task_type(
                y=y_cls, manual_override="classification"
            )
            logger.info(
                f"Classification dataset detected as: {task_type_cls} (method: {method_cls})"
            )

            if task_type_cls == "classification":
                logger.info(
                    "✅ Classification dataset correctly detected as classification"
                )
                classification_detection_correct = True
            else:
                logger.error(
                    f"❌ Classification dataset incorrectly detected as {task_type_cls}"
                )
    ...
    def test_regression_metrics_calculation(self):
        ...
        try:
            ...
            logger.info(
                f"Regression metrics calculated: {list(metrics_regression.keys())}"
            )

            # Check for regression-specific metrics
            expected_regression_metrics = ["mse", "rmse", "mae", "r2_score"]
            unexpected_classification_metrics = [
                "accuracy",
                "balanced_accuracy",
                "f1_macro",
                "precision_macro",
            ...
            if has_regression_metrics:
                logger.info(
                    f"✅ Found expected regression metrics: {expected_regression_metrics}"
                )
            else:
                missing = [
                    m
                    for m in expected_regression_metrics
                    if m not in metrics_regression
                ]
    ...
    def test_classification_metrics_calculation(self):
        ...
        try:
            ...
            logger.info(
                f"Classification metrics calculated: {list(metrics_classification.keys())}"
            )
            ...
            has_classification_metrics = all(
            ...
            if has_classification_metrics:
                ...
                missing = [
                    m
                    for m in expected_classification_metrics
                    if m not in metrics_classification
                ]
            ...
            logger.error(
                f"❌ Error during classification metrics calculation test: {e}"
            )
            import traceback
    ...
    def test_marvis_tsne_regression_integration(self):
        ...
        try:
            ...
            X, y, feature_names = self.create_regression_dataset(
                n_samples=50, n_features=3
            )

            logger.info(
                f"Created regression dataset: {X.shape} features, y range: {y.min():.3f} to {y.max():.3f}"
            )
            ...
            if classifier.task_type == "regression":
                ...
                logger.error(
                    f"❌ MARVIS T-SNE incorrectly detected task type as {classifier.task_type}"
                )
            ...
            try:
                ...
                has_regression_metrics = any(
                    metric in results for metric in expected_regression_metrics
                )
                has_classification_metrics = any(
                    metric in results for metric in unexpected_classification_metrics
                )

                if has_regression_metrics:
                    logger.info("✅ Found regression metrics in MARVIS T-SNE results")
                    found_reg_metrics = [
                        m for m in expected_regression_metrics if m in results
                    ]
                    logger.info(f"   Regression metrics found: {found_reg_metrics}")
                else:
                    logger.error(
                        "❌ No regression metrics found in MARVIS T-SNE results"
                    )

                if not has_classification_metrics:
                    logger.info(
                        "✅ No classification metrics found in regression results"
                    )
                else:
                    found_cls_metrics = [
                        m for m in unexpected_classification_metrics if m in results
                    ]
                    logger.error(
                        f"❌ Found unexpected classification metrics: {found_cls_metrics}"
                    )

                return (
                    task_type_correct
                    and has_regression_metrics
                    and not has_classification_metrics
                )

            except Exception as prediction_error:
                logger.warning(
                    f"⚠️ Prediction failed (expected in test environment): {prediction_error}"
                )
                logger.info(
                    "This is expected if VLM is not available - the important part is task type detection"
                )
                return task_type_correct

        except Exception as e:
            logger.error(
                f"❌ Error during MARVIS T-SNE regression integration test: {e}"
            )
            import traceback
...
def main():
    ...
    tests = [
        ...
        (
            ...
            test_suite.test_regression_metrics_calculation,
        ),
        (
            "Classification Metrics Calculation",
            test_suite.test_classification_metrics_calculation,
        ),
        (
            "MARVIS T-SNE Regression Integration",
            test_suite.test_marvis_tsne_regression_integration,
        ),
    ]
