...
class JoltConfigLoadingTestSuite:
    ...
    def create_test_config(
        self, task_id: int, dataset_name: str, config_dir: Path
    ) -> Dict[str, Any]:
        ...
        test_config = {
            ...
            "class_descriptions": {"0": "Class zero", "1": "Class one"},
        }
    ...
    def create_legacy_test_config(
        self, dataset_name: str, config_dir: Path
    ) -> Dict[str, Any]:
            ...
            for test_case in test_cases:
                if test_case["create_task_config"]:
                    self.create_test_config(
                        test_case["task_id"], test_case["dataset_name"], config_dir
                    )
                if test_case["create_legacy_config"]:
                    self.create_legacy_test_config(
                        test_case["dataset_name"], config_dir
                    )
            ...
            for test_case in test_cases:
                ...
                if test_case["should_find_config"]:
                    if config_loaded is not None:
                        logger.info(
                            f"✅ Successfully loaded config for {test_case['name']}"
                        )
                        logger.info(
                            f"   Config dataset_name: {config_loaded.get('dataset_name')}"
                        )
                        logger.info(
                            f"   Config has {len(config_loaded.get('column_descriptions', {}))} column descriptions"
                        )
                        passed_tests += 1
                    else:
                        logger.error(
                            f"❌ Failed to load config for {test_case['name']} (expected to find config)"
                        )
                else:
                    if config_loaded is None:
                        logger.info(
                            f"✅ Correctly found no config for {test_case['name']}"
                        )
                        passed_tests += 1
                    else:
                        logger.error(
    ...
    def load_jolt_config_for_dataset(
        self, dataset: Dict[str, Any], config_dir: Path
        ...
        try:
            ...
            if task_config_path.exists():
                ...
                logger.info(
                    f"Using JOLT metadata for task {dataset['id']} ({dataset['name']}) from task-based config"
                )
            else:
                ...
                if name_config_path.exists():
                    with open(name_config_path, "r") as f:
                        jolt_config = json.load(f)
                    logger.info(
                        f"Using JOLT metadata for {dataset['name']} from legacy name-based config"
                    )
                else:
                    logger.info(
                        f"No JOLT metadata found for task {dataset['id']} ({dataset['name']})"
                    )
        ...
        try:
            ...
            semantic_info = {
                ...
                "columns": [
                    {
                        "name": "feature1",
                        "semantic_description": "First feature for testing",
                    },
                    {
                        "name": "feature2",
                        "semantic_description": "Second feature for testing",
                    },
    ...
    def test_end_to_end_config_usage(self):
        ...
        try:
            ...
            if jolt_config_used:
                logger.info(
                    "✅ Config loaded successfully - jolt_config_used would be True"
                )
                logger.info(f"   Config context: {loaded_config.get('context', 'N/A')}")
                logger.info(
                    f"   Config column descriptions: {len(loaded_config.get('column_descriptions', {}))}"
                )
                return True
            else:
...
def test_jolt_config_loading_logic():
    ...
    result = test_suite.test_config_loading_logic()
    assert result, "JOLT config loading logic test failed"


@pytest.mark.integration
def test_jolt_synthesize_and_load_integration():
    """Test integration between synthesize_jolt_data.py and config loading."""
    test_suite = JoltConfigLoadingTestSuite()
    result = test_suite.test_synthesize_and_load_integration()
    assert result, "JOLT synthesize and load integration test failed"


@pytest.mark.integration
def test_jolt_end_to_end_config_usage():
    """Test end-to-end JOLT config usage."""
    test_suite = JoltConfigLoadingTestSuite()
    result = test_suite.test_end_to_end_config_usage()
    assert result, "JOLT end-to-end config usage test failed"
...
def main():
    ...
    tests = [
        ...
        (
            "Synthesize and Load Integration",
            test_suite.test_synthesize_and_load_integration,
        ),
