...
class ExampleParameterValidator:
        ...
        try:
            ...
            self.errors.append(
                ParameterValidationError(
                    str(file_path), 0, "FILE_ERROR", f"Error reading file: {e}"
                )
            )
    ...
    def _check_line_for_issues(self, file_path: Path, line_num: int, line: str):
        ...
        for deprecated, preferred in DEPRECATED_PARAMS.items():
            ...
            if (
                f"getattr(args, '{deprecated}'" in line
                or f'getattr(args, "{deprecated}"' in line
            ):
                self.errors.append(
                    ParameterValidationError(
                        str(file_path),
    ...
    def run_validation(self) -> bool:
        ...
        print(
            f"🔍 Validating parameter usage in examples directory: {self.examples_dir}"
        )
        print("=" * 80)
    ...
    def _report_results(self) -> bool:
        ...
        if not self.errors:
            print(
                f"✅ SUCCESS: All {self.files_checked} example files use correct parameter names!"
            )
            print()
        ...
        print(
            f"❌ VALIDATION FAILED: Found {len(self.errors)} issues in {self.files_checked} files"
        )
        print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate parameter usage in MARVIS examples"
    )
    parser.add_argument(
        "--examples-dir",
        default="examples",
