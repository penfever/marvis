...
def create_test_args(
    task_id: int = 23, output_dir: str = None, max_context_length: int = 4096
):
    ...
    class TestArgs:
        def __init__(self):
            ...
            self.api_model = None
            self.tabllm_model = (
                "microsoft/phi-3-mini-128k-instruct"  # Small model for testing
            )
...
def test_task_id_resolution():
    ...
    for test_id, expected, description in test_cases:
        ...
        try:
            config_data, feature_mapping = load_tabllm_config_by_openml_id(
                test_id, original_feature_count=10
            )
            ...
            if config_data is not None:
                ...
                if feature_mapping:
                    ...
                    if semantic_info:
                        logger.info(
                            f"   Semantic info available: {list(semantic_info.keys())}"
                        )
                    else:
...
def test_semantic_feature_expansion():
    ...
    for target_count, description in test_cases:
        ...
        try:
            ...
            test_feature_names = (
                [f"feature_{i}" for i in range(target_count)]
                if target_count > 0
                else None
            )
            expanded_info = expand_semantic_features(
                test_semantic_info, target_count, test_feature_names
            )
            ...
            if "columns" in expanded_info:
                ...
                if target_count > 0:
                    ...
                    if len(unique_names) == len(feature_names):
                        ...
                        logger.error(
                            f"   ❌ Duplicate semantic feature names found: {len(feature_names)} total, {len(unique_names)} unique"
                        )
                        duplicates = [
                            name
                    ...
                    if test_feature_names:
                        if set(feature_names) == set(test_feature_names):
                            logger.info(
                                "   ✅ Semantic feature names exactly match actual feature names"
                            )
                        else:
                            logger.error(
                    ...
                    if expanded_count == target_count:
                        ...
                        logger.error(
                            f"   ❌ Count mismatch: got {expanded_count}, expected {target_count}"
                        )

                # Test 4: All semantic features have descriptions
                missing_descriptions = [
                    col["name"]
                    for col in expanded_info["columns"]
                ...
                if not missing_descriptions:
                    ...
                    logger.error(
                        f"   ❌ Features missing descriptions: {missing_descriptions}"
                    )
...
def test_semantic_feature_alignment():
    ...
    expanded_semantic = expand_semantic_features(
        original_semantic, len(retained_features), retained_features
    )
    ...
    if "columns" in expanded_semantic:
        ...
        if len(semantic_names) == len(retained_features):
            ...
            logger.error(
                f"❌ Count mismatch: {len(semantic_names)} semantic vs {len(retained_features)} retained"
            )

        # Test 2: Names match exactly
        if set(semantic_names) == set(retained_features):
            logger.info(
                "✅ Semantic feature names exactly match retained feature names"
            )
        else:
        ...
        test_data = pd.Series(
            [i * 10 + 5 for i in range(len(retained_features))], index=retained_features
        )

        try:
            note = generate_note_from_row(
                test_data, expanded_semantic, retained_features, exclude_target=True
            )
...
def test_online_note_generation():
    ...
    # Test data row
    test_row = pd.Series(
        [39, "State-gov", "Bachelors"], index=["age", "workclass", "education"]
    )
    ...
    try:
        ...
        expected_elements = [
            "age in years is 39",
            "type of employment is State-gov",
            "education level achieved is Bachelors",
        ]
    ...
    try:
        ...
        expanded_semantic = expand_semantic_features(
            semantic_info, 5, expanded_attributes
        )

        # Create expanded test row
        expanded_row = pd.Series(
            [39, "State-gov", "Bachelors", 50000, "Married"], index=expanded_attributes
        )

        expanded_note = generate_note_from_row(
            expanded_row, expanded_semantic, expanded_attributes
        )
...
def test_context_aware_truncation():
    ...
    try:
        ...
        total_tokens = estimate_prompt_tokens(
            few_shot_examples, test_note, question, task_description, tokenizer
        )
    ...
    for limit in context_limits:
        try:
            ...
            logger.info(
                f"   Context limit {limit}: {len(truncated)}/{len(few_shot_examples)} examples kept"
            )
...
def test_tabllm_pipeline_with_mocked_llm():
    ...
    expanded_semantic = expand_semantic_features(
        semantic_info, len(retained_features), retained_features
    )
    ...
    if len(semantic_names) == len(retained_features):
        ...
        logger.error(
            f"❌ Count mismatch: {len(semantic_names)} vs {len(retained_features)}"
        )
    ...
    missing_desc = [
        col["name"]
        for col in expanded_semantic["columns"]
        if not col.get("semantic_description")
    ]
    if not missing_desc:
        logger.info("✅ All semantic features have descriptions")
    ...
    for idx in range(len(X_test)):
        ...
        try:
            note = generate_note_from_row(
                row, expanded_semantic, retained_features, exclude_target=True
            )
            if note and len(note.strip()) > 0:
                notes_generated += 1
    ...
    if len(predictions) == len(X_test):
        ...
        logger.error(
            f"❌ Prediction count mismatch: {len(predictions)} vs {len(X_test)}"
        )
    ...
    if tests_passed == total_tests:
        ...
        logger.error(
            "❌ TabLLM pipeline tests FAILED - feature alignment issues detected"
        )
...
def test_tabllm_integration():
    ...
    logging.getLogger("marvis.utils.model_loader").setLevel(logging.WARNING)
    logging.getLogger("transformers").setLevel(logging.WARNING)
    logging.getLogger("examples.tabular.llm_baselines.tabllm_baseline").setLevel(
        logging.WARNING
    )
    ...
    try:
        ...
        if isinstance(results, dict):
            ...
            if inspection_dir.exists():
                ...
                if inspection_files:
                    logger.info(
                        f"   ✅ Note inspection files created: {len(inspection_files)} files"
                    )
                    ...
                    if "sample_notes" in inspection_data:
                        ...
                        if num_notes > 0:
                            ...
                            logger.info(
                                f"      First note preview: {first_note[:100]}..."
                            )
                else:
        ...
        logger.info(
            "   This is often due to model availability or API limits in test environment"
        )
        ...
        try:
            ...
            for i, is_categorical in enumerate(categorical_indicator):
                if is_categorical and i < len(attribute_names):
                    unique_values = pd.Series(X[:, i]).unique()[
                        :5
                    ]  # Sample first 5 unique values
                    for val in unique_values:
                        real_ngrams.extend(extract_ngrams(str(val)))

            logger.info(
                f"   Extracted {len(set(real_ngrams))} unique n-grams from real data"
            )

            # Load semantic metadata
            logger.info(f"Loading semantic metadata for task {task_id}...")
            config_data, feature_mapping = load_tabllm_config_by_openml_id(
                task_id, original_feature_count=len(attribute_names)
            )

            if config_data is None or feature_mapping is None:
                logger.warning(
                    f"⚠️ No semantic data found for task {task_id}, skipping validation"
                )
                continue
            ...
            if "columns" in semantic_info:
                for col in semantic_info["columns"]:
                    ...
                    if "semantic_description" in col:
                        semantic_ngrams.extend(
                            extract_ngrams(col["semantic_description"])
                        )

            # From feature descriptions (alternative format)
            if "feature_description" in semantic_info:
                for feature, description in semantic_info[
                    "feature_description"
                ].items():
            ...
            logger.info(
                f"   Extracted {len(set(semantic_ngrams))} unique n-grams from semantic data"
            )
            ...
            if common_ngrams:
                ...
                common_counter = Counter(
                    [ng for ng in real_ngrams + semantic_ngrams if ng in common_ngrams]
                )
                top_common = common_counter.most_common(5)
                logger.info(
                    f"   Top common n-grams: {[ng for ng, count in top_common]}"
                )
            ...
            min_meaningful_common = max(
                3, len(attribute_names) // 10
            )  # At least 3 or 10% of features
            ...
            if meaningful_common:
                ...
                domain_keywords = {
                    ...
                    23: [
                        ...
                        "children",
                        "husband",
                    ],  # demographic terms for task 23
                }

                expected_domain_terms = domain_keywords.get(task_id, [])
                domain_matches = [
                    term
                    for term in meaningful_common
                    if any(keyword in term.lower() for keyword in expected_domain_terms)
                ]
                domain_specific_score = len(domain_matches) / max(
                    1, len(expected_domain_terms)
                )
            ...
            if len(meaningful_common) >= min_meaningful_common:
                reasons.append(
                    f"Meaningful common n-grams: {len(meaningful_common)} >= {min_meaningful_common}"
                )
                validation_passed = True
            else:
                reasons.append(
                    f"Insufficient meaningful common n-grams: {len(meaningful_common)} < {min_meaningful_common}"
                )

            if overlap_score >= min_overlap_score:
                reasons.append(
                    f"Overlap score: {overlap_score:.3f} >= {min_overlap_score}"
                )
                validation_passed = True
            else:
                reasons.append(
                    f"Low overlap score: {overlap_score:.3f} < {min_overlap_score}"
                )
            ...
            if validation_passed:
                ...
                if meaningful_common:
                    logger.info(
                        f"   Meaningful common n-grams: {meaningful_common[:10]}"
                    )
            else:
            ...
            try:
                ...
                note = generate_note_from_row(
                    test_row, semantic_info, attribute_names, exclude_target=True
                )
...
def test_real_cc18_semantic_data():
    ...
    try:
        config_data, feature_mapping = load_tabllm_config_by_openml_id(
            23, original_feature_count=9
        )

        if config_data is None or feature_mapping is None:
            logger.warning(
                "⚠️ Could not load real CC18 data for task 23 - skipping test"
            )
            return
        ...
        if "feature_description" in real_semantic_info:
            ...
            logger.info(
                f"   Original features in semantic data: {original_feature_count}"
            )
            ...
            logger.info(
                f"   Simulated retained features: {len(simulated_retained_features)}"
            )
            ...
            if "feature_description" in expanded_semantic:
                expanded_features = list(
                    expanded_semantic["feature_description"].keys()
                )

                # Test 1: Count matches
                if len(expanded_features) == len(simulated_retained_features):
                    logger.info(
                        "✅ Real data: Semantic feature count matches retained count"
                    )
                    tests_passed += 1
                else:
                    logger.error(
                        f"❌ Real data: Count mismatch {len(expanded_features)} vs {len(simulated_retained_features)}"
                    )

                # Test 2: Names match exactly
                if set(expanded_features) == set(simulated_retained_features):
                    logger.info(
                        "✅ Real data: Semantic feature names match retained names"
                    )
                    tests_passed += 1
                else:
                ...
                missing_desc = [
                    feat
                    for feat, desc in expanded_semantic["feature_description"].items()
                    if not desc
                ]
                if not missing_desc:
                    ...
                    logger.error(
                        f"❌ Real data: Missing descriptions for: {missing_desc}"
                    )
                ...
                try:
                    ...
                    if note and len(note.strip()) > 0:
                        ...
                        if "Age of wife in years" in note and "29" in note:
                            logger.info(
                                "   ✅ Note contains semantic descriptions from real data"
                            )
                        else:
                            logger.warning(
                                "   ⚠️ Note may not contain expected semantic content"
                            )
                    else:
                ...
                logger.error(
                    "❌ Expanded semantic data missing feature_description structure"
                )
        else:
            logger.warning(
                "⚠️ Real semantic data doesn't use feature_description structure"
            )
...
def test_balance_scale_empty_notes_bug():
    ...
    try:
        ...
        expanded_semantic = expand_semantic_features(
            semantic_info, 4, actual_feature_names
        )
        ...
        if "columns" in expanded_semantic:
            ...
            if set(expanded_names) == set(actual_feature_names):
                logger.info(
                    "✅ Bug fix working: Semantic names match actual feature names"
                )
                tests_passed += 1
            else:
            ...
            if not missing_desc:
                logger.info(
                    "✅ Bug fix working: All features have semantic descriptions"
                )
                tests_passed += 1
            else:
                logger.error(
                    f"❌ Bug fix issue: Missing descriptions for: {missing_desc}"
                )
...
def test_note_inspection_system():
    ...
    few_shot_examples = [
        (
            "The age is 25. The workclass is Private. The education is Bachelors.",
            "<=50K",
        ),
        (
            "The age is 38. The workclass is Self-emp-not-inc. The education is HS-grad.",
            ">50K",
        ),
        (
            "The age is 45. The workclass is Local-gov. The education is Masters.",
            ">50K",
        ),
    ]
    ...
    try:
        save_sample_notes_for_inspection(
            few_shot_examples, test_dataset, args, test_semantic_info
        )
...
def main():
    ...
    parser.add_argument(
        "--task_id", type=int, default=23, help="OpenML task ID to test with"
    )
    parser.add_argument(
        "--output_dir", type=str, help="Output directory for test files"
    )
    parser.add_argument(
        "--skip_integration", action="store_true", help="Skip integration test"
    )
