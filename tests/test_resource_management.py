...
def test_config_path_resolution():
    ...
    tabllm_mapping = rm.path_resolver.get_config_path(
        "tabllm", "openml_task_mapping.json"
    )
...
def test_metadata_validation():
    ...
    assert any(
        "No JOLT config found for OpenML task ID" in error for error in result["errors"]
    )
