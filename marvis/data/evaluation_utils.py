...
def load_datasets_for_evaluation(args) -> List[Dict[str, Any]]:
    ...
    if args.dataset_name:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Could not resolve OpenML identifiers for {dataset_name}: {resolve_error}"
                )
            ...
            logger.info(
                f"Successfully loaded dataset {dataset_name} (ID: {dataset_id}, task_id: {task_id})"
            )
    ...
    # Case 2b: Load from task IDs
        ...
        logger.info(
            f"Loading {len(task_ids)} datasets from provided task IDs: {task_ids}"
        )
        preserve_regression = getattr(args, "preserve_regression", False)
        ...
        for task_id in task_ids:
            try:
                ...
                logger.info(
                    f"Successfully loaded dataset {dataset_name} (task ID: {task_id}, dataset ID: {dataset_id})"
                )
            except Exception as e:
    ...
    # Case 3: Load from directory of CSV files
        ...
        for csv_file in csv_files:
            try:
                ...
                if len(df.columns) < 2:
                    logger.warning(
                        f"Dataset {csv_file} has fewer than 2 columns, skipping"
                    )
                    continue
    ...
    # Case 4: Sample random datasets from OpenML
        ...
        try:
            ...
            for dataset_name, dataset_id in dataset_items:
                ...
                try:
                    X, y, categorical_indicator, attribute_names, loaded_name = (
                        load_dataset(
                            str(dataset_id),
                            bypass_size_check=True,
                            preserve_regression=preserve_regression,
                    ...
                    logger.info(
                        f"Successfully loaded random dataset {loaded_name} (ID: {dataset_id})"
                    )
                    loaded_count += 1
            ...
            logger.info(
                f"Successfully loaded {loaded_count} out of {args.num_datasets} requested random datasets"
            )
...
def validate_training_sample_args(args) -> None:
    ...
    if balanced_few_shot and num_few_shot_examples is None:
        raise ValueError(
            "--balanced_few_shot requires --num_few_shot_examples to be specified"
        )

    if num_few_shot_examples is not None and num_few_shot_examples <= 0:
        raise ValueError(
            f"--num_few_shot_examples must be positive, got {num_few_shot_examples}"
        )
...
def standardize_training_samples(
    X_train: np.ndarray, y_train: np.ndarray, args, random_state: int = 42
) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    if balanced_few_shot and max_train_samples is not None:
        logger.warning(
            "Both --balanced_few_shot and --max_train_samples specified. Using --balanced_few_shot approach."
        )

    if num_few_shot_examples is not None and not balanced_few_shot:
        logger.warning(
            "--num_few_shot_examples specified without --balanced_few_shot. Parameter will be ignored. Use --balanced_few_shot to enable per-class sampling."
        )
    ...
    if balanced_few_shot and num_few_shot_examples is not None:
        ...
        logger.info(
            f"Using balanced few-shot: {num_few_shot_examples} examples per class ({num_classes} classes) = {effective_max_samples} total samples"
        )
        ...
        logger.info(
            f"Using max_train_samples: {effective_max_samples} total samples with {effective_strategy} strategy"
        )

    else:
        # No sampling limits specified
        logger.info(
            "No training sample limits specified, using all available training data"
        )
        return X_train, y_train

    # Apply sampling using existing function
    return apply_sampling_strategy(
        X_train, y_train, effective_max_samples, effective_strategy, random_state
    )
...
def preprocess_dataset_for_evaluation(dataset: Dict[str, Any], args) -> Dict[str, Any]:
    ...
    task_type, detection_method = detect_task_type(
        dataset=dataset, y=y, manual_override=manual_task_type, task_id=task_id
    )
    ...
    X_train, y_train = standardize_training_samples(
        X_train, y_train, args, random_state=args.seed
    )
...
def preprocess_datasets_for_evaluation(
    datasets: List[Dict[str, Any]], args
) -> List[Dict[str, Any]]:
    """
    ...
    logger.info(
        f"Successfully preprocessed {len(processed_datasets)} out of {len(datasets)} datasets"
    )
...
def validate_dataset_for_evaluation(dataset: Dict[str, Any]) -> bool:
    ...
    if len(dataset["X_train"]) < 10:
        logger.warning(
            f"Dataset {dataset['name']} has too few training samples: {len(dataset['X_train'])}"
        )
        return False

    if len(dataset["X_test"]) < 5:
        logger.warning(
            f"Dataset {dataset['name']} has too few test samples: {len(dataset['X_test'])}"
        )
        return False
    ...
    if len(unique_classes) < 2:
        logger.warning(
            f"Dataset {dataset['name']} has only {len(unique_classes)} class(es)"
        )
        return False
