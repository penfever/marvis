...
def load_whisper_model(
    model_name: str = "large-v2", device: Optional[str] = None
) -> Tuple[Any, Any]:
    """
...
def get_clap_embeddings(
    ...
    if cache_dir is not None:
        ...
        dataset_hash = generate_audio_dataset_hash(
            audio_paths, list(range(len(audio_paths))), f"clap_{version}", dataset_name
        )
    ...
    try:
        ...
        with torch.no_grad():
            for i in range(0, len(audio_paths), batch_size):
                batch_paths = audio_paths[i : i + batch_size]
                logger.info(
                    f"Processing batch {i//batch_size + 1}/{(len(audio_paths) + batch_size - 1)//batch_size}"
                )
