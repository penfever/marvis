...
def _load_failed_dataset_cache() -> Set[int]:
    ...
    try:
        ...
        if cached_data and isinstance(cached_data, list):
            ...
            logger.info(
                f"Loaded {len(_FAILED_DATASET_CACHE)} failed dataset IDs from managed cache"
            )
        ...
        try:
            with open(legacy_cache_path, "r") as f:
                ...
                if isinstance(cache_data, list):
                    ...
                    logger.info(
                        f"Loaded {len(_FAILED_DATASET_CACHE)} failed dataset IDs from legacy cache"
                    )
...
def _save_failed_dataset_cache() -> None:
    ...
    try:
        ...
        success = rm.cache_manager.save_to_cache(
            "system", cache_key, list(_FAILED_DATASET_CACHE), ".json"
        )

        if success:
            logger.info(
                f"Saved {len(_FAILED_DATASET_CACHE)} failed dataset IDs to managed cache"
            )
        else:
            ...
            logger.info(
                f"Saved {len(_FAILED_DATASET_CACHE)} failed dataset IDs to legacy cache"
            )
    except Exception as e:
...
def get_dataset_info(dataset_id_or_name: Union[str, int]) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ImportError(
            "OpenML package is required. Install it with 'pip install openml'."
        )
        ...
        if dataset_id_or_name in dataset_ids:
            ...
            try:
                datasets_df = openml.datasets.list_datasets(
                    output_format="dataframe", search=dataset_id_or_name
                )
                if not datasets_df.empty:
                    ...
                    raise ValueError(
                        f"Could not find any dataset matching '{dataset_id_or_name}' on OpenML."
                    )
            except Exception as e:
                logger.error(f"Error searching for dataset: {e}")
                raise ValueError(
                    f"Could not find dataset '{dataset_id_or_name}'. Please provide a valid OpenML dataset ID or name."
                )
    ...
    try:
        ...
        if dataset.features is None:
            logger.warning(
                f"Dataset {dataset_id} has no features information available"
            )
        ...
        info = {
            ...
            "num_features": (
                len(dataset.features) if dataset.features is not None else 0
            ),
            "num_samples": dataset.qualities.get("NumberOfInstances", "unknown"),
            "num_classes": dataset.qualities.get("NumberOfClasses", "unknown"),
            "num_missing_values": dataset.qualities.get(
                "NumberOfMissingValues", "unknown"
            ),
...
def load_dataset(
    dataset_name: str,
    bypass_size_check: bool = False,
    preserve_regression: bool = False,
) -> Tuple[np.ndarray, np.ndarray, List[bool], List[str], str]:
    """
    ...
    try:
        ...
        raise ImportError(
            "OpenML package is required to load datasets. Install it with 'pip install openml'."
        )
    ...
    # Try to find by name
        ...
        try:
            ...
            datasets_df = openml.datasets.list_datasets(
                output_format="dataframe", search=dataset_name
            )
            if not datasets_df.empty:
                # Get the first match
                first_match = datasets_df.iloc[0]
                dataset_id = first_match["did"]
                logger.info(
                    f"Found dataset matching '{dataset_name}': ID {dataset_id} - {first_match['name']}"
                )
            else:
                raise ValueError(
                    f"Could not find any dataset matching '{dataset_name}' on OpenML."
                )
        except Exception as e:
            logger.error(f"Error searching for dataset: {e}")
            raise ValueError(
                f"Could not find or load dataset '{dataset_name}'. Please provide a valid OpenML dataset name or ID."
            )

    # Check if the dataset ID is in the failed dataset cache
    if dataset_id in _FAILED_DATASET_CACHE:
        logger.warning(
            f"Dataset ID {dataset_id} has previously failed to load and will be skipped."
        )
        ...
        if len(available_datasets) > 0:
            ...
            suggestions = random.sample(
                list(available_datasets.items()), min(5, len(available_datasets))
            )
        ...
        raise ValueError(
            f"Dataset with ID {dataset_id} was previously marked as failed. {suggestion_msg}"
        )
    ...
    try:
        ...
        if num_samples != "unknown" and int(num_samples) < 1000:
            if bypass_size_check:
                logger.warning(
                    f"Dataset ID {dataset_id} has only {num_samples} samples, which is fewer than the minimum 1000 required, but bypassing size check."
                )
            else:
                logger.warning(
                    f"Dataset ID {dataset_id} has only {num_samples} samples, which is fewer than the minimum 1000 required."
                )
                ...
                if len(available_datasets) > 0:
                    ...
                    # Pick 5 random datasets to suggest
                    suggestions = random.sample(
                        list(available_datasets.items()),
                        min(5, len(available_datasets)),
                    )
                    for name, did in suggestions:
                        suggestion_msg += f"  - {name} (ID: {did})\n"
                    suggestion_msg += f"\nUse list_available_datasets() to see all {len(available_datasets)} predefined datasets."
                ...
                raise ValueError(
                    f"Dataset ID {dataset_id} has only {num_samples} samples (minimum 1000 required). {suggestion_msg}"
                )
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            data_df, _, categorical_indicator, attribute_names = silence_pandas_warning(
                dataset.get_data, dataset_format="dataframe", target=None
            )

            # Log the default target attribute if it exists
            if dataset.default_target_attribute:
                logger.info(
                    f"Dataset has default target attribute: '{dataset.default_target_attribute}'"
                )
                ...
                if dataset.default_target_attribute in data_df.columns:
                    ...
                    logger.info(
                        f"Default target not found in columns, using last column '{target_column}' as target"
                    )
            else:
                # If no default target is specified, use the last column
                target_column = data_df.columns[-1]
                logger.info(
                    f"No default target attribute specified, using last column '{target_column}' as target"
                )
            ...
            if sparse_columns:
                logger.info(
                    f"Found {len(sparse_columns)} sparse columns in the dataset: {', '.join(sparse_columns[:5])}{'...' if len(sparse_columns) > 5 else ''}"
                )
                ...
                for col in sparse_columns:
                    try:
                        ...
                        if hasattr(data_df[col], "sparse") and hasattr(
                            data_df[col].sparse, "to_dense"
                        ):
                            data_df[col] = pd.Series(
                                data_df[col].sparse.to_dense(), index=data_df.index
                            )
                        else:
                            ...
                            try:
                                ...
                                dense_series = pd.Series(
                                    [float(x) for x in data_df[col]],
                                    index=data_df.index,
                                )
                                data_df[col] = dense_series
                            except Exception as err:
                                logger.warning(
                                    f"Direct sparse conversion failed for column '{col}': {err}. Will try element-wise later."
                                )
                    except Exception as e:
                        logger.warning(
                            f"Issue pre-converting sparse column '{col}': {e}. Will try different method during feature processing."
                        )
            ...
            for col in X_df.columns:
                ...
                if hasattr(col_dtype, "name") and "category" in col_dtype_str:
                    logger.info(
                        f"Converting categorical column '{col}' to numeric codes"
                    )
                    X_df[col] = pd.Series(
                        X_df[col].astype("category").cat.codes, index=X_df.index
                    )
                    ...
                    try:
                        ...
                        if hasattr(X_df[col], "sparse"):
                            try:
                                ...
                                X_df[col] = pd.Series(
                                    X_df[col].sparse.to_dense(), index=X_df.index
                                )
                            except Exception as err1:
                                logger.warning(
                                    f"Error using sparse.to_dense() for '{col}': {err1}. Trying alternative approach."
                                )
                                # Try converting manually from sparse array
                                if hasattr(X_df[col], "_values") and hasattr(
                                    X_df[col], "_indices"
                                ):
                                    ...
                                    X_df[col] = pd.Series(
                                        np.array(X_df[col].tolist(), dtype=np.float32),
                                        index=X_df.index,
                                    )
                        else:
                            ...
                            try:
                                ...
                                if sp.issparse(X_df[col].iloc[0]):
                                    ...
                                    X_df[col] = pd.Series(
                                        np.array(dense_list, dtype=np.float32),
                                        index=X_df.index,
                                    )
                                else:
                                ...
                                logger.warning(
                                    f"Failed to convert sparse column '{col}' using scipy: {err2}. Using zeros."
                                )
                                X_df[col] = pd.Series(
                                    np.zeros(len(X_df)), index=X_df.index
                                )
                    except Exception as sparse_err:
                        logger.warning(
                            f"All sparse conversion methods failed for '{col}': {sparse_err}. Using zeros instead."
                        )
                        X_df[col] = pd.Series(np.zeros(len(X_df)), index=X_df.index)

                # Handle other problematic dtypes
                elif not np.issubdtype(col_dtype, np.number) and not np.issubdtype(
                    col_dtype, np.bool_
                ):
                    logger.warning(
                        f"Converting column '{col}' with dtype {col_dtype} to string and then numeric codes"
                    )
                    try:
                        ...
                        le = LabelEncoder()
                        X_df[col] = pd.Series(
                            le.fit_transform(X_df[col]), index=X_df.index
                        )
                    except Exception as conv_err:
                        logger.warning(
                            f"Error converting column '{col}': {conv_err}. Using zeros instead."
                        )
                        X_df[col] = pd.Series(np.zeros(len(X_df)), index=X_df.index)
            ...
            if (
                categorical_indicator is not None
                and len(categorical_indicator) > X.shape[1]
            ):
                ...
                logger.debug(
                    f"Adjusted categorical_indicator length from {len(categorical_indicator)+1} to {len(categorical_indicator)}"
                )
            ...
            if attribute_names is not None and len(attribute_names) > X.shape[1]:
                ...
                if target_column in attribute_names:
                    attribute_names = [
                        name for name in attribute_names if name != target_column
                    ]
                    logger.debug(
                        f"Removed target column '{target_column}' from attribute_names"
                    )
            ...
            logger.warning(
                f"Error getting data in dataframe format: {e}. Trying alternative approach."
            )
            ...
            try:
                            ...
                            for i in range(min(10, X.shape[0])):
                                for j in range(min(10, X.shape[1])):
                                    if "sparse" in str(type(X[i, j])).lower():
                                        sparse_in_array = True
                                        logger.warning(
                                            f"Detected sparse elements in numpy array at position [{i},{j}]: {type(X[i, j])}"
                                        )
                                        break
                                if sparse_in_array:
                                    break
                ...
                if sparse_in_array:
                    logger.info(
                        "Converting sparse elements in numpy array to dense values"
                    )
                    try:
                        ...
                        for i in range(X.shape[0]):
                            for j in range(X.shape[1]):
                                try:
                                    ...
                                    elif hasattr(val, "sparse") and hasattr(
                                        val.sparse, "to_dense"
                                    ):
                                        X_dense[i, j] = val.sparse.to_dense()
                                    elif hasattr(val, "_values") and hasattr(
                                        val, "_indices"
                                    ):
                                        # Manually extract from COO-like format
                                        X_dense[i, j] = (
                                            val._values[0]
                                            if len(val._values) > 0
                                            else 0
                                        )
                                    else:
                                    ...
                                    logger.debug(
                                        f"Error converting element at [{i},{j}]: {elem_err}"
                                    )
                                    X_dense[i, j] = 0
                        X = X_dense
                    except Exception as conv_err:
                        logger.warning(
                            f"Error during sparse array conversion: {conv_err}"
                        )

                # Ensure X is a proper numpy array with numeric types
                if not isinstance(X, np.ndarray) or not np.issubdtype(
                    X.dtype, np.number
                ):
                    logger.warning(
                        f"Converting X to numeric array. Original type: {type(X)}, dtype: {X.dtype if hasattr(X, 'dtype') else 'unknown'}"
                    )
                ...
                if not isinstance(y, np.ndarray):
                    logger.warning(
                        f"Converting y to numpy array. Original type: {type(y)}"
                    )
                    try:
                        # For classification, try to convert to integers
                        if (
                            len(np.unique(y)) < 100
                        ):  # Assume classification if less than 100 unique values
                            from sklearn.preprocessing import LabelEncoder
                ...
                raise ValueError(
                    f"Could not load dataset {dataset_id} in either dataframe or array format"
                )
        ...
        if (
            ...
            if max_safe_bins < 2:
                logger.warning(
                    f"Too few samples ({n_samples}) or unique targets ({n_unique_targets}) for quantile binning, using uniform strategy"
                )
                ...
                discretizer = KBinsDiscretizer(
                    n_bins=max_safe_bins, encode="ordinal", strategy="quantile"
                )
                n_bins = max_safe_bins

            y = discretizer.fit_transform(y.reshape(-1, 1)).flatten().astype(int)
            logger.info(
                f"Binned continuous target into {n_bins} classes using {discretizer.strategy} strategy"
            )
        elif (
            preserve_regression
            and np.issubdtype(y.dtype, np.floating)
        ...
        if np.any(np.isnan(y)) or np.any(np.isinf(y)):
            ...
            after_count = len(y)
            logger.warning(
                f"Removed {before_count - after_count} samples with NaN/inf in target variable"
            )
        ...
        if np.any(np.isnan(X)) or np.any(np.isinf(X)):
            ...
            after_count = len(X)
            logger.warning(
                f"Removed {before_count - after_count} samples with NaN/inf in features"
            )
        ...
        num_categorical = (
            sum(categorical_indicator) if categorical_indicator else "unknown"
        )
        num_numerical = (
            num_features - num_categorical if categorical_indicator else "unknown"
        )

        logger.info(
            f"Dataset info: {num_samples} samples, {num_features} features "
        ...
        raise ValueError(
            f"Failed to load dataset with ID {dataset_id}: {e}{suggestion_msg}"
        )
...
def create_llm_dataset(
    ...
    if any(arr.dtype.kind == "f" for arr in [y_train, y_val, y_test]):
        logger.warning(
            "Detected float labels in dataset, will be encoded as distinct classes"
        )
    ...
    try:
        ...
        logger.info(
            f"Used fallback label encoding with {len(unique_labels)} unique classes"
        )
    ...
    if num_classes > len(class_token_ids):
        raise ValueError(
            f"Not enough class tokens for {num_classes} classes (have {len(class_token_ids)})"
        )

    logger.info(
        f"TabPFN embedding shapes - Train: {train_embeddings.shape}, Val: {val_embeddings.shape}, Test: {test_embeddings.shape}"
    )
    ...
    prefix_embeddings, prefix_class_labels = prepare_tabpfn_embeddings_for_prefix(
        train_embeddings,
        y_train_encoded,
        num_embeddings=num_few_shot_examples,
        random_state=42,
    )

    logger.info(
        f"Prepared prefix data - Embeddings: {prefix_embeddings.shape}, Class labels: {prefix_class_labels.shape}"
    )
    ...
    np.savez(
        prefix_data_file, embeddings=prefix_embeddings, class_labels=prefix_class_labels
    )
    logger.info(f"Saved prefix data to {prefix_data_file}")

    # Check that length of embeddings and labels match
    if len(val_embeddings) != len(y_val_encoded) or len(test_embeddings) != len(
        y_test_encoded
    ):
        logger.warning(
            f"Mismatch between embeddings and labels: val embeddings: {len(val_embeddings)}, val labels: {len(y_val_encoded)}, "
        ...
        logger.info(
            f"Aligned label and embedding counts: now using {val_length} validation samples and {test_length} test samples"
        )
    else:
        val_length = len(val_embeddings)
        test_length = len(test_embeddings)
    ...
    if max_test_samples is not None and val_length > max_test_samples:
        logger.info(
            f"Limiting validation set from {val_length} to {max_test_samples} samples"
        )
    ...
    if max_test_samples is not None and test_length > max_test_samples:
        logger.info(
            f"Limiting test set from {test_length} to {max_test_samples} samples"
        )
        ...
        val_examples.append(
            {
                "label_id": int(
                    y_val_encoded[i]
                ),  # Use numeric label ID instead of string
    ...
    if max_train_samples is not None and len(train_dataset) > max_train_samples:
        logger.info(
            f"Limiting training set from {len(train_dataset)} to {max_train_samples} samples"
        )
        train_dataset = train_dataset[:max_train_samples]

    logger.info(
        f"Created datasets - Train: {len(train_dataset)}, Eval: {len(eval_dataset)}, Test: {len(test_examples)}"
    )
...
def load_datasets(args):
    ...
    if hasattr(args, "dataset_name") and args.dataset_name:
        ...
        try:
            ...
            X, y, categorical_indicator, attribute_names, dataset_name = load_dataset(
                args.dataset_name,
    ...
    # Case 4: Sample random datasets from OpenML
        ...
        try:
            ...
            if len(benchmark_datasets) >= args.num_datasets:
                ...
                logger.warning(
                    f"Using all {len(dataset_ids)} benchmark datasets (less than requested {args.num_datasets})"
                )
