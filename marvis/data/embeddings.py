...
def generate_dataset_hash(
    X: np.ndarray, y: np.ndarray, embedding_size: int, dataset_name: str = None
) -> str:
    """
    ...
    config = {
        ...
        "class_distribution": {
            str(k): int(v) for k, v in zip(*np.unique(y_np, return_counts=True))
        },
        "embedding_size": int(embedding_size),
        # Convert to float32 to ensure consistent representation across platforms
        "data_hash": hashlib.md5(X_np[:100].astype(np.float32).tobytes()).hexdigest()[
            :10
        ],
...
def get_tabpfn_embeddings(
    ...
    try:
        ...
        raise ImportError(
            "TabPFN package is required to get embeddings. Install it with 'pip install tabpfn'."
        )
    ...
    if cache_dir:
        ...
        if use_managed_cache:
            ...
            cache_key = rm.cache_manager.get_cache_key(
                ...
                task_type=task_type,
                n_features=X_train.shape[1],
                data_hash=generate_dataset_hash(
                    X_train, y_train, embedding_size, dataset_name
                ),
            )
        else:
        ...
        if use_managed_cache:
                ...
                try:
                    ...
                    if cache_data and isinstance(cache_data, dict):
                        ...
                        if train_embeddings.shape[1] != embedding_size:
                            logger.warning(
                                f"Cached embeddings size ({train_embeddings.shape[1]}) doesn't match requested size ({embedding_size}). Resizing..."
                            )
                    ...
                    logger.warning(
                        f"Error loading cached embeddings from managed cache: {e}. Recomputing..."
                    )
            ...
            cache_file = os.path.join(
                cache_dir, f"{prefix}tabpfn_embeddings_{dataset_hash}.npz"
            )
            ...
            if os.path.exists(cache_file) and not force_recompute:
                ...
                try:
                    ...
                    cache_metadata = (
                        cache["metadata"].item() if "metadata" in cache else {}
                    )

                    logger.info(
                        f"Loaded embeddings from cache - Train: {train_embeddings.shape}, Val: None, Test: {test_embeddings.shape}"
                    )
                    logger.info(f"Cache metadata: {cache_metadata}")
                    ...
                    logger.warning(
                        f"Error loading cached embeddings: {e}. Recomputing..."
                    )
                    # Continue with normal computation if loading fails
    ...
    if task_type == "classification":
        ...
        if min_class_count < MIN_SAMPLES_PER_CLASS:
            raise ValueError(
                f"Minimum class count is {min_class_count}, which is less than {MIN_SAMPLES_PER_CLASS} required for stratified sampling"
            )

        if len(unique_classes) < 2:
            raise ValueError(
                f"Only {len(unique_classes)} unique classes found, need at least 2 classes"
            )
    else:
        # For regression, log target statistics
        logger.info(
            f"Original train set target statistics: min={y_train.min():.3f}, max={y_train.max():.3f}, mean={y_train.mean():.3f}, std={y_train.std():.3f}"
        )
    ...
    MAX_FEATURES_FOR_TABPFN = (
        1000  # TabPFN typically works well with less than 1000 features
    )
    if X_train.shape[1] > MAX_FEATURES_FOR_TABPFN:
        ...
        selected_features = rng.choice(
            range(X_train.shape[1]), n_components, replace=False
        )
    ...
    if len(X_train) > max_samples:
        ...
        if task_type == "classification":
            ...
            if min_expected_samples < MIN_SAMPLES_PER_CLASS:
                ...
                adjusted_max_samples = min(
                    len(X_train), int(len(X_train) * required_ratio * 1.1)
                )  # Add 10% buffer
                logger.warning(
                    f"Adjusting sample size from {max_samples} to {adjusted_max_samples} to ensure minimum class representation"
                )
            ...
            try:
                ...
                _, X_train_sample, _, y_train_sample = train_test_split(
                    X_train, y_train, test_size=sample_ratio, random_state=42
                )
        else:
        ...
        logger.info(
            f"Sampled {len(X_train_sample)} training samples from {len(X_train)} total samples"
        )
    else:
    ...
    if task_type == "classification":
        ...
        logger.info(
            f"Sampled train set class distribution: {dict(zip(sampled_classes, sampled_counts))}"
        )
    else:
        logger.info(
            f"Sampled train set target statistics: min={y_train_sample.min():.3f}, max={y_train_sample.max():.3f}, mean={y_train_sample.mean():.3f}"
        )
    ...
    if len(test_embeddings_raw.shape) == 2 and len(train_embeddings_raw.shape) == 3:
        ...
        test_embeddings_raw = test_embeddings_raw[
            :, np.newaxis, :
        ]  # (8, 192) -> (8, 1, 192)
    ...
    logger.info(
        f"RobustScaler statistics - Center (median): {scaler.center_[:5]}... Scale (IQR): {scaler.scale_[:5]}..."
    )
...
def get_embeddings_in_chunks(
    ...
    logger.info(
        f"Processing {dataset_name} embeddings in chunks (total: {total_samples} samples)"
    )

    # Calculate number of chunks needed
    n_chunks = (
        total_samples + max_chunk_size - 1
    ) // max_chunk_size  # Ceiling division
    chunk_embeddings = []
    ...
    if (
        len(chunk_embeddings[0].shape) == 3
    ):  # Shape is (n_ensemble, n_samples, emb_size)
    ...
    logger.info(
        f"Final concatenated {dataset_name} embeddings shape: {concatenated.shape}"
    )
    return concatenated
...
def prepare_tabpfn_embeddings_for_prefix(
    ...
    if len(y_train) != len(train_embeddings):
        raise ValueError(
            f"Length of y_train ({len(y_train)}) doesn't match train_embeddings ({len(train_embeddings)})"
        )
    ...
    # Verify shapes
    assert len(combined_embeddings) == len(
        combined_labels
    ), f"Mismatch between embeddings ({len(combined_embeddings)}) and labels ({len(combined_labels)})"

    return combined_embeddings, combined_labels


def load_dinov2_model(
    model_name: str = "dinov2_vitb14", device: Optional[str] = None
) -> torch.nn.Module:
    """
...
def prepare_image_for_dinov2(image_input, size: int = 224) -> torch.Tensor:
    """
    Prepare an image for DINOV2 processing.

    Args:
        image_input: Can be a path to image file (str), numpy array, or PIL Image
    ...
    try:
        ...
        from marvis.utils.image_utils import normalize_image_input
...
def get_dinov2_embeddings(
    ...
    if isinstance(image_inputs, np.ndarray):
        ...
        if image_inputs.ndim == 2:
            # Assume each row is a flattened image
            image_list = [image_inputs[i] for i in range(len(image_inputs))]
        else:
            # Handle single image or other formats
            image_list = [image_inputs]
    elif isinstance(image_inputs, (list, tuple)):
        # Already a list/tuple of images
        image_list = list(image_inputs)
    else:
        # Single image input
        image_list = [image_inputs]
    ...
    log_device_usage(
        f"DINOV2 embeddings {model_name}", device, {"batch_size": batch_size}
    )

    # Setup caching if enabled
    cache_file = None
    if cache_dir and dataset_name:
        # Generate hash based on image inputs and model
        # For numpy arrays, use array properties; for paths, use paths
        if isinstance(image_inputs, np.ndarray):
        ...
        if os.path.exists(cache_file) and not force_recompute:
            ...
            try:
                ...
                if embeddings.shape[1] != embedding_size:
                    logger.warning(
                        f"Cached embeddings size ({embeddings.shape[1]}) doesn't match requested size ({embedding_size}). Resizing..."
                    )
