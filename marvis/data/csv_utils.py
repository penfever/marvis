...
def find_csv_file(dataset_id: str, search_dirs: List[str]) -> Optional[str]:
    ...
    try:
        ...
        logger.debug(
            f"Resource manager CSV search failed, falling back to legacy method: {e}"
        )
        ...
        if os.path.exists(y_file_path):
            ...
            try:
                ...
                if X.shape[0] != y.shape[0]:
                    logger.warning(
                        f"Mismatch in sample count: X has {X.shape[0]} samples, y has {y.shape[0]} samples"
                    )
...
def load_dataset_with_metadata(dataset_dir: str) -> Dict[str, Any]:
    ...
    dataset_info["has_embeddings"] = (
        os.path.exists(prefix_data_file) or len(embedding_files) > 0
    )

    return dataset_info


def find_csv_with_fallbacks(
    dataset_id: str,
    ...
    try:
        ...
        logger.debug(
            f"Resource manager CSV search failed: {e}, falling back to legacy method"
        )
