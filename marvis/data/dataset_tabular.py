...
def preprocess_features(
    X: np.ndarray, categorical_indicator: List[bool], preserve_categorical: bool = False
) -> np.ndarray:
    """
    ...
    for col_idx in range(df.shape[1]):
        ...
        if col.dtype == "object" or is_categorical:
            if preserve_categorical:
                ...
                logger.info(
                    f"  Preserved categorical feature with {col_filled.nunique()} unique values"
                )
            else:
                ...
                try:
                    ...
                    if encoded_values.max() <= 127 and encoded_values.min() >= -128:
                        ...
                        encoded_values.max() <= 32767 and encoded_values.min() >= -32768
                    ):
                        target_dtype = "int16"
                    else:
                        target_dtype = "int32"
                    ...
                    df.iloc[:, col_idx] = pd.Series(
                        encoded_values, index=df.index, dtype=target_dtype
                    )
                    logger.info(
                    ...
                    df.iloc[:, col_idx] = pd.Series(
                        np.zeros(len(df), dtype="int32"), index=df.index
                    )
        else:
            ...
            if preserve_categorical:
                ...
                if col.dtype == "object":
                    ...
                    logger.info(
                        f"  Converted object column {col_idx} to numeric for CatBoost"
                    )
                ...
                if col.isna().any():
                    ...
                    if df.iloc[:, col_idx].dtype != filled_col.dtype:
                        df.iloc[:, col_idx] = df.iloc[:, col_idx].astype(
                            filled_col.dtype
                        )
...
def compute_frequency_distribution(
    labels: np.ndarray, label_names: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
...
def compute_label_frequency_mapping(
    true_labels: np.ndarray, predicted_labels: np.ndarray, threshold: float = 0.05
    ...
    if len(true_classes) != len(pred_classes):
        logger.warning(
            f"Different number of classes: true={len(true_classes)}, pred={len(pred_classes)}"
        )
        return None
    ...
    if avg_cost_per_class / total_samples > threshold:
        logger.warning(
            f"Label mapping has high cost ({avg_cost_per_class:.2f} avg per class), may not be reliable"
        )
        return None

    logger.info(
        f"Created label mapping with average cost {avg_cost_per_class:.2f} per class"
    )
    return label_mapping


def apply_label_mapping(
    predictions: np.ndarray, label_mapping: Dict[int, int]
) -> np.ndarray:
    """
    ...
    for original_label, new_label in label_mapping.items():
        ...
        logger.debug(
            f"Mapped {np.sum(mask)} predictions from {original_label} to {new_label}"
        )
...
def compute_baseline_probabilities(
    train_labels: np.ndarray, test_labels: np.ndarray, strategy: str = "frequency"
    ...
    if strategy == "frequency":
        ...
        baseline_predictions = np.random.choice(
            unique_classes, size=num_test_samples, p=class_probs[unique_classes]
        )

    elif strategy == "uniform":
        # Uniform random predictions
        baseline_predictions = np.random.choice(unique_classes, size=num_test_samples)
...
def process_tabular_dataset_for_training(
    ...
    dataset["categorical_indicator_raw"] = (
        categorical_indicator.copy()
        if hasattr(categorical_indicator, "copy")
    ...
    # Determine task type
    is_classification = _determine_task_type(dataset, y, task_type, preserve_regression)
...
def _determine_task_type(
    ...
    if task_type:
        ...
        logger.info(
            f"Dataset {dataset['name']}: Using manual task type override: {task_type}"
        )
    ...
    try:
        ...
        detected_task_type, detection_method = detect_task_type(
            dataset=dataset, y=y, manual_override=task_type, task_id=task_id
        )

        is_classification = detected_task_type == "classification"
        logger.info(
            f"Dataset {dataset['name']}: Detected {detected_task_type} task using {detection_method}"
        )
        return is_classification
    ...
    if preserve_regression:
        ...
        if len(unique_vals) <= 10 and (
            y.dtype.kind == "O"  # String labels
            or (y.dtype.kind in ("i", "u"))  # Integer with few values
            or (
                y.dtype.kind == "f"
            ...
            logger.info(
                f"Overriding to classification due to {len(unique_vals)} discrete values"
            )
        else:
            logger.info(
                f"Preserving as regression task with {len(unique_vals)} unique values"
            )
    else:
...
def _process_labels(
    y: np.ndarray, is_classification: bool, dataset: Dict[str, Any]
) -> np.ndarray:
    """Process labels based on task type."""
    ...
    if is_classification:
        ...
        if y.dtype.kind == "O":
            ...
            unique_vals = np.unique(y)
            logger.info(
                f"Dataset {dataset['name']} has {len(unique_vals)} unique integer values for classification"
            )
    else:
        ...
        if y.dtype.kind == "O":
            ...
            logger.warning(
                f"Dataset {dataset['name']} has string labels but is detected as regression. Attempting conversion."
            )
            try:
                import pandas as pd
            ...
            unique_vals = np.unique(y)
            logger.info(
                f"Dataset {dataset['name']} has {len(unique_vals)} unique values for regression (range: {y.min():.3f} to {y.max():.3f})"
            )
...
def _apply_training_sample_limit(
    X_train: np.ndarray,
    y_train: np.ndarray,
    max_samples: int,
    strategy: str,
    ...
    logger.info(
        f"Limiting training data to {max_samples} samples (from {len(X_train)} available) using {strategy} sampling"
    )
        ...
        for i, class_label in enumerate(unique_classes):
            ...
            if samples_from_class > 0:
                chosen_indices = np.random.choice(
                    class_indices, samples_from_class, replace=False
                )
                selected_indices.extend(chosen_indices)

            logger.info(
                f"  Class {class_label}: selected {samples_from_class} out of {len(class_indices)} samples"
            )
    ...
    X_train_sampled = (
        X_train.iloc[indices] if hasattr(X_train, "iloc") else X_train[indices]
    )
    y_train_sampled = (
        y_train.iloc[indices] if hasattr(y_train, "iloc") else y_train[indices]
    )
...
def _compute_tabpfn_embeddings(
    dataset: Dict[str, Any],
    embedding_size: int,
    ...
    logger.info(
        f"Computing TabPFN embeddings for dataset {dataset['name']} with size {embedding_size}"
    )
