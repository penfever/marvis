...
class ClassNameExtractor:
    ...
    def extract_class_names(
        ...
        if semantic_names is None and all(
        ...
        if semantic_names:
            try:
                ...
                if len(validated_names) == num_classes:
                    logger.info(
                        f"Successfully extracted and validated {num_classes} semantic class names: {validated_names}"
                    )
                    return validated_names, True
                else:
                    logger.warning(
                        f"Semantic names count mismatch: got {len(validated_names)}, expected {num_classes}"
                    )
    ...
    def _get_builtin_mapping(
        self, dataset_name: str, num_classes: int
    ) -> Optional[List[str]]:
        ...
        for key, mapping in self.builtin_mappings.items():
            if key in dataset_key or dataset_key in key:
                if len(mapping) >= num_classes:
                    logger.info(
                        f"Using builtin mapping '{key}' for dataset '{dataset_name}'"
                    )
                    return mapping[:num_classes]

        return None

    def _load_from_semantic_file(
        self, semantic_file: str, num_classes: int
    ) -> Optional[List[str]]:
    ...
    def _extract_classes_from_semantic_data(
...
def get_actually_plotted_classes(
    labels: Union[List, np.ndarray], data_coords: Union[List, np.ndarray], ax=None
) -> np.ndarray:
    """
    ...
    if len(labels) != len(data_coords):
        raise ValueError(
            f"Mismatch between labels ({len(labels)}) and coordinates ({len(data_coords)})"
        )
