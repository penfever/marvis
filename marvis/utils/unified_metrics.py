...
class MetricsLogger:
    ...
    def __init__(
        ...
        if self.use_wandb and not WANDB_AVAILABLE:
            self.logger.warning(
                "Weights & Biases requested but not available. Metrics will not be logged to W&B."
            )
        ...
        if confusion_matrix is not None:
            ...
            if self.use_wandb:
                try:
                    ...
                    plt.ylabel("True")
                    plt.title(
                        f"Confusion Matrix - {self.model_name} - {self.dataset_name}"
                    )
                    ...
                    self.logger.warning(
                        f"Failed to create confusion matrix visualization: {e}"
                    )

            # Store the raw matrix
            self.logged_metrics[self._get_metric_key("confusion_matrix")] = (
                confusion_matrix
            )
    ...
    def log_explanation_metrics(
        ...
        if explanation_samples is not None and self.use_wandb:
            try:
                ...
                wandb.log(
                    {key: wandb.Table(dataframe=pd.DataFrame(explanation_samples))}
                )
            except Exception as e:
    ...
    def log_all_metrics(self, result_dict: Dict[str, Any]) -> None:
        ...
        self.log_core_metrics(
            ...
            f1_macro=result_dict.get(
                "f1_macro", result_dict.get("f1_score", result_dict.get("f1"))
            ),
        ...
        self.log_regression_metrics(
            ...
            mean_absolute_percentage_error=result_dict.get(
                "mean_absolute_percentage_error"
            ),
        )
    ...
    def log_aggregated_metrics(
        self, model_results: list, prefix: str = "average"
    ) -> None:
        """
