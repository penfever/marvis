...
def add_model_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--model_path", type=str, help=argparse.SUPPRESS  # Hide from help
    )
    parser.add_argument(
        "--model_id", type=str, help=argparse.SUPPRESS  # Hide from help
    )
    parser.add_argument(
        "--embedding_size",
        type=int,
...
def add_baseline_model_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--catboost_depth", type=int, default=6, help="Tree depth for CatBoost"
    )
    parser.add_argument(
...
def add_evaluation_wandb_args(
    parser: argparse.ArgumentParser, default_project: str = "marvis-evaluation"
):
...
def add_tsne_visualization_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--tsne_perplexity", type=int, default=30, help="Perplexity parameter for t-SNE"
    )
    parser.add_argument(
        "--tsne_max_iter",
    ...
    parser.add_argument(
        "--image_dpi", type=int, default=100, help="DPI for generated visualizations"
    )
    parser.add_argument(
        "--force_rgb_mode",
        action="store_true",
...
def add_dataset_selection_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--datasets", type=str, help="Comma-separated list of datasets to evaluate on"
    )
    parser.add_argument(
        "--audio_datasets",
        nargs="+",
...
def create_evaluation_parser(
    description: str, default_wandb_project: str = "marvis-evaluation"
...
def create_audio_evaluation_parser(
    description: str = "Evaluate MARVIS and baseline models on audio datasets",
) -> argparse.ArgumentParser:
...
def create_vision_evaluation_parser(
    description: str = "Evaluate MARVIS and baseline models on vision datasets",
) -> argparse.ArgumentParser:
...
def create_tabular_llm_evaluation_parser(
    description: str = "Evaluate LLM baselines on tabular datasets",
) -> argparse.ArgumentParser:
    ...
    def parse_args_with_deprecated_handling(*args, **kwargs):
        ...
        if hasattr(parsed_args, "use_3d_tsne") and parsed_args.use_3d_tsne:
            if not hasattr(parsed_args, "use_3d") or not parsed_args.use_3d:
                ...
                logger.warning(
                    "--use_3d_tsne is deprecated, please use --use_3d instead"
                )
...
def create_multimodal_evaluation_parser(
    description: str = "Evaluate MARVIS models across multiple modalities",
) -> argparse.ArgumentParser:
