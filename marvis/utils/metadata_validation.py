...
def validate_tabllm_metadata(
    openml_task_id: int, feature_count: Optional[int] = None
) -> Dict[str, Any]:
    """
...
def validate_metadata_for_models(
    openml_task_id: int, models: List[str], feature_count: Optional[int] = None
    ...
    for model in models:
        ...
        if model_lower in ["tabllm", "jolt"]:
            results[model] = resource_manager.validate_model_metadata(
                openml_task_id, model_lower
            )
        else:
            results[model] = {
...
def generate_metadata_coverage_report(
    task_ids: List[int] = None, models: List[str] = None
) -> Dict[str, Any]:
    """
    ...
    if task_ids is None:
        ...
        for model in models:
            if model in ["tabllm", "jolt"]:
                task_mapping = resource_manager.config_manager.get_openml_task_mapping(
                    model
                )
                if task_mapping:
    ...
    for model in models:
        total = (
            report["summary"]["coverage_by_model"][model]["valid"]
            + report["summary"]["coverage_by_model"][model]["invalid"]
        )
        if total > 0:
...
def print_metadata_coverage_report(report: Dict[str, Any]) -> None:
    ...
    for model, stats in summary["coverage_by_model"].items():
        print(
            f"  {model.upper()}: {stats['valid']}/{stats['valid'] + stats['invalid']} "
