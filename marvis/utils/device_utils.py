...
def get_device_info(device: str) -> dict:
    ...
    """
    info = {"device": device, "platform": sys.platform, "available_devices": []}
...
def configure_device_for_model(
    model_type: str, device: Optional[str] = None, batch_size: Optional[int] = None
) -> Tuple[str, int]:
    """
    ...
    logger.debug(
        f"Device configuration for {model_type}: device={device}, batch_size={batch_size}"
    )
    return device, batch_size


def log_device_usage(
    model_name: str, device: str, additional_info: Optional[dict] = None
):
    """
