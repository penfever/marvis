...
class PathResolver:
    ...
    def get_config_path(self, model_type: str, filename: str) -> Optional[Path]:
        ...
        try:
            ...
            logger.debug(
                f"Error accessing package resources for {model_type}/{filename}: {e}"
            )
...
class DatasetRegistry:
    ...
    def __init__(self, path_resolver: PathResolver):
        ...
        self._registry = {}
        self._lock_timeout = float(
            os.environ.get("MARVIS_REGISTRY_LOCK_TIMEOUT", "10.0")
        )
        self._retry_attempts = int(
            os.environ.get("MARVIS_REGISTRY_RETRY_ATTEMPTS", "3")
        )
        self._sync_interval = float(
            os.environ.get("MARVIS_REGISTRY_SYNC_INTERVAL", "30.0")
        )
        self._last_sync_time = 0
        self._disable_registry = os.environ.get(
            "MARVIS_DISABLE_REGISTRY", ""
        ).lower() in ("true", "1", "yes")
    ...
    def _get_local_registry_path(self) -> Path:
        ...
        return (
            self.path_resolver.get_base_dir()
            / f"dataset_registry_{pid}_{timestamp}.json"
        )
    ...
    def _acquire_file_lock(
        self, lock_path: Path, timeout: float = None
    ) -> Optional[int]:
        """
        ...
        try:
            ...
            try:
                ...
                while time.time() - start_time < timeout:
                    try:
                        lock_fd = os.open(
                            str(lock_path), os.O_CREAT | os.O_TRUNC | os.O_RDWR
                        )
                ...
                logger.warning(
                    "File locking not available on this platform. Registry may have race conditions."
                )
    ...
    def _load_registry_with_retry(self) -> dict:
        ...
        for attempt in range(self._retry_attempts):
            try:
                ...
                if self._validate_registry_data(data):
                    ...
                    logger.warning(
                        f"Registry data validation failed on attempt {attempt + 1}"
                    )
        ...
        logger.warning(
            "Failed to load registry after all retry attempts, starting with empty registry"
        )
        return {}
            ...
            try:
                ...
                if merged_count > 0:
                    ...
                    if self._atomic_write_json(main_registry, registry_path):
                        logger.debug(
                            f"Successfully merged {merged_count} registry entries"
                        )
    ...
    def _refresh_if_needed(self):
        ...
        if (
            ...
            or time.time() - self._last_sync_time < self._sync_interval
        ):
            return
    ...
    def register_dataset(self, dataset_id: str, metadata: Dict[str, Any]):
        ...
        if not success:
            logger.warning(
                f"Could not save registry for dataset {dataset_id}, but operation will continue"
            )
    ...
    def unregister_dataset(self, dataset_id: str):
        ...
        if dataset_id in self._registry:
            ...
            if not success:
                logger.warning(
                    f"Could not save registry after unregistering dataset {dataset_id}"
                )
                ...
                try:
                    ...
                    if len(parts) >= 4:
                        ...
                        if age_days > max_age_days:
                            local_file.unlink()
                            logger.debug(
                                f"Removed stale local registry file: {local_file}"
                            )

                except Exception as e:
                    logger.debug(
                        f"Error cleaning up local registry file {local_file}: {e}"
                    )
...
class CacheManager:
    ...
    def save_to_cache(
        self, cache_type: str, cache_key: str, data: Any, extension: str = ""
    ) -> bool:
        """Save data to cache."""
        try:
            cache_path = self.get_cache_path(cache_type, cache_key, extension)
...
class ConfigManager:
    ...
    def find_tabllm_notes(self, dataset_name: str) -> Optional[Path]:
        ...
        notes_path = (
            self.path_resolver.get_configs_dir() / "tabllm" / "notes" / filename
        )
    ...
    def get_openml_task_mapping(self, model_type: str) -> Optional[Dict[str, int]]:
        ...
        if mapping_path and mapping_path.exists():
            try:
                ...
                logger.warning(
                    f"Error loading OpenML task mapping for {model_type}: {e}"
                )
...
class MarvisResourceManager:
    ...
    def __init__(self, config: Optional[ResourceConfig] = None):
        ...
        self.dataset_preparer = DatasetPreparer(
            self.path_resolver, self.dataset_registry, self.cache_manager
        )

        logger.debug(
            f"Initialized MARVIS resource manager with base dir: {self.path_resolver.get_base_dir()}"
        )
    ...
    def _check_and_invalidate_caches(self) -> None:
        ...
        try:
            ...
            if caches_invalidated:
                logger.info(
                    "Data directory changes detected, caches have been invalidated"
                )

        except Exception as e:
            # Don't fail initialization if cache checking fails
            logger.warning(
                f"Could not check data directory for cache invalidation: {e}"
            )
    ...
    def find_csv_file(
        self, dataset_id: str, additional_search_dirs: Optional[List[str]] = None
    ) -> Optional[Path]:
        ...
        logger.debug(
            f"CSV file not found for dataset {dataset_id} in search directories"
        )
        return None

    def resolve_openml_identifiers(
        self,
        ...
        if task_id:
            ...
            if api_result:
                ...
                logger.info(
                    f"Task ID {task_id} resolves to dataset ID {result['dataset_id']}"
                )
                return result
            ...
            if task_id in mapping:
                ...
                logger.warning(
                    f"Using fallback mapping for task {task_id} -> dataset {result['dataset_id']}"
                )
                return result

            logger.error(
                f"Could not resolve task_id {task_id} from either OpenML API or fallback mapping"
            )
        ...
        logger.warning(
            "Resolution by dataset_id or dataset_name is temporarily disabled to force task_id-based OpenML API usage"
        )

        return result

    def validate_model_metadata(
        self, openml_task_id: int, model_type: str
    ) -> Dict[str, Any]:
            ...
            if jolt_dir and jolt_dir.exists():
                ...
                if config_path.exists():
                    ...
                    try:
                        ...
                        result["dataset_name"] = config_data.get(
                            "dataset_name", f"task_{openml_task_id}"
                        )
                        logger.debug(
                            f"Found JOLT config for task {openml_task_id}: {config_path}"
                        )
                    except Exception as e:
                        result["valid"] = False
                        result["errors"].append(
                            f"JOLT config file is invalid for task {openml_task_id}: {e}"
                        )
                else:
                    ...
                    if task_mapping:
                        ...
                        if dataset_name:
                            old_config_path = (
                                jolt_dir / f"jolt_config_{dataset_name}.json"
                            )
                            if old_config_path.exists():
                                result["dataset_name"] = dataset_name
                                result["warnings"].append(
                                    f"Using old JOLT config format for {dataset_name}"
                                )
                            else:
                                result["valid"] = False
                                result["errors"].append(
                                    f"No JOLT config found for OpenML task ID {openml_task_id}"
                                )
                        else:
                            result["valid"] = False
                            result["errors"].append(
        ...
        # Handle TabLLM with new task ID-based approach
            ...
            if tabllm_dir and tabllm_dir.exists():
                ...
                if template_path.exists():
                    ...
                    logger.debug(
                        f"Found TabLLM template for task {openml_task_id}: {template_path}"
                    )
                else:
                    # Try fallback to old dataset name approach
                    ...
                    if task_mapping:
                        ...
                        if dataset_name:
                            old_template_path = (
                                tabllm_dir / f"templates_{dataset_name}.yaml"
                            )
                            if old_template_path.exists():
                                result["dataset_name"] = dataset_name
                                result["warnings"].append(
                                    f"Using old TabLLM template format for {dataset_name}"
                                )
                            else:
                                result["valid"] = False
                                result["errors"].append(
                                    f"No TabLLM template found for OpenML task ID {openml_task_id}"
                                )
                        else:
                            result["valid"] = False
                            result["errors"].append(
...
class DatasetPreparer:
    ...
    def prepare_dataset(
        ...
        if not force_redownload and not force_reorganize:
            try:
                if check_function(dataset_dir):
                    logger.info(
                        f"Dataset {dataset_id} already prepared, skipping preparation"
                    )
                    return True
            except Exception as e:
        ...
        if not download_needed:
            try:
                ...
                download_needed = not self._check_raw_data_exists(
        ...
        if organize_function:
            ...
            if organize_needed:
                ...
                logger.info(
                    f"Dataset {dataset_id} already organized, skipping organization"
                )
    ...
    def prepare_cifar_dataset(
        self, dataset_type: str = "cifar10", force_redownload: bool = False
    ) -> tuple:
        """
        ...
        def download_and_organize_cifar(dataset_dir: Path):
            ...
            def save_cifar_images(
                dataset, base_dir: Path, class_names: list, split: str
            ) -> tuple:
            ...
            logger.info(
                f"{dataset_type.upper()} prepared: {len(train_paths)} train, {len(test_paths)} test images"
            )
    ...
    def get_cached_color_mapping(self, dataset_id: str, unique_classes: list) -> dict:
        ...
        if self.cache_manager is None:
            logger.warning(
                "No cache manager available, generating color mapping without caching"
            )
        ...
        cached_mapping = self.cache_manager.load_from_cache(
            cache_type="color_mappings", cache_key=cache_key, extension=".json"
        )
        ...
        logger.info(
            f"Generating new color mapping for dataset {dataset_id} with {len(unique_classes)} classes"
        )
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Failed to cache color mapping for dataset {dataset_id}"
                )

            # Return mapping in original format
            return {"class_to_color": class_to_color, "color_to_class": color_to_class}

        except Exception as e:
            logger.error(
                f"Error generating color mapping for dataset {dataset_id}: {e}"
            )
            # Return empty mappings as fallback
            return {"class_to_color": {}, "color_to_class": {}}

    def _restore_color_mapping_types(
        self, cached_mapping: dict, unique_classes: list
    ) -> dict:
        """
...
def prepare_cifar_dataset(
    dataset_type: str = "cifar10", force_redownload: bool = False
) -> tuple:
    """
    ...
    """
    resource_manager = get_resource_manager()
    return resource_manager.dataset_preparer.prepare_cifar_dataset(
        dataset_type, force_redownload
    )
