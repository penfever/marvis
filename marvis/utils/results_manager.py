...
class ResultsManager:
    ...
    def __init__(self, resource_manager: Optional[MarvisResourceManager] = None):
        ...
        logger.debug(
            f"Initialized ResultsManager with base dir: {self.resource_manager.path_resolver.get_base_dir()}"
        )
    ...
    def get_experiment_dir(
        self, model_name: str, dataset_id: str, modality: str
    ) -> Path:
        """
        ...
        if modality not in self._supported_modalities:
            logger.warning(
                f"Unsupported modality '{modality}'. Supported: {self._supported_modalities}"
            )

        experiment_dir = (
            self.get_results_base_dir() / modality / str(dataset_id) / model_name
        )
    ...
    def save_evaluation_results(
        ...
        with self._lock:
            ...
            if results_file.exists() and not overwrite:
                logger.warning(
                    f"Results already exist at {results_file} and overwrite=False"
                )
                return experiment_dir
            ...
            if experiment_metadata is None:
                metadata_obj = ExperimentMetadata(
                    model_name=model_name, dataset_id=dataset_id, modality=modality
                )
            ...
            logger.info(
                f"Successfully saved results for {model_name} on {dataset_id} to {experiment_dir}"
            )
            return experiment_dir
    ...
    def load_evaluation_results(
        ...
        if not results_file.exists():
            logger.debug(
                f"No results found for {model_name} on {dataset_id} in {experiment_dir}"
            )
            return None
    ...
    def list_experiments(
        ...
        return sorted(
            experiments, key=lambda x: (x["modality"], x["dataset_id"], x["model_name"])
        )
            ...
            if isinstance(artifact_path, list):
                ...
                for i, path in enumerate(artifact_path):
                    if copy_files and os.path.exists(path):
                        dest_path = (
                            artifacts_dir / f"{artifact_name}_{i}_{Path(path).name}"
                        )
                ...
                if copy_files and os.path.exists(artifact_path):
                    dest_path = (
                        artifacts_dir / f"{artifact_name}_{Path(artifact_path).name}"
                    )
    ...
    def create_summary_report(
        self, modality: Optional[str] = None, output_file: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def cleanup_old_results(
            ...
            try:
                ...
                if results_file.exists():
                    ...
                    if mod_time < cutoff_date:
                        ...
                        # Calculate size
                        size_mb = sum(
                            f.stat().st_size for f in exp_path.rglob("*") if f.is_file()
                        ) / (1024 * 1024)
                        ...
                        if not dry_run:
                            ...
                            logger.info(
                                f"Deleted old experiment: {exp['modality']}/{exp['dataset_id']}/{exp['model_name']}"
                            )
                        else:
                            logger.info(
                                f"Would delete: {exp['modality']}/{exp['dataset_id']}/{exp['model_name']} ({size_mb:.1f} MB)"
                            )
        ...
        logger.info(
            f"Cleanup completed: {stats['deleted']}/{stats['marked_for_deletion']} deleted, {stats['freed_size_mb']:.1f} MB freed"
        )
        return stats
    ...
    def _dict_to_evaluation_results(self, data: Dict[str, Any]) -> EvaluationResults:
        ...
        result_fields = {
            f.name for f in EvaluationResults.__dataclass_fields__.values()
        }
    ...
    def _dict_to_artifacts(self, data: Dict[str, Any]) -> ResultsArtifacts:
        ...
        artifact_fields = {
            f.name for f in ResultsArtifacts.__dataclass_fields__.values()
        }
    ...
    def _save_artifacts_manifest(
        self, artifacts: ResultsArtifacts, experiment_dir: Path
    ):
...
def get_results_manager(
    resource_manager: Optional[MarvisResourceManager] = None,
) -> ResultsManager:
