...
def fetch_wandb_data(entity, projects):
    ...
    for run in all_runs:
        ...
        is_llm_baseline_run = any(
            key
            for key in run.summary._json_dict.keys()
            if (
                # Model hierarchical format: model/{model}/dataset/{dataset}/{metric}
                (
                    any(
                        key.startswith(f"model/{model}/dataset/")
                        for model in ["tabllm", "tabula_8b", "jolt", "marvis_tsne"]
                    )
                    and any(
                        metric in key.lower()
                        for metric in [
                ...
                (
                    any(
                        key.startswith(f"{model}/dataset/")
                        for model in ["tabllm", "tabula_8b", "jolt", "marvis_tsne"]
                    )
                    and any(
                        metric in key.lower()
        ...
        is_baseline_run = any(
            ...
            if (
                ...
                (
                    ...
                    and any(model in key.lower() for model in baseline_model_names)
                    and any(
                        metric in key.lower()
                        for metric in [
                            "accuracy",
                ...
                (
                    any(model in key.lower() for model in baseline_model_names)
                    and any(
                        metric in key.lower()
                        for metric in [
                            "accuracy",
        ...
        if not (
            is_eval_run
            or is_llm_baseline_run
            or is_baseline_run
            or is_train_dataset_run
        ):
            continue
    ...
    logger.info(
        f"Fetched {len(runs_df)} total runs from W&B ({eval_runs_count} eval runs, {llm_baseline_runs_count} LLM baseline runs, {baseline_runs_count} traditional baseline runs, {train_dataset_runs_count} train_tabular_dataset runs)"
    )
    return runs_df
...
def fetch_wandb_train_data(entity, projects):
    ...
    for run in all_runs:
        ...
        if runtime is not None:
            runtime_info[task_id][split_idx][model_name]["training_time"] = float(
                runtime
            )

        if prediction_time is not None:
            runtime_info[task_id][split_idx][model_name]["prediction_time"] = float(
                prediction_time
            )
...
def extract_variables_from_wandb_data(summary, config, run_name, run_type="unknown"):
    ...
    """
    result = {"dataset_name": None, "dataset_id": None, "task_id": None}
    ...
    if run_type == "standard_baseline":
        ...
        for key in summary.keys():
            for model in llm_models:
                if key.startswith(f"model/{model}/dataset/") and key.endswith(
                    "/accuracy"
                ):
        ...
        if not result["dataset_name"]:
            for key in summary.keys():
                for model in llm_models:
                    if key.startswith(f"{model}/dataset/") and key.endswith(
                        "/accuracy"
                    ):
                        # Extract dataset name from hierarchical path
    ...
    if result["task_id"] is None:
        ...
        if result["task_id"] is None and result["dataset_id"] is not None:
            try:
                ...
                logger.debug(
                    f"Failed to impute task_id from dataset_id {result['dataset_id']}: {e}"
                )
...
def should_exclude_failed_run(summary, model_name=None, dataset_name=None, debug=False):
    ...
    for key, value in summary.items():
        ...
        if any(
            error_word in key.lower() for error_word in ["error", "failed", "exception"]
        ):
            ...
            if (
                ...
                and value.strip()
                and not any(
                    success_word in value.lower()
                    for success_word in ["success", "completed", "none"]
                )
            ):
                if debug:
    ...
    if model_name and dataset_name:
        ...
        completed_samples_key_model = (
            f"model/{model_name}/dataset/{dataset_name}/completed_samples"
        )
        if completed_samples_key_model in summary:
            ...
            if completed_samples == 0:
                if debug:
                    logger.debug(
                        f"Excluding run due to {completed_samples_key_model} = 0"
                    )
                return True

        # Check new hierarchical format: model/dataset/dataset_name/completed_samples
        completed_samples_key_new = (
            f"{model_name}/dataset/{dataset_name}/completed_samples"
        )
        if completed_samples_key_new in summary:
            ...
            if completed_samples == 0:
                if debug:
                    logger.debug(
                        f"Excluding run due to {completed_samples_key_new} = 0"
                    )
                return True
        ...
        if completed_samples_key_old in summary:
            ...
            if completed_samples == 0:
                if debug:
                    logger.debug(
                        f"Excluding run due to {completed_samples_key_old} = 0"
                    )
                return True
        ...
        if accuracy is not None and accuracy == 0.0:
            ...
            if valid_metrics == 0:
                if debug:
                    logger.debug(
                        f"Excluding run {model_name}_{dataset_name} due to accuracy=0 with no other valid metrics"
                    )
                return True
...
def analyze_zero_accuracy_run(
    summary, run_name, config, model_name=None, dataset_name=None
):
    """
    ...
    if model_name and dataset_name:
        ...
        if (
            completed_samples_key in summary
            and summary.get(completed_samples_key, 0) == 0
        ):
    ...
    if error_keys:
        analysis["failure_indicators"].extend(
            [f"{key}: {summary[key]}" for key in error_keys]
        )

    # Check for timeout errors in summary values
    for key, value in summary.items():
        if isinstance(value, str) and (
            "timeout" in value.lower() or "timed out" in value.lower()
        ):
    ...
    if has_failure_indicators:
        ...
        analysis["reasoning"] = (
            f"Has failure indicators: {', '.join(analysis['failure_indicators'])}"
        )
    elif has_other_metrics:
        analysis["is_legitimate"] = True
        analysis["reasoning"] = (
            f"Has other valid metrics: {list(analysis['other_performance_metrics'].keys())}"
        )
    else:
        analysis["is_legitimate"] = False
        analysis["reasoning"] = (
            "No other performance metrics found, likely a failed run"
        )
...
def extract_model_metrics_from_summary(summary, debug=False):
    ...
    for key, value in summary.items():
        ...
        if match and (
            is_numeric(value) or isinstance(value, dict)
        ):  # Allow dict for classification_report
            ...
            if debug:
                logger.debug(
                    f"Matched hierarchical pattern: {key} -> model={model_name}, dataset={dataset_name}, metric={metric_name}, value_type={type(value)}"
                )

            # Check if this model-dataset combination failed
            if should_exclude_failed_run(
                summary, model_name, dataset_name, debug=debug
            ):
                if debug:
                    logger.debug(
                        f"Skipping {model_name}_{dataset_name} metrics due to failed run detection"
                    )
            ...
            if metric_name == "classification_report" and isinstance(value, dict):
                ...
                if debug:
                    logger.debug(
                        f"Processing classification report for {model_name}/{dataset_name}"
                    )
                ...
                if "weighted avg" in value:
                    ...
                    if "f1-score" in weighted_avg:
                        baseline_metrics[model_name][dataset_name]["f1_weighted"] = (
                            safe_float_convert(weighted_avg["f1-score"])
                        )
                        if debug:
                            logger.debug(
                                f"Extracted f1_weighted from classification report: {model_name}/{dataset_name} = {weighted_avg['f1-score']}"
                            )
                    if "precision" in weighted_avg:
                        baseline_metrics[model_name][dataset_name][
                            "precision_weighted"
                        ] = safe_float_convert(weighted_avg["precision"])
                        if debug:
                            logger.debug(
                                f"Extracted precision_weighted from classification report: {model_name}/{dataset_name} = {weighted_avg['precision']}"
                            )
                    if "recall" in weighted_avg:
                        baseline_metrics[model_name][dataset_name][
                            "recall_weighted"
                        ] = safe_float_convert(weighted_avg["recall"])
                        if debug:
                            logger.debug(
                                f"Extracted recall_weighted from classification report: {model_name}/{dataset_name} = {weighted_avg['recall']}"
                            )
                ...
                if "macro avg" in value:
                    ...
                    if "f1-score" in macro_avg:
                        baseline_metrics[model_name][dataset_name]["f1_macro"] = (
                            safe_float_convert(macro_avg["f1-score"])
                        )
                        if debug:
                            logger.debug(
                                f"Extracted f1_macro from classification report: {model_name}/{dataset_name} = {macro_avg['f1-score']}"
                            )
                    if "precision" in macro_avg:
                        baseline_metrics[model_name][dataset_name][
                            "precision_macro"
                        ] = safe_float_convert(macro_avg["precision"])
                        if debug:
                            logger.debug(
                                f"Extracted precision_macro from classification report: {model_name}/{dataset_name} = {macro_avg['precision']}"
                            )
                    if "recall" in macro_avg:
                        baseline_metrics[model_name][dataset_name]["recall_macro"] = (
                            safe_float_convert(macro_avg["recall"])
                        )
                        if debug:
                            logger.debug(
                                f"Extracted recall_macro from classification report: {model_name}/{dataset_name} = {macro_avg['recall']}"
                            )
            ...
            # Handle flattened classification report metrics (dot notation format)
                ...
                if debug:
                    logger.debug(
                        f"Processing flattened classification report metric for {model_name}/{dataset_name}: {report_suffix}"
                    )
                ...
                if report_suffix == "weighted avg.f1-score":
                    ...
                    if debug:
                        logger.debug(
                            f"Extracted f1_weighted from flattened classification report: {model_name}/{dataset_name} = {value}"
                        )
                elif report_suffix == "weighted avg.precision":
                    baseline_metrics[model_name][dataset_name]["precision_weighted"] = (
                        safe_float_convert(value)
                    )
                    if debug:
                        logger.debug(
                            f"Extracted precision_weighted from flattened classification report: {model_name}/{dataset_name} = {value}"
                        )
                elif report_suffix == "weighted avg.recall":
                    baseline_metrics[model_name][dataset_name]["recall_weighted"] = (
                        safe_float_convert(value)
                    )
                    if debug:
                        logger.debug(
                            f"Extracted recall_weighted from flattened classification report: {model_name}/{dataset_name} = {value}"
                        )
                ...
                # Handle macro averages
                    ...
                    if debug:
                        logger.debug(
                            f"Extracted f1_macro from flattened classification report: {model_name}/{dataset_name} = {value}"
                        )
                elif report_suffix == "macro avg.precision":
                    baseline_metrics[model_name][dataset_name]["precision_macro"] = (
                        safe_float_convert(value)
                    )
                    if debug:
                        logger.debug(
                            f"Extracted precision_macro from flattened classification report: {model_name}/{dataset_name} = {value}"
                        )
                    ...
                    if debug:
                        logger.debug(
                            f"Extracted recall_macro from flattened classification report: {model_name}/{dataset_name} = {value}"
                        )
                ...
                baseline_metrics[model_name][dataset_name][metric_name] = (
                    safe_float_convert(value)
                )
                if debug and metric_name == "balanced_accuracy":
                    logger.debug(
                        f"Extracted balanced_accuracy: {model_name}/{dataset_name} = {value}"
                    )
        ...
        # Try old baseline pattern for backward compatibility
            ...
            if match and is_numeric(value):
                ...
                baseline_metrics[model_name]["default"][metric_name] = (
                    safe_float_convert(value)
                )
    ...
    llm_baseline_pattern_new = (
        r"^(tabllm|tabula_8b|jolt|marvis_tsne)/dataset/([^/]+)/(.+)$"
    )
    llm_baseline_pattern_model = (
        r"^model/(tabllm|tabula_8b|jolt|marvis_tsne)/dataset/([^/]+)/(.+)$"
    )
    ...
    for key, value in summary.items():
        ...
        if match and is_numeric(value):
            ...
            if debug:
                logger.debug(
                    f"Matched model LLM pattern: {key} -> model={model_name}, dataset={dataset_name}, metric={metric_name}"
                )
            ...
            # Store metric value
            llm_baseline_metrics[model_name][dataset_name][metric_name] = (
                safe_float_convert(value)
            )
        else:
            ...
            if match and is_numeric(value):
                ...
                if debug:
                    logger.debug(
                        f"Matched new LLM pattern: {key} -> model={model_name}, dataset={dataset_name}, metric={metric_name}"
                    )
                ...
                if match and is_numeric(value):
                    ...
                    if debug:
                        logger.debug(
                            f"Matched old LLM pattern: {key} -> model={model_name}, dataset={dataset_name}, metric={metric_name}"
                        )
    ...
    for model_name, dataset_metrics in baseline_metrics.items():
        if dataset_metrics:  # Only add if we have metrics
            ...
            for common_metric in common_metrics:
                ...
                if metric_values:
                    all_metrics[f"aggregated_{common_metric}"] = sum(
                        metric_values
                    ) / len(metric_values)
                    if debug:
                        logger.debug(
                            f"Computed aggregated {common_metric} for {model_name}: {all_metrics[f'aggregated_{common_metric}']} from {len(metric_values)} datasets"
                        )
...
def extract_results_from_wandb(
    wandb_df, runtime_info=None, debug=False, save_zero_accuracy_details=True
):
    """
    ...
    for _, row in wandb_df.iterrows():
        ...
        if save_zero_accuracy_details:
            ...
            if "accuracy" in summary and summary["accuracy"] == 0.0:
                ...
                analysis["would_be_filtered"] = should_exclude_failed_run(
                    summary, debug=debug
                )
            ...
            for key, value in summary.items():
                ...
                if key.endswith("/accuracy") and value == 0.0:
                    ...
                    if (
                        ...
                        and parts[0] == "model"
                        and parts[2] == "dataset"
                    ):  # model/model_name/dataset/dataset_name/accuracy
                        model_name = parts[1]
                        dataset_name = parts[3]
                        if model_name in ["tabllm", "tabula_8b", "jolt", "marvis_tsne"]:
                            analysis = analyze_zero_accuracy_run(
                                summary, run_name, config, model_name, dataset_name
                            )
                            analysis["would_be_filtered"] = should_exclude_failed_run(
                                summary, model_name, dataset_name, debug=debug
                            )
                        ...
                        len(parts) >= 4 and parts[1] == "dataset"
                    ):  # model_name/dataset/dataset_name/accuracy
                        model_name = parts[0]
                        dataset_name = parts[2]
                        if model_name in ["tabllm", "tabula_8b", "jolt", "marvis_tsne"]:
                            analysis = analyze_zero_accuracy_run(
        ...
        is_standard_baseline_run = any(
            key
            for key in summary.keys()
            if key.startswith("model/")
            and "/dataset/" in key
            and any(
                metric in key.lower() for metric in ["accuracy", "balanced_accuracy"]
            )
        )
        ...
        variables = extract_variables_from_wandb_data(
            summary, config, run_name, run_type
        )
        ...
        if is_llm_baseline_run:
            ...
            for dataset_name_from_key in llm_datasets:
                ...
                temp_variables = extract_variables_from_wandb_data(
                    summary, config, run_name, run_type
                )
            ...
            if dataset_name is None:
                logger.warning(
                    f"No dataset_name found in train_tabular_dataset run: {run_name}"
                )
                continue

            if task_id is None:
                logger.warning(
                    f"No task_id could be extracted for train_tabular_dataset run: {run_name}"
                )
            ...
            # Store model results
            for model_name, metrics in model_metrics.items():
                results[task_id]["splits"][split_idx][model_name] = {"metrics": metrics}

            # If no model metrics were found, log a warning
            if not model_metrics:
                logger.warning(
                    f"No model metrics found in summary for train_tabular_dataset run: {run_name}"
                )
                if debug:
            ...
            if task_id is None:
                logger.warning(
                    f"No task_id could be extracted for standard baseline run: {run_name}"
                )
                # Use dataset name as fallback
                task_id = dataset_name
            ...
            if not model_metrics:
                logger.warning(
                    f"No model metrics found in summary for standard baseline run: {run_name}"
                )
                if debug:
                    logger.info(f"Run summary: {summary}")
            ...
            if task_id is None:
                ...
                tasks_without_id += 1
                logger.warning(
                    f"Could not extract task ID from dataset name or run name: {dataset_name}, {run_name}. Using {task_id}"
                )
                ...
                for model_name, model_runtime in split_runtime.items():
                    ...
                    for metric_name, metric_value in model_runtime.items():
                        if metric_value is not None and is_numeric(metric_value):
                            ...
                            if (
                                "metrics"
                                not in results[task_id]["splits"][split_idx][model_name]
                            ):
                            ...
                            if debug:
                                logger.debug(
                                    f"Added runtime metric {metric_name}={metric_value} to task {task_id}, split {split_idx}, model {model_name}"
                                )
    ...
    if save_zero_accuracy_details and zero_accuracy_details:
        ...
        try:
            with open(filename, "w") as f:
                legitimate_runs = [
                    d for d in serializable_details if d.get("is_legitimate", False)
                ]
                failed_runs = [
                    d for d in serializable_details if not d.get("is_legitimate", False)
                ]
                filtered_runs = [
                    d for d in serializable_details if d.get("would_be_filtered", False)
                ]
                unfiltered_runs = [
                    d
                    for d in serializable_details
                    if not d.get("would_be_filtered", False)
                ]
                ...
                json.dump(
                    {
                        ...
                        "summary": {
                            ...
                            "unfiltered_failed_runs_count": len(
                                [
                                    d
                                    for d in unfiltered_runs
                                    if not d.get("is_legitimate", False)
                                ]
                            ),
                        },
...
def detect_run_types(wandb_df):
    ...
    llm_baseline_runs = wandb_df[
        wandb_df["summary"].apply(
            lambda x: any(
                key
                for key in x.keys()
                if (
