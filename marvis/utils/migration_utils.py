...
class MigrationManager:
        ...
        try:
            ...
            if not dry_run:
                ...
                if success:
                    ...
                    logger.info(
                        f"Migrated failed dataset cache, backed up to {backup_path}"
                    )
                else:
                ...
                logger.info(
                    f"[DRY RUN] Would migrate {len(failed_ids)} failed dataset IDs"
                )
    ...
    def migrate_embedding_caches(self, dry_run: bool = False) -> int:
        ...
        search_dirs = [
            ...
            Path("./cache"),
            Path.cwd(),
            self.rm.path_resolver.base_dir.parent
            / "data",  # Check if there's a sibling data dir
        ]
        ...
        try:
            ...
            if not dry_run:
                ...
                if not new_cache_path.exists():
                    ...
                    logger.info(
                        f"Migrated embedding cache: {cache_file} -> {new_cache_path}"
                    )
                    return True
            else:
        ...
        for config_file in jolt_dir.glob("jolt_config_*.json"):
            try:
                ...
                if not dry_run:
                    ...
                    new_config_path = self.rm.path_resolver.get_config_path(
                        "jolt", dataset_name
                    )
                    if new_config_path and not new_config_path.exists():
                        ...
                        logger.info(
                            f"Migrated JOLT config: {config_file} -> {new_config_path}"
                        )
                        migrated_count += 1
                else:
    ...
    def migrate_tabllm_configs(self, dry_run: bool = False) -> int:
        ...
        tabllm_dir = (
            project_root / "examples" / "tabular" / "llm_baselines" / "tabllm_like"
        )
            ...
            try:
                ...
                if not dry_run:
                    ...
                    if new_config_path and not new_config_path.exists():
                        ...
                        logger.info(
                            f"Migrated TabLLM template: {template_file} -> {new_config_path}"
                        )
                        migrated_count += 1
                else:
                    logger.info(
                        f"[DRY RUN] Would migrate TabLLM template: {template_file}"
                    )
                    migrated_count += 1
        ...
        if mapping_file.exists():
            try:
                if not dry_run:
                    new_mapping_path = (
                        self.rm.path_resolver.get_configs_dir()
                        / "tabllm"
                    ...
                    if not new_mapping_path.exists():
                        ...
                        logger.info(
                            f"Migrated TabLLM mapping: {mapping_file} -> {new_mapping_path}"
                        )
                        migrated_count += 1
                else:
                    logger.info(
                        f"[DRY RUN] Would migrate TabLLM mapping: {mapping_file}"
                    )
                    migrated_count += 1
            except Exception as e:
    ...
    def migrate_cc18_semantic(self, dry_run: bool = False) -> int:
        ...
        for semantic_file in semantic_dir.glob("*.json"):
            try:
                ...
                if not dry_run:
                    new_config_path = self.rm.path_resolver.get_config_path(
                        "cc18_semantic", dataset_id
                    )
                    if new_config_path and not new_config_path.exists():
                        new_config_path.parent.mkdir(parents=True, exist_ok=True)
                        shutil.copy2(semantic_file, new_config_path)
                        logger.info(
                            f"Migrated CC18 semantic: {semantic_file} -> {new_config_path}"
                        )
                        migrated_count += 1
                else:
                    logger.info(
                        f"[DRY RUN] Would migrate CC18 semantic: {semantic_file}"
                    )
                    migrated_count += 1
    ...
    def discover_existing_datasets(
    ...
    def register_discovered_datasets(
        self, discovered: List[Tuple[str, Path]], dry_run: bool = False
    ) -> int:
            ...
            try:
                ...
                if not dry_run:
                    ...
                    try:
                        ...
                        df = pd.read_csv(
                            file_path, nrows=1
                        )  # Just read first row for structure
                    ...
                    logger.info(
                        f"[DRY RUN] Would register dataset: {dataset_name} from {file_path}"
                    )
                    registered_count += 1
...
def run_migration(dry_run: bool = True) -> Dict[str, int]:
    ...
    results["registered_datasets"] = migration_manager.register_discovered_datasets(
        discovered, dry_run
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Migrate MARVIS resources to new management system"
    )
    parser.add_argument(
        "--live",
        action="store_true",
