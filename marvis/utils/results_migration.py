...
class ResultsFormatDetector:
    ...
    def extract_metadata(
        ...
        if format_type == "tabular_llm":
            ...
            if any(x in model_lower for x in ["tabllm", "jolt", "tabula"]):
                ...
                x in model_lower for x in ["marvis_tsne", "dinov2", "vlm", "vision"]
            ):
                modality = "vision"
        ...
        metadata = ExperimentMetadata(
            model_name=model_name, dataset_id=str(dataset_id), modality=modality
        )
    ...
    def extract_results(
        ...
        results.prediction_time_per_sample = result_dict.get(
            ...
            if result_dict.get("visualizations_saved"):
                ...
                if output_dir:
                    artifacts.visualizations = {
                        "main_plot": f"{output_dir}/main_visualization.png"
                    }
...
class ResultsMigrator:
    ...
    def migrate_file(
        ...
        try:
            ...
            if dry_run:
                logger.info(
                    f"Would migrate: {metadata.model_name} on {metadata.dataset_id} ({metadata.modality})"
                )
                return True
    ...
    def migrate_directory(
        self, source_dir: str, pattern: str = "*_results.json", dry_run: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Migration complete: {stats['successful']}/{stats['total_files']} successful"
        )
        return stats

    def _extract_info_from_path(
        self, file_path: Path
...
def create_migration_adapters():
    ...
    def tabular_llm_adapter(
        model_name: str, dataset_name: str, results: Dict[str, Any], args: Any = None
    ) -> Dict[str, Any]:
        ...
        if args:
            ...
            results["num_few_shot_examples"] = getattr(
                args, "num_few_shot_examples", None
            )
...
def migrate_legacy_results(
    source_dir: str, pattern: str = "*_results.json", dry_run: bool = True
) -> Dict[str, Any]:
    """
...
def validate_result_file(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "has_artifacts": artifacts.plots is not None
            or artifacts.visualizations is not None,
            "warnings": [],
        }

    except Exception as e:
        return {"status": "invalid", "error": str(e), "warnings": []}
