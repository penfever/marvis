...
DEFAULT_CACHE_FILES = ["openml_mappings.pkl", "metadata_cache.pkl", "dataset_cache.pkl"]
...
class DataDirectoryWatcher:
    ...
    def compute_directory_hash(self) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Computed directory hash for {len(all_files)} files: {hash_value[:16]}..."
            )
            return hash_value
    ...
    def save_hash_info(
        self, directory_hash: str, invalidated_caches: List[str] = None
    ) -> None:
        """
    ...
    def check_and_invalidate(
        self, cache_patterns: List[str] = None, force: bool = False
    ) -> bool:
        """
        ...
        logger.info(
            f"Data directory changed (hash: {stored_hash[:16] if stored_hash else 'None'} -> {current_hash[:16]})"
        )
        ...
        if invalidated_caches:
            logger.info(
                f"Invalidated {len(invalidated_caches)} cache files: {', '.join(invalidated_caches)}"
            )
        else:
...
def create_data_directory_watcher(
    data_dir: Optional[Path] = None, cache_dir: Optional[Path] = None
...
def check_and_invalidate_caches(
