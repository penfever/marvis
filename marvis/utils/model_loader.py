...
class BaseModelWrapper(ABC):
    ...
    @abstractmethod
    def generate(
        self, inputs: Union[str, List[str]], config: GenerationConfig
    ) -> Union[str, List[str]]:
...
class VLLMModelWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        vllm_kwargs.update(
            {k: v for k, v in self.kwargs.items() if k not in wrapper_specific_kwargs}
        )
        ...
        if image is not None:
            ...
            if self._tokenizer and hasattr(self._tokenizer, "apply_chat_template"):
                try:
                    ...
                    logger.debug(
                        f"Applied chat template, prompt: {formatted_prompt[:200]}..."
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to apply chat template: {e}. Using fallback formatting."
                    )
                    # Fallback to manual formatting
                    formatted_prompt = self._format_prompt_with_placeholder(
                        text_content
                    )
            else:
            ...
            prompt = {"prompt": formatted_prompt, "multi_modal_data": {"image": image}}
        else:
            # Text-only prompt
            prompt = text_content
...
class TransformersModelWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        if actual_device == "cuda" and torch.cuda.is_available():
            model_kwargs.update(
                {"torch_dtype": self.torch_dtype, "device_map": self.device_map}
            )
        elif (
            actual_device == "mps"
    ...
    def generate(
        ...
        with torch.no_grad():
            outputs = self._model.generate(**tokenized, **safe_kwargs)
...
class OpenAIModelWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Successfully connected to OpenAI API for model: {self.model_name}"
            )

        except ImportError:
            raise ImportError(
                "OpenAI package not available. Install with: pip install 'marvis[api]'"
            )
        except Exception as e:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if is_rate_limit and attempt < max_retries:
                        ...
                        logger.warning(
                            f"Rate limit error (attempt {attempt + 1}/{max_retries + 1}). Retrying in {delay}s..."
                        )
                        import time
...
class OpenAIVisionModelWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        try:
            ...
            self._client = openai.OpenAI(api_key=api_key)
            logger.info(
                f"Successfully connected to OpenAI API for VLM: {self.model_name}"
            )
        ...
        try:
            ...
            for message in conversation:
                if isinstance(message.get("content"), list):
                    ...
                    messages.append(
                        {"role": message.get("role", "user"), "content": openai_content}
                    )
                else:
                    # Text-only content
                    messages.append(
...
class GeminiModelWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Successfully connected to Gemini API for model: {self.model_name}"
            )

        except ImportError:
            raise ImportError(
                "Google GenerativeAI package not available. Install with: pip install 'marvis[api]'"
            )
        except Exception as e:
    ...
    def generate(
        ...
        for text_input in inputs:
            try:
                response = self._model.generate_content(text_input, **generation_kwargs)
...
class GeminiVisionModelWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        try:
            ...
            self._model = genai.GenerativeModel(self.model_name)
            logger.info(
                f"Successfully connected to Gemini API for VLM: {self.model_name}"
            )
    ...
    def generate_from_conversation(
        ...
        try:
                        ...
                        if content_item.get("type") == "text":
                            ...
                            if image is not None:
                                ...
                                if hasattr(image, "size") and hasattr(image, "mode"):
                                    logger.debug(
                                        f"Adding image to Gemini request: size={image.size}, mode={image.mode}"
                                    )
                                else:
                                    logger.warning(
                                        f"Image object doesn't have expected PIL attributes: {type(image)}"
                                    )
            ...
            response = self._model.generate_content(content_parts, **generation_kwargs)
...
class LlamaCPPModelWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        if not LLAMACPP_AVAILABLE:
            raise ImportError(
                "llama-cpp-python not available. Install with: pip install 'marvis[llamacpp]'"
            )
    ...
    def generate(
        ...
        for text_input in inputs:
            try:
                ...
                response = self._model(text_input, **generate_kwargs)
        ...
        try:
            ...
            for message in conversation:
                if isinstance(message.get("content"), list):
                    ...
                    messages.append(
                        {"role": message.get("role", "user"), "content": combined_text}
                    )
                else:
                    # Text-only message
                    messages.append(
            ...
            if hasattr(self._model, "create_chat_completion"):
                ...
                response = self._model(prompt, **generate_kwargs)
...
class ModelLoader:
    ...
    def load_llm(
        self, model_name: str, backend: str = "auto", device: str = "auto", **kwargs
    ) -> BaseModelWrapper:
        """
        ...
        if backend == "auto":
            ...
            if is_gguf_url(model_name):
                backend = "llamacpp"
                logger.info(
                    f"Auto-selected LlamaCPP backend for GGUF model: {model_name}"
                )
            else:
                ...
                if any(model in model_name for model in openai_llm_models):
                    ...
                    logger.info(
                        f"Auto-selected {backend} backend for local LLM: {model_name}"
                    )
        ...
        if backend == "openai":
            ...
            if not LLAMACPP_AVAILABLE:
                raise ImportError(
                    "LlamaCPP not available. Install with: pip install 'marvis[llamacpp]'"
                )
    ...
    def load_vlm(
        self, model_name: str, device: str = "auto", backend: str = "auto", **kwargs
    ) -> Union[
        VisionLanguageModelWrapper,
        VLLMVisionModelWrapper,
        OpenAIVisionModelWrapper,
        GeminiVisionModelWrapper,
        LlamaCPPVisionModelWrapper,
    ]:
        """
        ...
        if backend == "auto":
            ...
            if is_gguf_url(model_name):
                backend = "llamacpp"
                logger.info(
                    f"Auto-selected LlamaCPP backend for GGUF VLM: {model_name}"
                )
            else:
                ...
                if any(model in model_name for model in openai_vlm_models):
                    ...
                    supports_vllm = any(
                        pattern in model_name for pattern in vlm_supported_models
                    )
                    ...
                    if supports_vllm and VLLM_AVAILABLE:
                        ...
                        backend = "transformers"
                        logger.info(
                            f"Auto-selected transformers backend for VLM: {model_name}"
                        )
    ...
    def unload_model(
        self, model_name: str, backend: str = None, device: str = None
    ) -> None:
        ...
        if backend and device:
            ...
            keys_to_remove = [
                k for k in self._loaded_models.keys() if k.startswith(model_name)
            ]
