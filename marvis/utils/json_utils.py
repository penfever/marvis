    ...
    if isinstance(obj, (np.integer, np.int8, np.int16, np.int32, np.int64)):
        ...
        for key, value in obj.items():
            ...
            if isinstance(key, (np.integer, np.int8, np.int16, np.int32, np.int64)):
                ...
                converted_key = str(
                    key
                )  # Convert any remaining non-JSON types to string
        ...
        try:
            ...
            for result in results:
                ...
                try:
                    ...
                    logger.info(
                        f"Successfully saved {model_name} results using unified manager"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to save {model_name} results using unified manager: {e}"
                    )
                    logger.info("Falling back to legacy save method")
                    # Fall back to legacy method
                    _save_results_legacy(
                        result, output_dir, dataset_name, model_name, logger
                    )
        except ImportError:
...
def _save_results_legacy(result, output_dir, dataset_name, model_name, logger):
    ...
    success = safe_json_dump(
        result, results_file, logger=logger, minimal_fallback=True, indent=2
    )
