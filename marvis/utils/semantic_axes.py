...
class SemanticAxesComputer:
    ...
    def compute_semantic_axes(
        ...
        if feature_names is None and metadata is None:
            logger.warning(
                "No feature names or metadata provided, cannot compute semantic axes"
            )
            return {}
        ...
        if metadata is not None:
            ...
            feature_descriptions = {
                col.name: col.semantic_description for col in metadata.columns
            }
        else:
        ...
        if self.method == "pca_loadings":
            ...
            if (
                ...
                or reduction_func is None
            ):
                logger.warning(
                    "Perturbation method requires original_features, embedding_model, and reduction_func"
                )
                ...
                if self.use_smart_feature_selection:
                    semantic_axes = (
                        self._compute_perturbation_based_axes_with_preselection(
                            original_features,
                            reduced_coords,
                            labels,
                            feature_names,
    ...
    def _compute_pca_based_axes(
        ...
        try:
            ...
            pca_config = VisualizationConfig(
                use_3d=(n_components == 3), random_state=42
            )
            ...
            embeddings_clean = np.nan_to_num(
                embeddings, nan=0.0, posinf=1e3, neginf=-1e3
            )
    ...
    def _compute_importance_based_axes(
        self,
        embeddings: np.ndarray,
        reduced_coords: np.ndarray,
        labels: np.ndarray,
        ...
        try:
            ...
            for i, axis_name in enumerate(axis_names):
                ...
                selector = SelectKBest(
                    ...
                    k=min(self.top_k_features * 2, embeddings.shape[1]),
                )
                ...
                for idx in top_indices:
                    if (
                        ...
                        and feature_scores[idx] >= self.min_loading_threshold
                    ):
                        feature_name = feature_names[idx]
                        description = feature_descriptions.get(
                            feature_name, feature_name
                ...
                if top_features:
                    axis_description = (
                        f"{axis_name}-axis: {', '.join(top_features[:2])}"
                    )
                    semantic_axes[axis_name] = axis_description
                else:
    ...
    def _compute_perturbation_based_axes(
        self,
        ...
        try:
            ...
            if original_features.shape[0] > self.max_perturbation_dataset_size:
                logger.info(
                    f"Subsampling dataset from {original_features.shape[0]} to {self.max_perturbation_dataset_size} samples for perturbation analysis"
                )
                # Use random sampling to get a representative subset
                sample_indices = np.random.choice(
                    original_features.shape[0],
                    self.max_perturbation_dataset_size,
                    replace=False,
                )
            ...
            logger.info(
                f"Computing perturbation-based semantic axes with {self.perturbation_samples} samples on {original_features_subset.shape[0]} data points"
            )
            ...
            try:
                ...
                if hasattr(original_features, "select_dtypes"):
                    numeric_features = original_features.select_dtypes(
                        include=[np.number]
                    )
                    if numeric_features.empty:
                        logger.error(
                            "No numeric features found for perturbation analysis"
                        )
                        return {name: f"{name}-axis" for name in axis_names}
                    ...
                    if len(feature_names) != numeric_features.shape[1]:
                        logger.warning(
                            f"Feature names length mismatch: {len(feature_names)} vs {numeric_features.shape[1]}"
                        )
                ...
                for sample_idx in range(self.perturbation_samples):
                    ...
                    if feature_std > 0:  # Avoid division by zero for constant features
                        noise = np.random.normal(
                            0,
                            self.perturbation_strength * float(feature_std),
                    ...
                    try:
                        ...
                        if perturbed_reduced.shape != baseline_reduced_subset.shape:
                            logger.warning(
                                f"Shape mismatch in perturbation: {perturbed_reduced.shape} vs {baseline_reduced_subset.shape}"
                            )
                            continue

                        # Measure shift in reduced coordinates
                        coordinate_shift = np.mean(
                            np.abs(perturbed_reduced - baseline_reduced_subset), axis=0
                        )
                        perturbation_shifts.append(coordinate_shift)

                    except Exception as e:
                        logger.warning(
                            f"Error in perturbation sample {sample_idx} for feature {feature_idx}: {e}"
                        )
                        continue

                if perturbation_shifts:
                    # Average sensitivity across all perturbation samples
                    feature_sensitivities[feature_idx] = np.mean(
                        perturbation_shifts, axis=0
                    )
                else:
            ...
            for dim_idx, axis_name in enumerate(axis_names):
                ...
                # Find top contributing features
                top_indices = np.argsort(dim_sensitivities)[-self.top_k_features :][
                    ::-1
                ]
                ...
                max_sensitivity = (
                    np.max(dim_sensitivities) if len(dim_sensitivities) > 0 else 0
                )
                ...
                for idx in top_indices:
                    ...
                    if (
                        max_sensitivity > 0
                        and sensitivity >= self.min_loading_threshold * max_sensitivity
                    ):
                        if idx < len(feature_names):
                            feature_name = feature_names[idx]
                            description = feature_descriptions.get(
                            ...
                            sensitivity_pct = (
                                (sensitivity / max_sensitivity * 100)
                                if max_sensitivity > 0
                                else 0
                            )
                            significant_features.append(
                                f"+{description} ({sensitivity_pct:.0f}%)"
                            )
                ...
                if significant_features:
                    ...
                    if fallback_features:
                        axis_description = (
                            f"{axis_name}-axis: {', '.join(fallback_features)} (weak)"
                        )
                    else:
                ...
                # Debug logging
                logger.debug(
                    f"Axis {axis_name}: top sensitivities = {dim_sensitivities[top_indices[:3]]}"
                )
    ...
    def _select_candidate_features(
        ...
        try:
            ...
            logger.info(
                f"Selecting {max_features} candidate features from {n_features} total using {self.feature_selection_method}"
            )
            ...
            if self.feature_selection_method == "pca_variance":
                ...
                for dim_idx in range(reduced_coords.shape[1]):
                    ...
                    coords_binned = pd.cut(
                        reduced_coords[:, dim_idx], bins=5, labels=False
                    )

                    # Compute mutual information
                    mi_scores = mutual_info_classif(
                        X_numeric, coords_binned, random_state=42
                    )
                ...
                if len(np.unique(labels)) > 1:
                    ...
                    feature_scores += f_scores_labels / np.max(
                        f_scores_labels
                    )  # Normalize
                ...
                for dim_idx in range(reduced_coords.shape[1]):
                    ...
                    if len(np.unique(coords_binned)) > 1:
                        ...
                        feature_scores += f_scores_coords / np.max(
                            f_scores_coords
                        )  # Normalize
            ...
            logger.info(
                f"Selected features: {[feature_names[i] if i < len(feature_names) else f'feature_{i}' for i in top_features]}"
            )
    ...
    def _compute_perturbation_based_axes_with_preselection(
        self,
        original_features: np.ndarray,
        baseline_reduced: np.ndarray,
        labels: np.ndarray,
        ...
        try:
            ...
            candidate_feature_indices = self._select_candidate_features(
                original_features_numeric,
                baseline_reduced_subset,
                labels_subset,
                feature_names,
            )
            ...
            logger.info(
                f"Testing {n_candidates} candidate features (reduced from {n_features}) with {self.perturbation_samples} perturbations each"
            )
            ...
            for candidate_idx in candidate_feature_indices:
                ...
                for sample_idx in range(self.perturbation_samples):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Error in perturbation sample {sample_idx} for feature {candidate_idx}: {e}"
                        )
                        continue

                if perturbation_shifts:
                    # Average sensitivity across all perturbation samples
                    feature_sensitivities[candidate_idx] = np.mean(
                        perturbation_shifts, axis=0
                    )
            ...
            for dim_idx, axis_name in enumerate(axis_names):
                ...
                if len(tested_sensitivities) > 0:
                    ...
                    top_local_indices = np.argsort(tested_sensitivities)[
                        -self.top_k_features :
                    ][::-1]
                ...
                significant_features = []
                max_sensitivity = (
                    np.max(tested_sensitivities) if len(tested_sensitivities) > 0 else 0
                )
                ...
                semantic_axes[axis_name] = axis_description
                logger.debug(
                    f"Axis {axis_name}: top sensitivities among tested features = {tested_sensitivities[top_local_indices[:3]] if len(top_local_indices) > 0 else []}"
                )

            end_time = time.time()
            efficiency_gain = n_features / n_candidates if n_candidates > 0 else 1
            logger.info(
                f"Smart perturbation completed in {end_time - start_time:.2f}s with {model_calls} model calls ({efficiency_gain:.1f}x fewer than full analysis)"
            )
...
def create_semantic_axis_legend(
    semantic_axes: Dict[str, str], figsize: Tuple[float, float] = (8, 6)
) -> str:
    """
...
def create_compact_axis_labels(
    semantic_axes: Dict[str, str], max_chars_per_line: int = 40, max_lines: int = 3
) -> Dict[str, str]:
    """
...
def create_bottom_legend_text(
    semantic_axes: Dict[str, str], max_chars_per_line: int = 80, max_lines: int = 3
) -> str:
    """
...
def enhance_visualization_with_semantic_axes(
    embeddings: np.ndarray,
    reduced_coords: np.ndarray,
    labels: np.ndarray,
    metadata: Optional[DatasetMetadata] = None,
