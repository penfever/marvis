    ...
    try:
        ...
        if hasattr(task, "task_type") and task.task_type:
            ...
            if "classif" in task_type_str:
                logger.debug(
                    f"Detected classification from OpenML task.task_type: {task.task_type}"
                )
                return "classification"
            elif "regress" in task_type_str:
                logger.debug(
                    f"Detected regression from OpenML task.task_type: {task.task_type}"
                )
                return "regression"
        ...
        if hasattr(task, "task_type_id"):
            ...
            if task.task_type_id == 1:
                logger.debug(
                    f"Detected classification from OpenML task_type_id: {task.task_type_id}"
                )
                return "classification"
            elif task.task_type_id == 2:
                logger.debug(
                    f"Detected regression from OpenML task_type_id: {task.task_type_id}"
                )
                return "regression"
        ...
        if hasattr(task, "class_labels") and task.class_labels:
            logger.debug(
                f"Detected classification from OpenML class_labels: {len(task.class_labels)} classes"
            )
            return "classification"
...
def detect_task_type_from_target(
    y: Union[np.ndarray, pd.Series, list],
    target_name: Optional[str] = None,
    classification_threshold: int = 50,
) -> str:
    """
    ...
    try:
        ...
        if len(y_clean) == 0:
            logger.warning(
                "Target variable contains only NaN values, defaulting to classification"
            )
            return "classification"
        ...
        try:
            ...
            if target_type in ["binary", "multiclass", "multilabel-indicator"]:
                logger.info(
                    f"Detected classification task (sklearn type: {target_type})"
                )
                return "classification"
            ...
            logger.debug(
                f"sklearn type_of_target failed: {e}, falling back to manual analysis"
            )
        ...
        if is_numeric:
            ...
            is_integer_like = np.allclose(
                y_clean, np.round(y_clean), rtol=1e-10, atol=1e-10
            )
        ...
        if not is_numeric:
            ...
            logger.info(
                f"Detected classification: non-numeric target with {num_unique} unique values"
            )
            return "classification"
        ...
        if num_unique <= 2:
            ...
            logger.info(
                f"Detected classification: binary target with values {unique_values}"
            )
            return "classification"
        ...
        if is_integer_like and num_unique <= classification_threshold:
            ...
            logger.info(
                f"Detected classification: {num_unique} unique integer-like values (≤ {classification_threshold})"
            )
            return "classification"
        ...
        if not is_integer_like or num_unique > classification_threshold:
            ...
            logger.info(
                f"Detected regression: {num_unique} unique values, integer-like: {is_integer_like}"
            )
            return "regression"

        # Default fallback
        logger.warning(
            f"Uncertain task type for target with {num_unique} unique values, defaulting to classification"
        )
        return "classification"
    ...
    try:
        ...
        if "target_attribute" in dataset_info and dataset_info["target_attribute"]:
            ...
            if any(hint in target_name for hint in classification_hints):
                logger.debug(
                    f"Detected classification from target name hints: {target_name}"
                )
                return "classification"
            ...
            if any(hint in target_name for hint in regression_hints):
                logger.debug(
                    f"Detected regression from target name hints: {target_name}"
                )
                return "regression"
...
def detect_task_type(
    ...
    if task_id is not None:
        ...
        if task_id in SPECIAL_TASK_IDS:
            ...
            modality = (
                "vision"
                if task_id in [VISION_CLASSIFICATION_TASK_ID, VISION_REGRESSION_TASK_ID]
                else "audio"
            )
            logger.info(
                f"Using special {modality} task type: {detected_type} (task_id={task_id})"
            )
        ...
        try:
            ...
            if detected_type:
                logger.info(
                    f"Detected task type from OpenML task {task_id}: {detected_type}"
                )
                return detected_type, f"openml_task_id_{task_id}"
            else:
                raise ValueError(
                    f"OpenML task {task_id} does not have recognizable task type metadata"
                )
        except Exception as e:
            ...
            try:
                ...
                if task_id_int in SPECIAL_TASK_IDS:
                    ...
                    modality = (
                        "vision"
                        if task_id_int
                        in [VISION_CLASSIFICATION_TASK_ID, VISION_REGRESSION_TASK_ID]
                        else "audio"
                    )
                    logger.info(
                        f"Using special {modality} task type from dataset: {detected_type} (task_id={task_id_int})"
                    )
                    return (
                        detected_type,
                        f"special_{modality}_task_id_{task_id_int}_from_dataset",
                    )

                # Otherwise, try OpenML lookup
                logger.debug(
                    f"Found task_id {task_id_int} in dataset, attempting OpenML lookup"
                )
                import openml
                ...
                if detected_type:
                    logger.info(
                        f"Detected task type from dataset task_id {task_id_int}: {detected_type}"
                    )
                    return detected_type, f"openml_task_id_{task_id_int}_from_dataset"
                else:
                    raise ValueError(
                        f"OpenML task {task_id_int} does not have recognizable task type metadata"
                    )
            except (ValueError, TypeError) as e:
                raise ValueError(
                    f"Could not parse dataset task_id '{dataset_task_id}' as integer: {e}"
                )
            except Exception as e:
    ...
    if task is not None:
        ...
        if detected_type:
            ...
            raise ValueError(
                "Provided OpenML task object does not have recognizable task type metadata"
            )
...
def get_target_statistics(y: Union[np.ndarray, pd.Series, list]) -> Dict[str, Any]:
    ...
    try:
        ...
        if np.issubdtype(y_clean.dtype, np.number):
            ...
            stats.update({"q25": float(percentiles[0]), "q75": float(percentiles[1])})
        else:
...
if __name__ == "__main__":
    ...
    for y_test, description in test_cases:
        ...
        print(
            f"    Stats: {stats.get('unique_count', 'N/A')} unique values, "
