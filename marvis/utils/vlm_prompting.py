...
3. One red star point which is the query sample I want you to classify"""
    ...
    if multi_viz_info:
        ...
        nonlinear_methods = [
            v
            for v in multi_viz_info
            if v["method"].lower() in ["tsne", "umap", "isomap", "lle"]
        ]

        if linear_methods and nonlinear_methods:
            considerations.append(
                "Whether linear methods (PCA) and nonlinear methods show consistent or different cluster assignments"
            )

        local_methods = [
            v for v in multi_viz_info if v["method"].lower() in ["tsne", "lle"]
        ]
        global_methods = [
            v
            for v in multi_viz_info
            if v["method"].lower() in ["umap", "isomap", "mds"]
        ]

        if local_methods and global_methods:
            considerations.append(
                "How local structure methods (t-SNE, LLE) compare with global structure methods (UMAP, Isomap)"
            )

        if include_spectrogram and modality == "audio":
            considerations.append(
                "The audio spectrogram patterns that might provide additional context"
            )
        ...
        considerations = [
            "The spatial clustering patterns"
            + (" across all four 3D views" if use_3d else " in the t-SNE visualization")
        ]
        considerations.append(
        ...
        considerations = [
            f"The spatial relationships in {'3D space by examining all four views' if use_3d else 'the t-SNE visualization'}"
        ]
        considerations.append(
    ...
    consider_text = "\n".join(
        [f"- {consideration}" for consideration in considerations]
    )
    ...
    if multi_viz_info:
        ...
        analysis_type = (
            "3D spatial clustering patterns you observe across the multiple views"
        )
    else:
...
3. One red star point which is the query sample I want you to predict a value for"""
    ...
    if multi_viz_info:
        ...
        if linear_methods and nonlinear_methods:
            considerations.append(
                "Whether linear methods (PCA) and nonlinear methods show similar target value predictions"
            )
        ...
        if local_methods and global_methods:
            considerations.append(
                "How local structure methods (t-SNE, LLE) compare with global methods (UMAP, Isomap) for target value estimation"
            )
        ...
        considerations.append(
            "The target values of the nearest neighbors (connected by red lines)"
        )
        considerations.append(
            "The distance-weighted average of the nearest neighbor values"
        )
        ...
        considerations.append(
            "The color intensity/gradient of the nearby training points"
        )
        considerations.append(
            "Which areas of the colormap the red star is positioned within or closest to"
        )
...
def find_best_class_match(
    class_part: str, unique_classes: List, logger_instance: logging.Logger
) -> Any:
    """
    ...
    logger_instance.debug(
        f"Finding match for: '{class_part_clean}' (normalized: '{class_part_norm}')"
    )
    ...
    for cls in unique_classes:
        ...
        if cls_norm == class_part_norm and cls_norm:  # Ensure not empty
            logger_instance.debug(
                f"Normalized match: '{class_part_clean}' -> {cls} (both normalize to '{cls_norm}')"
            )
            return cls
    ...
    for cls in unique_classes:
        ...
        if (
            class_part_lower.startswith(cls_str) and len(cls_str) > 2
        ):  # Avoid very short matches
            logger_instance.debug(
                f"Partial match (starts with): '{class_part_clean}' -> {cls}"
            )
            return cls
    ...
    for cls in unique_classes:
        cls_norm = normalize_class_name(str(cls))
        if (
            class_part_norm.startswith(cls_norm) and len(cls_norm) > 2
        ):  # Avoid very short matches
            logger_instance.debug(
                f"Partial normalized match: '{class_part_clean}' -> {cls}"
            )
            return cls
    ...
    for cls in unique_classes:
        ...
        pattern = (
            r"\b"
            + re.escape(cls_str).replace(r"\ ", r"[\s_\-]*").replace(r"\-", r"[\s_\-]*")
            + r"\b"
        )
    ...
    for cls in unique_classes:
        cls_norm = normalize_class_name(str(cls))
        if (
            cls_norm in class_part_norm and len(cls_norm) > 3
        ):  # Avoid very short matches
            logger_instance.debug(
                f"Substring normalized match: '{class_part_clean}' -> {cls}"
            )
            return cls
...
def parse_vlm_response(
    response: str,
    unique_classes: List = None,
    logger_instance: Optional[logging.Logger] = None,
    use_semantic_names: bool = False,
    task_type: str = "classification",
    target_stats: Optional[Dict] = None,
    color_to_class_map: Optional[Dict] = None,
) -> Any:
    """
    ...
    logger_instance.debug(
        f"Parsing VLM response: '{response}' (task_type={task_type}, use_semantic_names={use_semantic_names})"
    )
    ...
    if task_type == "regression":
            ...
            try:
                ...
                if len(response_parts) > 1:
                    ...
                    if "|" in after_value:
                        ...
                        reasoning_split = re.split(
                            r"\s*(?:\||reasoning|because|since|explanation|rationale|the\s|this\s)",
                            after_value,
                            flags=re.IGNORECASE,
                        )
                    ...
                    try:
                        ...
                        logger_instance.debug(
                            f"Parsed structured value: '{value_part}' -> {parsed_value}"
                        )
                        return parsed_value
                    except ValueError:
                        pass
        ...
        try:
            ...
            logger_instance.debug(
                f"Fallback regression parsing: '{response}' -> {parsed_value}"
            )
            return parsed_value
        except ImportError:
            ...
            for pattern in numeric_patterns:
                ...
                if matches:
                    try:
                        ...
                        logger_instance.debug(
                            f"Fallback numeric extraction: '{response}' -> {parsed_value}"
                        )
                        return parsed_value
                    except ValueError:
                        continue
            ...
            if target_stats and "mean" in target_stats:
                logger_instance.warning(
                    f"Could not parse value from response: '{response}'. Using target mean: {target_stats['mean']}"
                )
                return float(target_stats["mean"])
            else:
                logger_instance.warning(
                    f"Could not parse value from response: '{response}'. Using fallback: 0.0"
                )
                return 0.0
            ...
            if len(response_parts) > 1:
                ...
                if color_to_class_map:
                    for color_name, class_label in color_to_class_map.items():
                        if class_part.lower() == color_name.lower():
                            logger_instance.debug(
                                f"Color name match found: {class_part} -> {class_label}"
                            )
                            return class_label
                ...
                for unique_class in unique_classes:
                    if class_part.strip().lower() == str(unique_class).strip().lower():
                        logger_instance.debug(
                            f"Direct match found: {class_part} -> {unique_class}"
                        )
                        return unique_class
                    ...
                    if class_match:
                        try:
                            ...
                            if 0 <= class_num < len(unique_classes):
                                logger_instance.debug(
                                    f"Parsed Class {class_num} -> {unique_classes[class_num]}"
                                )
                ...
                best_match = find_best_class_match(
                    class_part, unique_classes, logger_instance
                )
        ...
        for pattern in class_patterns:
            ...
            if match:
                try:
                    ...
                    if 0 <= class_num < len(unique_classes):
                        logger_instance.debug(
                            f"Found Class {class_num} pattern -> {unique_classes[class_num]}"
                        )
                        return unique_classes[class_num]
                except (ValueError, IndexError):
                    continue
    ...
    if use_semantic_names:
        ...
        logger_instance.warning(
            f"Could not parse class from response: '{response}'. Using fallback: {unique_classes[0]}"
        )
        ...
        logger_instance.warning(
            f"Could not parse class from response: '{response}'. Using fallback: Class_0"
        )
        return unique_classes[0]
...
def create_direct_regression_prompt(
    target_stats: Dict[str, Any], dataset_description: Optional[str] = None
) -> str:
    """
...
def generate_multi_viz_reasoning_guidance(
    multi_viz_info: List[Dict[str, Any]], reasoning_focus: str = "comparison"
) -> str:
    """
    ...
    global_methods = [
        v for v in multi_viz_info if v["method"].lower() in ["umap", "isomap", "mds"]
    ]
    ...
    method_pairs = [
        (
            "tsne",
            "umap",
            "t-SNE may show tighter clusters while UMAP preserves more global structure",
        ),
        (
            "pca",
            "tsne",
            "PCA shows linear separability while t-SNE reveals nonlinear cluster structure",
        ),
        (
            "isomap",
            "lle",
            "Isomap preserves geodesic distances while LLE focuses on local linearity",
        ),
    ]
...
def create_comprehensive_multi_viz_prompt(
    ...
    if len(multi_viz_info) >= 2:
        reasoning_guidance = generate_multi_viz_reasoning_guidance(
            multi_viz_info, reasoning_focus
        )
