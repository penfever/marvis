...
def download_gguf_file(url: str, cache_dir: Optional[Path] = None, force_download: bool = False) -> Path:
    """
    ...
    try:
        ...
        if validate_gguf_file(tmp_path):
            ...
            raise RuntimeError(f"Downloaded file failed GGUF validation: {download_url}")
            
    except ImportError:
        logger.error("Missing dependencies for GGUF download. Install with: pip install requests tqdm")
        raise RuntimeError("Cannot download GGUF files without requests and tqdm packages")
    except Exception as e:
...
def suggest_gguf_files(repo_url: str) -> list[str]:
    ...
    common_patterns = [
        "q4_k_m.gguf", "q4_k_s.gguf", "q5_k_m.gguf", "q5_k_s.gguf",
        "q6_k.gguf", "q8_0.gguf", "f16.gguf", "f32.gguf"
    ]
