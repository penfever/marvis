...
class GPUMonitor:
    ...
    def __init__(
        self, log_interval: float = 30.0, enable_detailed_logging: bool = True
    ):
        """
    ...
    def get_gpu_stats(self) -> Dict[str, Any]:
        ...
        for i in range(self.gpu_count):
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    stats[f"gpu_{i}_memory_utilization_percent"] = (
        ...
        # GPUtil-based stats (alternative if PYNVML not available)
            ...
            GPUTIL_AVAILABLE
            and self.enable_detailed_logging
            and not self.nvml_initialized
        ):
            try:
                gpus = GPUtil.getGPUs()
...
def init_wandb_with_gpu_monitoring(
    ...
    if not WANDB_AVAILABLE:
        logging.getLogger(__name__).warning(
            "WandB not available, skipping GPU monitoring"
        )
        return None
