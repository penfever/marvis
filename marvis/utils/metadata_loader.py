        ...
        if "target_classes" in data:
            for class_data in data["target_classes"]:
                target_classes.append(
                    TargetClassMetadata(
                        name=class_data["name"], meaning=class_data["meaning"]
                    )
                )
...
class MetadataLoader:
    ...
    def __init__(self, metadata_base_dir: Optional[str] = None):
        ...
        if metadata_base_dir is None:
            ...
            )  # Go up from marvis/utils/ to project root
        ...
        logger.info(
            f"MetadataLoader initialized with base directory: {self.metadata_base_dir}"
        )
    ...
    def detect_metadata_file(self, dataset_id: Union[str, int]) -> Optional[Path]:
        ...
        for pattern in patterns:
            ...
            if candidate_path.exists():
                logger.info(
                    f"Found metadata file for dataset '{dataset_id}': {candidate_path}"
                )
        ...
        if self.metadata_base_dir.exists():
            logger.debug(
                f"Performing recursive search for dataset '{dataset_id}' in {self.metadata_base_dir}"
            )
            ...
            for json_file in self.metadata_base_dir.rglob("*.json"):
                ...
                if (
                    dataset_str == stem
                    or dataset_str.lower() == stem.lower()
                    or f"task_{dataset_str}" == stem
                    or f"dataset_{dataset_str}" == stem
                ):
                    logger.info(
                        f"Found metadata file for dataset '{dataset_id}': {json_file}"
                    )
                    return json_file

        logger.debug(
            f"No metadata file found for dataset '{dataset_id}' in {self.metadata_base_dir}"
        )
        return None

    def load_metadata(
        self, dataset_id: Union[str, int, Path, Dict[str, Any]], use_cache: bool = True
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded metadata for dataset from {metadata_path}"
            )
            return metadata
...
def create_metadata_summary(metadata: DatasetMetadata, max_columns: int = 10) -> str:
    ...
    if metadata.target_classes:
        ...
        for target_class in metadata.target_classes:
            meaning = (
                target_class.meaning[:100] + "..."
                if len(target_class.meaning) > 100
                else target_class.meaning
            )
    ...
    if metadata.columns:
        ...
        for col in columns_to_show:
            desc = (
                col.semantic_description[:80] + "..."
                if len(col.semantic_description) > 80
                else col.semantic_description
            )
    ...
    if metadata.inference_notes:
        notes = (
            metadata.inference_notes[:200] + "..."
            if len(metadata.inference_notes) > 200
            else metadata.inference_notes
        )
