...
def estimate_tokens_for_feature(
    feature_name: str, feature_values: np.ndarray, tokenizer, num_examples: int = 5
) -> float:
...
def categorize_features(
    X, feature_names, categorical_indicator=None
) -> Dict[str, List[int]]:
    ...
    return {"semantic": semantic_indices, "numeric": numeric_indices}
...
def select_features_for_token_limit(
    ...
    """
    logger.info(
        f"Selecting features for {num_few_shot_examples} few-shot examples with {max_tokens} max tokens"
    )
    ...
    if num_features != num_feature_names:
        logger.warning(
            f"Dimension mismatch: X has {num_features} features but {num_feature_names} feature names provided"
        )
        # Use the minimum to avoid index errors
        max_valid_index = min(num_features, num_feature_names) - 1
        feature_names = feature_names[: max_valid_index + 1]
        logger.warning(
            f"Truncated feature names to {len(feature_names)} to match data dimensions"
        )
    ...
    logger.info(
        f"Found {len(semantic_indices)} semantic and {len(numeric_indices)} numeric features"
    )
    ...
    if prioritize_semantic:
        ...
        semantic_features = [
            (i, n, t) for i, n, t in feature_token_estimates if i in semantic_indices
        ]
        numeric_features = [
            (i, n, t) for i, n, t in feature_token_estimates if i in numeric_indices
        ]
    ...
    label_text = " ".join(
        [f"Label: {label}" for label in unique_labels[:num_few_shot_examples]]
    )
    ...
    total_overhead = (
        base_tokens
        + (overhead_per_example + label_tokens // num_few_shot_examples)
        * num_few_shot_examples
    )
    available_for_features = max_tokens - total_overhead - 100  # Leave some buffer

    logger.info(
        f"Token budget: {max_tokens} total, {total_overhead} overhead, {available_for_features} for features"
    )
    ...
    for idx, name, avg_tokens in sorted_features:
        ...
        if estimated_tokens <= available_for_features:
            ...
            logger.debug(
                f"Selected feature {name} (idx={idx}, ~{avg_tokens:.1f} tokens/example)"
            )
        else:
            logger.debug(
                f"Skipping feature {name} (would exceed limit: {estimated_tokens} > {available_for_features})"
            )

    estimated_total = total_overhead + total_feature_tokens
    logger.info(
        f"Selected {len(selected_indices)} features, estimated {estimated_total} total tokens"
    )
...
def create_reduced_dataset(
    X, feature_indices: List[int], feature_names: List[str]
) -> Tuple[Any, List[str]]:
    ...
    valid_indices = [
        i for i in feature_indices if 0 <= i <= max_data_index and i <= max_name_index
    ]
    ...
    if len(valid_indices) != len(feature_indices):
        invalid_indices = [
            ...
            if i < 0 or i > max_data_index or i > max_name_index
        ]
        logger.warning(f"Skipping invalid feature indices: {invalid_indices}")
        logger.warning(
            f"Valid range: 0-{max_data_index} (data), 0-{max_name_index} (names)"
        )
...
def test_feature_selection(
    X, y, feature_names, tokenizer, num_few_shot_examples, categorical_indicator=None
):
        ...
        results.append(
            {
                ...
                "selected_features": [
                    feature_names[i] for i in selected_indices[:5]
                ],  # First 5 for display
