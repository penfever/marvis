...
def _numpy_to_pil_image(array: np.ndarray) -> Image.Image:
    ...
    if array.ndim == 2:
        ...
        if array.shape[-1] == 1:
            ...
            raise ValueError(
                f"Unsupported number of channels: {array.shape[-1]}. "
                "Expected 1, 3, or 4 channels."
            )
    else:
        raise ValueError(
            f"Unsupported array dimensions: {array.ndim}. "
...
def _reshape_1d_to_2d(array: np.ndarray) -> np.ndarray:
    ...
    if side_length * side_length == length:
        logger.debug(
            f"Reshaping 1D array of length {length} to {side_length}x{side_length}"
        )
    ...
    raise ValueError(
        f"Cannot reshape 1D array of length {length} into a valid image. "
