...
class QwenWithVQPrefixEmbedding(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if prefix_data is not None and input_ids is not None:
            ...
            for i in range(input_ids.shape[0]):
                ...
                for start_idx_pos, end_idx_pos in zip(
                    ...
                    if (
                        isinstance(query_embedding, (tuple, list))
                        and len(query_embedding) == 2
                    ):
                        ...
                        for emb in embeddings:
                            ...
                            quantized_emb, emb_vq_loss, emb_perplexity = (
                                self.vector_quantizer(emb.unsqueeze(0))
                            )
                        ...
                        quantized_query, query_vq_loss, query_perplexity = (
                            self.vector_quantizer(query_embedding.unsqueeze(0))
                        )
                        ...
                        projected_query = self.embedding_projector(
                            quantized_query.squeeze(0)
                        )
                        ...
                        for j in range(max_examples):
                            # Example embedding
                            all_embeddings[example_offset + j * 2] = (
                                projected_quantized_embeddings[j]
                            )
                        ...
                        for j in range(
                            min(num_tokens, 10)
                        ):  # Limit to 10 tokens for single query
        ...
        if num_quantized > 0:
            ...
            with torch.no_grad():
                ...
                if self._num_vq_updates % 100 == 0:
                    ...
                    logger.info(
                        f"VQ Stats - Avg Loss: {avg_total_vq_loss:.6f}, Avg Perplexity: {avg_perplexity:.2f}"
                    )
        ...
        kwargs = {
            k: v.to(device) if torch.is_tensor(v) else v for k, v in kwargs.items()
        }
    ...
    def save_pretrained(self, save_directory, **kwargs):
        ...
        final_class_token_ids = kwargs.pop(
            "final_class_token_ids", self.class_token_ids
        )
    ...
    def from_pretrained(cls, pretrained_model_path, **kwargs):
        ...
        class_token_ids = (
            model_info.get("final_class_token_ids", model_info["class_token_ids"])
            if use_permuted
            else model_info["class_token_ids"]
        )
        ...
        if os.path.exists(vector_quantizer_path):
            ...
            logger.warning(
                f"Vector quantizer weights not found at {vector_quantizer_path}"
            )
        ...
        if os.path.exists(embedding_projector_path):
            ...
            logger.warning(
                f"Embedding projector weights not found at {embedding_projector_path}"
            )
...
def prepare_qwen_with_vq_prefix_embedding(
    ...
    logger.info(
        f"Added special tokens: PREFIX_START_ID={prefix_start_id}, PREFIX_END_ID={prefix_end_id}"
    )
...
def load_vq_pretrained_model(
    model_path: str,
    device_map: str = "auto",
    embedding_size: int = 1000,
    model_id: str = None,
) -> Tuple[torch.nn.Module, Any, int, int, List[int], bool]:
    """
    ...
    if is_vq:
        logger.info(
            f"Found vector quantizer at {vector_quantizer_path}, loading as VQ model"
        )
        ...
        if os.path.exists(model_info_path):
            ...
            try:
                ...
                logger.info(
                    f"Successfully loaded VQ model with codebook size {model.vq_num_embeddings}"
                )
                return (
                    model,
                    tokenizer,
