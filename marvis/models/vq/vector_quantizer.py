...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, return_indices: bool = False) -> Union[
    ...
    def _update_usage(self, indices: torch.Tensor) -> None:
        ...
        if self.usage_count % self.reset_usage_every == 0:
            ...
            logger.info(
                f"Codebook utilization: {total_used}/{self.num_embeddings} vectors used ({usage_pct:.2f}%)"
            )

            # Check if usage is too low and log a warning
            if usage_pct < 25 and self.usage_count > self.reset_usage_every * 2:
                logger.warning(
                    f"Low codebook utilization: only {usage_pct:.2f}% in use. Consider reducing codebook size."
                )
    ...
    def _update_codebook_ema(
        ...
        self._ema_cluster_size = (
            self._ema_cluster_size * self.decay + (1 - self.decay) * n
        )
