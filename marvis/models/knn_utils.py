...
def find_knn_in_embedding_space(
    ...
    if len(query_embeddings) > 1:
        logger.info(
            f"Finding {k} nearest neighbors for {len(query_embeddings)} query points"
        )
    ...
    if len(query_embeddings) > 1:
        logger.info(
            f"KNN completed. Average distance to nearest neighbor: {np.mean(distances[:, 0]):.4f}"
        )
...
def find_influential_training_points(
    ...
    all_influential = np.unique(
        np.concatenate([influential_indices, uncertain_neighbors])
    )
    ...
    logger.info(
        f"Found {len(all_influential)} influential points ({metadata['influence_percentage']:.2f}%)"
    )
...
def get_neighbor_distances_and_indices(
    ...
    """
    logger.info(
        f"Getting detailed neighbor information for {len(test_embeddings)} test points"
    )
    ...
    neighbor_info = {"distances": distances, "indices": indices, "k": k}
    ...
    if return_class_distribution:
        ...
        overall_unique, overall_counts = np.unique(
            all_neighbor_classes, return_counts=True
        )
        neighbor_info["overall_class_distribution"] = dict(
            zip(overall_unique, overall_counts)
        )
...
def analyze_knn_performance(
    ...
    for k in k_values:
        ...
        results[f"k_{k}"] = {"k": k, "accuracy": accuracy, "predictions": predictions}
    ...
    logger.info(
        f"Best KNN performance: k={results['best_k']}, accuracy={results['best_accuracy']:.4f}"
    )
