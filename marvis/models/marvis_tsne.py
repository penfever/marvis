...
class BioClip2EmbeddingExtractor:
    ...
    def __init__(
        self, model_name: str = "hf-hub:imageomics/bioclip-2", device: str = "auto"
    ):
    ...
    def load_model(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"BioCLIP requires open_clip library: {e}. Install with: pip install open-clip-torch"
            )
    ...
    def extract_embeddings(self, image_paths: list, batch_size: int = 32) -> np.ndarray:
        ...
        embeddings = []
        self.logger.info(
            f"Extracting BioClip2 embeddings for {len(image_paths)} images with batch_size={batch_size}"
        )
        ...
        for batch_start in range(0, len(image_paths), batch_size):
            ...
            if batch_start % (batch_size * 10) == 0:  # Log every 10 batches
                self.logger.info(
                    f"Processing batch {batch_start//batch_size + 1}/{(len(image_paths) + batch_size - 1)//batch_size}"
                )
            ...
            try:
                ...
                for image_path in batch_paths:
                    try:
                        ...
                        self.logger.error(
                            f"Error processing image {image_path}: {individual_e}"
                        )
                        raise RuntimeError(
                            f"Failed to extract embedding for {image_path}: {individual_e}"
                        )
...
class MarvisTsneClassifier:
    ...
    def __init__(
        ...
        device: str = "auto",
        backend: str = "auto",
        vlm_backend: Optional[
            str
        ] = None,  # Alias for backend (for backward compatibility)
        ...
        **kwargs,
        ...
        if "use_3d_tsne" in kwargs:
            ...
            temp_logger.warning(
                "Parameter 'use_3d_tsne' is deprecated. Use 'use_3d' instead."
            )
        ...
        self.class_color_name_map = (
            None  # Maps class labels to color names (e.g., {0: "Blue", 5: "Color_111"})
        )
        self.color_to_class_map = (
            None  # Maps color names to class labels (e.g., {"Blue": 0, "Color_111": 5})
        )
    ...
    def _get_embedding_method(self):
        ...
        if self.modality == "tabular":
            ...
            if embedding_model == "whisper":
                ...
                raise ValueError(
                    f"Unsupported audio embedding model: {embedding_model}"
                )
        elif self.modality == "vision":
            ...
            if embedding_backend == "bioclip2":
                ...
                raise ValueError(
                    f"Unsupported vision embedding backend: {embedding_backend}. Supported: 'dinov2', 'bioclip2'"
                )
        else:
    ...
    def _apply_dimensionality_reduction(self, embeddings, n_components):
        ...
        if hasattr(self, "_tsne_params"):
            ...
            max_iter = self._tsne_params.get(
                "max_iter", self._tsne_params.get("n_iter", 1000)
    ...
    def _load_vlm(self):
        ...
        if self.is_api_model:
            ...
            if actual_device == "cuda" and torch.cuda.is_available():
                ...
                vlm_kwargs.update({"low_cpu_mem_usage": True})
        ...
        try:
            self.logger.info(
                f"Loading VLM with parameters: model={model_to_load}, backend={backend}, device={actual_device}"
            )
            ...
            if self.vlm_wrapper is None:
                self.logger.error(
                    f"Model loader returned None for model {model_to_load}"
                )
                raise RuntimeError(
                    f"Model loader returned None for model {model_to_load}"
                )
            ...
            self.logger.error(
                f"VLM loading failed with exception type: {type(e).__name__}"
            )
            import traceback
    ...
    def _initialize_multi_viz_composer(self, X_train, y_train, X_test=None):
        ...
        for viz_method in self.visualization_methods:
            ...
            self.context_composer.add_visualization(
                viz_method, config=method_config, viz_config=viz_config
            )

        self.logger.info(
            f"Added {len(self.context_composer.visualizations)} visualization methods"
        )
        ...
        try:
            ...
            if self.dataset_metadata is not None:
                metadata = load_dataset_metadata(
                    self.dataset_metadata, metadata_base_dir=self.metadata_base_dir
                )
                if metadata:
                    self.logger.info(
                        f"Loaded metadata from explicit parameter: {metadata.dataset_name}"
                    )
            ...
            if metadata is None and self.auto_load_metadata:
                ...
                if dataset_info and isinstance(dataset_info, dict):
                    ...
                    self.logger.info(
                        f"Trying to load metadata for dataset_id: {dataset_id} from dataset_info: {dataset_info}"
                    )
                ...
                if dataset_id:
                    try:
                        metadata = load_dataset_metadata(
                            dataset_id, metadata_base_dir=self.metadata_base_dir
                        )
                        if metadata:
                            self.logger.info(
                                f"Auto-loaded metadata for dataset ID '{dataset_id}': {metadata.dataset_name}"
                            )
                        else:
                            self.logger.info(
                                f"No metadata found for dataset ID '{dataset_id}'"
                            )
                    except Exception as e:
                        self.logger.warning(
                            f"Failed to load metadata for dataset ID '{dataset_id}': {e}"
                        )
            ...
            if metadata is None and dataset_info and self.use_metadata:
                ...
                if metadata:
                    self.logger.info(
                        f"Created basic metadata from dataset info: {metadata.dataset_name}"
                    )
            ...
            if metadata:
                self.logger.info(
                    f"Metadata loaded successfully: {len(metadata.columns)} columns, {len(metadata.target_classes)} target classes"
                )
            else:
            ...
            self.logger.warning(
                "Metadata loading not available - install metadata utilities"
            )
    ...
    def _create_basic_metadata_from_dataset_info(self, dataset_info):
        ...
        try:
            ...
            if hasattr(self, "class_names") and self.class_names:
                for i, class_name in enumerate(self.class_names):
                    target_classes.append(
                        TargetClassMetadata(
                            name=class_name, meaning=f"Class {i}: {class_name}"
                        )
                    )
            else:
                for i in range(n_classes):
                    target_classes.append(
                        TargetClassMetadata(
                            name=f"Class_{i}", meaning=f"Target class {i}"
                        )
                    )
            ...
            if dataset_info.get("data_source") == "openml":
                description += (
                    f" from OpenML task {dataset_info.get('task_id', 'unknown')}"
                )
    ...
    def _get_semantic_axes_legend(
        self, embeddings, reduced_coords, labels, feature_names=None
    ):
    ...
    def _compute_semantic_axes_labels(
        self, embeddings, reduced_coords, labels, feature_names=None
    ):
        ...
        try:
            ...
            if self.semantic_axes_method == "perturbation":
                ...
                if (
                    hasattr(self, "_original_features")
                    and hasattr(self, "_embedding_model")
                    and self._original_features is not None
                    and self._embedding_model is not None
                ):

                    # Create reduction function
                    def reduction_func(emb):
                        return self._apply_dimensionality_reduction(
                            emb, reduced_coords.shape[1]
                        )
                    ...
                    self.logger.warning(
                        "Perturbation method requires original features and embedding model, falling back to PCA"
                    )
    ...
    def fit(
        self, X_train, y_train, X_test=None, class_names=None, task_type=None, **kwargs
    ):
        """
        ...
        try:
            ...
            if task_id is None and self.modality == "vision":
                ...
                self.logger.debug(
                    f"Using special vision classification task_id: {task_id}"
                )
            elif task_id is None and self.modality == "audio":
                task_id = AUDIO_CLASSIFICATION_TASK_ID
                self.logger.debug(
                    f"Using special audio classification task_id: {task_id}"
                )
            ...
            self.logger.info(
                f"Task type: {self.task_type} (detected via: {detection_method})"
            )
        ...
        if self.modality == "tabular":
            ...
            X_train_array, X_test_array, _, self.selected_feature_indices = (
        ...
        if self.modality == "tabular":
                ...
                class TabPFNWrapper:
                    ...
                    def transform(self, X):
                        ...
                        if X.ndim == 1:
                            X = X.reshape(1, -1)
                            self.logger.debug(
                                f"TabPFNWrapper: Reshaped 1D input to 2D: {X.shape}"
                            )
            ...
            self.logger.info(
                f"Generating {self.modality_kwargs.get('embedding_model', 'whisper')} embeddings for audio..."
            )
            ...
            self.val_embeddings = (
                None  # Audio doesn't use validation split for embeddings
            )
            ...
            embedding_backend = self.modality_kwargs.get("embedding_backend", "dinov2")
            self.logger.info(
                f"Generating {embedding_backend.upper()} embeddings for images..."
            )
            ...
            if embedding_backend == "bioclip2":
                ...
                train_embeddings = embedding_method(
                    ...
                    model_name=self.modality_kwargs.get(
                        "bioclip2_model", "hf-hub:imageomics/bioclip-2"
                    ),
                    cache_dir=self.cache_dir,
                    device=self.device,
            ...
            self.val_embeddings = (
                None  # Vision doesn't use validation split for embeddings
            )
            self.y_train_sample = y_train_fit
        ...
        if self.task_type == "regression":
            ...
            if self.use_3d:
                ...
                self.train_tsne, self.test_tsne, base_fig = viz_methods[
                    "create_regression_tsne_3d_visualization"
                ](
                    self.train_embeddings,
                    self.y_train_sample,
                    test_embeddings_for_viz,
                    perplexity=self.tsne_perplexity,
                    max_iter=self.tsne_max_iter,
                    random_state=self.seed,
                    zoom_factor=self.zoom_factor,
                    cached_color_mapping={
                        "class_to_color": self.class_color_name_map,
                        "color_to_class": self.color_to_class_map,
                    },
                )
            else:
                dimension_str = "3D" if self.use_3d else "2D"
                self.logger.info(
                    f"Creating {dimension_str} regression t-SNE visualization..."
                )
                self.train_tsne, self.test_tsne, base_fig = viz_methods[
                    "create_regression_tsne_visualization"
                ](
        ...
        if self.task_type == "classification":
            ...
            if class_names is not None:
                ...
                if (
                    self.use_semantic_names
                    and self._loaded_metadata
                    and hasattr(self._loaded_metadata, "target_classes")
                    and self._loaded_metadata.target_classes
                    and len(self._loaded_metadata.target_classes)
                    == len(self.unique_classes)
                ):
                    # Extract class names from metadata
                    semantic_class_names = [
                        tc.name if hasattr(tc, "name") else f"Class_{i}"
                        for i, tc in enumerate(self._loaded_metadata.target_classes)
                    ]
                    self.logger.info(
                        f"Using semantic class names from metadata: {semantic_class_names}"
                    )
                else:
                    # Extract semantic class names with fallback
                    semantic_class_names, _ = extract_class_names_from_labels(
                        labels=(
                            self.unique_classes.tolist()
            ...
            if self.unique_classes is not None:
                self.class_to_semantic = {
                    cls: name
                    for cls, name in zip(
                        sorted(self.unique_classes), semantic_class_names
                    )
                }
            else:
                self.class_to_semantic = {}
        ...
        if self.task_type == "classification" and self.unique_classes is not None:
            try:
                ...
                color_mappings = (
                    resource_manager.dataset_preparer.get_cached_color_mapping(
                ...
                if self.class_color_name_map and self.color_to_class_map:
                    self.logger.info(
                        f"Using cached color mappings for dataset {dataset_id} with {len(self.unique_classes)} classes"
                    )
                else:
                    self.logger.warning(
                        f"Failed to get valid color mappings for dataset {dataset_id}"
                    )
        ...
        if (
            self.semantic_axes
            and self.train_embeddings is not None
            and self.train_tsne is not None
        ):
            ...
            if self.semantic_axes_labels:
                self.logger.info(
                    f"Computed semantic axes: X={self.semantic_axes_labels.get('X', 'N/A')}, Y={self.semantic_axes_labels.get('Y', 'N/A')}"
                )
        ...
        if self.enable_multi_viz:
            ...
            self._initialize_multi_viz_composer(
                X_train_fit, y_train_fit, X_test_for_multi_viz
            )
    ...
    def predict(
        ...
        if num_requested_samples > num_fitted_test_samples:
            raise ValueError(
                f"Cannot predict on {num_requested_samples} samples when model was fitted with {num_fitted_test_samples} test samples. "
        ...
        if num_requested_samples < num_fitted_test_samples:
            self.logger.info(
                f"Predicting on subset: {num_requested_samples} of {num_fitted_test_samples} fitted test samples"
            )
        ...
        if self.use_3d and "viewing_angles" in self.modality_kwargs:
            try:
                ...
                self.logger.warning(
                    f"Error parsing viewing angles: {e}. Using defaults."
                )
                viewing_angles = None
        ...
        self.logger.info(
            f"Starting VLM predictions for {len(self._prediction_indices)} test points..."
        )
        ...
        for idx, i in enumerate(self._prediction_indices):
            ...
            try:
                ...
                if (idx + 1) % 10 == 0:
                    self.logger.info(
                        f"Completed {idx + 1}/{len(self._prediction_indices)} predictions"
                    )
    ...
    def evaluate(
        ...
        results = {
            ...
            "accuracy": (
                float(metrics["accuracy"]) if metrics["accuracy"] is not None else None
            ),
            "balanced_accuracy": (
            ...
            "num_test_samples": (
                ...
                else len(self._prediction_indices)
            ),
            "completed_samples": completed_samples,
            "completion_rate": detailed_results["completion_rate"],
            "num_classes": (
                len(self.unique_classes) if self.unique_classes is not None else 0
            ),
            "predictions": (
                predictions_converted
                if "predictions_converted" in locals()
                else predictions
            ),
            "ground_truth": (
                (
            ...
            # Additional metrics
            "roc_auc": (
                float(metrics["roc_auc"]) if metrics["roc_auc"] is not None else None
            ),
            "f1_macro": (
                float(metrics["f1_macro"]) if metrics["f1_macro"] is not None else None
            ),
            "f1_micro": (
                float(metrics["f1_micro"]) if metrics["f1_micro"] is not None else None
            ),
            "f1_weighted": (
                float(metrics["f1_weighted"])
                if metrics["f1_weighted"] is not None
                else None
            ),
            "precision_macro": (
                float(metrics["precision_macro"])
                if metrics["precision_macro"] is not None
                else None
            ),
            "recall_macro": (
                float(metrics["recall_macro"])
                if metrics["recall_macro"] is not None
                else None
            ),
...
{full_context}"""
    ...
    def get_config(self):
        ...
        return {
            ...
            "dinov2_model": (
                self.modality_kwargs.get("dinov2_model")
                if self.modality == "vision"
                else None
            ),
            "embedding_backend": (
                self.modality_kwargs.get("embedding_backend")
                if self.modality == "vision"
                else None
            ),
            "bioclip2_model": (
                self.modality_kwargs.get("bioclip2_model")
                if self.modality == "vision"
                else None
            ),
            "use_pca_backend": (
                self.modality_kwargs.get("use_pca_backend")
                if self.modality == "vision"
                else None
            ),
        }
...
def evaluate_marvis_tsne(dataset, args):
    ...
    try:
        ...
        logger.info(
            f"Using pre-split data - Train: {X_train.shape}, Test: {X_test.shape}"
        )
        ...
        classifier = MarvisTsneClassifier(
            ...
            tsne_max_iter=getattr(
                args, "tsne_max_iter", getattr(args, "tsne_n_iter", 1000)
            ),
            ...
            zoom_factor=getattr(
                args, "zoom_factor", getattr(args, "tsne_zoom_factor", 2.0)
            ...
            feature_selection_threshold=getattr(
                args, "feature_selection_threshold", 500
            ),
        )
        ...
        dataset_info = {"name": dataset["name"], "task_id": task_id}
        classifier.fit(
            X_train,
            y_train,
            X_test,
        ...
        if results.get("accuracy") is not None:
            logger.info(
                f"MARVIS t-SNE accuracy on {dataset['name']}: {results['accuracy']:.4f}"
            )
        elif results.get("r2_score") is not None:
            logger.info(
                f"MARVIS t-SNE R² score on {dataset['name']}: {results['r2_score']:.4f}"
            )
        else:
