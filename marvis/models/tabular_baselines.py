...
def load_embeddings_with_limit(
    cache_file: str, max_test_samples: Optional[int] = None
    ...
    logger.info(
        f"Loaded embeddings with limits - Val: {val_to_load}/{val_count}, Test: {test_to_load}/{test_count}"
    )

    return train_embeddings, val_embeddings, test_embeddings, y_train_sample


def create_and_evaluate_baseline_model(
    model_name: str, dataset: Dict[str, Any], args
) -> Dict[str, Any]:
    """
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        f"Creating and evaluating {model_name} model on dataset {dataset['name']}"
    )
    ...
    if model_name == "catboost":
        ...
        categorical_indicator = dataset.get(
            "categorical_indicator_raw", [False] * X_train.shape[1]
        )
        ...
        X_train_processed = preprocess_features(
            X_train, categorical_indicator, preserve_categorical=True
        )
        X_test_processed = preprocess_features(
            X_test, categorical_indicator, preserve_categorical=True
        )
        ...
        logger.info(
            f"Using CatBoost-specific preprocessing: Train shape {X_train.shape}, Test shape {X_test.shape}"
        )
    ...
    logger.info(
        f"Using training data with {len(X_train)} samples for baseline model training"
    )
    ...
    if model_name == "catboost":
        try:
            ...
            if "categorical_indicator_raw" in dataset:
                ...
                categorical_features = [
                    i
                    for i, is_cat in enumerate(dataset["categorical_indicator_raw"])
                    if i < max_features and is_cat
                ]
                task_type = (
                    "classification" if dataset["is_classification"] else "regression"
                )
                logger.info(
                    f"CatBoost using {len(categorical_features)} categorical features for {task_type}: {categorical_features}"
                )
                ...
                for cat_idx in categorical_features[
                    :5
                ]:  # Log first 5 categorical features
                    if cat_idx < df_train.shape[1]:
                        logger.info(
                            f"  Feature {cat_idx}: dtype={df_train.iloc[:, cat_idx].dtype}, "
            ...
            # Train the model
            model.fit(
                X_train, y_train, cat_features=categorical_features, verbose=False
            )

        except ImportError:
            logger.error(
                "CatBoost not installed. Please install it with 'pip install catboost'."
            )
            return {
        ...
        try:
            ...
            logger.info(
                f"TabPFN v2 initialized with n_estimators={n_estimators}, device={device}"
            )
            ...
            if not dataset["is_classification"]:
                ...
                if np.any(train_nan_mask) or np.any(test_nan_mask):
                    logger.warning(
                        f"TabPFN v2: Found {np.sum(train_nan_mask)} NaN/INF values in training targets, "
                    ...
                    y_train_clean = target_imputer.fit_transform(
                        y_train_clean.reshape(-1, 1)
                    ).flatten()
                    y_test_clean = target_imputer.transform(
                        y_test_clean.reshape(-1, 1)
                    ).flatten()
                ...
                max_quantiles = min(
                    n_samples // 2, 1000
                )  # Conservative limit for quantiles

                if n_unique > max_quantiles:
                    logger.info(
                        f"TabPFN v2: Target has {n_unique} unique values but only {n_samples} samples. "
                    ...
                    y_train_processed = target_preprocessor.fit_transform(
                        y_train_clean.reshape(-1, 1)
                    ).flatten()
                    y_test_processed = target_preprocessor.transform(
                        y_test_clean.reshape(-1, 1)
                    ).flatten()

                    logger.info(
                        f"TabPFN v2: Preprocessed target from {n_unique} to {len(np.unique(y_train_processed))} unique values"
                    )
                else:
                    logger.info(
                        f"TabPFN v2: Target has {n_unique} unique values with {n_samples} samples - no preprocessing needed"
                    )
        ...
        if X_train.shape[1] > max_features:
            logger.info(
                f"Limiting gradient boosting features from {X_train.shape[1]} to {max_features}"
            )
            feature_selector = SelectKBest(score_func=score_func, k=max_features)
            X_train_selected = feature_selector.fit_transform(X_train, y_train)
            X_test = feature_selector.transform(X_test)
            logger.info(
                f"Selected {X_train_selected.shape[1]} features for gradient boosting"
            )
        else:
            X_train_selected = X_train
            logger.info(
                f"Using all {X_train.shape[1]} features for gradient boosting (under limit)"
            )
        ...
        if dataset["is_classification"]:
            ...
            model = LinearModel(n_jobs=-1)  # Use all available cores
    ...
    if model_name == "logistic_regression":
        ...
        y_prob = (
            ...
            if dataset["is_classification"] and hasattr(model, "predict_proba")
            else None
        )
    else:
        y_pred = model.predict(X_test)
    ...
    if (
        ...
        and not dataset["is_classification"]
        and "target_preprocessor" in locals()
        ...
        bin_edges = target_preprocessor.bin_edges_[
            0
        ]  # Get bin edges for the single feature

        # Map ordinal predictions to bin centers
        y_pred_original_scale = []
        for pred in y_pred:
            bin_idx = int(
                np.clip(pred, 0, len(bin_edges) - 2)
        ...
        logger.info(
            f"TabPFN v2: Transformed {len(y_pred)} predictions back to original scale"
        )
    ...
    if dataset["is_classification"]:
        ...
        try:
            ...
            logger.info(
                f"Accuracy for {model_name} on {dataset['name']}: {accuracy:.4f}, Balanced accuracy: {balanced_acc:.4f}"
            )
        except Exception as e:
            logger.warning(f"Could not compute balanced accuracy: {e}")
            balanced_acc = None
            logger.info(
                f"Accuracy for {model_name} on {dataset['name']}: {accuracy:.4f}"
            )
            ...
            try:
                ...
                if len(unique_classes) == 2:
                    ...
                    binary_truth = np.array(
                        [1 if y == pos_class_idx else 0 for y in y_test_for_eval]
                    )
                    ...
                    if y_prob.shape[1] > pos_class_idx:
                        ...
                        logger.info(
                            f"ROC AUC for {model_name} on {dataset['name']}: {roc_auc:.4f}"
                        )
                # For multi-class classification
                    ...
                    if y_prob.shape[1] >= len(unique_classes):
                        ...
                        logger.info(
                            f"ROC AUC (OVR) for {model_name} on {dataset['name']}: {roc_auc:.4f}"
                        )
            except Exception as e:
        ...
        try:
            ...
            logger.info(
                f"Generated classification report and confusion matrix for {model_name}"
            )
        except Exception as e:
        ...
        logger.info(
            f"Regression metrics for {model_name} on {dataset['name']}: MSE={mse:.4f}, RMSE={rmse:.4f}, MAE={mae:.4f}, R²={r2:.4f}"
        )
...
def create_and_evaluate_llm_model(
    model_id: str, dataset: Dict[str, Any], args, cached_model: Optional[Tuple] = None
) -> Dict[str, Any]:
    """
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        f"Creating and evaluating LLM model {model_id} on dataset {dataset['name']}"
    )
        ...
        try:
            model, tokenizer, prefix_start_id, prefix_end_id, class_token_ids = (
                prepare_qwen_with_prefix_embedding(
                    embedding_size=args.embedding_size, model_id=model_id
                )
            )
    ...
    if prediction_distribution is not None:
        ...
        formatted_results["ground_truth"] = (
    ...
    if "confusion_matrix" in results:
        formatted_results["confusion_matrix"] = (
    ...
    logger.info(
        f"Evaluation complete for {model_id}. Accuracy: {formatted_results['accuracy']:.4f}"
    )
