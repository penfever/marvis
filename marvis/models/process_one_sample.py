...
def _create_enhanced_legend(legend_text, classifier_instance):
    ...
    if (
        classifier_instance.semantic_axes
        and classifier_instance.train_embeddings is not None
    ):
...
def _generate_vlm_response(classifier_instance, image, prompt):
    ...
    gen_config = GenerationConfig(
        ...
        temperature=0.1,
        do_sample=True,
        enable_thinking=classifier_instance.enable_thinking
        and classifier_instance.is_api_model,
        thinking_summary=False,
    )

    return classifier_instance.vlm_wrapper.generate_from_conversation(
...
def _parse_prediction(response, classifier_instance, all_classes):
    ...
    if classifier_instance.task_type == "regression":
        ...
        if classifier_instance.use_semantic_names:
            all_class_names = [
                classifier_instance.class_to_semantic.get(cls, str(cls))
        ...
        if prediction in all_class_names and classifier_instance.use_semantic_names:
            semantic_to_numeric = {
                name: cls
                for cls, name in classifier_instance.class_to_semantic.items()
...
def _create_multi_visualization(
    ...
    if (
        save_outputs
        and classifier_instance.temp_dir
        and (i % visualization_save_cadence == 0)
    ):
...
def _create_single_visualization(
    ...
    if classifier_instance.task_type == "regression":
        ...
        if classifier_instance.use_knn_connections:
            ...
            if classifier_instance.use_3d:
                fig, legend_text, metadata = viz_methods[
                    "create_regression_tsne_3d_plot_with_knn"
                ](
                    classifier_instance.train_tsne,
                    classifier_instance.test_tsne,
                    classifier_instance.y_train_sample,
                    classifier_instance.train_embeddings,
                    classifier_instance.test_embeddings,
        ...
        if classifier_instance.use_knn_connections:
            ...
            if classifier_instance.use_3d:
                fig, legend_text, metadata = viz_methods[
                    ...
                    class_names=classifier_instance.class_names,
                    use_semantic_names=classifier_instance.use_semantic_names,
                    semantic_axes_labels=getattr(
                        classifier_instance, "semantic_axes_labels", None
                    ),
    ...
    fig.savefig(
        img_buffer,
        format="png",
        dpi=classifier_instance.image_dpi,
        bbox_inches="tight",
        facecolor="white",
    )
    img_buffer.seek(0)
...
def _process_image(classifier_instance, image):
    ...
    # Resize if needed
    if (
        image.width > classifier_instance.max_vlm_image_size
        or image.height > classifier_instance.max_vlm_image_size
    ):
        ratio = min(
            classifier_instance.max_vlm_image_size / image.width,
            classifier_instance.max_vlm_image_size / image.height,
        )
...
def _save_outputs(
    save_outputs, temp_dir, i, visualization_save_cadence, prompt, response
):
...
def _store_prediction_details(
    ...
    if return_detailed and y_test is not None and prediction_details is not None:
        ...
        tsne_coords = (
            test_tsne[i].tolist()
            if test_tsne is not None and i < len(test_tsne)
            else [0.0, 0.0]
...
def process_one_sample(
    ...
    if (
        classifier_instance.enable_multi_viz
        and classifier_instance.context_composer is not None
    ):
        ...
        if classifier_instance.task_type == "classification" and not visible_classes:
            ...
            viz_metadata = {
                viz.method_name: getattr(viz, "metadata", {})
                for viz in classifier_instance.context_composer.visualizations
            }
            raise ValueError(
        ...
        if classifier_instance.task_type == "regression":
            multi_viz_info = _create_multi_viz_info(
                classifier_instance.context_composer, classifier_instance.modality
            )
