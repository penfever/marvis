...
class QwenWithPrefixEmbedding(torch.nn.Module):
    ...
    def _register_with_accelerate(self):
        ...
        if hasattr(self.base_model, "_hf_hook"):
            try:
                ...
                logger.info(
                    f"Using execution device from base model's hook: {execution_device}"
                )
                ...
                if execution_device is not None:
                    logger.info(
                        f"Moving embedding_projector to execution_device: {execution_device}"
                    )
                    self.embedding_projector = self.embedding_projector.to(
                        execution_device
                    )
        ...
        if prefix_data is not None and input_ids is not None:
            ...
            for i in range(batch_size):
                ...
                for start_idx_pos, end_idx_pos in zip(
                    ...
                    cur_prefix = prefix_data[
                        i
                    ]  # Should contain both embeddings and class info
                    ...
                    if isinstance(cur_prefix, tuple) and len(cur_prefix) == 2:
                        ...
                        interleaved_length = min(
                            num_tokens, len(projected_embeddings) + len(class_labels)
                        )
                        ...
                        inputs_embeds[
                            i, start_pos + 1 : start_pos + 1 + interleaved_length, :
                        ] = interleaved_embeddings
                    else:
                        # Handle the case where we just have embeddings without class info
                        # Make sure embeddings are on the right device
                        embeddings_to_use = cur_prefix[
                            : min(num_tokens, len(cur_prefix))
                        ].to(current_device)
                        projected_embeddings = self.embedding_projector(
                            embeddings_to_use
                        )
                        ...
                        inputs_embeds[
                            i,
                            start_pos + 1 : start_pos + 1 + len(projected_embeddings),
                            :,
                        ] = projected_embeddings

        # Ensure all inputs are on the current device before the forward pass
        kwargs = {
            k: v.to(current_device) if torch.is_tensor(v) else v
    ...
    def from_pretrained(cls, pretrained_model_path, **kwargs):
        ...
        logger.info(
            f"Loading model with {'permuted' if use_permuted else 'original'} class token mapping"
        )
        if "final_class_token_ids" in model_info and use_permuted:
            logger.info(
                f"Using final epoch class token IDs: {model_info['final_class_token_ids']}"
            )
...
def prepare_qwen_with_prefix_embedding(
    embedding_size=192, model_id="Qwen/Qwen2.5-3B-Instruct"
):
    """
    ...
    # Create the model
    qwen_with_prefix = QwenWithPrefixEmbedding(
        model, embedding_size, prefix_start_id, prefix_end_id, class_token_ids
    )
...
def load_pretrained_model(
    ...
    if os.path.exists(model_info_path):
        ...
        logger.info(
            f"Loading model with stored class token mapping from {model_info_path}"
        )
        ...
        if os.path.exists(checkpoint_path):
            ...
            error_msg = (
                f"No valid model files found at specified path: {model_path}. "
