...
class TSNEGenerator:
    ...
    def __init__(
        self, perplexity: int = 30, max_iter: int = 1000, random_state: int = 42
    ):
        """
    ...
    def fit_transform(
        ...
        """
        self.logger.info(
            f"Generating {n_components}D t-SNE coordinates for {len(train_embeddings)} train and {len(test_embeddings)} test samples"
        )
        ...
        self.logger.info(
            f"Running t-SNE with perplexity={effective_perplexity}, max_iter={self.max_iter}"
        )
        tsne = TSNE(
    ...
    def _calculate_optimal_perplexity(
        ...
        if total_samples <= 20:
            ...
            self.logger.info(
                f"Few-shot scenario detected ({total_samples} samples) - using conservative perplexity"
            )
        elif total_samples <= 50:
            # Moderate reduction for small datasets
            suggested_perplexity = max(10, total_samples // 3)
            self.logger.info(
                f"Small dataset detected ({total_samples} samples) - reducing perplexity for better local structure"
            )
            ...
            self.logger.info(
                f"Very few training samples ({train_samples}) - using low perplexity for local detail"
            )
        else:
...
class BaseTSNEPlotter(ABC):
    ...
    def _create_figure(self) -> Tuple[plt.Figure, Union[plt.Axes, List[plt.Axes]]]:
        ...
        if self.use_3d:
            ...
            fig = plt.figure(
                figsize=(15, 12) if self.figsize == (10, 8) else self.figsize
            )
    ...
    def _add_semantic_legend(
        ...
        legend_text = create_bottom_legend_text(
            semantic_axes_labels, max_chars_per_line=80, max_lines=2
        )
        ...
        **kwargs,
        ...
        if self.use_3d:
            ...
            for i, ax in enumerate(axes):
                ...
                if highlight_test_idx is not None and 0 <= highlight_test_idx < len(
                    test_coords
                ):
                ...
                if abs(elev) < 10:  # Front view (elev=0) or near-front views
                    ...
                    abs(azim) < 10 or abs(azim - 360) < 10
                ):  # Side view (azim=0) or similar
    ...
    def _plot_points(
        ...
        **kwargs,
        ...
        highlight_test_indices = (
            [highlight_test_idx] if highlight_test_idx is not None else None
        )
...
class RegressionTSNEPlotter(BaseTSNEPlotter):
    ...
    def _plot_points(
        ...
        **kwargs,
        ...
        if self.use_3d:
            ax.scatter(
                test_coords[:, 0], test_coords[:, 1], test_coords[:, 2], **test_style
            )
        else:
            ax.scatter(test_coords[:, 0], test_coords[:, 1], **test_style)
        ...
        return {"legend_text": legend_text, "metadata": metadata}
...
class KNNMixin:
    ...
    def __init__(
        self, *args, knn_k: int = 5, task_type: str = "classification", **kwargs
    ):
        """
    ...
    def _create_knn_pie_chart(
        ...
        ax.set_title(
            f"K-NN Distribution (k={len(knn_labels)})", fontsize=10, weight="bold"
        )
    ...
    def _create_knn_regression_chart(
        self, ax: plt.Axes, knn_targets: np.ndarray, knn_distances: np.ndarray
    ) -> str:
        """
        ...
        if target_max > target_min:
            ...
            normalized_targets = (sorted_targets - target_min) / (
        ...
        for i, (target, distance) in enumerate(zip(sorted_targets, sorted_distances)):
            y_offset = (
                (target_max - target_min) * 0.02 if target_max > target_min else 0.1
            )
            ax.text(
                i,
                target + y_offset,
        ...
        **kwargs,
        ...
        if highlight_test_idx is not None:
            ...
            if train_embeddings is None:
                raise ValueError(
                    "KNN analysis requested but train_embeddings not provided"
                )
            if test_embeddings is None:
                raise ValueError(
                    "KNN analysis requested but test_embeddings not provided"
                )
            if not (0 <= highlight_test_idx < len(test_coords)):
                raise ValueError(
                    f"Invalid highlight_test_idx {highlight_test_idx} for {len(test_coords)} test points"
                )
        ...
        if show_knn:
            if not self.use_3d:
                ...
                gs = fig.add_gridspec(
                    1, 5, width_ratios=[2.5, 2.5, 2, 1.5, 1.5], hspace=0.1, wspace=0.15
                )
                ...
                if self.task_type == "regression":
                    ...
                    knn_description = self._create_knn_pie_chart(
                        ...
                        knn_info["labels"],
                        knn_info["distances"],
                        kwargs.get("class_names"),
                        kwargs.get("use_semantic_names", False),
                        all_classes=np.unique(train_data),
                        cached_color_mapping=getattr(
                            self, "cached_color_mapping", None
                        ),
                    )
                ...
                for i, ax in enumerate(axes):
                    # Apply zoom if highlighting a test point
                    if (
                        highlight_test_idx is not None
                        and 0 <= highlight_test_idx < len(test_coords)
                    ):
                        target_point = test_coords[highlight_test_idx]
                        self._apply_zoom(ax, target_point, train_coords, test_coords)
...
class TSNEVisualizer:
    ...
    def __init__(
        ...
        self.plotter = self._create_plotter(
            task_type, use_3d, use_knn, knn_k, figsize, zoom_factor
        )
        self.task_type = task_type
        self.use_3d = use_3d
    ...
    def create_visualization(
        ...
        **kwargs,
        ...
        train_coords, test_coords = self.generator.fit_transform(
            train_embeddings, test_embeddings, n_components=3 if self.use_3d else 2
        )
...
def create_combined_tsne_plot(
    ...
    plotter = ClassificationTSNEPlotter(
        figsize=figsize, zoom_factor=zoom_factor, use_3d=use_3d
    )
