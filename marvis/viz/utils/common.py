...
def cleanup_matplotlib_figures(warn_threshold: int = 10):
    ...
    if len(current_figures) >= warn_threshold:
        logger.warning(
            f"Found {len(current_figures)} open matplotlib figures, closing all to prevent memory leaks"
        )
...
def plot_to_image(
    ...
    try:
        ...
        fig.savefig(
            img_buffer, format=format, dpi=dpi, bbox_inches="tight", facecolor="white"
        )
        img_buffer.seek(0)
...
def save_visualization_with_metadata(
    ...
    image = plot_to_image(
        fig, dpi=dpi, force_rgb=force_rgb, max_size=max_size, close_fig=close_fig
    )
    image.save(save_path)

    # Save metadata if provided
    if metadata:
        metadata_path = save_path.replace(".png", "_metadata.json").replace(
            ".jpg", "_metadata.json"
        )
