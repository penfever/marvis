...
def create_distinct_color_map(
    unique_classes: np.ndarray, cached_color_mapping: Optional[Dict] = None
) -> Dict:
    """
...
def get_color_to_class_map(unique_classes: np.ndarray) -> Dict:
    ...
    color_to_class_map = {
        color_name: class_label
        for class_label, color_name in class_to_color_map.items()
    }
...
def extract_visible_classes_from_legend(
    unique_classes: np.ndarray,
    class_names: Optional[List[str]] = None,
    use_semantic_names: bool = False,
) -> List[Union[int, str]]:
...
def create_regression_color_map(
    target_values: np.ndarray, colormap: str = "RdBu_r", n_levels: int = 20
) -> Tuple[np.ndarray, mcolors.Colormap, float, float]:
    """
...
def apply_consistent_point_styling(
    ...
    if y is not None:
        ...
        if all_classes is not None:
            class_color_map = create_distinct_color_map(
                all_classes, cached_color_mapping
            )
        else:
            # Fallback to unique classes if all_classes not provided (for backward compatibility)
            class_color_map = create_distinct_color_map(
                unique_classes, cached_color_mapping
            )
        ...
        for class_label in actually_plotted_classes:
            ...
            color = class_color_map.get(
                class_label, (0.5, 0.5, 0.5)
            )  # Default to gray if not in map
            training_style = get_standard_training_point_style()

            # Format class label
            formatted_label = format_class_label(
                class_label, class_names, use_semantic_names
            )
        ...
        visible_class_color_map = {
            cls: class_color_map[cls]
            for cls in actually_plotted_classes
            if cls in class_color_map
        }
        legend_text = create_class_legend(
            actually_plotted_classes,
            visible_class_color_map,
            class_names,
            use_semantic_names,
            show_test_points,
        ...
        if use_3d:
            ...
            ax.scatter(transformed_data[:, 0], transformed_data[:, 1], **training_style)
    ...
    if test_data is not None and show_test_points:
        ...
        if use_3d:
            ax.scatter(test_data[:, 0], test_data[:, 1], test_data[:, 2], **test_style)
        else:
            ax.scatter(test_data[:, 0], test_data[:, 1], **test_style)
