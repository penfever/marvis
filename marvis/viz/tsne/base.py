...
class BaseTSNEVisualization(BaseVisualization):
    ...
    def get_test_transformed(self) -> Optional[np.ndarray]:
        ...
        """
        if (
            self._combined_transformed is not None
            and self._train_size is not None
            and len(self._combined_transformed) > self._train_size
        ):
            return self._combined_transformed[self._train_size :]
        return None

    def set_class_info(
        self, class_names: Optional[List[str]] = None, use_semantic_names: bool = False
    ):
        """
    ...
    def create_legend_text(
        ...
        legend_text = create_class_legend(
            unique_classes, class_color_map, self._class_names, self._use_semantic_names
        )
    ...
    def get_visible_classes(self, unique_classes: np.ndarray) -> List[Union[int, str]]:
        ...
        return extract_visible_classes_from_legend(
            unique_classes, self._class_names, self._use_semantic_names
        )
    ...
    def create_base_metadata(
        ...
        metadata = {
            ...
            "zoom_factor": (
                self.config.zoom_factor if highlight_test_idx is not None else None
            ),
        ...
        if unique_classes is not None:
            ...
            metadata.update({"visible_classes": [], "plot_type": "regression"})
