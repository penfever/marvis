...
class BaseVisualization(ABC):
    ...
    def transform(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self.supports_new_data:
            raise NotImplementedError(
                f"{self.method_name} does not support transforming new data"
            )
    ...
    def generate_plot(
        ...
        **kwargs,
        ...
        if self.config.task_type == "regression" and y is not None:
            ...
            plot_result = self._plot_classification(
                ax,
                transformed_data,
                y,
                highlight_indices,
        ...
        # Resize if needed
        if (
            image.width > self.config.max_image_size
            or image.height > self.config.max_image_size
        ):
            ratio = min(
                self.config.max_image_size / image.width,
                self.config.max_image_size / image.height,
            )
    ...
    def _plot_classification(
        ...
        **kwargs,
        ...
        if (
            self.config.use_knn_connections
            and test_data is not None
            and highlight_test_indices
            and y is not None
            and hasattr(self, "_training_embeddings")
        ):
            ...
            for test_idx in highlight_test_indices:
                if test_idx < len(test_data):
                    ...
                    query_embedding = getattr(self, "_test_embeddings", test_data)[
                        test_idx
                    ]
    ...
    def _compute_knn_analysis(
        ...
        knn = NearestNeighbors(
            n_neighbors=min(k, len(training_data)), metric="euclidean"
        )
    ...
    def _add_knn_connections_to_plot(
        ...
        for i in range(n_connections):
            ...
            max_distance = (
                np.max(neighbor_distances) if len(neighbor_distances) > 1 else 1.0
            )
            alpha = (
                max(0.1, 1.0 - (distance / max_distance)) if max_distance > 0 else 0.5
            )
