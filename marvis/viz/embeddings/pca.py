...
class PCAVisualization(BaseVisualization):
    ...
    def _robust_data_cleaning(self, X: np.ndarray) -> np.ndarray:
        ...
        if np.any(constant_mask):
            self.logger.debug(
                f"Found {np.sum(constant_mask)} constant/near-constant features, adding noise"
            )
        ...
        if len(feature_vars_updated) > 1:
            var_ratio = np.max(feature_vars_updated) / np.min(
                feature_vars_updated[feature_vars_updated > 0]
            )
            if var_ratio > 1e12:
                self.logger.debug(
                    f"Extreme variance ratio {var_ratio:.2e}, applying standardization"
                )
    ...
    def _get_default_description(self, n_samples: int, n_features: int) -> str:
        ...
        if self._transformer and hasattr(
            self._transformer, "explained_variance_ratio_"
        ):
    ...
    def _add_quality_metrics(self, result: VisualizationResult):
        ...
        if self._transformer is not None and hasattr(
            self._transformer, "explained_variance_ratio_"
        ):
            # Explained variance ratio
            result.explained_variance = float(
                np.sum(self._transformer.explained_variance_ratio_)
            )

            # Individual component variances
            result.metadata["explained_variance_ratio"] = (
                self._transformer.explained_variance_ratio_.tolist()
            )
            result.metadata["explained_variance"] = (
                self._transformer.explained_variance_.tolist()
            )
            result.metadata["cumulative_variance"] = np.cumsum(
                self._transformer.explained_variance_ratio_
            ).tolist()

            # Singular values if available
            if hasattr(self._transformer, "singular_values_"):
                result.metadata["singular_values"] = (
                    self._transformer.singular_values_.tolist()
                )
            ...
            self.logger.debug(
                f"Added PCA quality metrics: explained variance = {result.explained_variance:.3f}"
            )
    ...
    def get_explained_variance_plot(self) -> "VisualizationResult":
        ...
        """
        if not self._fitted or not hasattr(
            self._transformer, "explained_variance_ratio_"
        ):
...
def create_pca_visualization(
    ...
    **kwargs,
    ...
    config = VisualizationConfig(
        ...
        extra_params={"whiten": whiten, **kwargs},
    )
    ...
    result = viz.generate_plot(
        transformed_data=train_projection, y=y_train, test_data=test_projection
    )
