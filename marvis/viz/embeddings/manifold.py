...
class LocallyLinearEmbeddingVisualization(BaseVisualization):
    ...
    def _get_default_description(self, n_samples: int, n_features: int) -> str:
        ...
        if "n_neighbors" in params:
            description += (
                f" Using {params['n_neighbors']} neighbors for local reconstruction."
            )
        ...
        if self._transformer is not None:
            ...
            if hasattr(self._transformer, "reconstruction_error_"):
                result.reconstruction_error = float(
                    self._transformer.reconstruction_error_
                )
                result.metadata["reconstruction_error"] = float(
                    self._transformer.reconstruction_error_
                )
...
class SpectralEmbeddingVisualization(BaseVisualization):
    ...
    def _create_transformer(self, **kwargs) -> Any:
        ...
        if spectral_params["affinity"] == "nearest_neighbors":
            if spectral_params["n_neighbors"] is None:
                spectral_params["n_neighbors"] = max(
                    2, int(np.sqrt(kwargs.get("n_samples", 100)))
                )
        ...
        self.logger.info(
            f"Creating Spectral Embedding with parameters: {spectral_params}"
        )
    ...
    def _get_default_description(self, n_samples: int, n_features: int) -> str:
        ...
        if "n_neighbors" in params and affinity == "nearest_neighbors":
            description += (
                f" Using {params['n_neighbors']} neighbors for graph construction."
            )
    ...
    def _add_quality_metrics(self, result: VisualizationResult):
        ...
        if self._transformer is not None:
            ...
            result.metadata["n_neighbors"] = getattr(
                self._transformer, "n_neighbors", None
            )
            result.metadata["gamma"] = getattr(self._transformer, "gamma", None)

            self.logger.debug(
                f"Added Spectral Embedding quality metrics: {result.metadata}"
            )
...
class MDSVisualization(BaseVisualization):
    ...
    def _add_quality_metrics(self, result: VisualizationResult):
        ...
        if self._transformer is not None:
            ...
            self.logger.debug(
                f"Added MDS quality metrics: stress = {result.stress:.4f}"
            )
    ...
    for method in methods:
        ...
        try:
            ...
            result = viz.generate_plot(
                transformed_data=train_embedding, y=y_train, test_data=test_embedding
            )
