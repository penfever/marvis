...
class DecisionRegionsVisualization(BaseVisualization):
    ...
    def method_name(self) -> str:
        clf_name = (
            self.classifier.__class__.__name__ if self.classifier else "Classifier"
        )
        ...
        if embedding_method == "pca":
            ...
            try:
                ...
                transformer = umap.UMAP(
                    n_components=2, random_state=self.config.random_state
                )
            except ImportError:
    ...
    def fit_transform(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        if self.classifier is not None:
            ...
            self.logger.info(
                f"Fitted {self.classifier.__class__.__name__} on {self._embedding_method} embedding"
            )
    ...
    def generate_plot(
        ...
        **kwargs,
        ...
        if self.classifier is None:
            raise ValueError(
                "Classifier must be provided for decision regions visualization"
            )
...
def create_decision_regions_visualization(
    ...
    **kwargs,
    ...
    config = VisualizationConfig(
        ...
        extra_params={"embedding_method": embedding_method, **kwargs},
    )
