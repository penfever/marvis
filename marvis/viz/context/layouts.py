...
class LayoutManager:
    ...
    def _compose_grid_layout(self, results: List[VisualizationResult]) -> Image.Image:
        ...
        total_height = (
            n_rows * (base_height + title_height_px)
            + (n_rows - 1) * spacing_px
            + 2 * padding_px
        )
        ...
        composition = Image.new(
            "RGB", (total_width, total_height), self.layout_config.background_color
        )
        ...
        for i, result in enumerate(results):
            ...
            if result.image.size != (base_width, base_height):
                resized_image = result.image.resize(
            ...
            draw.text(
                (title_x - text_width // 2, title_y), title, fill=(0, 0, 0), font=font
            )
    ...
    def _compose_adaptive_grid_layout(
        ...
        secondary_rows = (
            (n_secondary + secondary_cols - 1) // secondary_cols
            if n_secondary > 0
            else 0
        )
        secondary_width = (
            secondary_cols * secondary_size + (secondary_cols - 1) * spacing
            if secondary_cols > 0
            else 0
        )
        ...
        total_height = (
            primary_size
            + (
                secondary_rows * secondary_size + (secondary_rows - 1) * spacing
                if secondary_rows > 0
                else 0
            )
            + 2 * padding
    ...
    def _compose_sequential_layout(
        self, results: List[VisualizationResult]
    ) -> Image.Image:
        ...
        total_width = (
            len(results) * target_width + (len(results) - 1) * spacing + 2 * padding
        )
    ...
    def _compose_hierarchical_layout(
        self, results: List[VisualizationResult]
    ) -> Image.Image:
        ...
        total_width = (
            max(
                level1_size,
                (
                    len(level2) * level2_size + (len(level2) - 1) * spacing
                    if level2
                    else 0
                ),
                (
                    len(level3) * level3_size + (len(level3) - 1) * spacing
                    if level3
                    else 0
                ),
            )
        ...
        total_height = (
            level1_size
            + (level2_size if level2 else 0)
            + (level3_size if level3 else 0)
            + 4 * padding
        )
        ...
        if level1:
            ...
            y = padding
            resized = level1[0].image.resize(
                (level1_size, level1_size), Image.Resampling.LANCZOS
            )
            composition.paste(resized, (x, y))
            self._add_text_label(
                composition,
                level1[0].method_name,
                x + level1_size // 2,
                y + level1_size + 10,
            )
        ...
        if level2:
            ...
            for i, result in enumerate(level2):
                ...
                resized = result.image.resize(
                    (level2_size, level2_size), Image.Resampling.LANCZOS
                )
                composition.paste(resized, (x, y))
                self._add_text_label(
                    composition,
        ...
        if level3:
            ...
            for i, result in enumerate(level3):
                ...
                resized = result.image.resize(
                    (level3_size, level3_size), Image.Resampling.LANCZOS
                )
                composition.paste(resized, (x, y))
                self._add_text_label(
                    composition,
    ...
    def _compose_focus_plus_context_layout(
        self, results: List[VisualizationResult]
    ) -> Image.Image:
        ...
        context_rows = (
            (len(context_results) + context_cols - 1) // context_cols
            if context_results
            else 0
        )

        context_width = (
            context_cols * context_size + (context_cols - 1) * spacing
            if context_cols > 0
            else 0
        )
        context_height = (
            context_rows * context_size + (context_rows - 1) * spacing
            if context_rows > 0
            else 0
        )

        total_width = (
            focus_size + spacing + context_width + 2 * padding
            if context_results
            else focus_size + 2 * padding
        )
        total_height = (
            max(focus_size, context_height) + 2 * padding + 50
        )  # Extra for labels
        ...
        focus_resized = focus_result.image.resize(
            (focus_size, focus_size), Image.Resampling.LANCZOS
        )
        composition.paste(focus_resized, (padding, padding))
        self._add_text_label(
            composition,
            f"Focus: {focus_result.method_name}",
            padding + focus_size // 2,
            padding + focus_size + 10,
        )
        ...
        if context_results:
            ...
            context_start_y = (
                padding + (focus_size - context_height) // 2
            )  # Center vertically
            ...
            for i, result in enumerate(context_results):
                ...
                context_resized = result.image.resize(
                    (context_size, context_size), Image.Resampling.LANCZOS
                )
    ...
    def _add_text_label(
        self, image: Image.Image, text: str, x: int, y: int, font_size: int = 16
    ):
    ...
    def _add_border_and_title(self, result: VisualizationResult) -> Image.Image:
        ...
        bordered = Image.new(
            "RGB", (new_width, new_height), self.layout_config.background_color
        )
        ...
        draw.rectangle(
            [
                padding - 2,
                padding - 2,
                padding + result.image.width + 2,
                padding + result.image.height + 2,
            ],
            outline=self.layout_config.border_color,
            width=self.layout_config.border_width,
        )
