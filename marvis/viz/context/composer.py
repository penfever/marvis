...
class CompositionConfig:
    ...
    vlm_model_type: str = (
        "general"  # "general", "vision_specialist", "reasoning_focused"
    )
...
class ContextComposer:
    ...
    def add_visualization(
        ...
        if viz_type == "decision_regions":
            ...
            if classifier_type == "svm":
                ...
                classifier = RandomForestClassifier(
                    n_estimators=100, random_state=viz_config.random_state
                )
            else:
                # Default to SVM
                classifier = SVC(kernel="rbf", random_state=viz_config.random_state)
        ...
        self.visualization_configs.append(
            {"type": viz_type, "config": config or {}, "viz_config": viz_config}
        )
                ...
                if X_test is not None and not viz.supports_new_data:
                    ...
                    if y_train is not None:
                        ...
                        combined_labels = np.concatenate(
                            [y_train, np.full(len(X_test), -1)]
                        )
                    else:
                        combined_labels = None

                    combined_transformed = viz.fit_transform(
    ...
    def compose_layout(
        ...
        if remaining_figures:
            self.logger.debug(
                f"Closing {len(remaining_figures)} remaining figures to prevent memory leaks"
            )
            ...
            multi_viz_info.append(
                {
                    ...
                    "description": viz.get(
                        "description",
                        f"Visualization of data using {viz.get('method', 'unknown')} method",
                    ),
                }
            )
    ...
    def get_visualization_comparison(self) -> Dict[str, Any]:
        ...
        if n_samples < 100:
            comparison["recommendations"].append(
                "Small dataset: Consider PCA or MDS for stable results"
            )
        if n_features > 1000:
            comparison["recommendations"].append(
                "High-dimensional data: UMAP or t-SNE may be most effective"
            )
        if self._y_train is not None and len(np.unique(self._y_train)) > 10:
            comparison["recommendations"].append(
                "Many classes: Consider spectral embedding or UMAP"
            )
