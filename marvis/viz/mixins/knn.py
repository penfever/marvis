...
def create_knn_regression_analysis(
    neighbor_targets: np.ndarray, neighbor_distances: np.ndarray, ax: plt.Axes, k: int
) -> str:
    """
    ...
    for i, (target, distance) in enumerate(zip(sorted_targets, sorted_distances)):
        ax.text(
            i,
            target + (target_max - target_min) * 0.02,
            f"d={distance:.3f}",
            ha="center",
            va="bottom",
            fontsize=8,
    ...
    description = (
        f"Neighbor Analysis: {k} nearest neighbors found in embedding space:\n"
    )
    for i, (target, distance) in enumerate(zip(sorted_targets, sorted_distances)):
...
class BaseKNNVisualization:
    ...
    def set_knn_data(
        ...
        self._knn_model = NearestNeighbors(
            n_neighbors=min(k, len(train_embeddings)), metric="euclidean"
        )
        self._knn_model.fit(train_embeddings)

    def find_knn_for_test_point(
        self, test_idx: int, k: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        if test_idx >= len(self._test_embeddings):
            raise ValueError(
                f"test_idx {test_idx} >= number of test points {len(self._test_embeddings)}"
            )
    ...
    def add_knn_connections_to_plot(
        ...
        for i, neighbor_coord in enumerate(neighbor_coords):
            ...
            alpha = (
                max(0.1, line_alpha * (1.0 - distance / max_distance))
                if max_distance > 0
                else line_alpha
            )
        ...
        if len(class_distribution) > 1:
            ...
            for class_label, count in sorted(class_distribution.items()):
                if (
                    use_semantic_names
                    and class_names
                    and class_label < len(class_names)
                ):
