...
class FrequentPatternsVisualization(BaseVisualization):
    ...
    def fit_transform(
        ...
        self.frequent_itemsets = apriori(
            self.binary_df, min_support=self.min_support, use_colnames=True
        )
        ...
        **kwargs,
        ...
        if len(self.frequent_itemsets) > 0:
            ...
            itemset_labels = [
                (
                    str(set(itemset))[:30] + "..."
                    if len(str(set(itemset))) > 30
                    else str(set(itemset))
                )
            ...
            axes[1, 0].text(
                0.5,
                0.5,
                "No frequent itemsets to display",
                ha="center",
                va="center",
        ...
        if hasattr(self, "discrete_df"):
            ...
            axes[1, 1].text(
                0.5,
                0.5,
                "No discretization data available",
                ha="center",
                va="center",
    ...
    def get_top_patterns(self, n_top: int = 10) -> Dict[str, Any]:
        ...
        result = {"top_itemsets": [], "top_rules": []}

        if len(self.frequent_itemsets) > 0:
            top_itemsets = self.frequent_itemsets.nlargest(n_top, "support")
            result["top_itemsets"] = [
                {"itemset": list(itemset), "support": float(support)}
                for itemset, support in zip(
                    top_itemsets["itemsets"], top_itemsets["support"]
                )
            ]
