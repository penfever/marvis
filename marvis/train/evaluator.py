...
def evaluate_llm_on_test_set(
    ...
    if allowed_classes is None:
        ...
        logger.info(
            f"Detected {len(allowed_classes)} classes in test data: {allowed_classes}"
        )

    # Check if model has Accelerate hooks
    has_accelerate = hasattr(model, "_hf_hook") or (
        hasattr(model, "base_model") and hasattr(model.base_model, "_hf_hook")
    )
    logger.info(
        f"Model is {'using' if has_accelerate else 'not using'} Accelerate hooks"
    )
            ...
            try:
                            ...
                            if j < len(class_labels):
                                ...
                                class_token_embedding = (
                                    model.base_model.get_input_embeddings()(
                                        torch.tensor(
                                            [class_token_id], device=param_device
                                        )
                                    ).squeeze(0)
                                )
                ...
                if allowed_classes is not None:
                    ...
                    allowed_mask = torch.zeros(
                        len(class_logits), dtype=torch.bool, device=param_device
                    )
                ...
                if return_raw_probabilities:
                    ...
                    evaluate_llm_on_test_set.raw_probabilities.append(
                ...
                if baseline_probabilities is not None:
                    ...
                    baseline_tensor = torch.tensor(
                        baseline_probabilities, device=param_device
                    )
                    ...
                    if torch.sum(calibrated_probs) == 0:
                        ...
                        calibrated_probs = torch.clamp(
                            class_probs - mean_baseline, min=0
                        )

                    # Normalize if needed (ensure they sum to positive value)
                    if torch.sum(calibrated_probs) > 0:
                        calibrated_probs = calibrated_probs / torch.sum(
                            calibrated_probs
                        )
                    else:
                    ...
                    if i % 20 == 0:
                        ...
                        logger.info(
                            f"Calibrated probs: {calibrated_probs.cpu().numpy()}"
                        )
                ...
                if i % 100 == 0:
                    logger.info(
                        f"Class token probabilities: {class_probs.cpu().numpy()}"
                    )
                    logger.info(
                        f"Selected token: {max_prob_token} (id={max_prob_token_id}) with prob={max_prob:.4f}"
                    )
                ...
                if i % 100 == 0:
                    logger.info(
                        f"Example {i}: Pred={predicted_label_id}, True={label_id}, Probability={max_prob:.4f}"
                    )
    ...
    try:
        ...
        logger.info(
            f"Test accuracy: {accuracy:.4f} ({correct}/{len(predictions)}), Balanced accuracy: {balanced_accuracy:.4f}"
        )
    except Exception as e:
            ...
            try:
                ...
                if raw_probs and len(unique_classes) > 1:
                    ...
                    if len(unique_classes) == 2:
                        ...
                        binary_truth = [
                            1 if y == pos_class_idx else 0 for y in valid_truth
                        ]
                        binary_probs = [
                            prob[pos_class_idx]
                            for prob in [raw_probs[i] for i in valid_indices]
                        ]
                    ...
                    # For multi-class classification, use one-vs-rest approach
                        ...
                        probs_array = np.array(
                            [raw_probs[i][unique_classes] for i in valid_indices]
                        )
                        # Normalize probabilities if necessary
                        probs_array = probs_array / np.sum(
                            probs_array, axis=1, keepdims=True
                        )
