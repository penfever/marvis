...
def train_llm_with_tabpfn_embeddings(
    ...
    prefix_embeddings_tensor = torch.tensor(
        original_prefix_embeddings, dtype=torch.float32
    )
    prefix_class_labels_tensor = torch.tensor(
        original_prefix_class_labels, dtype=torch.long
    )
    ...
    logger.info(
        f"Embeddings shape: {prefix_embeddings_tensor.shape}, Class labels shape: {prefix_class_labels_tensor.shape}"
    )

    # Log augmentation settings
    if permute_examples:
        logger.info(
            "Example permutation: ENABLED - Example order will be randomly shuffled each epoch"
        )
    else:
    ...
    if permute_labels:
        logger.info(
            "Label permutation: ENABLED - Class-to-label mapping will be randomly permuted each epoch"
        )
    else:
        logger.info(
            "Label permutation: DISABLED - Class-to-label mapping will remain fixed"
        )

    if variable_few_shot:
        logger.info(
            f"Variable few-shot: ENABLED - Random number between {few_shot_min} and {few_shot_max} examples"
        )
    else:
        logger.info(
            f"Variable few-shot: DISABLED - Using fixed {len(original_prefix_embeddings)} examples"
        )
    ...
    if resume_from_checkpoint:
        ...
        if os.path.exists(resume_from_checkpoint):
            ...
            if os.path.exists(training_state_path):
                ...
                class_predictions = training_state.get(
                    "class_predictions", {c: 0 for c in range(max_possible_classes)}
                )
                ...
                logger.info(
                    f"Resuming from step {global_step}, estimated epoch {initial_epoch}"
                )

                # Get the best loss if available
                best_checkpoint_path = os.path.join(
                    output_dir, "best_model", "training_state.pt"
                )
                if os.path.exists(best_checkpoint_path):
                    best_state = torch.load(best_checkpoint_path)
                    best_loss = best_state.get("running_loss", float("inf")) / max(
                        1, best_state.get("running_loss_count", 1)
                    )
                    ...
                    wandb_callback(
                        {
                            ...
                            "train/resumed_loss": running_loss
            ...
            logger.warning(
                f"Checkpoint directory {resume_from_checkpoint} not found, starting from scratch"
            )
    ...
    if max_train_samples and max_train_samples < len(train_dataset):
        ...
        logger.info(
            f"TRUNCATED training dataset to {len(train_dataset)} examples with class distribution: {dict(zip(trunc_unique, trunc_counts))}"
        )
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        collate_fn=lambda examples: {
            k: [ex[k] for ex in examples] for k in examples[0]
        },
    )
    ...
    total_steps = (
        ...
        else len(train_dataloader) * num_train_epochs // gradient_accumulation_steps
    )
    switch_point = int(
        0.4 * total_steps
    )  # Point at which to switch from base_rate_1 to decay

    logger.info(f"Training for {total_steps} total steps with learning rate schedule:")
    logger.info(
        f"  - First {switch_point} steps ({switch_point/total_steps:.1%}): Fixed rate {lr_initial}"
    )
    logger.info(
        f"  - Remaining steps: Exponential decay from {lr_initial} to {lr_final}"
    )
    logger.info(
        f"  - Temperature annealing: {temperature_initial} -> {temperature_final} over {temperature_anneal_steps} steps"
    )
    logger.info(
        f"  - Gradient penalty: weight={gradient_penalty_weight}, threshold={gradient_penalty_threshold}"
    )
    logger.info(
        f"  - Layer freezing: {'Freeze entire base LLM' if unfreeze_last_k_layers == 0 else f'Unfreeze last {unfreeze_last_k_layers} layers'}"
    )
    logger.info(f"  - Mixup augmentation with alpha={mixup_alpha}")
    logger.info(
        f"  - Minimum frequency regularization with weight={min_freq_weight}, target={min_freq_target}"
    )
    logger.info(
        f"  - {'Saving best model by training loss' if save_best_model else 'Saving final model only'}"
    )
    ...
    def freeze_base_model_layers(model, unfreeze_last_k):
        ...
        if unfreeze_last_k == 0:
            ...
            if hasattr(model.base_model, "get_input_embeddings"):
                ...
                for param in input_embeddings.parameters():
                    param.requires_grad = False
                logger.info(
                    "Explicitly froze token embeddings including class tokens (unfreeze_last_k_layers=0)"
                )
            else:
                logger.info(
                    "Token embeddings including class tokens are frozen (unfreeze_last_k_layers=0)"
                )

        if unfreeze_last_k > 0:
            # Identify the transformer layers in the base model
            # For Qwen models, layers are typically in model.layers
            if hasattr(model.base_model, "model") and hasattr(
                ...
                logger.info(
                    f"Unfroze last {min(unfreeze_last_k, num_layers)} layers (layers {start_idx}-{num_layers-1})"
                )
                ...
                logger.warning(
                    "Could not identify transformer layers structure. Attempting alternative approach."
                )
                ...
                if layer_modules:
                    ...
                    logger.warning(
                        "Could not find any layer modules. Base model remains fully frozen."
                    )
        ...
        logger.info(
            f"Layer freezing complete: {trainable_params:,} / {total_params:,} parameters are trainable ({trainable_params/total_params*100:.1f}%)"
        )
    ...
    class_token_lr_init = (
        class_token_lr_initial if class_token_lr_initial is not None else lr_initial
    )
    class_token_lr_fin = (
        class_token_lr_final if class_token_lr_final is not None else lr_final
    )
    ...
    for name, param in model.named_parameters():
        ...
        if has_vq and (
            "vq_layer" in name or "vector_quantizer" in name or "codebook" in name
        ):
    ...
    param_groups = []
    if vq_params:
        param_groups.append({"params": vq_params, "lr": vq_lr_init, "name": "vq"})
    ...
    if class_token_params:
        ...
        logger.info(
            f"Class token parameter group: {len(class_token_params)} parameters, lr={class_token_lr_init}"
        )

    if llm_params:
        param_groups.append({"params": llm_params, "lr": llm_lr_init, "name": "llm"})
        logger.info(
            f"LLM parameter group: {len(llm_params)} parameters, lr={llm_lr_init}"
        )
    ...
    if not param_groups:
        ...
        optimizer = torch.optim.AdamW(
            [p for p in model.parameters() if p.requires_grad], lr=lr_initial
        )
    else:
        ...
        def _compute_lr(self, lr_initial, lr_final, scheduler_type=None):
            ...
            if scheduler_type is None or scheduler_type == "legacy":
                ...
                if self.current_step <= self.switch_point:
                    ...
                    progress = (self.current_step - self.switch_point) / (
                        self.total_steps - self.switch_point
                    )
                ...
                progress = self.current_step / self.total_steps
                return lr_final + (lr_initial - lr_final) * 0.5 * (
        ...
        def step(self):
            ...
            for param_group in self.optimizer.param_groups:
                ...
                if group_name in self.lr_configs:
                    ...
                    if self.lr_configs:
                        lr_initial, lr_final, scheduler_type = list(
                            self.lr_configs.values()
                        )[0]
                        lr = self._compute_lr(lr_initial, lr_final, scheduler_type)
                    else:
    ...
    if resume_from_checkpoint and resume_optimizer:
        ...
        if os.path.exists(training_state_path):
            ...
            logger.info(
                f"Resuming from global step {global_step}, scheduler step {scheduler_step}"
            )
        else:
            logger.warning(
                "No optimizer state found in checkpoint, starting with fresh optimizer"
            )
    ...
    def save_model_checkpoint(step, is_best=False):
        ...
        if is_best:
            logger.debug(
                f"Saved new best model at step {step} with avg loss {best_loss:.4f}"
            )
    ...
    query_separator_id = tokenizer.convert_tokens_to_ids(
        "<|endoftext|>"
    )  # Use EOS as separator if no special token
    ...
    def process_batch(
        inputs, query_embeddings, label_ids, loss_weight, temperature=1.0
    ):
        """
        ...
        answer_start_tokens = tokenizer(
            answer_start, add_special_tokens=False
        ).input_ids
        ...
        for i in range(len(label_ids)):
            ...
            for pos in range(len(input_ids) - len(answer_start_tokens)):
                if torch.all(
                    input_ids[pos : pos + len(answer_start_tokens)]
                    == torch.tensor(answer_start_tokens, device=device)
                ):
        ...
        start_positions = (inputs["input_ids"] == prefix_start_id).nonzero(
            as_tuple=True
        )
        ...
        for i in range(inputs["input_ids"].shape[0]):
            ...
            for start_idx_pos, end_idx_pos in zip(
                ...
                num_tokens = (
                    int((end_pos - start_pos - 1).item())
                    if isinstance(end_pos, torch.Tensor)
                    else (end_pos - start_pos - 1)
                )

                # Reserve space for the query embedding (10 tokens)
                query_space = min(
                    10, num_tokens // 3
                )  # Reserve up to 1/3 of available space, max 10 tokens
                example_space = num_tokens - query_space

                # Make sure these are Python integers, not tensors
                query_space = (
                    int(query_space)
                    if isinstance(query_space, torch.Tensor)
                    else query_space
                )
                example_space = (
                    int(example_space)
                    if isinstance(example_space, torch.Tensor)
                    else example_space
                )
                ...
                if embeddings.device != device:
                    logger.warning(
                        f"Embeddings on {embeddings.device} but model on {device}, moving..."
                    )
                    embeddings = embeddings.to(device)
                if class_labels.device != device:
                    logger.warning(
                        f"Class labels on {class_labels.device} but model on {device}, moving..."
                    )
                ...
                if variable_few_shot and max_examples > few_shot_min:
                    ...
                    max_ex = (
                        int(max_ex)
                        if not isinstance(max_ex, torch.Tensor)
                        else int(max_ex.item())
                    )
                    ...
                    num_examples = (
                        int(max_examples)
                        if isinstance(max_examples, torch.Tensor)
                        else max_examples
                    )
                ...
                for j in range(num_examples):
                    ...
                    if j < len(class_labels):
                        ...
                        class_idx = (
                            int(class_labels[j].item())
                            if isinstance(class_labels[j], torch.Tensor)
                            else int(class_labels[j])
                        )
                        ...
                        all_embeddings[example_offset + j * 2 + 1] = (
        ...
        if (
            ...
            and model.embedding_projector.weight.grad is not None
        ):
            ...
            if grad_norm > gradient_penalty_threshold:
                gradient_penalty = (
                    ...
                    * (grad_norm - gradient_penalty_threshold) ** 2
                )
        ...
        for i, pos in class_positions:
            ...
            safe_class_token_ids = [
                int(tid) if isinstance(tid, (int, float)) else int(tid.item())
                for tid in class_token_ids
            ]

            # Only consider class token IDs for prediction
            class_logits = torch.stack(
                [token_logits[tid] for tid in safe_class_token_ids]
            )
        ...
        if predictions:
            ...
            if (
                total_predictions > max_possible_classes * 5
            ):  # Wait until we have a reasonable sample
                min_freq_loss = min_frequency_regularizer(
                    predictions_tensor, max_possible_classes
                )
                ...
                adaptive_weight = min_freq_weight * torch.clamp(
                    deviation * 10, min=0.1, max=1.0
                )

                # Add warmup schedule to prevent early oscillations
                warmup_steps = (
                    max_possible_classes * 50
                )  # Warmup over reasonable number of steps
                warmup_factor = (
                ...
                regularized_loss = (
                    outputs.loss + adaptive_weight * min_freq_loss + gradient_penalty
                )
            else:
    ...
    logger.info(
        f"Model supports {num_classes} classes, dataset has {actual_num_classes} unique classes"
    )
    logger.info(f"Unique labels in dataset: {sorted(unique_train_labels)}")

    if max_label >= num_classes:
        raise ValueError(
            f"Dataset contains label {max_label} but model only supports {num_classes} classes (0-{num_classes-1})"
        )
    ...
    if num_train_epochs is None:
        if max_steps is not None:
            ...
            logger.info(
                f"Setting num_train_epochs to {num_train_epochs} based on max_steps {max_steps}"
            )
        else:
            # If both are None, default to 1 epoch
            num_train_epochs = 1
            logger.warning(
                "Both num_train_epochs and max_steps are None, defaulting to 1 epoch"
            )
    ...
    for epoch in epoch_progress:
        ...
        prefix_embeddings_tensor = torch.tensor(
            original_prefix_embeddings, dtype=torch.float32, device=device
        )
        prefix_class_labels_tensor = torch.tensor(
            original_prefix_class_labels, dtype=torch.long, device=device
        )
        ...
        if permute_examples:
            ...
            logger.info(
                f"Permuted order of {len(perm_indices)} few-shot examples for epoch {epoch+1}"
            )
        ...
        if (
            permute_labels
            and actual_num_classes > 1
            and permute_labels_every_k_steps is None
            and not in_last_k_steps
        ):
            ...
            logger.info(
                f"Permuted label mapping for epoch {epoch+1}: {perm_mapping} (only showing actual classes)"
            )
        ...
        for batch_idx, batch in progress_bar:
            ...
            if no_permute_last_k is not None and max_steps is not None:
                ...
                if in_last_k_steps and label_permutation is not None:
                    ...
                    label_permutation = None
                    prefix_class_labels_tensor = (
                        original_prefix_labels_for_epoch.clone()
                    )
                    logger.info(
                        f"Disabled label permutation at step {global_step} (in last {no_permute_last_k} steps)"
                    )
            ...
            if (
                permute_labels
                and num_classes > 1
                and permute_labels_every_k_steps is not None
                and not in_last_k_steps
            ):
                if (
                    global_step > 0
                    and global_step % permute_labels_every_k_steps == 0
                    and global_step != last_permutation_step
                ):
                    ...
                    # Log the permutation for debugging
                    perm_mapping = {
                        i: label_permutation[i].item() for i in actual_indices
                    }
                    logger.info(
                        f"Permuted label mapping at step {global_step}: {perm_mapping} (only showing actual classes)"
                    )

                    # Apply permutation to prefix class labels
                    prefix_class_labels_tensor = label_permutation[
                        original_prefix_labels_for_epoch
                    ]
            ...
            query_embeddings = torch.tensor(
                np.array(batch["query_embedding"]), dtype=torch.float32
            ).to(device)
            ...
            if use_mixup:
                query_embeddings, orig_label_ids, mixed_label_ids, lam = (
                    mixup_embeddings(query_embeddings, label_ids, alpha=mixup_alpha)
                )
                ...
                for orig_id, mixed_id in zip(orig_label_ids, mixed_label_ids):
                    ...
                    orig_idx = (
                        orig_id.item()
                        if isinstance(orig_id, torch.Tensor)
                        else int(orig_id)
                    )
                    mixed_idx = (
                        mixed_id.item()
                        if isinstance(mixed_id, torch.Tensor)
                        else int(mixed_id)
                    )

                    # Original class token - use current epoch's mapping which may be permuted
                    orig_class_token = tokenizer.convert_ids_to_tokens(
                        class_token_ids[orig_idx]
                    )
                    prompts_orig.append(
                    ...
                    mixed_class_token = tokenizer.convert_ids_to_tokens(
                        class_token_ids[mixed_idx]
                    )
                    prompts_mixed.append(
                ...
                batch_loss_mixed = process_batch(
                    inputs_mixed,
                    query_embeddings,
                    mixed_label_ids,
                    1.0 - lam,
                ...
                batch_loss_value = (
                    batch_loss_orig.item() * lam + batch_loss_mixed.item() * (1.0 - lam)
                )
            else:
                ...
                current_temp = temp_scheduler.get_temperature()
                loss = process_batch(
                    inputs, query_embeddings, label_ids, 1.0, temperature=current_temp
                )
                ...
                if running_loss_count > 0:
                    ...
                    if avg_loss < best_loss and below_threshold:
                        ...
                        if save_best_model and (avg_loss < best_loss - 0.03):
                            ...
                            progress_bar.write(
                                f"New best model at step {global_step} with loss {best_loss:.4f}"
                            )
            ...
            progress_bar.set_postfix(
                ...
                lr=(
                    ", ".join(lr_display)
                    if lr_display
                    else f"{scheduler.optimizer.param_groups[0]['lr']:.2e}"
                ),
        ...
        avg_epoch_loss = (
            running_loss / running_loss_count if running_loss_count > 0 else 0
        )
        ...
        if avg_epoch_loss < best_loss and below_threshold:
            ...
            if save_best_model and (avg_epoch_loss < best_loss - 0.03):
                ...
                epoch_progress.write(
                    f"New best model at epoch {epoch+1} with loss {best_loss:.4f}"
                )
        ...
        # Check for early stopping at epoch level - only if below threshold
            ...
            if no_improve_steps >= early_stopping_patience:
                ...
                epoch_progress.set_postfix(
                    early_stopped=True, avg_loss=f"{avg_epoch_loss:.4f}"
                )
                break
        ...
        if max_steps is not None and global_step >= max_steps:
            ...
            epoch_progress.set_postfix(
                max_steps_reached=True, avg_loss=f"{avg_epoch_loss:.4f}"
            )
            break
    ...
    if save_best_model:
        logger.info(
            f"Best model was at step {best_model_step} with average loss {best_loss:.4f}"
        )
        ...
        if os.path.exists(best_model_path):
            ...
            if hasattr(model, "from_pretrained"):
                best_model = model.__class__.from_pretrained(
                    best_model_path, torch_dtype=torch.bfloat16, device_map="auto"
                )
            else:
                ...
                if hasattr(model, "embedding_projector"):
                    ...
                    best_model.embedding_projector.load_state_dict(
                        model.embedding_projector.state_dict()
                    )
            ...
            logger.warning(
                f"Best model path {best_model_path} not found, returning current model"
            )
