    ...
    if hasattr(model, "save_pretrained"):
        ...
        if class_token_ids is not None:
            model.save_pretrained(
                checkpoint_path, final_class_token_ids=class_token_ids.copy()
            )
        else:
...
def save_final_model(
    ...
    if final_class_token_ids is not None:
        if restore_original_mapping and original_class_token_ids is not None:
            logger.info(
                "Restoring original (unpermuted) class token mapping before saving final model"
            )
            save_token_ids = original_class_token_ids
        else:
            logger.info(
                "Saving final model with current (possibly permuted) class token mapping"
            )
