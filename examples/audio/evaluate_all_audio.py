        ...
        wandb.log(
            {
                ...
                "dataset": (
                    dataset_name or getattr(args, "datasets", ["unknown"])[0]
                ).upper(),
        ...
        metrics.update(
            {
                ...
                f"{model_name}/include_spectrogram": config.get(
                    "include_spectrogram", True
                ),
                f"{model_name}/vlm_model": config.get("vlm_model_id", "unknown"),
                f"{model_name}/embedding_model": config.get(
    ...
    if "detailed_results" in eval_results:
        ...
        if isinstance(detailed, dict):
            ...
            if "balanced_accuracy" in detailed:
                metrics[f"{model_name}/balanced_accuracy"] = detailed[
                    "balanced_accuracy"
                ]
...
def load_esc50_dataset(data_dir: str, args) -> tuple:
    ...
    try:
        ...
        splits = dataset.create_few_shot_split(
            k_shot=args.k_shot, val_size=0.2, test_size=0.3, random_state=42
        )
        ...
        logger.info(
            f"ESC-50 loaded: {len(train_paths)} train, {len(test_paths)} test samples"
        )
        logger.info(
...
def load_ravdess_dataset(data_dir: str, args) -> tuple:
    ...
    try:
        ...
        logger.info(
            f"RAVDESS loaded: {len(train_paths)} train, {len(test_paths)} test samples"
        )
        logger.info(
            f"Classes: {len(class_names)} - {', '.join(class_names[:5])}{'...' if len(class_names) > 5 else ''}"
        )
...
def load_urbansound8k_dataset(data_dir: str, args) -> tuple:
    ...
    try:
        ...
        logger.info(
            f"UrbanSound8K loaded: {len(train_paths)} train, {len(test_paths)} test samples"
        )
        logger.info(
            f"Classes: {len(class_names)} - {', '.join(class_names[:5])}{'...' if len(class_names) > 5 else ''}"
        )
    ...
    try:
        ...
        if dataset_name == "esc50":
            train_paths, train_labels, test_paths, test_labels, class_names = (
                load_esc50_dataset(data_dir, args)
            )
        elif dataset_name == "ravdess":
            train_paths, train_labels, test_paths, test_labels, class_names = (
                load_ravdess_dataset(data_dir, args)
            )
        elif dataset_name == "urbansound8k":
            train_paths, train_labels, test_paths, test_labels, class_names = (
                load_urbansound8k_dataset(data_dir, args)
            )
        else:
...
def run_models_on_dataset(
    ...
    if "marvis_tsne" in args.models:
        ...
        logger.info(
            f"Testing MARVIS {backend_name}{feature_str} ({args.embedding_model.upper()} → {backend_name} → VLM)..."
        )
        try:
            ...
            eval_results = classifier.evaluate(
                ...
                output_dir=(
                    os.path.join(args.output_dir, dataset_name.lower())
            ...
            logger.info(
                f"{dataset_name} MARVIS t-SNE completed: {eval_results['accuracy']:.4f} accuracy"
            )
    ...
    if "whisper_knn" in args.models:
        ...
        try:
            ...
            logger.info(
                f"{dataset_name} Whisper KNN completed: {eval_results['accuracy']:.4f} accuracy"
            )

            # Log to wandb
            if use_wandb_logging:
                log_results_to_wandb(
                    "whisper_knn", eval_results, args, class_names, dataset_name
                )
    ...
    if "clap_zero_shot" in args.models:
        ...
        try:
            classifier = CLAPZeroShotClassifier(
                ...
                use_cuda=(
                    False if sys.platform == "darwin" else None
                ),  # Auto-detect if not Mac
            ...
            logger.info(
                f"{dataset_name} CLAP zero-shot completed: {eval_results['accuracy']:.4f} accuracy"
            )

            # Log to wandb
            if use_wandb_logging:
                log_results_to_wandb(
                    "clap_zero_shot", eval_results, args, class_names, dataset_name
                )
    ...
    if results:
        ...
        return {"status": "error", "error": "No models specified for testing"}
...
def run_all_audio_tests(args):
    ...
    for dataset_key in args.datasets:
        if dataset_key in available_datasets:
            ...
            logger.warning(
                f"Unknown dataset: {dataset_key}. Available: {available_datasets}"
            )
    ...
    logger.info(
        f"Testing {len(datasets_to_test)} audio datasets: {', '.join(datasets_to_test)}"
    )
    ...
    for dataset_name in datasets_to_test:
        try:
            ...
            if result.get("status") == "success":
                ...
                logger.error(
                    f"✗ {dataset_name.upper()} failed: {result.get('error', 'Unknown error')}"
                )

        except Exception as e:
            logger.error(f"Unexpected error testing {dataset_name}: {e}")
            all_results[dataset_name.lower()] = {"status": "error", "error": str(e)}
...
def save_results(results: dict, output_dir: str, k_shot: int):
                        ...
                        summary_data.append(
                            {
                                ...
                                "train_samples": dataset_info.get(
                                    "train_samples", "N/A"
                                ),
                                "test_samples": dataset_info.get("test_samples", "N/A"),
                                "training_time": model_result.get("training_time", 0),
                                "prediction_time": model_result.get(
                                    "prediction_time", 0
                                ),
                                "error": None,
                            }
    ...
    for _, row in summary_df.iterrows():
        if row["status"] == "SUCCESS":
            success_count += 1
            logger.info(
                f"{row['dataset']:12s} {row['model']:15s}: ✓ {row['accuracy']:.4f} accuracy "
            ...
            logger.info(
                f"{row['dataset']:12s} {row['model']:15s}: ✗ ERROR - {row['error']}"
            )

    logger.info("\\n" + "-" * 80)
    logger.info(
        f"SUMMARY: {success_count}/{len(summary_data)} experiments successful across {total_datasets} datasets"
    )
        ...
        if "model" in summary_df.columns and len(summary_df["model"].unique()) > 1:
            ...
            for model in summary_df["model"].unique():
                ...
                if len(model_results) > 0:
                    ...
                    logger.info(
                        f"  {model:15s}: {model_mean:.4f} (across {len(model_results)} datasets)"
                    )
...
def parse_args():
    ...
    parser = create_audio_evaluation_parser(
        "Test audio datasets with MARVIS t-SNE and baselines"
    )
    ...
    parser.set_defaults(output_dir="./all_audio_test_results", datasets="esc50,ravdess")
...
def parse_args_old_implementation():
    ...
    parser.add_argument(
        "--quick_test", action="store_true", help="Run quick test with subset of data"
    )
    parser.add_argument(
        "--no_download", action="store_true", help="Skip automatic dataset downloads"
    )
    parser.add_argument(
        "--zoom_factor",
        type=float,
...
def main():
    ...
    if args.use_wandb and WANDB_AVAILABLE:
        ...
        total_experiments = sum(
            len(r.get("all_results", ["default"])) for r in results.values()
        )
