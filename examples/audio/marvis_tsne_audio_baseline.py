...
class MarvisAudioTsneClassifier:
    ...
    def __init__(
        ...
        if embedding_model not in ["whisper", "clap"]:
            raise ValueError(
                f"embedding_model must be 'whisper' or 'clap', got '{embedding_model}'"
            )
    ...
    def fit(
        ...
        """
        logger.info(
            f"Fitting MARVIS audio classifier with {len(train_paths)} training samples"
        )
        ...
        logger.info(
            f"Using all {len(train_paths)} training samples for t-SNE visualization"
        )
        ...
        if test_paths:
            logger.info(
                f"Extracting {self.embedding_model.upper()} embeddings for test samples..."
            )
            if self.embedding_model == "whisper":
        ...
        # Run t-SNE
        logger.info(
            f"Running {'PCA' if self.use_pca_backend else 't-SNE'} on embeddings..."
        )
        if self.use_3d:
    ...
    def predict(
        ...
        logger.info(
            f"Extracting {self.embedding_model.upper()} embeddings for test data..."
        )
        if self.embedding_model == "whisper":
            test_embeddings = get_whisper_embeddings(
                test_paths,
        ...
        for idx, (test_path, test_embedding) in enumerate(
            zip(test_paths, test_embeddings)
        ):
            try:
                ...
                should_save_viz = (
                    ...
                    and output_dir
                    and (
                        idx % self.save_every_n == 0
                        or idx == 0
                        or idx == total_samples - 1
                    )
                )
                viz_path = None
                if should_save_viz:
    ...
    def _create_test_visualization(
        self,
        ...
        Image.Image, Tuple[Image.Image, plt.Figure], Tuple[Image.Image, plt.Figure, str]
    ]:
        ...
        if self.use_knn_connections and not self.use_pca_backend:
            ...
            nbrs = NearestNeighbors(n_neighbors=5, metric="cosine").fit(
                self.plot_embeddings
            )
            ...
            test_tsne_approx = np.average(
                self.train_tsne[indices[0]], weights=weights, axis=0
            ).reshape(1, -1)

            if self.use_3d:
                # 3D KNN plot not implemented yet - fall back to standard 3D
                train_tsne, test_tsne, fig = create_tsne_3d_visualization(
                    self.plot_embeddings[:50],
                    self.plot_labels[:50],  # Limit for speed
                    test_embedding_2d,
            ...
            if self.use_3d:
                train_tsne, test_tsne, fig = create_tsne_3d_visualization(
                    self.train_embeddings[:20],
                    self.train_labels[:20],  # Limit for speed
                    test_embedding_2d,
                ...
                legend_text = (
                    "2D t-SNE visualization showing spatial clustering of audio samples"
                )
    ...
    def _create_audio_classification_prompt(self, legend_text: str = "") -> str:
        ...
        if self.embedding_model == "whisper":
            ...
            dataset_description = (
                f"Audio samples embedded using CLAP {self.clap_version} features"
            )
        else:
            dataset_description = (
                f"Audio samples embedded using {self.embedding_model} features"
            )
        ...
        elif len(self.class_names) == 50 or any(
            "sound" in str(name).lower() for name in self.class_names
        ):
            dataset_description += " for environmental sound classification"
        # Check if this is urban sound classification
        elif any(
            urban_word in str(name).lower()
            for name in self.class_names
            for urban_word in ["siren", "traffic", "urban", "street"]
        ):
        ...
        try:
            ...
            try:
                ...
                gen_config = GenerationConfig(
                    max_new_tokens=100, temperature=0.1, do_sample=True
                )
            except ImportError:
            ...
            response = self.vlm_wrapper.generate_from_conversation(
                conversation, gen_config
            )

            # Parse prediction using utility
            predicted_class = parse_vlm_response(
                response, np.array(self.class_names), logger, self.use_semantic_names
            )
    ...
    def evaluate(
        ...
        predictions, detailed_outputs = self.predict(
            test_paths, save_outputs=save_outputs, output_dir=output_dir
        )
