...
class WhisperKNNClassifier:
    ...
    def fit(
        ...
        """
        logger.info(
            f"Fitting Whisper KNN classifier with {len(train_paths)} training samples"
        )
...
class CLAPZeroShotClassifier:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.error(
                "msclap library not found. Please install with: pip install msclap"
            )
            raise ImportError(
                "msclap library required. Install with: pip install msclap"
            ) from e
        ...
        with torch.no_grad():
            for i in range(0, len(test_paths), self.batch_size):
                ...
                if i % (self.batch_size * 10) == 0:  # Log every 10 batches
                    logger.info(
                        f"Processing batch {i//self.batch_size + 1}/{(len(test_paths)-1)//self.batch_size + 1}"
                    )
                ...
                similarities = self.model.compute_similarity(
                    audio_embeddings, self.text_embeddings
                )
