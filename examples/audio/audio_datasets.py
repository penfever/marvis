...
class AudioDataset:
    ...
    def create_few_shot_split(
        ...
        logger.info(
            f"Dataset contains {len(paths)} samples across {len(np.unique(labels))} classes"
        )
...
class UrbanSound8KDataset(AudioDataset):
    ...
    def check_exists(self) -> bool:
        ...
        try:
            ...
            if valid_audio_count > 0:
                logger.debug(
                    f"soundata dataset exists with {valid_audio_count}/10 sample audio files"
                )
                return True
            else:
        ...
        soundata_exists = (self.root_dir / "UrbanSound8K").exists() and (
            self.root_dir / "UrbanSound8K" / "audio"
        ).exists()
        manual_exists = (self.root_dir / "UrbanSound8K" / "audio").exists() and (
            self.root_dir / "UrbanSound8K" / "metadata" / "UrbanSound8K.csv"
        ).exists()
    ...
    def download(self):
        ...
        try:
            ...
            logger.error(
                "soundata library is required for UrbanSound8K download. Install with: pip install soundata"
            )
    ...
    def load_metadata(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Loaded UrbanSound8K metadata using soundata API: {len(metadata_records)} clips"
            )
            ...
            logger.warning(
                f"Failed to load metadata using soundata API: {e}, falling back to CSV files"
            )
        ...
        possible_paths = [
            ...
            / "UrbanSound8K"
            / "metadata"
            / "UrbanSound8K.csv",  # standard structure
        ...
        raise FileNotFoundError(
            f"UrbanSound8K metadata not found using soundata API or CSV files in: {possible_paths}"
        )
    ...
    def get_samples(self, split: str = "all") -> Tuple[List[str], List[int], List[str]]:
        ...
        try:
            ...
            if len(paths) > 0:
                logger.info(
                    f"Loaded UrbanSound8K samples using soundata API: {len(paths)} samples"
                )
                if missing_files > 0:
                    logger.warning(
                        f"Some audio files were missing: {missing_files} files not found"
                    )
                return paths, labels, class_names
            else:
                logger.warning(
                    f"No valid audio files found using soundata API (missing: {missing_files}), falling back to CSV-based loading"
                )
            ...
            logger.warning(
                f"Failed to load samples using soundata API: {e}, falling back to CSV-based loading"
            )
        ...
        if audio_base is None:
            raise FileNotFoundError(
                f"UrbanSound8K audio directory not found in any of: {possible_audio_bases}"
            )
...
class RAVDESSDataset(AudioDataset):
    ...
    FILES = ["Audio_Speech_Actors_01-24.zip", "Audio_Song_Actors_01-24.zip"]
    ...
    def check_exists(self) -> bool:
        ...
        possible_dirs = [
            self.root_dir
            / "RAVDESS"
            / "Audio_Speech_Actors_01-24",  # Original expected structure
    ...
    def download(self):
        ...
        for filename in self.FILES:
            if (
                "Speech" in filename
                and (ravdess_dir / "Audio_Speech_Actors_01-24").exists()
            ):
                continue
            if (
                "Song" in filename
                and (ravdess_dir / "Audio_Song_Actors_01-24").exists()
            ):
                continue
            ...
            with zipfile.ZipFile(zip_path, "r") as zip_ref:
                ...
                logger.debug(
                    f"Extracted contents: {zip_ref.namelist()[:5]}..."
                )  # Show first 5 files
    ...
    def get_samples(
        self, split: str = "all", speech_only: bool = True
        ...
        if audio_dir is None:
            logger.error(
                f"No valid RAVDESS audio directory found. Checked: {possible_dirs}"
            )
            ...
            if ravdess_dir.exists():
                ...
                logger.debug(
                    f"RAVDESS directory contents: {[str(c) for c in contents]}"
                )
            return [], [], []
        ...
        for actor_dir in actor_dirs:
            ...
            logger.debug(
                f"Actor {actor_dir.name}: found {len(audio_files)} audio files"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"  Classes: {len(splits['class_names'])} - {', '.join(splits['class_names'][:5])}..."
    )
