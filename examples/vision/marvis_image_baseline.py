...
class MarvisImageClassifier:
    ...
    def __init__(
        ...
        self.tsne = TSNE(
            ...
            method=(
                "exact" if tsne_perplexity < 50 else "barnes_hut"
            ),  # Use exact method for small perplexity
        )
    ...
    def fit(
        ...
        """
        logger.info(
            f"Fitting MARVIS image classifier on {len(train_image_paths)} training images"
        )
        ...
        n_pca_components = min(
            50, train_embeddings_scaled.shape[1]
        )  # Reduce to 50 dims first
        self.pca = PCA(
            n_components=n_pca_components, random_state=self.tsne_random_state
        )
        train_embeddings_pca = self.pca.fit_transform(train_embeddings_scaled)

        # Apply t-SNE with numerical stability improvements
        logger.info(
            f"Applying t-SNE (components={self.tsne_components}, perplexity={self.tsne_perplexity})..."
        )
                ...
                self.scaler.transform(
                    self.scaler.inverse_transform(
                        self.scaler.transform(
                            np.random.random(
                                (
                                    len(self.train_tsne_embeddings),
                                    test_embeddings_scaled.shape[1],
                                )
                            )
                        )
                    )
                    ...
                    : len(self.train_tsne_embeddings)
                ],  # Placeholder for train embeddings
                test_embeddings_pca,
            ]
        )
    ...
    def _project_to_tsne_space(self, test_embeddings: np.ndarray) -> np.ndarray:
        ...
        """
        logger.info(
            "Re-fitting t-SNE on combined train+test data for better projection..."
        )
        ...
        if hasattr(self, "train_embeddings_original"):
            train_embeddings_scaled = self.scaler.transform(
                self.train_embeddings_original
            )
            train_embeddings_scaled = np.clip(train_embeddings_scaled, -5.0, 5.0)
        ...
        if train_embeddings_scaled is None:
            logger.warning(
                "Original training embeddings not stored, using approximation"
            )
            ...
            proxy_train = np.random.normal(
                0, 1, (len(self.train_tsne_embeddings), test_embeddings.shape[1])
            )
            nn.fit(proxy_train)
            ...
            test_tsne_embeddings = np.zeros(
                (len(test_embeddings), self.tsne_components)
            )
            ...
            for i, (dists, idxs) in enumerate(zip(distances, indices)):
                ...
                test_tsne_embeddings[i] = np.average(
                    self.train_tsne_embeddings[idxs], weights=weights, axis=0
                )
        ...
        adjusted_perplexity = min(
            self.tsne_perplexity, (len(combined_embeddings) - 1) / 3
        )
        ...
        combined_tsne = TSNE(
            ...
            method=(
                "exact" if len(combined_embeddings) < 1000 else "barnes_hut"
