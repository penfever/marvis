...
class GenericKNNBaseline:
    ...
    def __init__(
        self,
        embedding_extractor: GenericEmbeddingExtractor,
        n_neighbors: int = 5,
        metric: str = "cosine",
    ...
    def fit(self, train_paths: list, train_labels: list, class_names: list = None):
        """Fit the classifier."""
        logger.info(
            f"Fitting generic KNN classifier with {len(train_paths)} training samples"
        )
        ...
        if class_names is None:
            ...
            self.class_names = get_semantic_class_names_or_fallback(
                labels=unique_labels, dataset_name=getattr(self, "dataset_name", None)
            )
        else:
...
def download_and_prepare_awa2(data_dir: str = "./awa2_data") -> tuple:
    ...
    if (data_dir / "images").exists() and len(
    ...
    for filename, url in urls.items():
        ...
        extracted_indicator = data_dir / filename.replace(
            ".zip", ""
        )  # e.g., "AwA2-base", "AwA2-data"
    ...
    if (
        not (data_dir / "images").exists()
        or len(list((data_dir / "images").glob("*/*.jpg"))) < 1000
    ):
...
def organize_fishnet_data(data_dir: Path):
    ...
    if not class_dirs:
        ...
        if image_files:
            logger.warning(
                "FishNet data appears to be flat structure. Creating single class."
            )
...
def load_existing_plantdoc(data_dir: Path) -> tuple:
    ...
    if not (train_dir.exists() and test_dir.exists()):
        raise RuntimeError(
            f"PlantDoc data not found in {data_dir}. Expected train/ and test/ directories."
        )
    ...
    logger.info(
        f"Loaded PlantDoc: {len(train_paths)} train, {len(test_paths)} test images, {len(class_names)} classes"
    )
...
def load_existing_dataset(data_dir: Path, dataset_name: str) -> tuple:
    ...
    if not (train_dir.exists() and test_dir.exists()):
        raise RuntimeError(
            f"{dataset_name} data not found in {data_dir}. Expected images/train/ and images/test/ directories."
        )
    ...
    logger.info(
        f"Loaded {dataset_name}: {len(train_paths)} train, {len(test_paths)} test images, {len(class_names)} classes"
    )
    return train_paths, train_labels, test_paths, test_labels, class_names
...
def test_single_dataset(dataset_name: str, args):
    ...
    if dataset_name == "fishnet":
        train_paths, train_labels, test_paths, test_labels, class_names = (
            download_and_prepare_fishnet(dataset_dir)
        )
    elif dataset_name == "awa2":
        train_paths, train_labels, test_paths, test_labels, class_names = (
            download_and_prepare_awa2(dataset_dir)
        )
    elif dataset_name == "plantdoc":
        train_paths, train_labels, test_paths, test_labels, class_names = (
            download_and_prepare_plantdoc(dataset_dir)
        )
    else:
    ...
    if "bioclip2_knn" in args.models:
        ...
        try:
            ...
            logger.info(
                f"BioClip2 KNN completed: {eval_results['accuracy']:.4f} accuracy"
            )

            if use_wandb_logging:
                log_results_to_wandb(
                    "bioclip2_knn", eval_results, args, class_names, dataset_name
                )
    ...
    for api_backend, model_name in api_models_to_test:
        ...
        try:
            ...
            classifier.fit(
                class_names=class_names
            )  # API models don't need training data but need class names
            training_time = time.time() - start_time
            ...
            logger.info(
                f"{api_backend.title()} VLM completed: {eval_results['accuracy']:.4f} accuracy"
            )

            if use_wandb_logging:
                log_results_to_wandb(
                    model_key, eval_results, args, class_names, dataset_name
                )
    ...
    if "marvis_tsne_bioclip2" in args.models:
        ...
        try:
            classifier = MarvisImageTsneClassifier(
                ...
                bioclip2_model=getattr(
                    args, "bioclip2_model", "hf-hub:imageomics/bioclip-2"
                ),
            ...
            logger.info(
                f"MARVIS t-SNE BioClip2 completed: {eval_results['accuracy']:.4f} accuracy"
            )

            if use_wandb_logging:
                log_results_to_wandb(
                    "marvis_tsne_bioclip2",
                    eval_results,
                    args,
                    class_names,
                    dataset_name,
...
def save_results(results: dict, output_dir: str, args):
    ...
    results_file = os.path.join(
        output_dir, f"{args.dataset}_bioclip2_test_results.json"
    )
    ...
    for _, row in summary_df.iterrows():
        if row["status"] == "SUCCESS":
            logger.info(
                f"{row['model']:20s}: ✓ {row['accuracy']:.4f} accuracy "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test biological datasets with BioClip2 and MARVIS baselines"
    )
