...
class APIVLMBaseline:
    ...
    def __init__(
        ...
        if not any(model in model_name for model in supported_models):
            logger.warning(
                f"Model {model_name} may not be supported. Supported models: {supported_models}"
            )
        ...
        for batch_images, _ in test_loader:
            for i in range(len(batch_images)):
                ...
                if self.model_wrapper == "mock":
                    ...
                    prediction = self._predict_single_image(
                        image,
                        image_path=f"batch_{i}_sample_{i}",
    ...
    def _predict_single_image(
        ...
        try:
            response = self.model_wrapper.generate_from_conversation(
        ...
        if len(self.raw_responses) <= 10:
            logger.info(
                f"API VLM Response #{len(self.raw_responses)} for {image_path}:"
            )
    ...
    def save_raw_responses(self, output_path: str, benchmark_name: str = "unknown"):
        ...
        logger.info(
            f"Saved {len(self.raw_responses)} raw API responses to {output_path}"
        )
