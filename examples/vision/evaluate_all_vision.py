...
def download_and_prepare_cifar(
    data_dir: str = "./cifar_data", dataset_type: str = "cifar10"
) -> tuple:
    """
    ...
    if dataset_type == "cifar10":
        ...
        raise ValueError(
            f"Unsupported dataset type: {dataset_type}. Use 'cifar10' or 'cifar100'"
        )

    # Check if already prepared
    if images_dir.exists() and len(list(images_dir.glob("*/*.png"))) > min_images:
        logger.info(
            f"{dataset_type.upper()} already prepared, loading existing data..."
        )
...
def load_imagenet_dataset(args) -> tuple:
    ...
    logger.info(
        "Loading ImageNet-1k from HuggingFace (this may take a while on first run)..."
    )
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                f"Failed to load ImageNet from HuggingFace. You may need authentication. Error: {e2}"
            )
    ...
    def save_dataset_images(dataset, output_dir, max_samples=None):
        ...
        if max_samples:
            indices = np.random.choice(
                len(dataset), min(max_samples, len(dataset)), replace=False
            )
    ...
    )  # Use max_test_samples for train too if specified
    ...
    logger.info(
        f"ImageNet loaded: {len(train_paths)} train, {len(test_paths)} test images"
    )
...
def load_custom_dataset(dataset_path: str, num_classes: int) -> tuple:
    ...
    logger.info(
        f"Loaded custom dataset: {len(train_paths)} train, {len(test_paths)} test images"
    )
    ...
    try:
        ...
        if dataset_name in ["cifar10", "cifar100"]:
            ...
            train_paths, train_labels, test_paths, test_labels, class_names = (
                prepare_cifar_dataset(dataset_type=dataset_name, force_redownload=False)
            )
        elif dataset_name == "imagenet":
            # Load ImageNet dataset from HuggingFace
            train_paths, train_labels, test_paths, test_labels, class_names = (
                load_imagenet_dataset(args)
            )
            ...
            train_paths, train_labels, test_paths, test_labels, class_names = (
                load_custom_dataset(args.dataset_path, args.num_classes)
            )
        else:
        ...
        logger.info(
            f"  Classes: {len(class_names)} - {', '.join(class_names[:5])}{'...' if len(class_names) > 5 else ''}"
        )

        return run_models_on_dataset(
            dataset_name,
            train_paths,
            train_labels,
            test_paths,
...
def run_models_on_dataset(
    ...
    if getattr(args, "balanced_few_shot", False):
        ...
        logger.info(
            f"Sampled {len(train_paths)} training examples across {len(class_names)} classes"
        )

    # Apply max test samples limit if specified
    if (
        getattr(args, "max_test_samples", None)
        and len(test_paths) > args.max_test_samples
    ):
    ...
    if "marvis_tsne" in args.models:
        ...
        if args.use_knn_connections and args.use_pca_backend:
            logger.warning(
                "KNN connections are only supported with t-SNE, not PCA. KNN will be disabled for PCA backend."
            )

        logger.info(
            f"Testing MARVIS {backend_name}{feature_str} (DINOV2 → {backend_name} → VLM)..."
        )
        try:
            classifier = MarvisImageTsneClassifier(
                ...
                device=(
                    "cpu"
                    if sys.platform == "darwin" or args.device == "cpu"
                    else args.device
                ),
            ...
            logger.info(
                f"MARVIS t-SNE completed: {eval_results['accuracy']:.4f} accuracy"
            )

            # Log to wandb
            if use_wandb_logging:
                log_results_to_wandb(
                    "marvis_tsne", eval_results, args, class_names, dataset_name
                )
    ...
    if "marvis_simple" in args.models:
        ...
        try:
            ...
            eval_results = classifier.evaluate(
                test_paths, test_labels, return_detailed=True
            )
            eval_results["training_time"] = training_time
            eval_results["config"] = classifier.get_config()
            ...
            logger.info(
                f"Simple MARVIS completed: {eval_results['accuracy']:.4f} accuracy"
            )

            # Log to wandb
            if use_wandb_logging:
                log_results_to_wandb(
                    "marvis_simple", eval_results, args, class_names, dataset_name
                )
    ...
    if "dinov2_linear" in args.models:
        ...
        try:
            ...
            train_dataset = ImageNetDataset(
                train_paths, train_labels, get_image_transforms("train")
            )
            test_dataset = ImageNetDataset(
                test_paths, test_labels, get_image_transforms("test")
            )
            ...
            )  # Using test as validation for simplicity
            training_time = time.time() - start_time
            ...
            logger.info(
                f"DINOV2 Linear Probe completed: {eval_results['accuracy']:.4f} accuracy"
            )

            # Log to wandb
            if use_wandb_logging:
                log_results_to_wandb(
                    "dinov2_linear", eval_results, args, class_names, dataset_name
                )
    ...
    if "qwen_vl" in args.models:
        ...
        try:
            ...
            if use_wandb_logging:
                log_results_to_wandb(
                    "qwen_vl", eval_results, args, class_names, dataset_name
                )
    ...
    if "openai_vlm" in args.models:
        ...
        try:
            ...
            logger.info(
                f"OpenAI VLM completed: {eval_results['accuracy']:.4f} accuracy"
            )

            # Log to wandb
            if use_wandb_logging:
                log_results_to_wandb(
                    "openai_vlm", eval_results, args, class_names, dataset_name
                )
    ...
    if "gemini_vlm" in args.models:
        ...
        try:
            ...
            logger.info(
                f"Gemini VLM completed: {eval_results['accuracy']:.4f} accuracy"
            )

            # Log to wandb
            if use_wandb_logging:
                log_results_to_wandb(
                    "gemini_vlm", eval_results, args, class_names, dataset_name
                )
    ...
    if "api_vlm" in args.models:
        ...
        try:
            ...
            if hasattr(args, "openai_model") and args.openai_model:
                ...
                raise ValueError(
                    "Either --openai_model or --gemini_model is required when using api_vlm"
                )
            ...
            if use_wandb_logging:
                log_results_to_wandb(
                    "api_vlm", eval_results, args, class_names, dataset_name
                )
...
def log_results_to_wandb(
        ...
        wandb.log(
            {
                ...
                "dataset": (
                    dataset_name or getattr(args, "dataset", "unknown")
                ).upper(),
                "backend": getattr(args, "backend", "auto"),
    ...
    if model_name == "marvis_tsne":
        ...
        metrics.update(
            {
                ...
                f"{model_name}/use_knn_connections": config.get(
        ...
        if eval_results.get("visualizations_saved", False):
            ...
            metrics[f"{model_name}/output_directory"] = eval_results.get(
        ...
        if isinstance(class_report, dict):
            ...
            for avg_type in ["macro avg", "weighted avg"]:
                if avg_type in class_report:
                    ...
                    metrics.update(
                        {
                            f"{model_name}/precision_{avg_prefix}": avg_metrics.get(
                                "precision", 0
                            ),
                            f"{model_name}/recall_{avg_prefix}": avg_metrics.get(
                                "recall", 0
                            ),
                            f"{model_name}/f1_{avg_prefix}": avg_metrics.get(
                                "f1-score", 0
                            ),
                        }
                    )
...
def save_results(results: dict, output_dir: str, args):
    ...
    for _, row in summary_df.iterrows():
        if row["status"] == "SUCCESS":
            logger.info(
                f"{row['model']:15s}: ✓ {row['accuracy']:.4f} accuracy "
...
def parse_args():
    ...
    parser = create_vision_evaluation_parser(
        "Test image classification datasets with MARVIS and baselines"
    )
    ...
    parser.set_defaults(output_dir="./all_vision_test_results", datasets="cifar10")
...
def parse_args_old():
    ...
    parser = argparse.ArgumentParser(
        description="Test image classification datasets with MARVIS and baselines"
    )
...
def main():
    ...
    if getattr(args, "balanced_few_shot", False):
        logger.info(
            f"  Few-shot examples per class: {getattr(args, 'num_few_shot_examples', 5)}"
        )
    ...
    for dataset_name, results in all_results.items():
        ...
        if "status" in results and results["status"] == "error":
            ...
            for model_name, model_results in results.items():
                if "error" in model_results:
                    logger.info(
                        f"  {model_name:15s}: ✗ ERROR - {model_results['error']}"
                    )
                else:
                    ...
                    logger.info(
                        f"  {model_name:15s}: ✓ {accuracy:.4f} accuracy "
