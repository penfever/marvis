...
def load_imagenet_subset(
    ...
    logger.info(
        f"Found {len(valid_classes)} classes with at least {min_samples_per_class} samples"
    )
    ...
    if num_classes is not None and num_classes < len(valid_classes):
        ...
        selected_indices = np.random.choice(
            len(valid_classes), num_classes, replace=False
        )
    ...
    for label_idx, (class_name, image_files) in enumerate(valid_classes):
        ...
        if samples_per_class is not None and len(class_image_paths) > samples_per_class:
            selected_indices = np.random.choice(
                len(class_image_paths), samples_per_class, replace=False
            )
        ...
        logger.info(
            f"Class '{class_name}' (label {label_idx}): {len(class_image_paths)} samples"
        )

    logger.info(
        f"Total dataset: {len(image_paths)} images across {len(class_names)} classes"
    )
...
def split_dataset(
    ...
    logger.info(
        f"Dataset split - Train: {len(train_paths)}, Val: {len(val_paths)}, Test: {len(test_paths)}"
    )
...
def get_image_transforms(
    mode: str = "train", image_size: int = 224
) -> transforms.Compose:
    """
...
def create_data_loaders(
    ...
    train_dataset = ImageNetDataset(
        train_paths, train_labels, transform=get_image_transforms("train", image_size)
    )
    val_dataset = ImageNetDataset(
        val_paths, val_labels, transform=get_image_transforms("val", image_size)
    )
    test_dataset = ImageNetDataset(
        test_paths, test_labels, transform=get_image_transforms("test", image_size)
    )
