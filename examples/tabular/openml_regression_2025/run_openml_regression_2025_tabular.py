...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train and evaluate MARVIS on New OpenML Suite 2025 regression collection"
    )
    ...
    parser.add_argument(
        "--task_ids",
        type=str,
        help="Comma-separated list of OpenML task IDs to evaluate on. If not provided, runs on all regression 2025 tasks.",
    )
    parser.add_argument(
        "--dataset_name", type=str, help="Name of the OpenML dataset to evaluate on"
    )
    parser.add_argument(
        "--dataset_ids",
        type=str,
...
def train_on_task(task, split_idx, args):
    ...
    logger.info(
        f"Training on regression task {task_id} ({dataset_name}), split {split_idx+1}/{args.num_splits}"
    )

    # Create output directory
    model_output_dir = os.path.join(
        args.output_dir, f"task_{task_id}", f"split_{split_idx}", "model"
    )
    ...
    train_script = os.path.join(
        args.marvis_repo_path, "examples", "tabular", "train_tabular_dataset_tabular.py"
    )
    ...
    cmd = [
        ...
        "0.1",  # Lower threshold for regression (MSE-based)
        "--learning_rate",
        "1e-4",
        "--mixup_alpha",
        "0.0",  # Disable mixup for regression
        ...
        wandb_project,
        "--wandb_name",
        f"train_regression_task{task_id}_split{split_idx}",
        "--seed",
        str(args.seed + split_idx),  # Vary seed for different splits
        "--feature_selection_threshold",
        str(args.feature_selection_threshold),
        "--task_type",
        "regression",  # Explicitly specify regression task type
    ]
    ...
    try:
        ...
        logger.info(
            f"Training completed for regression task {task_id}, split {split_idx+1}"
        )
        return model_output_dir
    except subprocess.CalledProcessError as e:
        logger.error(
            f"Training failed for regression task {task_id}, split {split_idx+1}: {e}"
        )
        return None
...
def evaluate_model(task, split_idx, model_dir, args):
    ...
    logger.info(
        f"Evaluating on regression task {task_id} ({dataset_name}), split {split_idx+1}/{args.num_splits}"
    )

    # Create output directory
    eval_output_dir = os.path.join(
        args.output_dir, f"task_{task_id}", f"split_{split_idx}", "evaluation"
    )
    ...
    eval_script = os.path.join(
        args.marvis_repo_path, "examples", "tabular", "evaluate_on_dataset_tabular.py"
    )
    ...
    cmd = [
        ...
        wandb_project,
        "--wandb_name",
        f"eval_regression_task{task_id}_split{split_idx}",
    ...
    try:
        subprocess.run(cmd, check=True)
        logger.info(
            f"Evaluation completed for regression task {task_id}, split {split_idx+1}"
        )
        return eval_output_dir
    except subprocess.CalledProcessError as e:
        logger.error(
            f"Evaluation failed for regression task {task_id}, split {split_idx+1}: {e}"
        )
        return None
    ...
    for split_idx in range(args.num_splits):
        ...
        if not args.skip_training:
            ...
            if model_dir is None:
                logger.error(
                    f"Skipping evaluation for regression task {task_id}, split {split_idx+1} due to training failure"
                )
                continue
        else:
        ...
        if not args.skip_evaluation:
            ...
            if eval_dir is None:
                logger.error(
                    f"Evaluation failed for regression task {task_id}, split {split_idx+1}"
                )
...
def main():
    ...
    if args.task_ids:
        ...
        logger.info(
            f"No specific tasks provided, using all {len(tasks)} regression 2025 tasks"
        )
