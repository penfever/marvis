...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate baseline models on New OpenML Suite 2025 regression collection"
    )
...
def get_openml_regression_2025_tasks():
    ...
    try:
        ...
        logger.info(
            "Attempting to fetch regression 2025 using newer API method (get_suite)"
        )
        ...
        try:
            ...
            logger.error(
                "Could not fetch regression 2025 suite. Please check study_id=455 exists and contains regression tasks."
            )
            return []
...
def evaluate_baselines_on_task(task, split_idx, args):
    ...
    logger.info(
        f"Evaluating baselines on regression task {task_id} ({dataset_name}), split {split_idx+1}/{args.num_splits}"
    )

    # Create output directory
    eval_output_dir = os.path.join(
        args.output_dir, f"task_{task_id}", f"split_{split_idx}", "baselines"
    )
    ...
    cmd = [
        ...
        "--task_type",
        "regression",  # Explicitly specify task type
        "--seed",
        str(args.seed + split_idx),  # Use different seed for each split
    ]
    ...
    if not args.no_wandb:
        cmd.extend(
            [
                ...
                wandb_project,
                "--wandb_name",
                f"baselines_regression_task{task_id}_split{split_idx}",
            ]
        )
    ...
    try:
        ...
        logger.info(
            f"Baseline evaluation completed for regression task {task_id}, split {split_idx+1}"
        )
        return eval_output_dir
    except subprocess.CalledProcessError as e:
        logger.error(
            f"Baseline evaluation failed for regression task {task_id}, split {split_idx+1}: {e}"
        )
        return None
...
def process_task(task, args):
    ...
    # Save task metadata
    with open(
        os.path.join(task_dir, f"task_info_baselines_{run_timestamp}.json"), "w"
    ) as f:
        task_info = {
            "task_id": task_id,
    ...
    for split_idx in range(args.num_splits):
        ...
        if eval_dir is None:
            logger.error(
                f"Baseline evaluation failed for regression task {task_id}, split {split_idx+1}"
            )
