...
def parse_args():
    ...
    parser = create_tabular_llm_evaluation_parser(
        "Evaluate LLM baselines on New OpenML Suite 2025 regression collection"
    )
    ...
    for action_group in parser._mutually_exclusive_groups:
        if any(
            action.dest in ["dataset_name", "dataset_ids", "data_dir", "num_datasets"]
    ...
    parser.set_defaults(
        ...
        models=[
            ...
            "tabllm",
            "jolt",
        ],  # Remove tabula_8b for now as it may need adaptation
...
def get_openml_regression_2025_tasks():
    ...
    try:
        ...
        logger.info(
            "Attempting to fetch regression suite using newer API method (get_suite)"
        )
        suite = openml.study.get_suite(
            455
        )  # 455 is the ID for New_OpenML_Suite_2025_regression
        ...
        try:
            # Try fallback method
            logger.info(
                "Attempting fallback method with get_study and entity_type='task'"
            )
            ...
            logger.error(
                "Could not fetch regression suite. Please check study_id=455 exists and contains regression tasks."
            )
            return []
    ...
    for task_id in task_ids:
        try:
            ...
            if task.task_type.lower() != "supervised regression":
                logger.warning(
                    f"Task {task_id} is not a regression task (type: {task.task_type}), skipping"
                )
                continue
            tasks.append(task)
            logger.info(
                f"Retrieved regression task {task_id}: {task.get_dataset().name}"
            )
...
def evaluate_llm_baselines_on_task(task, split_idx, args):
    ...
    logger.info(
        f"Evaluating LLM baselines on regression task {task_id} ({dataset_name}), split {split_idx+1}/{args.num_splits}"
    )

    # Create output directory
    eval_output_dir = os.path.join(
        args.output_dir, f"task_{task_id}", f"split_{split_idx}", "llm_baselines"
    )
    ...
    if os.path.exists(results_file) and not getattr(args, "force_rerun", False):
        logger.info(
            f"Results already exist for task {task_id}, split {split_idx+1}. Skipping."
        )
    ...
    eval_script = os.path.join(
        args.marvis_repo_path,
        "examples",
        "tabular",
        "evaluate_llm_baselines_tabular.py",
    )
    ...
    cmd = [
        ...
        eval_script,
        "--task_ids",
        str(task_id),  # Pass task_id properly for regression
        "--output_dir",
        eval_output_dir,
        "--models",
    ]
    ...
    cmd.extend(
        [
            ...
            str(args.num_few_shot_examples),
            "--seed",
    ...
    cmd.extend(
        [
            ...
            "--tsne_perplexity",
            str(args.tsne_perplexity),
            "--tsne_max_iter",
            str(getattr(args, "tsne_max_iter", getattr(args, "tsne_n_iter", 1000))),
            "--max_tabpfn_samples",
            str(args.max_tabpfn_samples),
            "--nn_k",
            str(args.nn_k),
            "--max_vlm_image_size",
            str(args.max_vlm_image_size),
            "--image_dpi",
            str(args.image_dpi),
        ]
    )
    ...
    cmd.extend(
        [
            ...
            wandb_project,
            "--wandb_name",
            f"llm_baselines_regression_task{task_id}_split{split_idx}",
        ]
    )
    ...
    try:
        ...
        logger.info(
            f"LLM baseline evaluation completed for regression task {task_id}, split {split_idx+1}"
        )
        return eval_output_dir
    except subprocess.CalledProcessError as e:
        logger.error(
            f"LLM baseline evaluation failed for regression task {task_id}, split {split_idx+1}: {e}"
        )
        return None
...
def process_task(task, args):
    ...
    with open(os.path.join(task_dir, f"task_info_{run_timestamp}.json"), "w") as f:
        task_info = {
            ...
            "target_attribute": (
                task.target_name if hasattr(task, "target_name") else None
            ),
            "num_features": (
                len(task.get_dataset().features)
                if hasattr(task.get_dataset(), "features")
                and isinstance(task.get_dataset().features, dict)
                else None
            ),
    ...
    for split_idx in range(args.num_splits):
        ...
        if eval_dir is None:
            logger.error(
                f"LLM baseline evaluation failed for regression task {task_id}, split {split_idx+1}"
            )
            ...
            task_summary.append(
                {
                    ...
                    "target_attribute": (
                        task.target_name if hasattr(task, "target_name") else "unknown"
                    ),
                    "num_features": (
                        len(dataset.features)
                        if hasattr(dataset, "features")
                        and isinstance(dataset.features, dict)
                        else 0
                    ),
                    "num_instances": (
                        dataset.qualities.get("NumberOfInstances", 0)
...
def aggregate_results(args):
    ...
    for task_dir in os.listdir(args.output_dir):
        ...
        for split_dir in os.listdir(task_path):
            ...
            aggregated_file = os.path.join(
                llm_baselines_path, "aggregated_results.json"
            )
                ...
                try:
                    ...
                    for result in results:
                        ...
                        if "r2_score" in result:
                            summary_by_model[model_name]["r2_scores"].append(
                                result["r2_score"]
                            )
                            summary_by_model[model_name]["successful_tasks"] += 1
                        if "mae" in result:
                            summary_by_model[model_name]["mae_scores"].append(
                                result["mae"]
                            )
                        if "mse" in result:
                            summary_by_model[model_name]["mse_scores"].append(
                                result["mse"]
                            )
    ...
    all_results_file = os.path.join(
        args.output_dir, f"all_regression_results_{timestamp}.json"
    )
    ...
    for model_name, stats in summary_by_model.items():
        if stats["r2_scores"]:
            summary_stats[model_name] = {
                ...
                "mean_mae": (
                    np.mean(stats["mae_scores"]) if stats["mae_scores"] else None
                ),
                "std_mae": np.std(stats["mae_scores"]) if stats["mae_scores"] else None,
                "mean_mse": (
                    np.mean(stats["mse_scores"]) if stats["mse_scores"] else None
                ),
    ...
    summary_file = os.path.join(
        args.output_dir, f"regression_summary_stats_{timestamp}.json"
    )
    ...
    # Log summary
    logger.info(
        f"Aggregation complete. Found {len(all_results)} total regression results."
    )
    for model_name, stats in summary_stats.items():
        logger.info(
            f"{model_name}: Mean R² = {stats['mean_r2_score']:.4f} ± {stats['std_r2_score']:.4f}"
        )
        if stats["mean_mae"]:
            logger.info(
                f"  Mean MAE = {stats['mean_mae']:.4f} ± {stats['std_mae']:.4f}"
            )
        logger.info(
...
def main():
    ...
    logger.info(
        f"Processing regression tasks from index {start_idx} to {end_idx} (total: {len(tasks)})"
    )
    ...
    if args.skip_missing_metadata:
        ...
        logger.info(
            f"Filtered {original_count} regression tasks to {len(tasks)} tasks with valid metadata"
        )

        if len(tasks) == 0:
            logger.error(
                "No regression tasks have valid metadata for any of the requested models. Exiting."
            )
            return
