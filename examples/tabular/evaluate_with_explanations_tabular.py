...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate a pretrained MARVIS model with explanations"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    dataset_group.add_argument(
        "--data_dir", type=str, help="Directory containing CSV files to use as datasets"
    )
    dataset_group.add_argument(
        "--num_datasets",
        type=int,
...
def load_csv_dataset(
    ...
    try:
        ...
        logger.info(
            f"Dataset info: {X.shape[0]} samples, {X.shape[1]} features, {len(np.unique(y))} classes"
        )
        ...
        try:
            ...
            if len(benchmark_datasets) >= args.num_datasets:
                ...
                try:
                    ...
                    more_dataset_ids = [
                        id
                        for id in more_dataset_ids
                        if int(id) not in benchmark_datasets
                    ]
                    ...
                    if len(more_dataset_ids) >= args.num_datasets - len(dataset_ids):
                        random_ids = rng.sample(
                            more_dataset_ids, args.num_datasets - len(dataset_ids)
                        )
                        dataset_ids.extend([int(id) for id in random_ids])
                    else:
                        dataset_ids.extend([int(id) for id in more_dataset_ids])
                        logger.warning(
                            f"Could only find {len(dataset_ids)} datasets, less than requested {args.num_datasets}"
                        )
                except Exception as e:
...
def load_pretrained_model(model_path, device_map="auto", embedding_size=1000):
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        f"Loading pretrained model from {model_path} using core utility function"
    )

    return core_load_pretrained_model(
        model_path=model_path, device_map=device_map, embedding_size=embedding_size
    )
...
def evaluate_with_explanations(
    ...
    prefix_embeddings_tensor = torch.tensor(prefix_embeddings, dtype=torch.float32).to(
        param_device
    )
    prefix_class_labels_tensor = torch.tensor(prefix_class_labels, dtype=torch.long).to(
        param_device
    )
    ...
    if max_test_samples and max_test_samples < len(test_dataset):
        ...
        logger.info(
            f"TRUNCATED test dataset to {len(test_dataset)} examples for faster evaluation"
        )
        ...
        for i in tqdm(range(len(test_dataset)), desc="Evaluating with explanations"):
            ...
            try:
                ...
                query_embedding = torch.tensor(
                    np.array(example["query_embedding"]), dtype=torch.float32
                ).to(param_device)

                # First, get the initial embedding matrix from the tokens
                input_embeds = model.base_model.get_input_embeddings()(
                    inputs["input_ids"]
                ).to(param_device)
                ...
                end_positions = (inputs["input_ids"] == prefix_end_id).nonzero(
                    as_tuple=True
                )

                # Process each sequence in batch
                for seq_idx in range(inputs["input_ids"].shape[0]):
                    batch_start_positions = torch.where(start_positions[0] == seq_idx)[
                        0
                    ]
                    ...
                    for start_idx_pos, end_idx_pos in zip(
                        ...
                        if (
                            start_pos >= end_pos - 1
                        ):  # Need at least 1 token between markers
                            continue
                        ...
                        projected_query = model.embedding_projector(query_embedding).to(
                            param_device
                        )
                        ...
                        logger.info(
                            f"Unique classes in prefix data: {unique_prefix_classes}"
                        )

                        # Safety check - if we don't have at least 2 classes, we need to log a warning
                        if len(unique_prefix_classes) < 2:
                            logger.warning(
                                f"WARNING: Only {len(unique_prefix_classes)} classes found in prefix data! This may lead to poor performance."
                            )
                        ...
                        indices = torch.randperm(len(class_labels))[:num_examples].to(
                            param_device
                        )
                        ...
                        projected_examples = model.embedding_projector(
                            selected_embeddings
                        ).to(param_device)
                        ...
                        logger.info(
                            f"Selected {len(selected_classes)} unique classes for prefix examples: {selected_classes}"
                        )
                        ...
                        for j in range(len(indices)):
                            # Example embedding
                            all_embeddings[example_offset + j * 2] = projected_examples[
                                j
                            ]
                        ...
                        input_embeds[seq_idx, start_pos + 1 : end_pos, :] = (
                            all_embeddings
                        )
                ...
                class_token_text = tokenizer.decode(
                    [max_prob_token_id], skip_special_tokens=False
                )
                ...
                explanation_prompt = get_explanation_prompt(
                    explanation_type, class_token_text
                )
                ...
                explanation_inputs = {
                    k: v.to(param_device) for k, v in explanation_inputs.items()
                }

                # We need to handle the embeddings for this new input too
                explanation_embeds = model.base_model.get_input_embeddings()(
                    explanation_inputs["input_ids"]
                ).to(param_device)

                # Find positions of PREFIX_START and PREFIX_END in the new input
                explanation_start_positions = (
                    explanation_inputs["input_ids"] == prefix_start_id
                ).nonzero(as_tuple=True)
                explanation_end_positions = (
                    explanation_inputs["input_ids"] == prefix_end_id
                ).nonzero(as_tuple=True)

                # If the token contains prefix placeholders, fill them the same way we did before
                if (
                    len(explanation_start_positions[0]) > 0
                    and len(explanation_end_positions[0]) > 0
                ):
                    ...
                    for seq_idx in range(explanation_inputs["input_ids"].shape[0]):
                        batch_start_positions = torch.where(
                            explanation_start_positions[0] == seq_idx
                        )[0]
                        batch_end_positions = torch.where(
                            explanation_end_positions[0] == seq_idx
                        )[0]
                        ...
                        for start_idx_pos, end_idx_pos in zip(
                            ...
                            explanation_embeds[seq_idx, start_pos + 1 : end_pos, :] = (
                                all_embeddings
                            )
                            logger.info(
                                "Using same prefix embeddings for explanation generation"
                            )
                ...
                with torch.no_grad():
                    ...
                    full_output_text = tokenizer.decode(
                        explanation_output[0], skip_special_tokens=False
                    )
                ...
                if explanation_marker in full_output_text:
                    explanation_start = full_output_text.find(explanation_marker) + len(
                        explanation_marker
                    )
                ...
                if i % 5 == 0:
                    ...
                    logger.info(
                        f"Prediction: {predicted_label_id}, True: {label_id}, Probability: {max_prob:.4f}"
                    )
    ...
    try:
        ...
        if valid_pred:
            ...
            logger.info(
                f"Dataset has {len(unique_classes)} unique classes: {unique_classes}"
            )
            ...
            report = classification_report(
                valid_truth, valid_pred, target_names=class_names, output_dict=True
            )
...
def process_dataset(
    ...
    logger.info(
        f"Dataset {dataset_name} shapes - Train: {X_train.shape}, Val: {X_val.shape}, Test: {X_test.shape}"
    )
    ...
    train_embeddings, val_embeddings, test_embeddings, tabpfn, y_train_sample = (
        get_tabpfn_embeddings(
            X_train,
            y_train,
            X_val,
    ...
    logger.info(
        f"Evaluating model on {dataset_name} test set with {args.explanation_type} explanations"
    )
    ...
    logger.info(
        f"Evaluation complete. {dataset_name} test accuracy: {results['accuracy']:.4f}"
    )
    ...
    results_file = os.path.join(
        dataset_output_dir,
        f"{dataset_name}_{args.explanation_type}_evaluation_results.json",
    )
    ...
    if args.save_explanations:
        ...
        explanations_file = os.path.join(
            dataset_output_dir,
            f"{dataset_name}_{args.explanation_type}_explanations.json",
        )
        ...
        if "classification_report" in result_summary:
            ...
            for class_name, metrics in report.items():
                if isinstance(metrics, dict):  # Skip avg/total sections
                    wandb.log(
                        {
                            f"{dataset_name}/class_{class_name}/precision": metrics[
                                "precision"
                            ],
                            f"{dataset_name}/class_{class_name}/recall": metrics[
                                "recall"
                            ],
                            f"{dataset_name}/class_{class_name}/f1": metrics[
                                "f1-score"
                            ],
                        }
                    )
            ...
            if "explanations" in results:
                ...
                correct_indices = [
                    ...
                    for i, (p, g) in enumerate(
                        zip(results["predictions"], results["ground_truth"])
                    )
                    if p == g
                ]
                incorrect_indices = [
    ...
    for i in range(num_examples):
        ...
        logger.info(
            f"Pred: {results['predictions'][i]}, True: {results['ground_truth'][i]}"
        )
...
def main():
    ...
    if args.use_wandb:
        if not WANDB_AVAILABLE:
            logger.warning(
                "Weights & Biases (wandb) package not available. Will not log to W&B."
            )
        else:
    ...
    model, tokenizer, prefix_start_id, prefix_end_id, class_token_ids = (
        load_pretrained_model(
            args.model_path, device_map=args.device, embedding_size=args.embedding_size
        )
    )
    ...
    for i, dataset in enumerate(datasets):
        logger.info(
            f"Processing dataset {i+1}/{len(datasets)}: {dataset['name']} (ID: {dataset['id']})"
        )
        ...
        try:
            ...
            all_results.append({"dataset_name": dataset["name"], "error": str(e)})
    ...
    if valid_results:
        ...
        logger.info(
            f"Average accuracy across {len(valid_results)} datasets: {avg_accuracy:.4f}"
        )
        ...
        summary = {
            ...
            "dataset_accuracies": {
                r["dataset_name"]: r["accuracy"] for r in valid_results
            },
            "timestamp": timestamp,
        }

        summary_file = os.path.join(
            args.output_dir, f"summary_{args.explanation_type}_{timestamp}.json"
        )
        ...
        if args.use_wandb and WANDB_AVAILABLE:
            ...
            ax.set_title(
                f"Accuracy across datasets ({args.explanation_type} explanations)"
            )
    ...
    logger.info(
        f"Evaluation with explanations complete! Results saved to {args.output_dir}"
    )
