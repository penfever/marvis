...
def process_dataset(dataset: Dict[str, Any], args) -> Dict[str, Any]:
    ...
    if "X_train" in dataset and "y_train" in dataset:
        logger.info(
            f"Dataset {dataset['name']} already preprocessed, adding embeddings if needed"
        )
        ...
        if "train_embeddings" not in dataset:
            logger.info(
                f"Computing TabPFN embeddings for dataset {dataset['name']} with size {args.embedding_size}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Computed embeddings - Train: {train_embeddings.shape}, Val: {val_embeddings.shape}, Test: {test_embeddings.shape}"
                )

            except Exception as e:
                logger.error(
                    f"Failed to compute TabPFN embeddings for dataset {dataset['name']}: {e}"
                )
                raise
...
def get_token_ids_from_model(tokenizer):
    ...
    class_token_ids = [
        tokenizer.convert_tokens_to_ids(f"<CLASS_{i}>") for i in range(10)
    ]
...
def load_pretrained_model(
    model_path, device_map="auto", embedding_size=1000, model_id=None
):
    ...
    if has_vq_files:
        ...
        try:
            ...
            model, tokenizer, prefix_start_id, prefix_end_id, class_token_ids, is_vq = (
...
def evaluate_dataset(
    ...
    if args.label_fitting:
        logger.info(
            f"Label fitting enabled with holdout ratio {args.label_fitting_holdout_ratio}"
        )
        ...
        n_holdout = min(
            int(n_embeddings * args.label_fitting_holdout_ratio), n_embeddings
        )
        ...
        holdout_indices = holdout_rng.choice(
            n_embeddings, size=n_holdout, replace=False
        )
        ...
        holdout_dataset, _, _, _, _ = create_llm_dataset(
            ...
            y_holdout,
            empty_X,
            empty_y,  # Empty validation set
            empty_X,
            empty_y,  # Empty test set
            embeddings_holdout,
            empty_embeddings,
            empty_embeddings,
            tokenizer,
        ...
        logger.info(
            f"Getting predictions on {len(holdout_dataset)} holdout samples for label fitting"
        )
    ...
    if label_mapping is not None and "predictions" in results:
        ...
        remapped_predictions = apply_label_mapping(
            results["predictions"], label_mapping
        )
        ...
        results["classification_report"] = classification_report(
            y_test, remapped_predictions, output_dict=True
        )
        results["confusion_matrix"] = confusion_matrix(y_test, remapped_predictions)

        logger.info(
            f"Applied label mapping: accuracy changed from {original_accuracy:.4f} to {remapped_accuracy:.4f}"
        )
    ...
    if "balanced_accuracy" in results and results["balanced_accuracy"] is not None:
        logger.info(
            f"Test balanced accuracy on {dataset['name']}: {results['balanced_accuracy']:.4f}"
        )
    ...
    if args.label_fitting:
        ...
        if label_mapping is not None:
            ...
            result_summary["label_fitting"]["original_accuracy"] = float(
                results.get("original_accuracy", results["accuracy"])
            )
            result_summary["label_fitting"]["remapped_accuracy"] = float(
                results["accuracy"]
            )
        else:
    ...
    if "confusion_matrix" in results:
        result_summary["confusion_matrix"] = (
            results["confusion_matrix"].tolist()
            if not isinstance(results["confusion_matrix"], list)
...
def main():
    ...
    if hasattr(args, "run_all_baselines") and args.run_all_baselines:
        raise ValueError(
            "--run_all_baselines is deprecated. Use --models all_baselines instead."
        )

    if hasattr(args, "baselines_only") and args.baselines_only:
        raise ValueError(
            "--baselines_only is deprecated. Use --models with baseline model names instead (e.g., --models catboost tabpfn_v2)."
        )

    if hasattr(args, "model_path") and args.model_path:
        raise ValueError(
            "--model_path is deprecated. Use --models with the model path instead (e.g., --models /path/to/model)."
        )

    if hasattr(args, "model_id") and args.model_id:
        raise ValueError(
            "--model_id is deprecated. Use --models with the model identifier instead (e.g., --models Qwen/Qwen2.5-7B-Instruct)."
        )
    ...
    if args.use_wandb:
        if not WANDB_AVAILABLE:
            ...
            logger.info(
                f"Initialized Weights & Biases run with GPU monitoring: {args.wandb_name}"
            )
    ...
    all_classification = all(
        dataset["is_classification"] for dataset in processed_datasets
    )
    all_regression = all(
        not dataset["is_classification"] for dataset in processed_datasets
    )
    ...
    if all_classification:
        ...
        logger.info(
            "All datasets are classification tasks - using full baseline model set"
        )
    elif all_regression:
        baseline_models = [
            "catboost",
        ...
        logger.info(
            "All datasets are regression tasks - using regression-compatible baseline models"
        )
    else:
        baseline_models = [
            "catboost",
            ...
            "gradient_boosting",
            "logistic_regression",
        ]  # Safe set for mixed tasks
        logger.info(
            "Mixed classification/regression tasks - using universally compatible baseline models"
        )
    ...
    for model_spec in args.models:
        if model_spec == "all_baselines":
            ...
            model_spec == "marvis_tsne"
            or "/" in model_spec
            or model_spec.startswith(".")
            or os.path.exists(model_spec)
        ):
            ...
            if model_spec == "marvis_tsne":
                ...
                logger.error(
                    "marvis_tsne requires a model path. Please specify the path to your trained MARVIS model."
                )
                continue
            elif (
                os.path.exists(model_spec)
                or model_spec.startswith(".")
                or model_spec.startswith("/")
            ):
                # It's a local path
    ...
    if not models_to_evaluate:
        raise ValueError(
            "No valid models specified. Use --models to specify models to evaluate."
        )
    ...
    for model_type, model_identifier in models_to_evaluate:
        ...
        logger.info(
            f"Evaluating model {model_identifier} (type: {model_type}) on {len(processed_datasets)} datasets"
        )
            ...
            if cache_key not in model_cache:
                ...
                try:
                    ...
                    (
                        ...
                        embedding_size=args.embedding_size, model_id=model_identifier
                    )
                    model_cache[cache_key] = (
                        model,
                        tokenizer,
                    ...
                    logger.info(
                        f"Successfully loaded and cached model {model_identifier}"
                    )
                except Exception as e:
                ...
                if cached_value is None:
                    logger.error(
                        f"Previously failed to load model {model_identifier}, skipping"
                    )
                    continue
                model, tokenizer, prefix_start_id, prefix_end_id, class_token_ids = (
                    cached_value
        ...
        for dataset in processed_datasets:
            try:
                if model_type == "marvis":
                    ...
                    results = create_and_evaluate_baseline_model(
                    ...
                    # Add model type to results
                    results["model_type"] = (
                        "llm_baseline" if model_type == "llm_baseline" else "llm"
                    )
                ...
                if args.use_wandb and WANDB_AVAILABLE:
                    ...
                    if (
                        "prediction_distribution" in results
                        and "ground_truth_distribution" in results
                        and results["prediction_distribution"] is not None
                    ):
                ...
                logger.error(
                    f"Error evaluating {model_identifier} on dataset {dataset['name']}: {e}"
                )
                import traceback
        ...
        if valid_results:
            ...
            if classification_results:
                avg_accuracy = sum(r["accuracy"] for r in classification_results) / len(
                    classification_results
                )
                logger.info(
                    f"Average accuracy for {model_identifier}: {avg_accuracy:.4f} over {len(classification_results)} datasets"
                )

            if regression_results:
                avg_mse = sum(r["mse"] for r in regression_results) / len(
                    regression_results
                )
                avg_r2 = (
                    sum(r["r2"] for r in regression_results if not np.isnan(r["r2"]))
                    / len([r for r in regression_results if not np.isnan(r["r2"])])
                    if any(not np.isnan(r["r2"]) for r in regression_results)
                    else None
                )
                logger.info(
                    f"Average MSE for {model_identifier}: {avg_mse:.4f} over {len(regression_results)} datasets"
                )
                if avg_r2 is not None:
                    logger.info(
                        f"Average R² for {model_identifier}: {avg_r2:.4f} over {len(regression_results)} datasets"
                    )
            ...
            if args.use_wandb and WANDB_AVAILABLE:
                model_name = (
                    model_identifier if model_type != "baseline" else model_identifier
                )
                ...
                aggregated_metrics = {"num_valid_datasets": len(valid_results)}
                ...
                balanced_accs = [
                    r.get("balanced_accuracy")
                    for r in valid_results
                    if r.get("balanced_accuracy") is not None
                ]
                if balanced_accs:
                    avg_balanced_acc = sum(balanced_accs) / len(balanced_accs)
                    aggregated_metrics["balanced_accuracy"] = avg_balanced_acc
                    logger.info(
                        f"Average balanced accuracy for {model_identifier}: {avg_balanced_acc:.4f}"
                    )
                ...
                if roc_aucs:
                    ...
                    logger.info(
                        f"Average ROC AUC for {model_identifier}: {avg_roc_auc:.4f}"
                    )
    ...
    all_results_file = os.path.join(
        args.output_dir, f"all_evaluation_results_{timestamp}.json"
    )
    ...
    for model_type, model_identifier in models_to_evaluate:
        model_results = [
            ...
            if (
                ...
                and (
                    r.get("model_id") == model_identifier
                    or r.get("model_name") == model_identifier
                )
            )
        ]
        ...
        if valid_results:
            avg_accuracy = sum(r["accuracy"] for r in valid_results) / len(
                valid_results
            )
            ...
            model_summary = {
                ...
                "dataset_accuracies": {
                    r["dataset_name"]: float(r["accuracy"]) for r in valid_results
                },
            }
            ...
            balanced_accs = [
                r
                for r in valid_results
                if "balanced_accuracy" in r and r["balanced_accuracy"] is not None
            ]
            if balanced_accs:
                avg_balanced_acc = sum(
                    r["balanced_accuracy"] for r in balanced_accs
                ) / len(balanced_accs)
                model_summary["average_balanced_accuracy"] = float(avg_balanced_acc)
                model_summary["dataset_balanced_accuracies"] = {
                    r["dataset_name"]: float(r["balanced_accuracy"])
                    for r in balanced_accs
                }
                logger.info(
                    f"Average balanced accuracy for {model_identifier}: {avg_balanced_acc:.4f} over {len(balanced_accs)} datasets"
                )

            # Add ROC AUC information if available
            roc_aucs = [
                r for r in valid_results if "roc_auc" in r and r["roc_auc"] is not None
            ]
            if roc_aucs:
                avg_roc_auc = sum(r["roc_auc"] for r in roc_aucs) / len(roc_aucs)
                model_summary["average_roc_auc"] = float(avg_roc_auc)
                model_summary["dataset_roc_aucs"] = {
                    r["dataset_name"]: float(r["roc_auc"]) for r in roc_aucs
                }
                logger.info(
                    f"Average ROC AUC for {model_identifier}: {avg_roc_auc:.4f} over {len(roc_aucs)} datasets"
                )
    ...
    for model_type, model_identifier in models_to_evaluate:
        ...
        for result in model_results:
            if (
                "prediction_distribution" in result
                and "ground_truth_distribution" in result
                and result["prediction_distribution"] is not None
            ):
    ...
    if args.use_wandb and WANDB_AVAILABLE:
        ...
        for model_summary in summary["model_summaries"]:
            ...
            if "dataset_balanced_accuracies" in model_summary:
                balanced_accs = [
                    acc
                    for acc in model_summary["dataset_balanced_accuracies"].values()
                    if acc is not None
                ]
                if balanced_accs:
                    avg_balanced_acc = sum(balanced_accs) / len(balanced_accs)
            ...
            if "dataset_roc_aucs" in model_summary:
                roc_aucs = [
                    auc
                    for auc in model_summary["dataset_roc_aucs"].values()
                    if auc is not None
                ]
                if roc_aucs:
                    avg_roc_auc = sum(roc_aucs) / len(roc_aucs)
        ...
        try:
            ...
            model_names = [
                f"{ms['model_identifier']} ({ms['model_type']})" for ms in all_summaries
            ]
            accuracies = [ms["average_accuracy"] for ms in all_summaries]
            colors = [
                (
                    "#1f77b4"
                    if ms["model_type"] == "marvis" or ms["model_type"] == "llm"
                    else "#ff7f0e" if ms["model_type"] == "baseline" else "#2ca02c"
                )
