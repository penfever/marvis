...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("openml_cc18_run.log"), logging.StreamHandler()],
)
...
def train_on_task(task, split_idx, args):
    ...
    logger.info(
        f"Training on task {task_id} ({dataset_name}), split {split_idx+1}/{args.num_splits}"
    )
...
def evaluate_model(task, split_idx, model_dir, args):
    ...
    logger.info(
        f"Evaluating on task {task_id} ({dataset_name}), split {split_idx+1}/{args.num_splits}"
    )
    ...
    for split_idx in range(args.num_splits):
        ...
        if not args.skip_training:
            ...
            if model_dir is None:
                logger.error(
                    f"Skipping evaluation for task {task_id}, split {split_idx+1} due to training failure"
                )
                continue
        else:
        ...
        if not args.skip_evaluation:
            ...
            if eval_dir is None:
                logger.error(
                    f"Evaluation failed for task {task_id}, split {split_idx+1}"
                )
