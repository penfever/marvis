...
def parse_args():
    ...
    parser = create_tabular_llm_evaluation_parser(
        "Evaluate LLM baselines on OpenML CC18 collection"
    )
...
def evaluate_llm_baselines_on_task(task, split_idx, args):
    ...
    logger.info(
        f"Evaluating LLM baselines on task {task_id} ({dataset_name}), split {split_idx+1}/{args.num_splits}"
    )
    ...
    cmd.extend(
        [
            ...
            wandb_project,
            "--wandb_name",
            f"llm_baselines_task{task_id}_split{split_idx}",
        ]
    )
    ...
    try:
        ...
        logger.info(
            f"LLM baseline evaluation completed for task {task_id}, split {split_idx+1}"
        )
        return eval_output_dir
    except subprocess.CalledProcessError as e:
        logger.error(
            f"LLM baseline evaluation failed for task {task_id}, split {split_idx+1}: {e}"
        )
        return None
...
def process_task(task, args):
    ...
    with open(os.path.join(task_dir, f"task_info_{run_timestamp}.json"), "w") as f:
        task_info = {
            ...
            "num_classes": (
                len(task.class_labels) if hasattr(task, "class_labels") else None
            ),
            "num_features": (
    ...
    for split_idx in range(args.num_splits):
        ...
        if eval_dir is None:
            logger.error(
                f"LLM baseline evaluation failed for task {task_id}, split {split_idx+1}"
            )
...
def aggregate_results(args):
                ...
                try:
                    ...
                    for result in results:
                        ...
                        if "accuracy" in result:
                            summary_by_model[model_name]["accuracies"].append(
                                result["accuracy"]
                            )
                            summary_by_model[model_name]["successful_tasks"] += 1
                        if "balanced_accuracy" in result:
                            summary_by_model[model_name]["balanced_accuracies"].append(
        ...
        if stats["accuracies"]:
            summary_stats[model_name] = {
                ...
                "mean_balanced_accuracy": (
                    np.mean(stats["balanced_accuracies"])
                    if stats["balanced_accuracies"]
                    else None
                ),
                "std_balanced_accuracy": (
                    np.std(stats["balanced_accuracies"])
                    if stats["balanced_accuracies"]
                    else None
                ),
    ...
    for model_name, stats in summary_stats.items():
        logger.info(
            f"{model_name}: Mean accuracy = {stats['mean_accuracy']:.4f} ± {stats['std_accuracy']:.4f} "
...
def main():
    ...
    logger.info(
        f"Processing tasks from index {start_idx} to {end_idx} (total: {len(tasks)})"
    )
    ...
    if args.skip_missing_metadata:
        ...
        logger.info(
            f"Filtered {original_count} tasks to {len(tasks)} tasks with valid metadata"
        )

        if len(tasks) == 0:
            logger.error(
                "No tasks have valid metadata for any of the requested models. Exiting."
            )
            return
