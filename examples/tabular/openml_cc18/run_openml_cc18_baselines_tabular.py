...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate baseline models on OpenML CC18 collection"
    )
    ...
    parser.add_argument(
        "--task_ids",
        type=str,
        help="Comma-separated list of OpenML task IDs to evaluate on. If not provided, runs on all CC18 tasks.",
    )
    parser.add_argument(
...
def evaluate_baselines_on_task(task, split_idx, args):
    ...
    logger.info(
        f"Evaluating baselines on task {task_id} ({dataset_name}), split {split_idx+1}/{args.num_splits}"
    )
    ...
    cmd = [
        ...
        eval_output_dir,
        "--models",
        "all_baselines",  # Run all baseline models (replaces deprecated --run_all_baselines and --baselines_only)
    ...
    if not args.no_wandb:
        cmd.extend(
            [
                ...
                wandb_project,
                "--wandb_name",
                f"baselines_task{task_id}_split{split_idx}",
            ]
        )
    ...
    try:
        ...
        logger.info(
            f"Baseline evaluation completed for task {task_id}, split {split_idx+1}"
        )
        return eval_output_dir
    except subprocess.CalledProcessError as e:
        logger.error(
            f"Baseline evaluation failed for task {task_id}, split {split_idx+1}: {e}"
        )
        return None
...
def process_task(task, args):
    ...
    for split_idx in range(args.num_splits):
        ...
        if eval_dir is None:
            logger.error(
                f"Baseline evaluation failed for task {task_id}, split {split_idx+1}"
            )
