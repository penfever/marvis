...
def parse_args():
    ...
    parser = create_tabular_llm_evaluation_parser(
        "Evaluate LLM baselines (TabLLM, Tabula-8B, JOLT, MARVIS-T-SNe) on tabular datasets"
    )
    ...
    parser.set_defaults(output_dir="./llm_baseline_results")
...
def parse_args_old():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate LLM baselines (TabLLM, Tabula-8B, JOLT, MARVIS-T-SNe) on tabular datasets"
    )
    ...
    dataset_group.add_argument(
        "--dataset_name", type=str, help="Name of a single dataset to evaluate on"
    )
    dataset_group.add_argument(
        "--dataset_ids",
        type=str,
    ...
    dataset_group.add_argument(
        "--data_dir", type=str, help="Directory containing CSV files to evaluate on"
    )
    dataset_group.add_argument(
        "--num_datasets",
        type=int,
    ...
    parser.add_argument(
        "--gpu_index", type=int, default=0, help="GPU index to use when device is cuda"
    )
    parser.add_argument(
        "--timeout_minutes",
        type=int,
...
def apply_k_shot_split(dataset: Dict, k_shot: int, random_state: int = 42) -> Dict:
    ...
    if hasattr(X, "values"):
        ...
        X_array = np.array(X)
        original_columns = dataset.get(
            "attribute_names", [f"feature_{i}" for i in range(X_array.shape[1])]
        )
    ...
    logger.info(
        f"Applying k-shot split: {k_shot} samples per class, {len(unique_classes)} classes"
    )
    ...
    for class_label in unique_classes:
        ...
        if n_samples < k_shot:
            logger.warning(
                f"Class {class_label} only has {n_samples} samples, requested {k_shot}"
            )
    ...
    logger.info(
        f"K-shot dataset: {len(X_kshot)} total samples ({k_shot} per class * {len(unique_classes)} classes)"
    )

    return dataset_kshot


def apply_balanced_few_shot_selection(
    X_train, y_train, num_examples: int, random_state: int = 42
):
    """
...
def main():
    ...
    if args.k_shot is not None:
        logger.warning(
            "--k_shot is deprecated. Use --num_few_shot_examples with --balanced_few_shot instead."
        )
        ...
        if args.num_few_shot_examples is None and not args.balanced_few_shot:
            ...
            logger.info(
                f"Converted --k_shot {args.k_shot} to --num_few_shot_examples {args.k_shot} --balanced_few_shot"
            )
    ...
    for dataset in datasets:
        ...
        try:
            ...
            task_type, detection_method = detect_task_type(
                y=y_data, dataset=dataset, task_id=task_id
            )
            ...
            if args.skip_classification and task_type == "classification":
                logger.info(
                    f"Skipping classification dataset {dataset['name']} (--skip_classification enabled)"
                )
                continue
            elif args.skip_regression and task_type == "regression":
                logger.info(
                    f"Skipping regression dataset {dataset['name']} (--skip_regression enabled)"
                )
                continue

        except Exception as e:
            logger.warning(
                f"Could not detect task type for {dataset['name']}: {e}. Proceeding with evaluation."
            )
            task_type = "unknown"
        ...
        try:
            ...
            if openml_task_id:
                ...
                if hasattr(X_data, "shape"):
                    ...
                    feature_count = None
                metadata_results = validate_metadata_for_models(
                    openml_task_id, models_to_evaluate, feature_count
                )

                # Log validation results and filter models
                valid_models = []
                for model_name in models_to_evaluate:
                    validation = metadata_results.get(
                        model_name, {"valid": True, "errors": [], "warnings": []}
                    )
                    if validation["valid"]:
                        ...
                        if validation["warnings"]:
                            for warning in validation["warnings"]:
                                logger.warning(
                                    f"{model_name} metadata warning: {warning}"
                                )
                    else:
                        if args.skip_missing_metadata:
                            logger.warning(
                                f"Skipping {model_name} on {dataset['name']} due to metadata validation failures:"
                            )
                ...
                if len(valid_models) < len(models_to_evaluate):
                    ...
                    if args.skip_missing_metadata:
                        logger.warning(
                            f"Skipped models due to missing metadata: {', '.join(skipped)}"
                        )
                    else:
                        logger.warning(
                            f"Skipped models due to missing metadata: {', '.join(skipped)}"
                ...
                logger.warning(
                    f"No OpenML task ID found for dataset {dataset['name']}, skipping metadata validation"
                )
        ...
        for model_name in models_to_evaluate_filtered:
            ...
            try:
                ...
                with timeout_context(timeout_seconds):
                    if model_name.lower() == "tabllm":
                        ...
                        if effective_nn_k != original_nn_k:
                            logger.info(
                                f"Capping nn_k from {original_nn_k} to {effective_nn_k} (max of {n_classes * 2} classes*2 or 10% of {train_size} training samples)"
                            )
                        ...
                        model_name.lower() in ["openai_llm", "api_llm"]
                        ...
                        model_name.lower() in ["gemini_llm", "api_llm"]
                ...
                logger.warning(
                    f"Evaluation of {model_name} on {dataset['name']} timed out after {args.timeout_minutes} minutes"
                )
                result = {
            ...
            if args.use_wandb and WANDB_AVAILABLE:
                ...
                result_with_dataset_id["task_id"] = dataset[
                    "id"
                ]  # For MARVIS compatibility, task_id = dataset_id
    ...
    success = safe_json_dump(
        all_results, aggregated_file, logger=logger, minimal_fallback=True, indent=2
    )
        ...
        for model_name in models_to_evaluate:
            ...
            model_results = [
                ...
                if (
                    ...
                    and ("accuracy" in r or "r2_score" in r)
            ...
            if model_results:
                ...
                agg_metrics_logger.log_aggregated_metrics(
                    model_results, prefix="average"
                )
    ...
    for model_name in models_to_evaluate:
        ...
        if model_results:
            ...
            regression_results = [
                r
                for r in model_results
                if "r2_score" in r and r["r2_score"] is not None
            ]
            ...
            errors = len(
                [
                    r
                    for r in model_results
                    if "error" in r and not r.get("timeout", False)
                ]
            )
            ...
            if classification_results:
                ...
                logger.info(
                    f"{model_name}: Average accuracy = {avg_accuracy:.4f} ({len(classification_results)} classification datasets)"
                )
            ...
            if regression_results:
                ...
                logger.info(
                    f"{model_name}: Average R² = {avg_r2:.4f} ({len(regression_results)} regression datasets)"
                )
            ...
            if classification_results or regression_results:
                ...
                logger.info(
                    f"{model_name}: Completion rate = {avg_completion:.1%} ({total_valid} total datasets)"
                )
                if timeouts > 0:
