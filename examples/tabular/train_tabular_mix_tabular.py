    ...
    if scheduler_type == "constant":
        ...
        if num_warmup_steps > 0:
            ...
            def lr_lambda(current_step):
                ...
                return max(
                    args.lr_eta_min / args.learning_rate,
                    0.5
                    * (1.0 + math.cos(math.pi * args.lr_num_cycles * 2.0 * progress)),
                )
            ...
            scheduler = CosineAnnealingLR(
                optimizer, T_max=num_training_steps, eta_min=args.lr_eta_min
            )
        ...
        def lr_lambda(current_step):
            ...
            return max(
                args.lr_eta_min / args.learning_rate, (1.0 - progress) ** args.lr_power
            )
...
def patch_vector_quantizer_if_needed(model):
    ...
    def patched_forward(self, inputs, return_indices=False):
        ...
        perplexity = torch.exp(
            -torch.sum(avg_probs * torch.log(avg_probs + self.epsilon))
        )
    ...
    try:
        ...
        if not hasattr(vq, "_patched_by_fix"):
            ...
            logger.info(
                "Vector Quantizer forward method patched to improve codebook utilization"
            )
            return True
...
def diagnose_vq_issues(model, output_dir, log_to_wandb=False):
    ...
    try:
        ...
        report_lines.append(
            f"Active codes: {active_codes}/{total_codes} ({utilization_rate:.2%})"
        )
    ...
    if "active_codes" in locals() and "total_codes" in locals():
        if active_codes == 0:
            ...
            report_lines.append(
                "- Increase training epochs to at least a few thousand steps"
            )
            report_lines.append("- Reduce codebook size to 128 or 64")
            report_lines.append(
                "- Check if the VQ layer is correctly integrated in the forward pass"
            )
        ...
        report_lines.append(
            "- Unable to generate recommendations due to diagnostic errors"
        )
    ...
    if log_to_wandb and WANDB_AVAILABLE:
        try:
            ...
            wandb.log(
                {
                    "vq/diagnostic_report": wandb.Html(
...
def load_datasets(args) -> List[Dict[str, Any]]:
    ...
    if args.dataset_ids:
        ...
        logger.info(
            f"Loading {len(dataset_ids)} datasets from provided IDs: {dataset_ids}"
        )
        ...
        for dataset_id in dataset_ids:
            try:
                ...
                logger.info(
                    f"Successfully loaded dataset {dataset_name} (ID: {dataset_id})"
                )
            except Exception as e:
    ...
    # Case 2: Load from directory or directories of CSV files
        ...
        if not args.force_recompute_embeddings:
            raise ValueError(
                "When using --data_dir, you must also set --force_recompute_embeddings to ensure fresh embeddings are computed for the shuffled dataset order."
            )
        ...
        if not all_csv_files:
            raise ValueError(
                f"No CSV files found in any of the directories: {data_dirs}"
            )

        logger.info(
            f"Found {len(all_csv_files)} total CSV files across all directories"
        )
        ...
        non_xy_files = [
            f for f in all_csv_files if "_X.csv" not in f and "_y.csv" not in f
        ]

        # Use X files if available, otherwise use regular CSV files
        files_to_process = (
            x_files if x_files else non_xy_files if non_xy_files else all_csv_files
        )
        ...
        logger.info(
            f"Processing {len(files_to_process)} dataset files (shuffled with seed {args.seed})"
        )

        for file_path in files_to_process:
            try:
                X, y, categorical_indicator, attribute_names, dataset_name = (
                    load_csv_dataset(file_path)
                )
                datasets.append(
                    {
                ...
                logger.info(
                    f"Successfully loaded dataset {dataset_name} from {file_path}"
                )
            except Exception as e:
    ...
    # Case 3: Sample random datasets from OpenML
        ...
        try:
            ...
            if len(datasets_info) < args.num_datasets:
                logger.info(
                    f"Only found {len(datasets_info)} datasets, trying broader search"
                )
                try:
                    # Use dataframe output format to avoid FutureWarning
                    more_datasets_df = openml.datasets.list_datasets(
                        output_format="dataframe", limit=100
                    )
                    # Convert dataframe to dict format for compatibility
                    more_datasets = {
                        str(row["did"]): {} for _, row in more_datasets_df.iterrows()
                    }
                    datasets_info.update(more_datasets)
                    logger.info(
                        f"Got a total of {len(datasets_info)} datasets after broader search"
                    )
                except Exception as e:
            ...
            logger.info(
                f"Found {len(dataset_choices)} potential datasets to sample from"
            )

            # If we still don't have enough, use random IDs as a last resort
            if len(dataset_choices) < args.num_datasets:
                logger.info(
                    f"Not enough datasets found ({len(dataset_choices)}), adding random IDs to reach {args.num_datasets}"
                )
                ...
                logger.info(
                    f"Added {len(random_ids)} random dataset IDs to reach {len(dataset_choices)} total"
                )
            ...
            if len(known_working_datasets) >= args.num_datasets:
                ...
                random_choices = [
                    id for id in dataset_choices if id not in known_working_datasets
                ]
                random_ids = rng.sample(
                    random_choices, args.num_datasets - len(known_working_datasets)
                )
                dataset_ids = known_working_datasets + random_ids
                logger.info(
                    f"Using {len(known_working_datasets)} known working datasets plus {len(random_ids)} random datasets"
                )
            ...
            if args.num_datasets > len(available_datasets):
                logger.warning(
                    f"Requested {args.num_datasets} datasets, but only {len(available_datasets)} are predefined"
                )
                ...
                logger.info(
                    f"Using {len(predefined_ids)} predefined datasets plus {len(random_ids)} random IDs"
                )
            else:
                # Sample from predefined datasets
                sampled_datasets = rng.sample(
                    list(available_datasets.items()), args.num_datasets
                )
        ...
        attempted_ids = set()
        max_attempts = max(
            args.num_datasets * 8, 100
        )  # Try at least 100 datasets or 8x the requested number

        # Try loading datasets until we get enough or run out of options
        while (
            successful_loads < args.num_datasets and len(attempted_ids) < max_attempts
        ):
            ...
            if len(attempted_ids) >= len(dataset_ids):
                ...
                new_random_ids = []
                new_count = min(
                    50, args.num_datasets - successful_loads
                )  # Generate up to 50 new IDs at a time

                logger.info(
                    f"Exhausted initial dataset list. Generating {new_count} new random IDs to try."
                )
                ...
                logger.info(
                    f"Added {len(new_random_ids)} new random dataset IDs to try"
                )
            ...
            try:
                ...
                if X.shape[0] < 50:  # Skip if fewer than 50 samples
                    logger.warning(
                        f"Skipping dataset {dataset_name} (ID: {dataset_id}) - too few samples: {X.shape[0]}"
                    )
                    continue
                ...
                if unique_classes < 2:  # Skip if fewer than 2 classes
                    logger.warning(
                        f"Skipping dataset {dataset_name} (ID: {dataset_id}) - too few classes: {unique_classes}"
                    )
                    continue

                if unique_classes > 10:  # Skip if more than 10 classes
                    logger.warning(
                        f"Skipping dataset {dataset_name} (ID: {dataset_id}) - too many classes: {unique_classes}"
                    )
                    continue
                ...
                if X.shape[1] > 5000:  # Arbitrary limit to avoid memory issues
                    logger.warning(
                        f"Skipping dataset {dataset_name} (ID: {dataset_id}) - too many features: {X.shape[1]}"
                    )
                    continue
                ...
                if min_class_count < 10:
                    logger.warning(
                        f"Skipping dataset {dataset_name} (ID: {dataset_id}) - extreme class imbalance detected: class with only {min_class_count} examples"
                    )
                    continue

                # Log class distribution for reference
                class_distribution = {
                    int(cls): int(count) for cls, count in zip(unique_vals, counts)
                }
                class_imbalance_ratio = np.max(counts) / np.min(counts)
                logger.info(
                    f"Class distribution for {dataset_name} (ID: {dataset_id}): {class_distribution}, imbalance ratio: {class_imbalance_ratio:.2f}x"
                )
                ...
                if isinstance(X, np.ndarray) and X.size > 0:
                    ...
                    if num_rows_with_nan > 0:
                        ...
                        if num_rows_with_nan > 0.5 * len(df_X):
                            logger.warning(
                                f"Skipping dataset {dataset_name} (ID: {dataset_id}) - too many missing values: {num_rows_with_nan}/{len(df_X)} rows"
                            )
                            continue
                        ...
                        logger.info(
                            f"Removed {num_rows_with_nan} rows with missing values from dataset {dataset_name} (ID: {dataset_id})"
                        )

                        # If we have too few samples after cleaning, skip the dataset
                        if len(X) < 50:
                            logger.warning(
                                f"Skipping dataset {dataset_name} (ID: {dataset_id}) - too few samples after removing NaN values: {len(X)}"
                            )
                            continue
                ...
                logger.info(
                    f"Successfully loaded dataset {dataset_name} (ID: {dataset_id}) - {X.shape[0]} samples, {X.shape[1]} features, {len(np.unique(y))} classes"
                )
                successful_loads += 1
                ...
                if successful_loads % 5 == 0 or successful_loads == args.num_datasets:
                    logger.info(
                        f"Progress: Loaded {successful_loads}/{args.num_datasets} datasets ({len(attempted_ids)} attempts)"
                    )
        ...
        if successful_loads < args.num_datasets:
            logger.warning(
                f"Only able to load {successful_loads} datasets out of {args.num_datasets} requested ({len(attempted_ids)} attempts)"
            )

        logger.info(
            f"Successfully loaded {len(datasets)} datasets for training/testing"
        )

    if not datasets:
        raise ValueError(
            "No datasets could be loaded. Please check your input parameters."
        )
...
def split_datasets_train_test(
    datasets: List[Dict[str, Any]], test_ratio: float, seed: int
...
def preprocess_features(X: np.ndarray, categorical_indicator: List[bool]) -> np.ndarray:
    ...
    for col_idx in range(df.shape[1]):
        col = df.iloc[:, col_idx]
        is_categorical = (
            categorical_indicator[col_idx]
            if col_idx < len(categorical_indicator)
            else False
        )
        ...
        if col.dtype == "object" or is_categorical:
            ...
            try:
                ...
                df.iloc[:, col_idx] = pd.Series(
                    encoded_values, index=df.index, dtype=dest_dtype
                )
                logger.info(
                    f"  Encoded {len(encoder.classes_)} unique categories for column {col_idx}"
                )
            except Exception as e:
                logger.warning(f"  Error encoding column {col_idx}: {e}")
                # If encoding fails, replace with zeros - use Series to avoid dtype warning
                df.iloc[:, col_idx] = pd.Series(
                    np.zeros(len(df), dtype=int), index=df.index
                )
        else:
            ...
            if col.isna().any():
                ...
                logger.info(
                    f"  Filled {col.isna().sum()} missing values in column {col_idx}"
                )
...
def process_dataset(dataset: Dict[str, Any], args) -> Dict[str, Any]:
    ...
    if len(X) != len(y):
        raise ValueError(
            f"Dataset {dataset['name']}: X and y lengths do not match: {len(X)} vs {len(y)}"
        )

    # Check initial class distribution
    unique_classes, class_counts = np.unique(y, return_counts=True)
    logger.info(
        f"Dataset {dataset['name']} initial class distribution: {dict(zip(unique_classes, class_counts))}"
    )
    ...
    if len(valid_classes) < MIN_CLASSES:
        logger.warning(
            f"Dataset {dataset['name']} has only {len(valid_classes)} classes with at least {MIN_SAMPLES_PER_CLASS} samples. Skipping."
        )
        # Return a flag indicating this dataset should be skipped
        dataset["skip"] = True
        dataset["skip_reason"] = (
            f"Insufficient classes: only {len(valid_classes)} classes with >= {MIN_SAMPLES_PER_CLASS} samples"
        )
        return dataset
    ...
    if len(valid_classes) < len(unique_classes):
        ...
        y = y[mask]
        logger.info(
            f"Filtered out {len(mask) - mask.sum()} samples from classes with < {MIN_SAMPLES_PER_CLASS} members"
        )
    ...
    if isinstance(y, np.ndarray):
        ...
        if y.dtype.kind == "O":
            logger.info(
                f"Dataset {dataset['name']} has string labels. Encoding to integers."
            )
            from sklearn.preprocessing import LabelEncoder
        ...
        # If numeric, check if it's continuous or discrete
            ...
            logger.info(
                f"Dataset {dataset['name']} has {len(unique_vals)} unique integer values"
            )
            ...
            unique_vals = np.unique(y)
            if len(unique_vals) <= 10 and all(
                float(val).is_integer() for val in unique_vals
            ):
                # Convert to integers for classification
                logger.info(
                    f"Dataset {dataset['name']} has {len(unique_vals)} discrete float values. Converting to integers."
                )
                y = y.astype(int)
            else:
                # It's truly continuous - must bin for classification tasks
                logger.info(
                    f"Dataset {dataset['name']} has continuous target. Converting to classification by binning."
                )
                ...
                dataset["target_discretizer"] = (
                    discretizer  # Store for future reference
                )
    ...
    if min_class_count < MIN_SAMPLES_PER_CLASS:
        logger.warning(
            f"Dataset {dataset['name']}: After processing, minimum class count is {min_class_count}. Skipping."
        )
        dataset["skip"] = True
        dataset["skip_reason"] = (
            f"After processing, minimum class count is {min_class_count} < {MIN_SAMPLES_PER_CLASS}"
        )
        return dataset

    if len(unique_classes) < MIN_CLASSES:
        logger.warning(
            f"Dataset {dataset['name']}: After processing, only {len(unique_classes)} classes remain. Skipping."
        )
        dataset["skip"] = True
        dataset["skip_reason"] = (
            f"After processing, only {len(unique_classes)} classes remain < {MIN_CLASSES}"
        )
        return dataset
    ...
    try:
        ...
        X_val, X_test, y_val, y_test = train_test_split(
            ...
            y_temp,
            test_size=0.5,
            random_state=dataset_specific_seed,
            stratify=y_temp,
        )
    except ValueError as e:
        # If stratified split fails, it means some classes are too small
        logger.warning(
            f"Dataset {dataset['name']}: Stratified split failed: {e}. Skipping."
        )
        dataset["skip"] = True
    ...
    logger.info(
        f"Dataset {dataset['name']} shapes - Train: {X_train.shape}, Val: {X_val.shape}, Test: {X_test.shape}"
    )
    ...
    if (
        dataset.get("has_precomputed_embeddings", False)
        and precomputed_dir
        and not args.force_recompute_embeddings
    ):
        ...
        if not os.path.exists(prefix_data_path):
            ...
            cache_files = glob.glob(
                os.path.join(precomputed_dir, "*tabpfn_embeddings*.npz")
            )
            if cache_files:
                # Use the most recent cache file
                prefix_data_path = sorted(cache_files, key=os.path.getmtime)[-1]
                logger.info(
                    f"Found embedding cache file: {os.path.basename(prefix_data_path)}"
                )

        if os.path.exists(prefix_data_path):
            try:
                logger.info(
                    f"Loading precomputed embeddings for {dataset['name']} from {prefix_data_path}"
                )
                ...
                if (
                    "embeddings" in precomputed_prefix_data
                    and "class_labels" in precomputed_prefix_data
                ):
                ...
                # Check if the file has the raw embeddings
                    ...
                    key in precomputed_prefix_data
                    for key in ["train_embeddings", "val_embeddings", "test_embeddings"]
                ):
                    ...
                    if "y_train_sample" in precomputed_prefix_data:
                        ...
                        logger.warning(
                            "y_train_sample not found in cache, using y_train instead"
                        )
                    ...
                    if train_embeddings.shape[1] != args.embedding_size:
                        logger.warning(
                            f"Precomputed embeddings have size {train_embeddings.shape[1]}, but requested size is {args.embedding_size}"
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                f"Successfully resized embeddings to {args.embedding_size}"
                            )
                            has_precomputed_embeddings = True
                        except Exception as resize_err:
                            logger.warning(
                                f"Error resizing embeddings: {resize_err}. Will recalculate."
                            )
                            has_precomputed_embeddings = False
                    else:
                        logger.info(
                            f"Successfully loaded precomputed embeddings with shapes - Train: {train_embeddings.shape}, Val: {val_embeddings.shape}, Test: {test_embeddings.shape}"
                        )
                        has_precomputed_embeddings = True
                    ...
                    if has_precomputed_embeddings:
                        ...
                        try:
                            ...
                            logger.warning(
                                "TabPFN not available. Using None as a placeholder."
                            )
                            tabpfn = None
                else:
                    # Unknown format
                    logger.warning(
                        f"Unrecognized embedding file format in {prefix_data_path}"
                    )
                    has_precomputed_embeddings = False
            except Exception as e:
    ...
    if not has_precomputed_embeddings:
        ...
        logger.info(
            f"Generating TabPFN embeddings for {dataset['name']} with size {args.embedding_size}"
        )
            ...
            if not args.force_recompute_embeddings:
                ...
                try:
                    ...
                    cache_pattern = os.path.join(
                        cache_dir, f"*{dataset_identifier}*tabpfn_embeddings*.npz"
                    )
                    ...
                    if cache_files:
                        ...
                        logger.info(
                            f"Found existing embedding cache: {os.path.basename(cache_file)}"
                        )
                        ...
                        try:
                            ...
                            if all(
                                ...
                                logger.info(
                                    f"Successfully loaded cached embeddings with shapes - Train: {train_embeddings.shape}, Val: {val_embeddings.shape}, Test: {test_embeddings.shape}"
                                )
                            ...
                            logger.warning(
                                f"Error loading cached embeddings from {cache_file}: {e}"
                            )
                except Exception as e:
...
def create_llm_datasets(
    dataset: Dict[str, Any], model_info: Dict[str, Any], args
) -> Dict[str, Any]:
    """
    ...
    dataset_output_dir = os.path.join(
        args.output_dir, f"dataset_{dataset['id']}_{dataset_hash}"
    )
    os.makedirs(dataset_output_dir, exist_ok=True)

    logger.info(
        f"Created dataset directory: {os.path.basename(dataset_output_dir)} for {dataset['name']}"
    )
    ...
    train_dataset, eval_dataset, test_dataset, label_encoder, prefix_data_file = (
        create_llm_dataset(
            dataset["X_train"],
            dataset["y_train_sample"],
            dataset["X_val"],
            dataset["y_val"],
            dataset["X_test"],
            dataset["y_test"],
            dataset["train_embeddings"],
            dataset["val_embeddings"],
            dataset["test_embeddings"],
            model_info["tokenizer"],
            model_info["prefix_start_id"],
            model_info["prefix_end_id"],
            model_info["class_token_ids"],
...
def main():
    ...
    if args.use_wandb:
        if not WANDB_AVAILABLE:
            logger.warning(
                "Weights & Biases requested but not installed. Run 'pip install wandb' to install."
            )
        else:
            ...
            if args.resume and args.use_wandb and args.resume_wandb:
                ...
                if os.path.exists(wandb_id_file):
                    ...
                    logger.info(
                        "No wandb_id.txt found in checkpoint directory, starting new run"
                    )
            elif args.resume and args.use_wandb:
                logger.info(
                    "Resuming training from checkpoint, but starting a new W&B run (use --resume_wandb to resume the W&B run too)"
                )
            ...
            if resume_wandb == "must" and wandb_id:
                ...
                gpu_monitor = GPUMonitor(
                    log_interval=30.0, enable_detailed_logging=True
                )
                if torch.cuda.is_available():
            ...
            run_id = (
                wandb_run.id if (resume_wandb == "must" and wandb_id) else wandb.run.id
            )
            logger.info(
                f"Initialized Weights & Biases run: {args.wandb_name} (ID: {run_id})"
            )
    ...
    if not args.force_recompute_embeddings:
        logger.info(
            f"Checking for precomputed embeddings in model directory: {args.output_dir}"
        )
        ...
        if dataset_dirs:
            logger.info(
                f"Found {len(dataset_dirs)} potential dataset directories in the model directory"
            )
            ...
            for dataset_dir in dataset_dirs:
                try:
                    ...
                    if dataset_info.get("has_embeddings", False):
                        logger.info(
                            f"Found precomputed embeddings for dataset {dataset_info['name']} (ID: {dataset_info['id']})"
                        )
                        model_datasets.append(dataset_info)

                except Exception as e:
                    logger.warning(
                        f"Error checking dataset directory {dataset_dir}: {e}"
                    )
    else:
        logger.info(
            "Skipping check for precomputed embeddings in model directory due to --force_recompute_embeddings flag"
        )
    ...
    if len(model_datasets) >= min_datasets_needed:
        logger.info(
            f"Using {len(model_datasets)} datasets with precomputed embeddings found in model directory"
        )
        ...
        if args.dataset_ids:
            ...
            filtered_model_datasets = [
                d for d in model_datasets if str(d["id"]) in specified_ids
            ]

            if filtered_model_datasets:
                logger.info(
                    f"Filtered to {len(filtered_model_datasets)} datasets that match specified IDs"
                )
        ...
        if args.num_datasets and len(model_datasets) > args.num_datasets:
            ...
            logger.info(
                f"Using first {args.num_datasets} datasets with precomputed embeddings"
            )
        ...
        for dataset_info in model_datasets:
            try:
                ...
                if dataset_info.get("is_csv", False):
                    ...
                    if csv_file:
                        X, y, categorical_indicator, attribute_names, dataset_name = (
                            load_csv_dataset(csv_file)
                        )
                    else:
                        logger.warning(
                            f"Cannot find CSV file for dataset {dataset_id}. Skipping this dataset."
                        )
                        continue
                else:
                    # Load from OpenML
                ...
                logger.info(
                    f"Successfully loaded dataset {dataset_name} (ID: {dataset_id}) with precomputed embeddings"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to load dataset {dataset_info['id']} despite having precomputed embeddings: {e}"
                )
                # Check if this was likely a CSV dataset that we couldn't find
                if (
                    "episode" in str(dataset_info["id"]).lower()
                    or not str(dataset_info["id"]).isdigit()
                ):
                    logger.info(
                        f"Dataset {dataset_info['id']} appears to be a CSV dataset. Skipping since original data is not available."
                    )
                else:
                    logger.info(
                        f"Dataset {dataset_info['id']} appears to be an OpenML dataset that failed to load."
                    )

    # If we don't have enough datasets yet, load more using the standard approach
    if len(all_datasets) < min_datasets_needed or (
        args.num_datasets and len(all_datasets) < args.num_datasets
    ):
        logger.info(
            f"Need more datasets. Currently have {len(all_datasets)}, loading more..."
        )
        ...
        additional_datasets = [
            d for d in additional_datasets if str(d["id"]) not in existing_ids
        ]
    ...
    if len(all_datasets) < 2:
        raise ValueError(
            f"At least 2 datasets are required for training and testing, but only {len(all_datasets)} were loaded."
        )
    ...
    if int(len(all_datasets) * args.test_split_ratio) < min_test_datasets:
        ...
        logger.warning(
            f"Adjusting test_split_ratio from {args.test_split_ratio} to {effective_test_ratio} to ensure at least {min_test_datasets} test dataset(s)"
        )
    ...
    if int(len(all_datasets) * args.test_split_ratio) > max_test_datasets:
        ...
        logger.warning(
            f"Adjusting test_split_ratio from {args.test_split_ratio} to {effective_test_ratio} to ensure at least 1 training dataset"
        )
        args.test_split_ratio = effective_test_ratio
    ...
    logger.info(
        f"Split datasets: {len(train_datasets)} for training, {len(test_datasets)} for testing"
    )
    ...
    if args.use_vector_quantization:
        logger.info(
            f"Preparing model {args.model_id} with Vector Quantized prefix embedding"
        )
        logger.info(
            f"VQ Settings: Codebook size: {args.vq_num_embeddings}, Commitment cost: {args.vq_commitment_cost}, Decay: {args.vq_decay}"
        )
        ...
        if args.vq_commitment_cost == 0.25:
            logger.info(
                "Using default commitment cost of 0.25. For better VQ utilization, consider using --vq_commitment_cost 0.5 or higher."
            )
        ...
        if patched:
            ...
            logger.warning(
                "Could not patch Vector Quantizer - codebook utilization might be low"
            )

        # Pass VQ debugging flag to the model if supported
        if hasattr(model, "vector_quantizer") and hasattr(
            model.vector_quantizer, "set_debug_mode"
        ):
            model.vector_quantizer.set_debug_mode(args.vq_debug)

        # Set VQ warmup steps if supported
        if hasattr(model, "vector_quantizer") and hasattr(
            model.vector_quantizer, "set_warmup_steps"
        ):
        ...
        if hasattr(model, "vector_quantizer") and hasattr(
            model.vector_quantizer, "_codebook_usage"
        ):
            try:
        ...
        if hasattr(model, "vector_quantizer"):
            try:
                model.vector_quantizer.reset_usage_every = (
        ...
        model, tokenizer, prefix_start_id, prefix_end_id, class_token_ids = (
            prepare_qwen_with_prefix_embedding(
                embedding_size=args.embedding_size, model_id=args.model_id
            )
        )
    ...
    for dataset in train_datasets:
        ...
        if not processed_dataset.get("skip", False):
            ...
            logger.warning(
                f"Skipping dataset {dataset['name']}: {processed_dataset.get('skip_reason', 'Unknown reason')}"
            )
    ...
    datasets_info = {
        "train_datasets": [
            ...
            for d in llm_train_datasets
        ],
        "test_datasets": [
            {"id": d["dataset_id"], "name": d["dataset_name"]}
            for d in llm_test_datasets
        ],
    }
    ...
    logger.info(
        f"Using eval dataset from first dataset with {len(eval_dataset)} samples"
    )
    ...
    if args.permute_labels:
        if args.permute_labels_every_k_steps is not None:
            logger.info(
                f"Label permutation enabled: will permute every {args.permute_labels_every_k_steps} steps"
            )
        else:
        ...
        if args.no_permute_last_k is not None:
            logger.info(
                f"Label permutation will be disabled for the last {args.no_permute_last_k} steps"
            )
    ...
    logger.info(
        f"Learning rate schedule: {args.lr_scheduler} (initial: {lr_initial:.2e}, final: {lr_final:.2e})"
    )
    if args.lr_warmup_steps > 0 or args.lr_warmup_ratio > 0:
        warmup_steps = (
            args.lr_warmup_steps
            if args.lr_warmup_ratio == 0
            else int(args.total_steps * args.lr_warmup_ratio)
        )
        logger.info(
            f"  Warmup steps: {warmup_steps} (out of {args.total_steps} total steps)"
        )

    # Prepare component-specific learning rates
    # For VQ
    vq_lr_initial = (
        args.vq_learning_rate
        if args.vq_learning_rate is not None
        else args.learning_rate
    )
    vq_lr_scheduler = (
        args.vq_lr_scheduler if args.vq_lr_scheduler is not None else args.lr_scheduler
    )
    ...
    # For class tokens
    class_token_lr_initial = (
        args.class_token_learning_rate
        if args.class_token_learning_rate is not None
        else args.learning_rate
    )
    class_token_lr_scheduler = (
        args.class_token_lr_scheduler
        if args.class_token_lr_scheduler is not None
    ...
    llm_lr_initial = (
        ...
        if args.llm_learning_rate is not None
        else args.learning_rate
    )
    llm_lr_scheduler = (
        args.llm_lr_scheduler
        if args.llm_lr_scheduler is not None
        else args.lr_scheduler
    )
    ...
    if args.vq_learning_rate is not None or args.vq_lr_scheduler is not None:
        logger.info(
            f"VQ learning rate: {vq_lr_scheduler} (initial: {vq_lr_initial:.2e}, final: {vq_lr_final:.2e})"
        )
    if (
        args.class_token_learning_rate is not None
        or args.class_token_lr_scheduler is not None
    ):
        logger.info(
            f"Class token learning rate: {class_token_lr_scheduler} (initial: {class_token_lr_initial:.2e}, final: {class_token_lr_final:.2e})"
        )
    if args.llm_learning_rate is not None or args.llm_lr_scheduler is not None:
        logger.info(
            f"LLM learning rate: {llm_lr_scheduler} (initial: {llm_lr_initial:.2e}, final: {llm_lr_final:.2e})"
        )
    ...
    for dataset in llm_test_datasets:
        logger.info(
            f"Evaluating on dataset: {dataset['dataset_name']} (ID: {dataset['dataset_id']})"
        )
        ...
        results = evaluate_llm_on_test_set(
            ...
            model_info["prefix_start_id"],
            model_info["prefix_end_id"],
            final_class_token_ids,
            dataset["prefix_data_file"],
            max_test_samples=args.max_test_samples,
        )

        logger.info(
            f"Test accuracy on {dataset['dataset_name']}: {results['accuracy']:.4f}"
        )
    ...
    if args.use_wandb and WANDB_AVAILABLE:
        ...
        for dataset_id, results in all_results.items():
            wandb.log({f"test/{results['dataset_name']}_accuracy": results["accuracy"]})

        # Create a summary table
        dataset_table = wandb.Table(
            columns=["Dataset ID", "Dataset Name", "Accuracy", "Samples"]
        )
        for dataset_id, results in all_results.items():
    ...
    if (
        args.use_vector_quantization
        and hasattr(trained_model, "vector_quantizer")
        and hasattr(trained_model.vector_quantizer, "_codebook_usage")
    ):
        try:
            ...
            logger.info(
                f"Final VQ codebook utilization: {active_codes}/{total_codes} vectors used ({utilization_pct:.2f}%)"
            )
            ...
            with open(os.path.join(final_model_path, "vq_usage_stats.txt"), "w") as f:
                ...
                f.write(
                    f"Active entries: {active_codes} ({active_codes/total_codes:.2%})\n"
                )
                f.write("--\n")
                ...
                if active_codes == 0:
                    ...
                    f.write(
                        "- Reduce the codebook size (try 64 or 128 instead of 256/512)\n"
                    )
                    f.write("- Use the patched version with 'autopatch' enabled\n")

            # Run extended diagnostics
            diagnose_vq_issues(
                trained_model, final_model_path, log_to_wandb=args.use_wandb
            )
        except Exception as e:
