...
try:
    ...
    def evaluate_with_explanations(*args, **kwargs):
        logging.getLogger(__name__).error(
            "Failed to import evaluate_with_explanations. Please make sure the file is in the same directory."
        )
...
def main():
    ...
    logger.info(
        f"Dataset shapes - Train: {X_train.shape}, Val: {X_val.shape}, Test: {X_test.shape}"
    )
    ...
    trained_model, tokenizer, final_class_token_ids = train_llm_with_tabpfn_embeddings(
        ...
        * 0.16,  # Match the ratio from old defaults: 8e-6 / 5e-5 = 0.16
        ...
        class_token_lr_final=(
            (args.class_token_learning_rate * 0.16)
            if args.class_token_learning_rate
            else None
        ),
        class_token_lr_scheduler_type=args.class_token_lr_scheduler,
        llm_lr_initial=args.llm_learning_rate,
        llm_lr_final=(
            (args.llm_learning_rate * 0.16) if args.llm_learning_rate else None
        ),
        llm_lr_scheduler_type=args.llm_lr_scheduler,
        wandb_callback=(
            wandb_log_callback if (args.use_wandb and WANDB_AVAILABLE) else None
        ),
    ...
    if args.evaluate_with_explanations:
        logger.info(
            f"Using explanation-based evaluation with {args.explanation_type} explanations"
        )
        try:
    ...
    serializable_results = {
        ...
        "dataset_name": args.dataset_name,
        "evaluation_method": (
            "explanation_based" if args.evaluate_with_explanations else "standard"
        ),
    }
    ...
    if "confusion_matrix" in results:
        ...
        serializable_results["confusion_matrix"] = (
    ...
    if (
        args.evaluate_with_explanations
        and args.save_explanations
        and "explanations" in results
    ):
        ...
        explanations_file = os.path.join(
            args.output_dir,
            f"{args.dataset_name}_{args.explanation_type}_explanations.json",
        )
    ...
    if args.use_wandb and WANDB_AVAILABLE:
            ...
            if hasattr(wandb.run, "history") and wandb.run.history is not None:
                ...
                if "gpu.0.powerWatts" in history_df.columns:
                    ...
                    if len(power_data) > 0:
                        ...
                        logger.info(
                            f"GPU power estimation: {avg_power_watts:.1f}W avg, {total_energy_wh:.4f}Wh total energy"
                        )
        except Exception as e:
        ...
        if args.evaluate_with_explanations:
            ...
            mean_prob = None
            if "probabilities" in results:
                mean_prob = sum(results["probabilities"]) / len(
                ...
                try:
                    ...
                    if sample_indices:
                        explanation_samples = {
                            ...
                            "predicted": [
                                results["predictions"][i] for i in sample_indices
                            ],
                            "actual": [
                                results["ground_truth"][i] for i in sample_indices
                            ],
                            "correct": [
                                results["predictions"][i] == results["ground_truth"][i]
                                for i in sample_indices
                            ],
                            "explanation": [
                                results["explanations"][i] for i in sample_indices
                            ],
                        }
                except Exception as e:
