"""
Gemini LLM baseline for tabular classification.
...
class GeminiLLMBaseline:
    """
    Gemini LLM baseline for tabular data classification.
    Converts tabular data to text format and uses Gemini thinking models for classification.
    """
    ...
    def __init__(
        ...
        if not any(model in model_name for model in gemini_models):
            logger.warning(
                f"Model {model_name} may not be a Gemini model. Supported: {gemini_models}"
            )

    def load_model(self):
        """Load Gemini LLM model through the unified model loader."""
        try:
            logger.info(f"Loading Gemini LLM model: {self.model_name}")

            # Use the unified model loader which will auto-detect Gemini
            self.model_wrapper = model_loader.load_llm(
                model_name=self.model_name, backend="gemini"
            )

            logger.info("Gemini LLM model loaded successfully")

        except Exception as e:
            logger.error(f"Failed to load Gemini LLM model: {e}")
            raise
    ...
    def _format_tabular_data(
        ...
        for i, sample in enumerate(X):
            # Create a structured text representation optimized for Gemini
            sample_text = "Tabular data sample for classification:\n\n"

            # Group features into categories if possible
            sample_text += "Features:\n"
            for j, (feature_name, value) in enumerate(zip(self.feature_names, sample)):
                sample_text += f"  • {feature_name}: {value:.4f}\n"

            if include_labels and y is not None:
                sample_text += f"\nKnown classification: {self.class_names[y[i]]}\n"
    ...
    def _create_classification_prompt(self, sample_text: str) -> str:
        """Create a classification prompt optimized for Gemini thinking models."""
        class_list = ", ".join([f'"{name}"' for name in self.class_names])

        prompt = f"""You are an expert machine learning classifier analyzing tabular data. Your task is to classify the given sample into one of the available categories.
...
Classification:"""
    ...
    def predict(self, X_test: np.ndarray) -> np.ndarray:
        ...
        logger.info(f"Running Gemini LLM predictions on {len(X_test)} samples")

        predictions = []
        formatted_samples = self._format_tabular_data(X_test)

        # Create generation config optimized for Gemini thinking models
        generation_config = GenerationConfig(
            max_new_tokens=(
                50 if self.enable_thinking else 15
            ),  # More tokens for thinking
    ...
    def _parse_response(self, response: str) -> str:
        ...
        # Default to first class
        logger.warning(
            f"Could not parse class from response: '{response}'. Using fallback: {self.class_names[0]}"
        )
    ...
    def save_raw_responses(self, output_path: str, benchmark_name: str = "unknown"):
        ...
        logger.info(
            f"Saved {len(self.raw_responses)} raw Gemini responses to {output_path}"
        )
    ...
    def evaluate(
        ...
        """
        Evaluate Gemini LLM on test data.
...
def evaluate_gemini_llm(dataset: Dict[str, Any], args) -> Dict[str, Any]:
    """
    Evaluate Gemini LLM baseline on tabular dataset.
    ...
    try:
        ...
        if (
            hasattr(args, "max_test_samples")
            and args.max_test_samples
            and args.max_test_samples < len(X_test)
        ):
        ...
        # Get model name and thinking setting
        model_name = getattr(args, "gemini_model", "gemini-2.5-flash")
        ...
        logger.info(
            f"Gemini LLM achieved {results['accuracy']:.4f} accuracy on {dataset_name}"
        )
        ...
        logger.error(
            f"Error evaluating Gemini LLM on {dataset.get('name', 'unknown')}: {e}"
        )
        return {
            "model_name": getattr(args, "gemini_model", "gemini-2.5-flash"),
            ...
            "completed_samples": 0,
            "completion_rate": 0.0,
            "thinking_enabled": getattr(args, "enable_thinking", True),
            "api_model": True,
            "model_type": "gemini_llm",
        }
