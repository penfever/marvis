...
class OpenAILLMBaseline:
    ...
    def __init__(
        ...
        if not any(model in model_name for model in openai_models):
            logger.warning(
                f"Model {model_name} may not be an OpenAI model. Supported: {openai_models}"
            )
    ...
    def load_model(self):
        ...
        try:
            ...
            self.model_wrapper = model_loader.load_llm(
                model_name=self.model_name, backend="openai"
            )
...
Your classification:"""
    ...
    def save_raw_responses(self, output_path: str, benchmark_name: str = "unknown"):
        ...
        logger.info(
            f"Saved {len(self.raw_responses)} raw OpenAI responses to {output_path}"
        )
...
def evaluate_openai_llm(dataset: Dict[str, Any], args) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"OpenAI LLM achieved {results['accuracy']:.4f} accuracy on {dataset_name}"
        )
        ...
        logger.error(
            f"Error evaluating OpenAI LLM on {dataset.get('name', 'unknown')}: {e}"
        )
        return {
            "model_name": getattr(args, "openai_model", "gpt-4o"),
