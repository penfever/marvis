...
def evaluate_tabula_8b(dataset, args):
    ...
    if (
        ...
        and args.device != "cpu"
        and hasattr(args, "gpu_index")
        and args.gpu_index != 0
    ):
        ...
        if torch.cuda.device_count() > 0:
            ...
            )  # Should be specified GPU's name
    ...
    try:
        ...
        train_config = TrainConfig(
            model_name=args.tabula_model, context_length=args.max_context_length
        )
        ...
        model_wrapper = model_loader.load_llm(
            train_config.model_name, backend="auto", device=args.device, **model_kwargs
        )
        ...
        try:
            ...
            logger.warning(
                f"Could not initialize serializer: {e}. Using DefaultSerializer directly."
            )
            from rtfm.serialization.serializers import DefaultSerializer
        ...
        if tokenizer_config.add_serializer_tokens and hasattr(
            serializer, "special_tokens"
        ):
        ...
        if hasattr(X_train, "iloc"):
            ...
            if len(original_feature_names) == num_features:
                ...
                feature_names = [
                    name for name in original_feature_names if name != y_colname
                ]
            ...
            ), f"Feature names count {len(feature_names)} != X_train columns {num_features}"

            X_train_examples = pd.DataFrame(
                X_train[example_indices], columns=feature_names
            )
        ...
        for i in range(len(X_test)):
            ...
            if hasattr(X_test, "iloc"):
                ...
                if len(feature_names) == len(cur_row_data):
                    ...
                    cur_row = pd.Series(
                        cur_row_data,
                        index=[f"feature_{j}" for j in range(len(cur_row_data))],
                    )
            ...
            try:
                ...
                if is_oom_error(pred_error):
                    logger.warning(
                        f"Tabula-8B prediction failed for sample {i} due to OOM: {pred_error}"
                    )
                    ...
                    if drop_feature_for_oom(dropped_features, num_features, logger):
                        ...
                        dataset["attribute_names"] = [
                            dataset["attribute_names"][idx] for idx in keep_indices
                        ]
                        if "categorical_indicator" in dataset:
                            dataset["categorical_indicator"] = [
                                dataset["categorical_indicator"][idx]
                        ...
                        labeled_examples = pd.concat(
                            [X_train_examples, y_train_examples], axis=1
                        )
                    ...
                    logger.error(
                        f"Tabula-8B prediction failed for sample {i}: {pred_error}"
                    )
                ...
                example_inputs_outputs.append(
                    {
                        "sample_index": i,
                        "input_features": (
                            cur_row.drop(y_colname).to_dict()
                            if hasattr(cur_row, "drop")
                            else str(cur_row)
                        ),
                        "target_choices": [str(cls) for cls in unique_classes],
                        ...
                        "correct": str(prediction) == str(true_label)
                        or prediction == true_label,
                    }
                )
