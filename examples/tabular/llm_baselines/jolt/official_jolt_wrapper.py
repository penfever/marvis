...
def evaluate_jolt_official(dataset, args):
    ...
    if force_cpu:
        ...
        logger.info(
            f"Available CUDA devices after masking: {torch.cuda.device_count()}"
        )
        if torch.cuda.device_count() > 0:
            logger.info(f"Current CUDA device: {torch.cuda.current_device()}")
            logger.info(
                f"Device name: {torch.cuda.get_device_name(0)}"
            )  # Should be GPU 2's name
    ...
    try:
        ...
        jolt_config_path = rm.path_resolver.get_config_path(
            "jolt", task_config_filename
        )
        ...
        if jolt_config_path and jolt_config_path.exists():
            ...
            if jolt_config_path and jolt_config_path.exists():
                ...
                with open(jolt_config_path, "r") as f:
                    jolt_config = json.load(f)
                logger.info(
                    f"Using JOLT metadata for {dataset['name']} from managed config (legacy naming)"
                )
            else:
                logger.info(
                    f"No JOLT metadata found for task {dataset['id']} ({dataset['name']}), using default approach"
                )
    except Exception as e:
        logger.error(
            f"Could not load JOLT config for task {dataset['id']} ({dataset['name']}): {e}"
        )
        import traceback
        ...
        try:
            ...
            task_config_path = os.path.join(
                current_dir, f"jolt_config_task_{dataset['id']}.json"
            )
            if os.path.exists(task_config_path):
                ...
                with open(task_config_path, "r") as f:
                    jolt_config = json.load(f)
                logger.info(
                    f"Using JOLT metadata for task {dataset['id']} from legacy location"
                )
            else:
                # Try dataset name-based naming
                name_config_path = os.path.join(
                    current_dir, f"jolt_config_{dataset['name']}.json"
                )
                if os.path.exists(name_config_path):
                    ...
                    with open(name_config_path, "r") as f:
                        jolt_config = json.load(f)
                    logger.info(
                        f"Using JOLT metadata for {dataset['name']} from legacy location"
                    )
    ...
    logger.info(
        f"Starting JOLT evaluation with jolt_config={'loaded' if jolt_config else 'None'}"
    )
    ...
    try:
        ...
        task_type, detection_method = detect_task_type(
            dataset=dataset, y=y_train, task_id=task_id
        )
        ...
        def get_jolt_feature_names(original_names, jolt_config):
            ...
            if jolt_config and "column_descriptions" in jolt_config:
                ...
                logger.info(
                    f"JOLT config has {len(column_descriptions)} column descriptions"
                )
                ...
                for orig_name in original_names:
                    if orig_name in column_descriptions:
                        ...
                        logger.info(
                            f"JOLT column mapping: {orig_name} -> {column_descriptions[orig_name]}"
                        )
                    else:
                        ...
                        for desc_key in column_descriptions.keys():
                            ...
                            if (
                                orig_name.lower().replace("_", " ").replace("-", " ")
                                in desc_key.lower()
                                or desc_key.lower().replace("_", " ").replace("-", " ")
                                in orig_name.lower()
                            ):
                                best_match = column_descriptions[desc_key]
                                logger.info(
                                    f"JOLT fuzzy column mapping: {orig_name} -> {desc_key} -> {best_match}"
                                )
                                break
                        if best_match:
                            ...
                            logger.warning(
                                f"JOLT: No mapping found for column '{orig_name}', using original"
                            )
                ...
                logger.info(
                    "JOLT: No column descriptions in config, using original names"
                )
        ...
        if not isinstance(X_train, pd.DataFrame):
            ...
            if len(feature_names) > X_train.shape[1]:
                ...
                feature_names = list(feature_names) + [
                    f"feature_{i}" for i in range(len(feature_names), X_train.shape[1])
                ]
        ...
        if not isinstance(X_test, pd.DataFrame):
            ...
            if len(feature_names) > X_test.shape[1]:
                ...
                feature_names = list(feature_names) + [
                    f"feature_{i}" for i in range(len(feature_names), X_test.shape[1])
                ]
        ...
        logger.info(
            f"JOLT using model: {jolt_model_name}, shots: {effective_shots}, full training data: {len(X_train)} samples"
        )
        ...
        if is_regression:
            ...
            logger.info(
                f"Limiting regression training data to {max_train_for_jolt} samples (JOLT authors use 10-50)"
            )
        else:
            # JOLT authors use up to 32 shots for classification
            max_train_for_jolt = min(effective_shots * 2, 32, len(train_data))
            logger.info(
                f"Limiting classification training data to {max_train_for_jolt} samples (JOLT authors use up to 32)"
            )
        ...
        if original_train_size > max_train_for_jolt:
            ...
            logger.info(
                f"Reduced training data from {original_train_size} to {max_train_for_jolt} samples to match JOLT authors' approach"
            )
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_output_dir:
                ...
                if hasattr(args, "jolt_model") and args.jolt_model:
                    ...
                    if "/" in args.jolt_model:
                        ...
                        if "qwen" in args.jolt_model.lower():
                            if (
                                "2.5" in args.jolt_model
                                and "7b" in args.jolt_model.lower()
                            ):
                                llm_type = (
                                    "qwen2.5-7b-instruct"  # Use JOLT's specific naming
                                )
                    ...
                    llm_type = (
                        "phi-3-mini-128k-instruct"  # Even smaller than Gemma-2-2B
                    )

                logger.info(
                    f"Using JOLT config - llm_path: {llm_path}, llm_type: {llm_type}"
                )
                logger.info(
                    f"JOLT few-shot: user requested {user_few_shot}, using {effective_shots} shots, {len(train_data)} train samples available"
                )
                logger.info(
                    f"JOLT train_size_limit={effective_shots} to prevent massive prompts while preserving test indices"
                )
                ...
                if is_regression:
                    ...
                    logger.info(
                        f"Using aggressive memory limits for regression: max_tokens={max_tokens}, max_generated_length={max_generated_length}"
                    )
                else:
                    ...
                    max_generated_length = 256  # Minimum safe generation length
                    logger.info(
                        f"Using standard limits for classification: max_tokens={max_tokens}, max_generated_length={max_generated_length}"
                    )
                ...
                if is_regression:
                    ...
                    num_decimal_places_y = (
                        3  # Use 3 decimal places for regression targets
                    )
                else:
                ...
                jolt_args = Namespace(
                    ...
                    batch_size=(
                        1 if is_regression else 5
                    ),  # Smaller batch for regression, JOLT authors use 5 for classification
                    ...
                    train_end_index=len(
                        train_data
                    ),  # Use actual limited training data size
                    test_start_index=len(
                        train_data
                    ),  # Test data starts after limited training data
                    test_end_index=len(train_data)
                    + actual_test_size,  # Test data ends after all test samples
                ...
                try:
                    logger.info(
                        f"Loading JOLT model - llm_path: {llm_path}, llm_type: {llm_type}"
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            f"JOLT args: llm_path={llm_path}, llm_type={llm_type}"
                        )
                    ...
                    logger.error(
                        f"Error loading JOLT model {llm_type}: {e}. Using fallback model."
                    )
                    # Use the specified JOLT model as fallback, with better error handling
                    fallback_model = getattr(
                        args, "jolt_model", "Qwen/Qwen2.5-7B-Instruct"
                    )
                ...
                max_retries = max(
                    0, num_features - 2
                )  # Keep trying until only 2 features remain
                ...
                for retry in range(max_retries + 1):
                    try:
                        ...
                        logger.info(
                            f"  - max_generated_length: {jolt_args.max_generated_length}"
                        )
                        ...
                        with torch.no_grad():
                            ...
                            if not isinstance(jolt_results, dict):
                                raise ValueError(
                                    f"JOLT run_jolt() returned {type(jolt_results)}, expected dict"
                                )
                            ...
                            if is_regression:
                                if "gen" not in jolt_results:
                                    raise ValueError(
                                        "JOLT sample mode did not return 'gen' key - sample() function may have failed"
                                    )

                                gen_data = jolt_results["gen"]
                                if not isinstance(gen_data, list):
                                    raise ValueError(
                                        f"JOLT 'gen' data is {type(gen_data)}, expected list"
                                    )

                                if len(gen_data) == 0:
                                    raise ValueError(
                                        "JOLT 'gen' data is empty - no samples were generated"
                                    )

                                # Check if any generation results exist
                                total_samples = sum(
                                    len(gen_item) if isinstance(gen_item, list) else 0
                                    for gen_item in gen_data
                                )
                                if total_samples == 0:
                                    raise ValueError(
                                        "JOLT generated 0 valid samples - all generation attempts failed or were filtered out"
                                    )

                                logger.info(
                                    f"JOLT generated {total_samples} total samples across {len(gen_data)} test instances"
                                )

                                # Check if process_generated_results was called (should add 'metrics' key)
                                if "metrics" not in jolt_results:
                                    raise ValueError(
                                        "JOLT sample mode completed but 'metrics' key missing - process_generated_results() was not called or failed"
                                    )
                                ...
                                if not isinstance(metrics, list) or len(metrics) == 0:
                                    raise ValueError(
                                        f"JOLT metrics is {type(metrics)} with length {len(metrics) if hasattr(metrics, '__len__') else 'N/A'}, expected non-empty list"
                                    )

                                first_metric = metrics[0]
                                if not isinstance(first_metric, dict):
                                    raise ValueError(
                                        f"JOLT first metric is {type(first_metric)}, expected dict"
                                    )
                                ...
                                if missing_keys:
                                    raise ValueError(
                                        f"JOLT regression metrics missing required keys: {missing_keys}"
                                    )
                                ...
                                if (
                                    isinstance(y_test_median, list)
                                    and len(y_test_median) > 0
                                ):
                                    nan_count = sum(
                                        1 for val in y_test_median if np.isnan(val)
                                    )
                                    if nan_count == len(y_test_median):
                                        raise ValueError(
                                            "All JOLT regression predictions are NaN - generation or parsing failed"
                                        )
                                    elif nan_count > 0:
                                        logger.warning(
                                            f"JOLT has {nan_count}/{len(y_test_median)} NaN predictions"
                                        )
                        ...
                        if is_oom_error(jolt_error) and retry < max_retries:
                            ...
                            if drop_feature_for_oom(
                                dropped_features, num_features, logger, min_features=2
                            ):
                                logger.info(
                                    f"Retrying JOLT with {len(dropped_features)} dropped features"
                                )
                                ...
                                # Update datasets
                                X_train = (
                                    X_train.iloc[:, keep_indices]
                                    if hasattr(X_train, "iloc")
                                    else X_train[:, keep_indices]
                                )
                                X_test = (
                                    X_test.iloc[:, keep_indices]
                                    if hasattr(X_test, "iloc")
                                    else X_test[:, keep_indices]
                                )

                                # Update dataset info
                                dataset = original_dataset.copy()
                                dataset["attribute_names"] = [
                                    dataset["attribute_names"][i] for i in keep_indices
                                ]
                                if "categorical_indicator" in dataset:
                                    dataset["categorical_indicator"] = [
                                        dataset["categorical_indicator"][i]
                                        for i in keep_indices
                                    ]

                                # Recreate DataFrames with new feature names
                                feature_names = get_jolt_feature_names(
                                    dataset["attribute_names"], jolt_config
                                )
                                X_train = pd.DataFrame(X_train, columns=feature_names)
                                X_test = pd.DataFrame(X_test, columns=feature_names)
                                ...
                                logger.error(
                                    "No more features to drop, cannot continue"
                                )
                                raise
                        else:
                ...
                example_inputs_outputs = (
                    []
                )  # Store example inputs and outputs for debugging
                ...
                logger.info(
                    f"JOLT results keys: {list(jolt_results.keys()) if isinstance(jolt_results, dict) else 'Not a dict'}"
                )
                if isinstance(jolt_results, dict) and "data" in jolt_results:
                    logger.info(
                        f"JOLT data keys: {list(jolt_results['data'].keys()) if isinstance(jolt_results['data'], dict) else 'Data not a dict'}"
                    )
                ...
                if is_regression:
                            ...
                            if key == "data" and isinstance(value, dict):
                                ...
                                for subkey, subvalue in value.items():
                                    if hasattr(subvalue, "shape"):
                                        logger.info(
                                            f"      - {subkey}: array shape {subvalue.shape}"
                                        )
                                    elif isinstance(subvalue, (list, tuple)):
                                        logger.info(
                                            f"      - {subkey}: {type(subvalue).__name__} length {len(subvalue)}"
                                        )
                                    else:
                                        logger.info(
                                ...
                                logger.info(
                                    f"    - {key}: {type(value).__name__} with {len(value) if hasattr(value, '__len__') else 'no'} items"
                                )
                                if isinstance(value, list) and len(value) > 0:
                                    first_metric = value[0]
                                    if isinstance(first_metric, dict):
                                        logger.info(
                                            f"      - First metric keys: {list(first_metric.keys())}"
                                        )
                                        for metric_key in [
                                            "y_test_median",
                                            "y_test_mean",
                                            ...
                                            if metric_key in first_metric:
                                                ...
                                                if hasattr(metric_val, "__len__"):
                                                    logger.info(
                                                        f"        - {metric_key}: length {len(metric_val)}"
                                                    )
                                                else:
                                                    logger.info(
                                                        f"        - {metric_key}: {metric_val}"
                                                    )
                                            else:
                                                logger.info(
                                                    f"        - {metric_key}: MISSING"
                                                )
                    ...
                    if isinstance(jolt_results, dict):
                        ...
                        logger.info(
                            f"  - Expected sample mode results: gen={has_gen}, prompts={has_prompts}, metrics={has_metrics}"
                        )

                        if has_gen:
                            gen_data = jolt_results["gen"]
                            logger.info(
                                f"    - gen data type: {type(gen_data)}, length: {len(gen_data) if hasattr(gen_data, '__len__') else 'N/A'}"
                            )
                            if isinstance(gen_data, list) and len(gen_data) > 0:
                                first_gen = gen_data[0]
                                logger.info(
                                    f"    - first gen item type: {type(first_gen)}, length: {len(first_gen) if hasattr(first_gen, '__len__') else 'N/A'}"
                                )

                        if not has_metrics:
                            logger.error(
                                "REGRESSION ISSUE: JOLT sample mode did not produce 'metrics' - process_generated_results may have failed!"
                            )
                            logger.error(
                                "This suggests the sample() function completed but process_generated_results() was not called or failed"
                            )
                ...
                if is_regression:
                    ...
                    if (
                        isinstance(jolt_results, dict)
                        and "metrics" in jolt_results
                        and len(jolt_results["metrics"]) > 0
                    ):
                        # Extract regression predictions and metrics from JOLT
                        logger.info(
                            "Extracting regression predictions from JOLT metrics..."
                        )
                        try:
                            metrics = jolt_results["metrics"][
                                0
                            ]  # Get first column metrics for regression

                            # Extract regression predictions (use median as point prediction)
                            if (
                                "y_test_median" in metrics
                                and len(metrics["y_test_median"]) > 0
                            ):
                                ...
                                logger.info(
                                    f"Successfully extracted {completed_samples} regression predictions from JOLT"
                                )

                                # Extract JOLT's built-in regression metrics
                                if "mae" in metrics:
                                    jolt_regression_metrics["mae"] = float(
                                        metrics["mae"]
                                    )
                                    logger.info(
                                        f"JOLT built-in MAE: {jolt_regression_metrics['mae']:.4f}"
                                    )

                                # Store additional regression statistics if available
                                if "y_test_mean" in metrics:
                                    jolt_regression_metrics["predictions_mean"] = (
                                        metrics["y_test_mean"]
                                    )
                                if "y_test_std" in metrics:
                                    jolt_regression_metrics["predictions_std"] = (
                                        metrics["y_test_std"]
                                    )
                                if (
                                    "y_test_lower" in metrics
                                    and "y_test_upper" in metrics
                                ):
                                ...
                                logger.warning(
                                    "No y_test_median found in JOLT regression metrics"
                                )
                                predictions = []
                                completed_samples = 0
                        except Exception as e:
                            logger.error(
                                f"Error extracting regression predictions from JOLT metrics: {e}"
                            )
                            import traceback
                    ...
                    logger.info(
                        f"Successfully extracted {completed_samples} predictions from JOLT"
                    )
                    ...
                    logger.info(
                        "Attempting to extract predictions from JOLT metrics..."
                    )
                    try:
                        metrics = jolt_results["metrics"][0]  # Get first column metrics
                        if (
                            "probabilities_from_logits" in metrics
                            and len(metrics["probabilities_from_logits"]) > 0
                        ):
                            ...
                            if (
                                "categories" in jolt_results
                                and len(jolt_results["categories"]) > 0
                            ):
                                categories = jolt_results["categories"][
                                    0
                                ...
                                logger.info(
                                    f"Successfully extracted {completed_samples} predictions and probabilities from JOLT"
                                )
                            ...
                            logger.warning(
                                "No probabilities_from_logits found in JOLT metrics"
                            )
                            predictions = []
                            completed_samples = 0
                    except Exception as e:
                        logger.error(
                            f"Error extracting predictions from JOLT metrics: {e}"
                        )
                        import traceback
                ...
                if len(predictions) > len(X_test):
                    ...
                    logger.warning(
                        f"JOLT only predicted {len(predictions)} out of {len(X_test)} samples. "
                    ...
                    if is_regression:
                        ...
                        if len(predictions) > 0:
                            ...
                            if existing_predictions:
                                ...
                                logger.info(
                                    f"Using mean value {imputation_value:.3f} for regression imputation"
                                )
                            else:
                                # Fallback to training data mean
                                imputation_value = np.mean(y_train)
                                logger.info(
                                    f"Using training data mean {imputation_value:.3f} for regression imputation"
                                )
                        else:
                            imputation_value = np.mean(y_train)
                            logger.info(
                        ...
                        if len(predictions) > 0:
                            ...
                            logger.info(
                                f"Using majority class '{majority_class}' for classification imputation"
                            )
                        else:
                            ...
                            logger.info(
                                f"Using training majority class '{majority_class}' for classification imputation"
                            )
                    ...
                    logger.info(
                        f"Successfully imputed {missing_count} missing predictions. Total predictions: {len(predictions)}"
                    )

                # Calculate metrics
                if completed_samples > 0:
                    y_test_partial = (
                        y_test[:completed_samples]
                    ...
                    if len(y_test_partial) > 0:
                        target_type = type(
                            y_test_partial.iloc[0]
                            if hasattr(y_test_partial, "iloc")
                            else y_test_partial[0]
                        )
                            ...
                            try:
                                ...
                                if target_type == int:
                                    ...
                                    if isinstance(pred, str):
                                        ...
                                        converted_pred = int(
                                            float(pred)
                                        )  # Use float first to handle "1.0" -> 1
                                    else:
                    ...
                    if is_regression:
                        ...
                        logger.info(
                            f"JOLT regression metrics - R²: {r2:.4f}, MAE: {mae:.4f}, RMSE: {rmse:.4f}"
                        )
                        ...
                        if jolt_probabilities is not None and completed_samples > 0:
                            ...
                            all_class_log_probs = []
                            categories = (
                                jolt_results["categories"][0]
                                if "categories" in jolt_results
                                else unique_classes
                            )

                            for i in range(
                                min(completed_samples, len(jolt_probabilities))
                            ):
                                ...
                                for j, class_label in enumerate(categories):
                                    ...
                                    prob = (
                                        jolt_probabilities[i][j]
                                        if j < len(jolt_probabilities[i])
                                        else 1e-10
                                    )
                            ...
                            logger.info(
                                f"Converted JOLT probabilities to log probs for {len(all_class_log_probs)} samples"
                            )
                        ...
                        calculated_metrics = calculate_llm_metrics(
                            ...
                            predictions_partial,
                            unique_classes,
                            all_class_log_probs=all_class_log_probs,
                            logger=logger,
                            task_type=task_type,
                        )
                        ...
                        logger.info(
                            f"JOLT achieved {accuracy:.4f} accuracy on {completed_samples} samples"
                        )
                    ...
                    for i in range(num_examples_to_store):
                        true_label = (
                            y_test_partial.iloc[i]
                            if hasattr(y_test_partial, "iloc")
                            else y_test_partial[i]
                        )
                        ...
                        example_inputs_outputs.append(
                            {
                                "sample_index": i,
                                "input_features": (
                                    test_features.to_dict()
                                    if hasattr(test_features, "to_dict")
                                    else str(test_features)
                                ),
        ...
        results = {
            ...
            "completion_rate": (
                completed_samples / len(X_test) if len(X_test) > 0 else 0.0
            ),
            "num_features": X_train.shape[
                1
            ],  # Use X_train to get actual feature count after reduction
            "predictions": convert_to_serializable(
                predictions[:completed_samples] if completed_samples > 0 else []
            ),
            "ground_truth": convert_to_serializable(
                (
                    y_test[:completed_samples].tolist()
                    if hasattr(y_test[:completed_samples], "tolist")
                    else list(y_test)[:completed_samples]
                )
                if completed_samples > 0
                else []
            ...
            results.update(
                {
                    ...
                    "r2_score": (
                        float(calculated_metrics["r2_score"])
                        if calculated_metrics["r2_score"] is not None
                        else None
                    ),
                    "mae": (
                        float(calculated_metrics["mae"])
                        if calculated_metrics["mae"] is not None
                        else None
                    ),
                    "mse": (
                        float(calculated_metrics["mse"])
                        if calculated_metrics["mse"] is not None
                        else None
                    ),
                    "rmse": (
                        float(calculated_metrics["rmse"])
                        if calculated_metrics["rmse"] is not None
                        else None
                    ),
                    "regression_results": {
                        "r2_score": (
                        ...
                        "jolt_builtin_metrics": convert_to_serializable(
                            jolt_regression_metrics
                        ),
                    },
                }
            )
            ...
            results.update(
                {
                    ...
                    "balanced_accuracy": (
                        float(balanced_acc) if balanced_acc is not None else None
                    ),
                    "num_classes": len(unique_classes),
                    "roc_auc": (
                        float(calculated_metrics["roc_auc"])
                        if calculated_metrics["roc_auc"] is not None
                        else None
                    ),
                    "f1_macro": (
                        float(calculated_metrics["f1_macro"])
                        if calculated_metrics["f1_macro"] is not None
                        else None
                    ),
                    "f1_micro": (
                        float(calculated_metrics["f1_micro"])
                        if calculated_metrics["f1_micro"] is not None
                        else None
                    ),
                    "f1_weighted": (
                        float(calculated_metrics["f1_weighted"])
                        if calculated_metrics["f1_weighted"] is not None
                        else None
                    ),
                    "precision_macro": (
                        float(calculated_metrics["precision_macro"])
                        if calculated_metrics["precision_macro"] is not None
                        else None
                    ),
                    "recall_macro": (
                        float(calculated_metrics["recall_macro"])
                        if calculated_metrics["recall_macro"] is not None
                        else None
                    ),
        ...
        if results["completion_rate"] < min_completion_rate:
            error_msg = (
                f"JOLT failed to complete successfully: only {results['completed_samples']}/{results['num_test_samples']} "
        ...
        if not is_regression:
            if results["accuracy"] is None or results["accuracy"] == 0:
                error_msg = (
                    f"JOLT failed to produce valid classification results: "
            ...
            if results["mae"] is None:
                error_msg = (
                    f"JOLT failed to produce valid regression results: "
        ...
        logger.info(
            f"JOLT evaluation completed successfully with {results['completion_rate']:.1%} completion rate"
        )
        ...
        return {
            ...
            "completed_samples": 0,
            "completion_rate": 0.0,
            "used_jolt_config": jolt_config
            is not None,  # Include config status even in error case
        }
    finally:
