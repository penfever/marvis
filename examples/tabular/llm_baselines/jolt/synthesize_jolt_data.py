...
def extract_column_descriptions(semantic_info: Dict[str, Any]) -> Dict[str, str]:
    ...
    if "feature_description" in semantic_info and isinstance(
        semantic_info["feature_description"], str
    ):
        ...
        dataset_name = semantic_info.get(
            "dataset_name", semantic_info.get("dataset", "unknown")
        )
        ...
        if "fourier" in feat_desc_string.lower():
            for i in range(min(20, num_features)):
                column_descriptions[f"fourier_coeff_{i+1}"] = (
                    f"Fourier coefficient {i+1}"
                )
        elif "pixel" in feat_desc_string.lower():
            for i in range(min(20, num_features)):
                column_descriptions[f"pixel_{i+1}"] = f"Pixel value {i+1}"
        elif (
            "karhunen" in feat_desc_string.lower() or "pca" in feat_desc_string.lower()
        ):
            for i in range(min(20, num_features)):
                column_descriptions[f"kl_coeff_{i+1}"] = (
                    f"Karhunen-Loève coefficient {i+1}"
                )
            ...
            for i in range(min(20, num_features)):
                column_descriptions[f"protein_{i+1}"] = (
                    f"Protein expression level {i+1}"
                )
        elif (
            "sensor" in feat_desc_string.lower()
            or "acceleration" in feat_desc_string.lower()
        ):
            ...
            for i in range(min(20, num_features)):
                if i < len(sensor_types):
                    column_descriptions[f"feature_{i+1}"] = (
                        f"{sensor_types[i % len(sensor_types)]} measurement"
                    )
                else:
    ...
    elif "feature_description" in semantic_info and isinstance(
        semantic_info["feature_description"], dict
    ):
            ...
            if isinstance(feat_desc, str):
                ...
                if "," in feat_desc:
                    ...
                    clean_desc = (
                        ...
                        else feat_name.replace("-", " ").replace("_", " ")
                    )
            else:
                clean_desc = feat_name.replace("-", " ").replace("_", " ")
            column_descriptions[feat_name] = clean_desc
    ...
    if not column_descriptions:
        ...
        num_features = (
            semantic_info.get("features")
            or semantic_info.get("dataset_size", {}).get("features")
            or semantic_info.get("input_features", {}).get("number_of_features")
            or 5
        )
        ...
        if "phishing" in dataset_name or "phishing" in dataset_desc:
            for i in range(num_features):
                column_descriptions[f"feature_{i+1}"] = (
                    f"Website security feature {i+1}"
                )
...
def extract_class_information(semantic_info: Dict[str, Any]) -> Tuple[List[str], str]:
    ...
    if "target_description" in semantic_info:
        ...
        if isinstance(target_values, dict):
            ...
            class_meanings = [
                f"'{cls}' ({meaning})" for cls, meaning in target_values.items()
            ]
        ...
        if "values" in target_var and isinstance(target_var["values"], list):
            ...
            if description:
                ...
                if (
                    "phishing" in description.lower()
                    and "legitimate" in description.lower()
                ):
        ...
        if target_type == "multiclass":
            ...
            if "digit" in dataset_name or "mnist" in dataset_name:
                ...
                class_description = "Classes: " + ", ".join(
                    [f"'{i}' (Digit {i})" for i in range(10)]
                )
            elif "letter" in dataset_name:
                class_names = [chr(ord("A") + i) for i in range(26)]
                class_description = "Classes: " + ", ".join(
                    [
                        f"'{chr(ord('A') + i)}' (Letter {chr(ord('A') + i)})"
                        for i in range(26)
                    ]
                )
            else:
...
def create_task_context_prefix(
    ...
    if column_descriptions:
        column_list = ", ".join(
            list(column_descriptions.values())[:10]
    ...
    if "fourier" in feat_desc.lower():
        ...
        "medical" in str(semantic_info).lower()
        or "patient" in str(semantic_info).lower()
    ):
...
def create_jolt_config_for_dataset(semantic_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    task_prefix = create_task_context_prefix(
        semantic_info, column_descriptions, class_description
    )
    ...
    for json_file in sorted(json_files):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"  Created config: {jolt_config['num_features']} features, {jolt_config['num_classes']} classes"
                )
