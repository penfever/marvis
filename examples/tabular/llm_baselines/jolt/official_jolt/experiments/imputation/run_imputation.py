...
def main():
    ...
    mask = np.random.choice(
        ...
        p=[1 - args.missing_fraction, args.missing_fraction],
    )
    ...
    for row in range(num_rows):
        ...
        y_column_types = [
            "numerical"
        ] * num_columns_to_impute  # all colums are numerical here

        data = csv_parser.get_data(
            y_column_names=y_column_names, missing_mask=mask, test_row=row
        )

        if data is not None:
            tmp_path = os.path.join(
                "./tmp", next(tempfile._get_candidate_names()) + ".pkl"
            )
        ...
        if len(args.prefix) > 0:
            args.prefix = "Each example contains four columns: {}, {}, {}, and {} that describe what type and how many medals a country won at the Paris 2024 olympics.".format(
    ...
    final_results = {"mae": mae, "nll": nll}

    with open(
        os.path.join(args.output_dir, args.experiment_name + "_final.pkl"), "wb"
    ) as g:
