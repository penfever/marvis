    ...
    def get_data(self, y_column_names=[], missing_mask=None, test_row=None):
        ...
        for i, _ in self.df.iterrows():
            ...
            for j, name in enumerate(self.rearranged_column_names):
                if missing_mask[
                    i, self.all_column_names.index(name)
                ]:  # need to swizzle the mask indices as well
                    continue
        ...
        data = {
            ...
            "x_test": np.expand_dims(
                x_all[test_row] + self.column_separator, axis=0
            ),  # do put a column separator on x_test
