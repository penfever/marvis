    ...
    for dataset in datasets:
        for shot in shots:
            for seed in seeds:
                print("-------------------------------")
                print(
                    "model={}, dataset={}, shot={}, seed={}".format(
                        args.llm_type, dataset, shot, seed
                    )
                )
                print("-------------------------------")
                config_args = option_parser.parse_args(
                    args=[
                        "--experiment_name",
                        "{}_shot_{}_seed_{}_{}".format(
                            dataset, shot, seed, args.llm_type
                        ),
                        "--data_path",
                        os.path.join(
                            "./data/tabllm",
                            "tabllm_{}_{}_{}.pkl".format(dataset, shot, seed),
                        ),
                        "--output_dir",
                        "./experiments/classification/output",
                        "--mode",
                        "logpy_only",
                        "--seed",
                        "0",
