...
def run_jolt(args, model, tokenizer):
    ...
    if args.mode not in valid_modes:
        ...
        if not has_sample and not has_logpy:
            raise ValueError(
                f"Invalid mode '{args.mode}'. Valid modes are: {valid_modes}. "
                f"Mode must contain 'sample' for sampling mode or 'logpy' for log probability mode."
            )

        # Log warning for non-standard mode format
        print(
            f"WARNING: Mode '{args.mode}' uses non-standard format. Consider using: {valid_modes}"
        )
        ...
        for i, type in enumerate(args.y_column_types):
            if type == "categorical":
                results = compute_classification_metrics(
                    results=results, column_index=i
                )
