        ...
        for idx in tqdm(range(num_prompts), desc="Sampling"):
            ...
            while num_samples > 0:
                ...
                if elapsed_time > max_timeout_per_sample:
                    logging.warning(
                        f"JOLT sampling timeout ({max_timeout_per_sample}s) reached for sample {idx}. "
                    ...
                    for _ in range(num_samples):
                        ...
                        for col_idx in range(results["dim_y"]):
                            if hasattr(args, "y_column_types") and col_idx < len(
                                args.y_column_types
                            ):
                                if args.y_column_types[col_idx] == "categorical":
                                    ...
                                    if (
                                        ...
                                        and col_idx < len(results["categories"])
                                        and len(results["categories"][col_idx]) > 0
                                    ):
                                        fallback_sample.append(
                                            results["categories"][col_idx][0]
                                        )
                                    else:
                                    ...
                                    if (
                                        hasattr(args, "num_decimal_places_y")
                                        and args.num_decimal_places_y == 0
                                    ):
                ...
                if failed_attempts >= max_failed_attempts:
                    logging.warning(
                        f"JOLT sampling max failed attempts ({max_failed_attempts}) reached for sample {idx}. "
                        f"Filling remaining {num_samples} samples with fallback value 0.0"
                    )
                    # Fill remaining samples with fallback value - match expected data types
                    for _ in range(num_samples):
