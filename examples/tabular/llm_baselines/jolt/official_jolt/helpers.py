...
def get_predicted_values_from_generated_sample(generated_input, args, category_names):
    ...
    if len(column_values) < expected_num_y_columns:
        if args.print_sampling_rejections:
            print(
                "Number of column values={} in the generated response is fewer than expected={}.".format(
                    column_values, expected_num_y_columns
                )
            )
            print(generated_input)
        return None
    if len(column_values) > expected_num_y_columns:
        column_values = column_values[
            -expected_num_y_columns:
        ]  # take the last expected_num_y_columns
    ...
    for i in range(expected_num_y_columns):
        if args.y_column_types[i] == "categorical":
            ...
            if column_values[i] in category_names[i]:
                ...
                if args.print_sampling_rejections:
                    print(
                        "Generated response for column={} does not contain a known category.".format(
                            i
                        )
                    )
                    print(generated_input)
            ...
            if not number:
                if args.print_sampling_rejections:
                    print(
                        "Generated response for column={} does not contain a number.".format(
                            i
                        )
                    )
                    print(generated_input)
...
def floats_to_str(nums, num_decimal, dim=1):
    if np.ndim(nums) == 0:
        return _float_to_str(
            nums, num_decimal
        )  # when y_dim = 1, only a scalar is passed
    if dim > 1:  # can have multiple dimensions in x and y
        return [
            [_float_to_str(value, num_decimal) for value in group] for group in nums
        ]
    else:
...
def construct_prompts(x_train, y_train, x_test, args, dim_y):
    ...
    for i, column_type in enumerate(args.y_column_types):
        if column_type == "numerical":
            str_y_train[:, i] = np.expand_dims(
                np.array(
                    floats_to_str(
                        nums=y_train[:, i], num_decimal=args.num_decimal_places_y, dim=1
                    )
                ),
                axis=0,
            )
        ...
        if y_column_type == "numerical":
            ...
            if len(args.y_column_types) > 1:
                mae = np.mean(
                    np.abs(
                        y_test_median
                        - np.array(gen_results["data"]["y_test"])[:, column]
                    )
                )
            else:
                mae = np.mean(
                    np.abs(y_test_median - np.array(gen_results["data"]["y_test"]))
                )

            metrics["y_test"] = y_tests
            if (len(args.y_column_types) == 1) and (
                args.y_column_types[0] == "numerical"
            ):  # only used in black box opt with one output y
                metrics["y_test_max_x"] = gen_results["data"]["x_test"][
                    np.argmax(np.max(np.array(y_tests), axis=1))
                ]  # find argmax of the largest sample
            ...
            for i in range(len(gen_results["gen"])):
                ...
                if len(args.y_column_types) > 1:
                    ...
                    for category in gen_results["categories"][column]:
                        y_test_distribution[i].append(
                            (np.array(ys[:, column]) == category).sum()
                        )
                    y_test_mode[i] = gen_results["categories"][column][
                        np.argmax(y_test_distribution[i])
                    ]
                    probabilities_from_sampling[i] = (
                        y_test_distribution[i] / np.array(y_test_distribution[i]).sum()
                    )
                else:
                    for category in gen_results["categories"][column]:
                        y_test_distribution[i].append(
                            (np.array(gen_results["gen"][i]) == category).sum()
                        )
                    y_test_mode[i] = gen_results["categories"][column][
                        np.argmax(y_test_distribution[i])
                    ]
            ...
            # compute accuracy
            if len(args.y_column_types) > 1:
                accuracy_sampling = accuracy_score(
                    np.array(gen_results["data"]["y_test"], dtype=str)[:, column],
                    y_test_mode,
                )
            else:
                accuracy_sampling = accuracy_score(
                    np.array(gen_results["data"]["y_test"]), y_test_mode
                )
                if len(gen_results["categories"][column]) == 2:  # binary classification
                    auc = roc_auc_score(
                        np.array(gen_results["data"]["y_test"]),
                        np.array(probabilities_from_sampling)[:, 1],
                    )
...
def compute_classification_metrics(results, column_index):
    ...
    results["metrics"][column_index]["y_test_mode"] = np.array(
        results["categories"][column_index]
    )[pred_indices]

    # accuracy
    accuracy_logits = accuracy_score(
        np.array(
            results["data"]["y_test"][:, column_index],
            dtype=results["metrics"][column_index]["y_test_mode"].dtype,
        ),
