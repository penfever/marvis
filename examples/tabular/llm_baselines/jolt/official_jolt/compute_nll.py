...
def _get_y_logprobs_by_column(
    args, tokenizer, model, input_tokens, mask, y_ranges, column_type, column_index
):
    """
    ...
    if tensor_size_gb > 10.0:  # More than 10GB per tensor is unreasonable
        raise RuntimeError(
            f"JOLT tensor too large: {tensor_size_gb:.2f}GB. Reduce batch size or sequence length."
        )
    ...
    attn_mask = torch.zeros((bs, max_prompt_len), dtype=torch.long, device=model.device)
    for k, t in enumerate(input_tokens):
        # More efficient: create on CPU then move to GPU once
        input_ids[k, : len(t)] = torch.tensor(t, dtype=torch.long, device="cpu").to(
            model.device
        )
    ...
    for j in range(bs):
        ...
        y_logprobs_j.append(
            logprobs[j][
                y_ranges[j][column_index][0] - 1 : y_ranges[j][column_index][1] - 1
            ].cpu()
        )
...
def _get_y_logprobs_categorical(args, tokenizer, model, input_tokens, y_ranges):
    ...
    for j in range(bs):
        y_logprobs_j = []
        y_logprobs_j.append(
            logprobs[j][y_ranges[j][0][0] - 1 : y_ranges[j][0][1] - 1].cpu()
        )
        y_logprobs.append(y_logprobs_j)
...
def construct_y_str(y_test_true, categories, args, tokenizer, non_y):
    ...
    for i, y_type in enumerate(y_types):
        ...
        if y_type == "numerical":
            ...
            right_range = len(
                tokenizer.encode(y_strs[0] + prefix_y + y_partial_str + cur_sep)
            )
            ...
            for category in categories[i]:
                if str(category) != str(y_test_true[i]):
                    right_range = len(
                        tokenizer.encode(y_strs[0] + prefix_y + str(category) + cur_sep)
                    )
...
def compute_classification_probabilities(args, tokenizer, model, results):
    if "metrics" not in results.keys():
        results["metrics"] = [
            {"probabilities_from_logits": [], "y_logprobs": []}
            for _ in range(len(args.y_column_types))
        ]
    else:
    ...
    if len(results["data"]["y_train"]) > 0 and len(results["categories"][0]) > 0:
        ...
        if category_counts:
            ...
            logger.info(
                f"JOLT fallback category for missing labels: '{most_common_category}' (idx={fallback_category_idx})"
            )
        else:
            logger.info(
                f"JOLT fallback category: using first category '{available_categories[0]}' (idx=0)"
            )

    full_texts, enc_full_texts, y_ranges = [], [], []
    max_len = 0
    for x_test, y_test_true in tqdm(
        (zip(results["data"]["x_test"], results["data"]["y_test"])),
        ...
        str_y_test_true, token_y_test_true, y_range, cur_max_length = (
            construct_y_str_single_categorical(
                results["categories"], args, tokenizer=tokenizer, non_y=non_y
            )
        )
    ...
    print(
        f"[JOLT Memory Opt] Processing {len(enc_full_texts)} prompts individually to prevent OOM"
    )
    y_logprobs = []
    ...
    for itr, y_test_true in tqdm(
        enumerate(results["data"]["y_test"]), desc="Computing probabilities"
    ):  # this loops over number of test locations
        y_logprobs_given_x = y_logprobs[
            itr * num_categories : (itr + 1) * num_categories
        ]
        ...
        if idx_gt_column is None:
            ...
            available_categories = [str(cat) for cat in results["categories"][0]]
            logger.warning(
                f"Ground truth value '{gt_value_str}' not found in training categories: {available_categories}. "
        ...
        results["metrics"][0]["probabilities_from_logits"].append(
            np.exp(logprobs_normalized)
        )
...
def compute_nll(args, tokenizer, model, results):
    ...
    for x_test, y_test_true in tqdm(
        ...
        str_y_test_true, token_y_test_true, y_range, cur_max_length = construct_y_str(
            y_test_true, results["categories"], args, tokenizer=tokenizer, non_y=non_y
        )
    ...
    allowed_tokens = set([tokenizer.convert_tokens_to_ids(token) for token in allowed])
    ...
    for enc_full_text, y_range, y_test_true in tqdm(
        zip(enc_full_texts, y_ranges, results["data"]["y_test"]),
        ...
        for i, column in enumerate(args.y_column_types):
            if column == "numerical":
                nll_num_column = -(
                    gt_y_logprobs[0][i].sum().item()
                    + args.num_decimal_places_y * np.log(10)
                )
                ...
                results["metrics"][i]["probabilities_from_logits"].append(
                    np.exp(logprobs)
                )
