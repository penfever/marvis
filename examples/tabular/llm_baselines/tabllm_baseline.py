...
def create_regression_bins(
    y_train: np.ndarray, n_bins: int = 10
...
def convert_targets_to_bins(
    y: np.ndarray, bin_edges: np.ndarray, bin_labels: List[str]
) -> List[str]:
    """
...
def convert_bin_predictions_to_values(
    predictions: List[str], bin_edges: np.ndarray, bin_labels: List[str]
) -> np.ndarray:
    """
    ...
    try:
        if template_path and template_path.exists():
            ...
            CustomLoader.add_constructor(
                "!TemplateMetadata", template_metadata_constructor
            )
            ...
            logger.info(
                f"Found TabLLM config for OpenML task {openml_task_id} (dataset: {dataset_name})"
            )
            ...
            semantic_file_exists = (
                hasattr(semantic_file, "exists") and semantic_file.exists()
            ) or (isinstance(semantic_file, str) and os.path.exists(semantic_file))
            ...
            if semantic_file_exists and original_feature_count is not None:
                try:
                    ...
                    logger.warning(
                        f"Could not validate feature count from semantic info: {e}"
                    )
        ...
        return _load_tabllm_config_by_dataset_name_fallback(
            openml_task_id, original_feature_count
        )
        ...
        logger.error(
            f"Error loading TabLLM config for OpenML task {openml_task_id}: {e}"
        )
        return None, None


def _load_tabllm_config_by_dataset_name_fallback(
    openml_task_id, original_feature_count=None
):
    ...
    try:
        ...
        mapping_path = (
            rm.path_resolver.get_configs_dir() / "tabllm" / "openml_task_mapping.json"
        )
    ...
    logger.info(
        f"Found TabLLM config for OpenML task {openml_task_id} (dataset: {dataset_name}) via fallback"
    )
    ...
    if original_feature_count is not None:
        ...
        feature_mapping = {"task_id": openml_task_id, "dataset_name": dataset_name}
...
def load_tabllm_template(dataset_name):
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    template_path = os.path.join(
        current_dir, "tabllm_like", f"templates_{dataset_name}.yaml"
    )
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            f"Could not load template for {dataset_name}: {e}"
        )
        return None


def create_feature_mapping_after_preprocessing(
    ...
    if "columns" in semantic_info:
        for col in semantic_info["columns"]:
            if col.get("name") != "target":
                original_descriptions[col["name"]] = col.get(
                    "semantic_description", col["name"]
                )
    ...
    for i, processed_name in enumerate(processed_feature_names):
        if i < len(original_feature_names):
            ...
            if original_name in original_descriptions:
                processed_mapping["semantic_info"][processed_name] = (
                    original_descriptions[original_name]
                )
            # Also try to match by name similarity for robustness
                ...
                for orig_key, description in original_descriptions.items():
                    if (
                        orig_key.lower() in processed_name.lower()
                        or processed_name.lower() in orig_key.lower()
                    ):
...
def expand_semantic_features(
    semantic_info: Dict[str, Any],
    target_feature_count: int,
    actual_feature_names: List[str] = None,
) -> Dict[str, Any]:
    """
    ...
    if "columns" in semantic_info:
        ...
        needs_realignment = (
            ...
            and len(actual_feature_names) == target_feature_count
            and current_count <= target_feature_count
        )
        ...
        if needs_expansion or needs_realignment:
            ...
            if needs_expansion:
                logger.info(
                    f"Expanding {current_count} semantic features to {target_feature_count} by duplicating with actual feature names..."
                )
            else:
                logger.info(
                    f"Realigning {current_count} semantic features to match actual feature names..."
                )
            expanded_columns = []
            ...
            if (
                ...
                for i, actual_name in enumerate(actual_feature_names):
                    ...
                    if "semantic_description" in new_feature:
                        variant_num = (
                            (i // current_count) + 1 if i >= current_count else ""
                        )
                        variant_suffix = (
                            f" (variant {variant_num})" if variant_num else ""
                        )
                        new_feature["semantic_description"] = (
                            f"{base_feature['semantic_description']}{variant_suffix}"
                        )
                ...
                for i in range(target_feature_count):
                    ...
                    new_feature = base_feature.copy()
                    new_feature["name"] = (
                        f"{base_feature['name']}_{suffix_num}"
                        if i >= current_count
                        else base_feature["name"]
                    )
                    if "semantic_description" in new_feature:
                        variant_suffix = (
                            f" (variant {suffix_num})" if i >= current_count else ""
                        )
                        new_feature["semantic_description"] = (
                            f"{base_feature['semantic_description']}{variant_suffix}"
                        )
        ...
        if needs_expansion or needs_realignment:
            ...
            if (
                ...
                for i in range(target_feature_count):
                    ...
                    new_name = (
                        f"{base_name}_{suffix_num}" if i >= current_count else base_name
                    )
                    variant_suffix = (
                        f" (variant {suffix_num})" if i >= current_count else ""
                    )
...
def generate_note_from_row(
    ...
    if "columns" in semantic_info:
        semantic_features = [
            (col["name"], col.get("semantic_description", col["name"]))
            for col in semantic_info["columns"]
            if col.get("name") != "target"
        ]
    elif "feature_descriptions" in semantic_info:
        semantic_features = [
            (name, desc) for name, desc in semantic_info["feature_descriptions"].items()
        ]
    elif "feature_description" in semantic_info:
        semantic_features = [
            (name, desc) for name, desc in semantic_info["feature_description"].items()
        ]
    elif "feature_names" in semantic_info:
        semantic_features = [
            (name, meaning) for name, meaning in semantic_info["feature_names"].items()
        ]
    ...
    if len(semantic_features) == feature_count:
        ...
        logger.warning(
            f"Feature count mismatch - Row: {feature_count}, Semantic: {len(semantic_features)}, Names: {len(attribute_names)}"
        )
...
def truncate_few_shot_examples_for_context(
    few_shot_examples: List[Tuple[str, str]],
    test_note: str,
    question: str,
    task_description: str,
    ...
    test_query = f"{test_note}\n\n{question}\nAnswer:"
    reserved_tokens = (
        estimate_note_tokens(test_query, tokenizer)
    ...
    if available_tokens <= 0:
        logger.warning(
            f"No tokens available for few-shot examples after reserving {reserved_tokens} tokens"
        )
        return []
    ...
    if len(selected_examples) < len(few_shot_examples):
        logger.info(
            f"Truncated few-shot examples from {len(few_shot_examples)} to {len(selected_examples)} "
...
def save_sample_notes_for_inspection(
    few_shot_examples: List[Tuple[str, str]],
    ...
    try:
        ...
        inspection_data = {
            "metadata": {
                ...
                "generation_method": (
                    "online_with_expanded_semantics"
                    if semantic_info
                    else "online_basic_fallback"
                ),
            },
        ...
        if semantic_info:
            if "columns" in semantic_info:
                ...
                inspection_data["semantic_expansion_info"][
                    "original_semantic_features"
                ] = original_count
                inspection_data["semantic_expansion_info"]["expansion_applied"] = (
                    original_count < num_features
                )
            elif "feature_descriptions" in semantic_info:
        ...
        logger.info(
            f"Saved {num_samples} sample notes for inspection to: {notes_filepath}"
        )
...
def evaluate_tabllm(dataset, args):
    ...
    if "task_id" in dataset:
        ...
        try:
            ...
            if openml_task_id:
                logger.debug(
                    f"Resolved dataset_id {dataset_id} to task_id {openml_task_id}"
                )
    ...
    if openml_task_id:
        ...
        if template_data:
            logger.info(
                f"Using TabLLM metadata for OpenML task {openml_task_id} (dataset: {dataset['name']})"
            )
        else:
            logger.info(
                f"No TabLLM metadata found for OpenML task {openml_task_id} (dataset: {dataset['name']}), using default approach"
            )
    else:
        logger.warning(
            f"No OpenML task ID found for dataset {dataset['name']}, cannot load TabLLM config"
        )
        ...
        try:
            ...
            if semantic_file and semantic_file.exists():
                with open(semantic_file, "r") as f:
                    semantic_info = json.load(f)
                logger.info(
                    f"Loaded semantic info from {semantic_file} for online note generation"
                )
            else:
                logger.warning(
                    f"No semantic information found for dataset {dataset['name']} or task {openml_task_id}"
                )
        except Exception as e:
    ...
    if semantic_info is not None:
        ...
        logger.info(
            f"Expanding semantic features for online note generation: {current_feature_count} features after reduction"
        )
        semantic_info = expand_semantic_features(
            semantic_info, current_feature_count, processed_feature_names
        )
    ...
    try:
        ...
        if hasattr(args, "openai_model") and args.openai_model:
            ...
            model_name = (
                args.tabllm_model
                if hasattr(args, "tabllm_model")
                else "Qwen/QwQ-32B-Preview"
            )
            backend = "auto"
        ...
        try:
            ...
            if backend == "auto" and "Qwen-2.5" in model_name:
                ...
                model_wrapper = model_loader.load_llm(
                    model_name, backend=backend, device=args.device, **model_kwargs
                )
            else:
                raise
        ...
        task_type, detection_method = detect_task_type(
            ...
            task_id=task_id,
            dataset=dataset,
            classification_threshold=getattr(args, "regression_bins", 10)
            * 2,  # Use 2x bins as threshold
        )
        ...
        if task_type == "regression":
            ...
            bin_edges, bin_labels, min_val, max_val = create_regression_bins(
                y_train, n_bins
            )
            logger.info(
                f"Created {n_bins} bins for regression: range [{min_val:.3g}, {max_val:.3g}]"
            )
        ...
        if (
            ...
            and template_data
            and "templates" in template_data
        ):
            template_info = next(iter(template_data["templates"].values()))
            answer_choices_str = template_info.get(
                "answer_choices", " ||| ".join([str(cls) for cls in unique_classes])
            )
            answer_choices = [
                choice.strip() for choice in answer_choices_str.split("|||")
            ]
            ...
            if not class_to_name:
                logger.warning(
                    f"TabLLM mapping failed: {len(answer_choices)} answer choices vs {len(unique_classes)} unique classes"
                )
        ...
        if getattr(args, "balanced_few_shot", False):
            ...
            logger.info(
                f"Using balanced few-shot selection: {len(example_indices)} examples ({examples_per_class}+ per class)"
            )
        else:
        ...
        for idx in example_indices:
            ...
            if semantic_for_note is not None:
                ...
                if not note.strip():
                    ...
                    logger.warning(
                        f"  Current feature names ({len(current_feature_names)}): {current_feature_names[:5]}..."
                    )
                    logger.warning(
                        f"  Row feature names ({len(row_feature_names)}): {row_feature_names[:5]}..."
                    )
                    if "columns" in semantic_for_note:
                        semantic_names = [
                            col["name"] for col in semantic_for_note["columns"]
                        ]
                        logger.warning(
                            f"  Semantic feature names ({len(semantic_names)}): {semantic_names[:5]}..."
                        )
        ...
        logger.info(
            f"Created {len(few_shot_examples)} few-shot examples (requested: {args.num_few_shot_examples})"
        )
        ...
        if max_context_length and tokenizer is not None and len(X_test) > 0:
            logger.info(
                f"Applying context-aware truncation with max_context_length={max_context_length}"
            )
            ...
            if semantic_info is not None:
                sample_test_note = generate_note_from_row(
                    sample_test,
                    semantic_info,
                ...
                sample_test_note = " ".join(
                    [f"Feature {i} is {val}" for i, val in enumerate(sample_test)]
                )
            ...
            original_tokens = estimate_prompt_tokens(
                few_shot_examples,
                sample_test_note,
                question,
                task_description,
            ...
            logger.info(
                f"Estimated {original_tokens} tokens for full prompt with {len(few_shot_examples)} examples"
            )
            ...
            if len(few_shot_examples) == 0:
                logger.warning(
                    "All few-shot examples were truncated due to context length limits. Using zero-shot approach."
                )
            else:
                final_tokens = estimate_prompt_tokens(
                    few_shot_examples,
                    sample_test_note,
                    question,
                    task_description,
                ...
                logger.info(
                    f"After truncation: {len(few_shot_examples)} examples, ~{final_tokens} tokens"
                )
        elif max_context_length is None:
            logger.info("No max_context_length specified, using all few-shot examples")
        elif tokenizer is None:
            logger.info(
                "No tokenizer available (API model), skipping context-aware truncation"
            )
        else:
        ...
        save_sample_notes_for_inspection(
            few_shot_examples, dataset, args, semantic_info
        )
        ...
        for i in range(len(X_test)):
            ...
            if i % 10 == 0 and torch.cuda.is_available():
                try:
                    ...
                    if "assert" in str(cache_error).lower():
                        logger.warning(
                            f"CUDA cache clear failed due to assertion error: {cache_error}. Continuing without cache clearing."
                        )
                    else:
            ...
            semantic_for_note = semantic_info  # Use the expanded semantic info
            if semantic_for_note is not None:
                test_note = generate_note_from_row(
                    test_sample,
                    semantic_for_note,
                    dataset["attribute_names"],
                    exclude_target=True,
            ...
            if tokenizer is not None:
                ...
                reserved_tokens = (
                    len(test_tokens) + answer_tokens_estimate + 50
                )  # Buffer
                ...
                available_tokens = (
                    args.max_context_length - reserved_tokens - task_description_tokens
                )
            else:
                ...
                available_tokens = (
                    ...
                    - reserved_tokens
                    - task_description_tokens_estimate
                )
            ...
            if i == 0 and len(example_parts) < num_shots:
                print(
                    f"Sample {i}: Reduced few-shot examples from {num_shots} to {len(example_parts)} due to context length limit"
                )
            ...
            try:
                ...
                if backend in ["openai", "gemini"]:
                    ...
                    prediction_result = unified_llm_predict(
                        full_prompt=full_prompt.replace(
                            " Answer:", ""
                        ),  # Remove " Answer:" as unified function adds it
                        ...
                        test_first_sample=(
                            i == 0
                        ),  # Only test methods on the first sample
                    )
                    ...
                    example_inputs_outputs.append(
                        {
                            ...
                            "test_note": test_note,
                            "few_shot_examples": [
                                (note, label) for note, label in selected_examples
                            ],
                            "num_shots_used": len(selected_examples),
                            "full_prompt": (
                                full_prompt
                                if len(full_prompt) < 5000
                                else full_prompt[:5000] + "... [truncated]"
                            ),
                            "question": question,
                            ...
                            "true_class_name": class_to_name.get(
                ...
                if any(
                    ...
                    logger.error(
                        "This error indicates a configuration problem that needs to be fixed"
                    )
                ...
                # Check if this is an OOM error
                    ...
                    logger.warning(
                        f"Unified prediction failed for sample {i} due to OOM: {e}"
                    )

                    # Try dropping a feature and retry this sample
                    if drop_feature_for_oom(
                        dropped_features, len(dataset["attribute_names"]), logger
                    ):
                        logger.info(
                            f"Retrying sample {i} with {len(dropped_features)} dropped features"
                        )
                        # Need to regenerate few-shot examples with dropped features
                        semantic_for_regenerate = (
                            processed_feature_mapping.get("semantic_info")
                            if processed_feature_mapping
                            else None
                        )
                        if semantic_for_regenerate is None and feature_mapping:
                            semantic_for_regenerate = feature_mapping.get(
                                "semantic_info"
                            )
            ...
            if i < 10:
                if class_log_probs:
                    print(
                        f"Sample {i}: {dict(sorted(class_log_probs.items(), key=lambda x: x[1], reverse=True))}"
                    )
                true_label = y_test.iloc[i] if hasattr(y_test, "iloc") else y_test[i]
                print(
                    f"Sample {i} ({prediction_method}): Predicted: {predicted_class_name} -> {predicted_class}, True: {class_to_name.get(true_label, str(true_label))} -> {true_label}"
                )
            elif i == 10:
        ...
        prediction_time = (
            total_time  # For LLMs, prediction time includes model loading and inference
        )
        ...
        if completed_samples > 0:
            ...
            if task_type == "regression":
                ...
                predicted_values = convert_bin_predictions_to_values(
                    predicted_bin_labels, bin_edges, bin_labels
                )
                ...
                logger.info(
                    f"Regression metrics: R²={r2:.4f}, MAE={mae:.4f}, RMSE={rmse:.4f}"
                )
                ...
                if task_id is None:
                    ...
                    try:
                        ...
                        if dataset_id:
                            identifiers = rm.resolve_openml_identifiers(
                                dataset_id=dataset_id
                            )
                            task_id = identifiers.get("task_id")
                    except Exception as e:
                        logger.warning(
                            f"Could not resolve task_id from dataset_id {dataset.get('id')}: {e}"
                        )
        ...
        results = {
            ...
            "prediction_time": float(
                prediction_time
            ),  # Time for inference (includes model loading for LLMs)
            "total_time": float(
                total_time
            ),  # Same as prediction_time for LLMs (no separate training phase)
            ...
            "precision_macro": (
                float(precision_macro) if precision_macro is not None else None
            ),
        ...
        if task_type == "regression":
            logger.info(
                f"TabLLM R² on {dataset['name']}: {regression_results.get('r2_score', 'N/A'):.4f}"
            )
        else:
        ...
        if hasattr(args, "output_dir") and args.output_dir:
            ...
            logger.info(
                f"Sample generated notes saved for inspection in: {inspection_dir}"
            )
