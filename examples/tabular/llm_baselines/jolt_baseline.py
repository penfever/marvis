...
def load_jolt_config_by_openml_id(openml_task_id, original_feature_count=None):
    ...
    try:
        ...
        jolt_config_path = os.path.join(
            jolt_dir, f"jolt_config_task_{openml_task_id}.json"
        )
    ...
    try:
        if jolt_config_path and jolt_config_path.exists():
            ...
            logger.info(
                f"Found JOLT config for OpenML task {openml_task_id} (dataset: {dataset_name})"
            )
        ...
        return _load_jolt_config_by_dataset_name_fallback(
            openml_task_id, jolt_dir_str, original_feature_count
        )
...
def _load_jolt_config_by_dataset_name_fallback(
    openml_task_id, jolt_dir, original_feature_count=None
):
    ...
    try:
        if os.path.exists(jolt_config_path):
            ...
            logger.info(
                f"Found JOLT config for OpenML task {openml_task_id} (dataset: {dataset_name}) via fallback"
            )
        ...
        logger.debug(
            f"Error loading JOLT config via fallback for OpenML task {openml_task_id}: {e}"
        )
        return None, None
...
def evaluate_jolt_legacy(dataset, args):
    ...
    if openml_task_id:
        ...
        if jolt_config:
            logger.info(
                f"Using JOLT metadata for OpenML task {openml_task_id} (dataset: {dataset['name']})"
            )
        else:
            logger.info(
                f"No JOLT metadata found for OpenML task {openml_task_id} (dataset: {dataset['name']}), using default approach"
            )
    else:
        logger.warning(
            f"No OpenML task ID found for dataset {dataset['name']}, cannot load JOLT config"
        )
    ...
    try:
        ...
        try:
            ...
            model_wrapper = model_loader.load_llm(
                args.jolt_model, backend="auto", device=args.device, **model_kwargs
            )
            ...
            logger.warning(
                f"Error loading model with model loader: {e}. Falling back to direct loading..."
            )
            # Fallback to direct loading
            model = AutoModelForCausalLM.from_pretrained(
                args.jolt_model, low_cpu_mem_usage=True, use_cache=False
            )
            ...
            try:
                ...
                prediction_result = unified_llm_predict(
                    full_prompt=full_prompt.replace(
                        " Class:", ""
                    ),  # Remove " Class:" as unified function handles this
        ...
        if completed_samples > 0:
            ...
            for pred in predictions:
                try:
                    ...
                    logger.warning(
                        f"Could not convert prediction '{pred}' to type {target_type}: {e}"
                    )
        ...
        return {"model_name": "JOLT", "dataset_name": dataset["name"], "error": str(e)}
    ...
    try:
        ...
        if not os.path.exists(wrapper_file):
            ...
            raise FileNotFoundError(
                f"Official JOLT wrapper not found at {wrapper_file}"
            )
        ...
        spec = importlib.util.spec_from_file_location(
            "official_jolt_wrapper", wrapper_file
        )
