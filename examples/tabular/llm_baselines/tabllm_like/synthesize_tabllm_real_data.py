...
def generate_note_from_row(
    row: pd.Series, semantic_info: Dict[str, Any], exclude_target: bool = True
) -> str:
...
def process_dataset_to_notes(
    json_file: str, max_notes: int = NOTES_PER_DATASET
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        dataset_name = semantic_info.get(
            "dataset_name", semantic_info.get("dataset", Path(json_file).stem)
        )
        task_id = (
            semantic_info.get("openml_task")
            or semantic_info.get("_metadata", {}).get("task_id")
            or Path(json_file).stem
        )
        ...
        if dataset_id and task_id:
            print(
                f"Processing dataset {dataset_name} (Task ID: {task_id}, Dataset ID: {dataset_id})..."
            )
        elif task_id:
            ...
            if RESOURCE_MANAGER:
                try:
                    ...
                    identifiers = RESOURCE_MANAGER.resolve_openml_identifiers(
                        task_id=task_id_int
                    )
                    dataset_id = identifiers.get("dataset_id")
                    if dataset_id:
                        print(
                            f"Processing dataset {dataset_name} (Task ID: {task_id}, Dataset ID: {dataset_id} - resolved)..."
                        )
                    else:
                        print(
                            f"Processing dataset {dataset_name} (Task ID: {task_id}, could not resolve to dataset ID)..."
                        )
                except (ValueError, TypeError) as e:
                    print(
                        f"Processing dataset {dataset_name} (Task ID: {task_id}, error resolving: {e})..."
                    )
            else:
                ...
                try:
                    ...
                    print(
                        f"Processing dataset {dataset_name} (ID: {dataset_id}, assuming task_id=dataset_id)..."
                    )
                except (ValueError, TypeError):
                    print(
                        f"Processing dataset {dataset_name} (invalid ID: {task_id})..."
                    )
        else:
        ...
        if df is not None and len(df) > 0:
            ...
            feature_columns = [
                col for col in df.columns if col not in ["target", "class"]
            ]
            ...
            semantic_info = expand_semantic_features(
                semantic_info, actual_feature_count, feature_columns
            )
            ...
            if dataset_id is None:
                print(
                    f"  Could not resolve task_id {task_id} to dataset_id - generating synthetic examples for {dataset_name}..."
                )
            else:
                print(
                    f"  Could not load OpenML dataset {dataset_id} - generating synthetic examples for {dataset_name}..."
                )
            ...
            for i in range(min(10, max_notes)):  # Generate fewer synthetic examples
                ...
                if "columns" in semantic_info:
                    for col in semantic_info["columns"]:
                        ...
                        if "binary" in data_type:
                            ...
                            if "(" in data_type and ")" in data_type:
                                values = data_type[
                                    data_type.find("(") + 1 : data_type.find(")")
                                ].split("/")
                                example_features[col_name] = np.random.choice(values)
                            else:
                                example_features[col_name] = (
                                    f"category_{np.random.randint(1, 4)}"
                                )
                        else:
                    ...
                    for feat_name, feat_desc in semantic_info[
                        "feature_description"
                    ].items():
                        if feat_desc.lower() == "continuous":
                            if "age" in feat_name.lower():
                                ...
                                example_features[feat_name] = np.random.randint(
                                    10000, 1000000
                                )
                            ...
                            example_features[feat_name] = (
                                f"value_{np.random.randint(1, 5)}"
                            )

                elif (
                    "feature_names" in semantic_info
                    or "feature_descriptions" in semantic_info
                ):
                    feature_list = semantic_info.get(
                        "feature_descriptions", semantic_info.get("feature_names", {})
                    )
                    for feat_name in feature_list.keys():
                        example_features[feat_name] = np.random.randint(
                            0, 16
                        )  # Based on letter dataset scale

                # Create Series for note generation
                row_series = pd.Series(example_features)
                note = generate_note_from_row(
                    row_series, semantic_info, exclude_target=True
                )

                # Generate synthetic target
                if "target_classes" in semantic_info:
                    target = np.random.choice(
                        [tc["name"] for tc in semantic_info["target_classes"]]
                    )
                elif "target_values" in semantic_info:
                    target = np.random.choice(
                        list(semantic_info["target_values"].keys())
                    )
                else:
...
def write_yaml_template(template_data: Dict[str, Any], output_path: str):
    ...
    yaml.add_representer(
        dict,
        lambda dumper, data: (
            template_representer(dumper, data)
            if data.get("__tag__") == "!Template"
            else (
                template_metadata_representer(dumper, data)
                if data.get("__tag__") == "!TemplateMetadata"
                else dumper.represent_dict(data)
            )
        ),
    )
...
def create_notes_bank_files():
    ...
    for json_file in sorted(json_files):
        ...
        try:
            ...
            task_id = (
                semantic_info.get("_metadata", {}).get("task_id")
                or Path(json_file).stem
            )
            dataset_id = semantic_info.get("openml_dataset")
            ...
            if dataset_id:
                ...
                if df is not None and len(df) > 0:
                    ...
                    template_semantic_info = expand_semantic_features(
                        semantic_info, actual_feature_count, feature_columns
                    )

            # Create and save YAML template using task_id for consistent lookup
            template_data = create_template_for_dataset(
                template_semantic_info, dataset_name
            )
...
def create_template_for_dataset(
    semantic_info: Dict[str, Any], dataset_name: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(classes) == 2 and any(
            keyword in " ".join(classes).lower()
